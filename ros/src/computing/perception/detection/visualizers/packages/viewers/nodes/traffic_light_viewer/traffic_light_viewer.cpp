...
static void signalState_cb(const autoware_msgs::TrafficLight::ConstPtr &msg) {
  ...
  cv::circle(result, cv::Point(WINDOW_SIZE / 2, WINDOW_SIZE / 2), RADIUS,
             signalColor, CV_FILLED);
  ...
  cv::putText(result, label, textOrg, fontFace, fontScale, textColor,
              fontThickness, CV_AA);
...
int main(int argc, char *argv[]) {
  ...
  ros::Subscriber signalState_sub =
      n.subscribe("/light_color", 1, signalState_cb);
