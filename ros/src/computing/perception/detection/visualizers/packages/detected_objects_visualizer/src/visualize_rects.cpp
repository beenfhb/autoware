...
VisualizeRects::VisualizeRects() {
  ...
  private_nh_.param<std::string>("object_src", object_src_topic,
  ...
  std::size_t found_pos = ros_namespace.rfind(
      "/"); // find last / from topic name to extract namespace
...
cv::Mat VisualizeRects::ObjectsToRects(
  ...
  for (auto const &object : in_objects->objects) {
    if (IsObjectValid(object)) {
      ...
      // draw rectangle
      cv::rectangle(final_image, rect, cv::Scalar(244, 134, 66), 4, CV_AA);
      ...
      cv::rectangle(final_image, text_origin + cv::Point(0, baseline),
