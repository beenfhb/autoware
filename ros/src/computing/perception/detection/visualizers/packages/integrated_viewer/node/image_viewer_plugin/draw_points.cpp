...
void DrawPoints::Draw(const autoware_msgs::PointsImage::ConstPtr &points,
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      int color_id = distance_range
                         ? ((distance - min_distance) * 255 / distance_range)
                         : 128;
      ...
      cv::rectangle(image, cv::Point(x - minus_offset, y - minus_offset),
                    cv::Point(x + plus_offset, y + plus_offset),
                    CV_RGB(red, green, blue), CV_FILLED);
    }
  }
