...
TrafficLightPlugin::TrafficLightPlugin(QWidget *parent) : rviz::Panel(parent) {
  ...
  signal_state_sub_ = node_handle_.subscribe(
      "/light_color", 1, &TrafficLightPlugin::SignalStateCallback, this);

} // TrafficLightPlugin::TrafficLightPlugin()

void TrafficLightPlugin::SignalStateCallback(
...
void TrafficLightPlugin::SetStateInfo(const StateInfo info) {
  ...
  cv::Mat circle_mat(DEFAULT_WINDOW_SIZE, DEFAULT_WINDOW_SIZE, CV_8UC3,
                     cv::Scalar(0));
  ...
  cv::circle(circle_mat,
             ...
             DEFAULT_RADIUS, info.view_color,
  ...
  ui_.state_view_->setPixmap(viewed_image_.scaled(
