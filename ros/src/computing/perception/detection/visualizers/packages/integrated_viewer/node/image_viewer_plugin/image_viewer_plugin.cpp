...
const QString ImageViewerPlugin::kDetectedObjectDataTypeBase =
    "autoware_msgs/DetectedObjectArray";
const QString ImageViewerPlugin::kPointDataType = "autoware_msgs/PointsImage";
const QString ImageViewerPlugin::kLaneDataType =
    "autoware_msgs/ImageLaneObjects";
const QString ImageViewerPlugin::kBlankTopic = "-----";

ImageViewerPlugin::ImageViewerPlugin(QWidget *parent) : rviz::Panel(parent) {
...
void ImageViewerPlugin::UpdateTopicList(void) {
  ...
  int image_topic_index =
      ui_.image_topic_combo_box_->findText(image_topic_current);
  int rect_topic_index =
      ui_.rect_topic_combo_box_->findText(rect_topic_current);
  int point_topic_index =
      ui_.point_topic_combo_box_->findText(point_topic_current);
  int lane_topic_index =
      ui_.lane_topic_combo_box_->findText(lane_topic_current);

  if (image_topic_index != -1) {
    ui_.image_topic_combo_box_->setCurrentIndex(image_topic_index);
  } else {
    ui_.image_topic_combo_box_->setCurrentIndex(
        ui_.image_topic_combo_box_->findText(kBlankTopic));
    image_sub_.shutdown();
  ...
  if (rect_topic_index != -1) {
    ...
    ui_.rect_topic_combo_box_->setCurrentIndex(
        ui_.rect_topic_combo_box_->findText(kBlankTopic));
    rect_sub_.shutdown();
    detected_objects_msg_ = NULL;
  }
  ...
  if (point_topic_index != -1) {
    ...
    ui_.point_topic_combo_box_->setCurrentIndex(
        ui_.point_topic_combo_box_->findText(kBlankTopic));
    point_sub_.shutdown();
    points_msg_ = NULL;
  }
  ...
  if (lane_topic_index != -1) {
    ...
    ui_.lane_topic_combo_box_->setCurrentIndex(
        ui_.lane_topic_combo_box_->findText(kBlankTopic));
    lane_sub_.shutdown();
    lane_msg_ = NULL;
  }
...
void ImageViewerPlugin::on_image_topic_combo_box__activated(int index) {
  ...
  std::string selected_topic =
      ui_.image_topic_combo_box_->itemText(index).toStdString();
  ...
  image_sub_ = node_handle_.subscribe<sensor_msgs::Image>(
      selected_topic, 1, &ImageViewerPlugin::ImageCallback, this);
...
void ImageViewerPlugin::on_rect_topic_combo_box__activated(int index) {
  // Extract selected topic name from combo box
  std::string selected_topic =
      ui_.rect_topic_combo_box_->itemText(index).toStdString();
  if (selected_topic == kBlankTopic.toStdString() || selected_topic == "") {
    rect_sub_.shutdown();
    detected_objects_msg_ = NULL;
    return;
  ...
  rect_sub_ = node_handle_.subscribe<autoware_msgs::DetectedObjectArray>(
      selected_topic, 1, &ImageViewerPlugin::DetectedObjCallback, this);
} // ImageViewerPlugin::on_detectedobj_topic_combo_box__activated()

void ImageViewerPlugin::DetectedObjCallback(
...
void ImageViewerPlugin::on_point_topic_combo_box__activated(int index) {
  // Extract selected topic name from combo box
  std::string selected_topic =
      ui_.point_topic_combo_box_->itemText(index).toStdString();
  if (selected_topic == kBlankTopic.toStdString() || selected_topic == "") {
    point_sub_.shutdown();
    points_msg_ = NULL;
    return;
  ...
  // if selected topic is not blank or empty , start callback function
  point_sub_ = node_handle_.subscribe<autoware_msgs::PointsImage>(
      selected_topic, 1, &ImageViewerPlugin::PointCallback, this);

} // ImageViewerPlugin::on_point_topic_combo_box__activated()

void ImageViewerPlugin::PointCallback(
    const autoware_msgs::PointsImage::ConstPtr &msg) {
  points_msg_ = msg;
...
void ImageViewerPlugin::on_lane_topic_combo_box__activated(int index) {
  // Extract selected topic name from combo box
  std::string selected_topic =
      ui_.lane_topic_combo_box_->itemText(index).toStdString();
  if (selected_topic == kBlankTopic.toStdString() || selected_topic == "") {
    lane_sub_.shutdown();
    lane_msg_ = NULL;
    return;
  ...
  lane_sub_ = node_handle_.subscribe<autoware_msgs::ImageLaneObjects>(
      selected_topic, 1, &ImageViewerPlugin::LaneCallback, this);
} // void ImageViewerPlugin::on_lane_topic_combo_box__activated()

void ImageViewerPlugin::LaneCallback(
    const autoware_msgs::ImageLaneObjects::ConstPtr &msg) {
  lane_msg_ = msg;
}
...
void ImageViewerPlugin::ShowImageOnUi(void) {
  ...
  ui_.view_->setPixmap(view_on_ui.scaled(width, height, Qt::KeepAspectRatio,
...
void ImageViewerPlugin::load(const rviz::Config &config) {
  ...
  if (config.mapGetString("Image topic", &image_topic)) {
    ...
    if (selected_topic != kBlankTopic.toStdString() && selected_topic != "") {
      ...
      // If the load topic doesn't exist, load it anyway to wait for the topic
      // to become active
      if (topic_index == -1) {
