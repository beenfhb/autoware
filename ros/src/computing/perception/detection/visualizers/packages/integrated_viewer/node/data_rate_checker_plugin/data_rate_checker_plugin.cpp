...
DataRateCheckerPlugin::DataRateCheckerPlugin(QWidget *parent)
  ...
  ui_.topic_frequency_lcd_->setMinimumWidth(ui_.topic_frequency_lcd_->width() +
                                            2);
...
void DataRateCheckerPlugin::UpdateTopicList(void) {
  ...
  if (topic_index != -1) {
    ...
    ui_.topic_combo_box_->setCurrentIndex(
        ui_.topic_combo_box_->findText(kBlankTopic));
    topic_sub_.shutdown();
    timer_.stop();
    ui_.status_icon_->setStyleSheet("QLabel {color: #b3b3b3;}");
    ui_.status_text_->setText(QString("No topic selected"));
...
void DataRateCheckerPlugin::on_topic_combo_box__activated(int index) {
  ...
  std::string selected_topic =
      ui_.topic_combo_box_->itemText(index).toStdString();
  ...
  topic_sub_ = node_handle_.subscribe(
      selected_topic, 1, &DataRateCheckerPlugin::MessageCallback, this);

  timer_ = node_handle_.createWallTimer(
      ros::WallDuration(1.0), &DataRateCheckerPlugin::TimerCallback, this);
...
void DataRateCheckerPlugin::TimerCallback(const ros::WallTimerEvent &event) {
  ...
  double avg_delay =
      std::accumulate(times_.begin(), times_.end(), 0.0) / times_.size();
