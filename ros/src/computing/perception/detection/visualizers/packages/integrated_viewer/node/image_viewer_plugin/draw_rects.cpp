...
DrawRects::DrawRects(void) {
  ...
  pedestrian_image_ =
      cv::imread(DEFAULT_PATH + "pedestrian.png", cv::IMREAD_UNCHANGED);

} // DrawRects::DrawRects()

void DrawRects::OverlayImage(const cv::Mat &in_background,
                             const cv::Mat &in_foreground, cv::Mat &output,
                             cv::Point2i in_location) {
  ...
  for (int y = std::max(in_location.y, 0); y < in_background.rows; ++y) {
    ...
    for (int x = std::max(in_location.x, 0); x < in_background.cols; ++x) {
      ...
      double opacity =
          ((double)in_foreground.data[fY * in_foreground.step +
                                      fX * in_foreground.channels() + 3]) /
          255.;
      for (int c = 0; opacity > 0 && c < output.channels(); ++c) {
        unsigned char foregroundPx =
            in_foreground.data[fY * in_foreground.step +
                               fX * in_foreground.channels() + c];
        unsigned char in_backgroundPx =
            in_background.data[y * in_background.step +
                               x * in_background.channels() + c];
...
void DrawRects::DrawImageRect(
    const autoware_msgs::DetectedObjectArray::ConstPtr &detected_objects,
    cv::Mat &image) {
  ...
  for (const auto &detected_object : detected_objects->objects) {
    if (detected_object.x >= 0 && detected_object.y >= 0 &&
        detected_object.width > 0 && detected_object.height > 0) {
      ...
      if (detected_object.dimensions.x > 0 &&
        ...
        cv::Mat image_roi =
            image(cv::Rect(cv::Point(detected_object.x, detected_object.y),
                           cv::Point(x2, y2)));
        cv::Mat color_fill(image_roi.size(), CV_8UC3, color_map_[0]);
        double alpha = 0.3;
        cv::addWeighted(color_fill, alpha, image_roi, 1.0 - alpha, 0.0,
                        image_roi);
      }

      // Draw rectangle
      cv::rectangle(image, cv::Point(detected_object.x, detected_object.y),
                    cv::Point(x2, y2), color_map_[0], kRectangleThickness,
                    CV_AA, 0);
    }
  }
} // DrawRects::DrawImageRect()

void DrawRects::DrawImageBox(
    const autoware_msgs::DetectedObjectArray::ConstPtr &detected_objects,
    cv::Mat &image) {
  if (detected_objects == NULL) {
    return;
  ...
  for (const auto &detected_object : detected_objects->objects) {
    ...
    cv::rectangle(image, cv::Point(detected_object.x, detected_object.y),
                  ...
                  color_map_[0], kRectangleThickness, CV_AA, 0);
  }
} // DrawRects::DrawImageRect()

void DrawRects::DrawLabel(
    const autoware_msgs::DetectedObject &in_detected_object, cv::Mat &image) {
  ...
  cv::Size label_size = cv::getTextSize("0123456789", font_face, font_scale,
                                        font_thickness, &font_baseline);

  cv::Point label_origin =
      cv::Point(rectangle_origin.x, rectangle_origin.y - font_baseline -
                                        kRectangleThickness * 2 - icon_height);

  double distance = sqrt(
      in_detected_object.pose.position.x * in_detected_object.pose.position.x +
  ...
  if (in_detected_object.label == "car" ||
      in_detected_object.label == "truck") {
  ...
  cv::putText(image, label_one.str(), label_origin, font_face, font_scale,
              CV_RGB(255, 255, 255), 1, CV_AA);
  label_origin.y += text_holder_rect.height / 3;
  cv::putText(image, label_two.str(), label_origin, font_face, font_scale,
              CV_RGB(255, 255, 255), 1, CV_AA);
