...
RegionTLRMxNetROSNode::RegionTLRMxNetROSNode()
    : image_topic_name_("/image_raw"), network_definition_file_name_(""),
      pretrained_model_file_name_(""), use_gpu_(false), gpu_id_(0),
      kAdvertiseInLatch_(true), kTrafficLightRed(0), kTrafficLightGreen(1),
      kTrafficLightUnknown(2), kStringRed("red signal"),
      kStringGreen("green signal"), kStringUnknown("") {}
...
void RegionTLRMxNetROSNode::ROISignalCallback(
  ...
  for (Context &context : contexts_) {
    ...
    LightState current_state =
        recognizer.RecognizeLightState(roi, score_threshold_);
  ...
  // Save timestamp of this frame so that same frame has never been process
  // again
...
void RegionTLRMxNetROSNode::GetROSParam() {
  ...
  private_node_handle.param<std::string>("image_raw_topic", image_topic_name_,
                                         "/image_raw");
  ...
  ROS_INFO("network_definition_file: %s",
           network_definition_file_name_.c_str());
  ...
  private_node_handle.param<int>("change_state_threshold",
  ...
  if (network_definition_file_name_.empty()) {
    ROS_FATAL(
        "No Network Definition File was specified. Terminate program... ");
    exit(EXIT_FAILURE);
  }
...
void RegionTLRMxNetROSNode::StartSubscribersAndPublishers() {
  ...
  image_subscriber = node_handle.subscribe(
      image_topic_name_, 1, &RegionTLRMxNetROSNode::ImageRawCallback, this);
  roi_signal_subscriber = node_handle.subscribe(
      "/roi_signal", 1, &RegionTLRMxNetROSNode::ROISignalCallback, this);
  superimpose_sub = node_handle.subscribe(
      "/config/superimpose", 1, &RegionTLRMxNetROSNode::SuperimposeCb, this);

  // Register publishers
  signal_state_publisher =
      node_handle.advertise<autoware_msgs::TrafficLight>("light_color", 1);
  signal_state_string_publisher =
      node_handle.advertise<std_msgs::String>("/sound_player", 1);
  marker_publisher = node_handle.advertise<visualization_msgs::MarkerArray>(
      "tlr_result", 1, kAdvertiseInLatch_);
  superimpose_image_publisher =
      node_handle.advertise<sensor_msgs::Image>("tlr_superimpose_image", 1);

} // RegionTLRMxNetROSNode::StartSubscribersAndPublishers()

/*!
 * DetermineState works as a latch to reduce the chance of sudden changes in the
 * state of the traffic light, caused by mis classifications in the detector. To
 * change the traffic light state, the new candidate should be found at least
 * kChangeStateThreshold times.
 * @param current_state the current state of the traffic light as reported by
 * the classifier.
 * @param in_out_signal_context the object containing the data of the current
...
void RegionTLRMxNetROSNode::DetermineState(LightState in_current_state,
  ...
  if (in_current_state != in_out_signal_context.lightState) {
    ...
    if (in_current_state != in_out_signal_context.newCandidateLightState) {
      ...
      // if classifier returned the same result previously increase its
      // confidence
      in_out_signal_context.stateJudgeCount++;
    }
  }
  // if new candidate has been found enough times, change state to the new
  // candidate
...
void RegionTLRMxNetROSNode::PublishTrafficLight(std::vector<Context> contexts) {
  ...
  for (const auto ctx : contexts) {
    ...
    // This program assume that the signal which has the largest estimated
    // radius equal the nearest one from camera.
...
void RegionTLRMxNetROSNode::PublishImage(std::vector<Context> contexts) {
  ...
  for (const auto ctx : contexts_) {
    ...
    circle(result_image, ctx.redCenter, ctx.lampRadius, CV_RGB(255, 0, 0), 1,
           0);
    circle(result_image, ctx.yellowCenter, ctx.lampRadius, CV_RGB(255, 255, 0),
           1, 0);
    circle(result_image, ctx.greenCenter, ctx.lampRadius, CV_RGB(0, 255, 0), 1,
           0);
    ...
    cv::Point label_origin =
        cv::Point(ctx.topLeft.x, ctx.botRight.y + font_baseline);

    cv::putText(result_image, label, label_origin, kFontFace, kFontScale,
                label_color);
  }
...
void RegionTLRMxNetROSNode::SuperimposeCb(
    const std_msgs::Bool::ConstPtr &config_msg) {
