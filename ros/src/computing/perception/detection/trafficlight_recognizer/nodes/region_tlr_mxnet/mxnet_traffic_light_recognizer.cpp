...
void MxNetTrafficLightRecognizer::Init(const char *in_network_definition_buffer,
  ...
  const mx_uint input_shape_data[4] = {1, static_cast<mx_uint>(num_channels_),
                                       static_cast<mx_uint>(height_),
                                       static_cast<mx_uint>(width_)};
  prediction_handle_ = 0;

  MXPredCreate(in_network_definition_buffer, in_pretrained_model_buffer,
               in_pretrained_model_length, dev_type, dev_id, num_input_nodes,
               input_keys, input_shape_indptr, input_shape_data,
               &prediction_handle_);
  assert(prediction_handle_);
}

void MxNetTrafficLightRecognizer::PreProcessImage(
    const cv::Mat &in_image, mx_float *out_image_data, const int in_channels,
...
LightState
MxNetTrafficLightRecognizer::RecognizeLightState(const cv::Mat &in_image,
                                                 double in_score_threshold) {
  ...
  PreProcessImage(in_image, mx_image_data.data(), num_channels_,
