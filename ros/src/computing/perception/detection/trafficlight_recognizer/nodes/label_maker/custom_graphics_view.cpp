...
CustomGraphicsView::CustomGraphicsView(QWidget *parent)
    : QGraphicsView(parent), scheduled_scalings_(0), rectangle_item_(nullptr),
      k_initial_position_(QPoint(-1, -1)), start_position_(k_initial_position_),
      end_position_(k_initial_position_), dragging_(false) {
...
void CustomGraphicsView::wheelEvent(QWheelEvent *event) {
  ...
  // if user moved the wheel in another direction, we reset previously scheduled
  // scalings
...
void CustomGraphicsView::mouseMoveEvent(QMouseEvent *mouse_event) {
  ...
  if (dragging_) {
    ...
    rectangle_item_->setBrush(
        QColor(0, 0, 255, 125)); // Show in transparent blue
...
QPoint CustomGraphicsView::ConvertPointOnImage(QPoint point) {
  ...
  QPoint limit(original_image_.size().width() - 1,
               original_image_.size().height() - 1);
...
bool CustomGraphicsView::GetSelectedArea(QPoint *left_upper,
                                         QPoint *right_bottom) {
  ...
  // Assign surely "Left-Upper" and "Right-Bottom" coordinate value of selected
  // area
...
QSize CustomGraphicsView::GetImageSize() { return original_image_.size(); }
