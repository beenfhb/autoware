...
static bool g_use_vector_map_server; // Switch flag whether vecter-map-server
...
public:
  ...
  autoware_msgs::Lane waypoints() const { return waypoints_; }

  void set_pose(const geometry_msgs::PoseStamped &pose) { pose_ = pose; }
...
bool project2(const Point3 &pt, int &u, int &v, bool useOpenGLCoord = false) {
  ...
  if (u < 0 || imageWidth < u || v < 0 || imageHeight < v ||
      _pt.z() < nearPlane || farPlane < _pt.z()) {
    u = -1, v = -1;
    return false;
  }
...
double ConvertDegreeToRadian(double degree) { return degree * M_PI / 180.0f; }

double ConvertRadianToDegree(double radian) { return radian * 180.0f / M_PI; }
...
double GetSignalAngleInCameraSystem(double hang, double vang) {
  ...
  signal_orientation_in_map_system.setRPY(0, signal_pitch_in_map,
                                          signal_yaw_in_map);

  tf::Quaternion signal_orientation_in_cam_system =
      trf * signal_orientation_in_map_system;
  double signal_roll_in_cam;
  double signal_pitch_in_cam;
  double signal_yaw_in_cam;
  tf::Matrix3x3(signal_orientation_in_cam_system)
      .getRPY(signal_roll_in_cam, signal_pitch_in_cam, signal_yaw_in_cam);

  return ConvertRadianToDegree(
      signal_pitch_in_cam); // holizontal angle of camera is represented by
                            // pitch
...
void echoSignals2(ros::Publisher &pub, bool useOpenGLCoord = false) {
  ...
  if (g_use_vector_map_server) {
    ...
    if (g_ros_client.call(service)) {
      ...
      for (const auto &response : service.response.objects.data) {
        ...
        vmap.signals.insert(
            std::map<int, Signal>::value_type(signal.id, signal));
      }
      ROS_INFO("[feat_proj] VectorMapServer available. Publishing only "
               "TrafficSignals on the current lane");
    }
  }
  ...
  for (const auto &signal_map : vmap.signals) {
    ...
    Point3 signalcenterx(signalcenter.x(), signalcenter.y(),
                         signalcenter.z() + SignalLampRadius);
    ...
    if (project2(signalcenter, u, v, useOpenGLCoord) == true) {
      ...
      sign.u = u + adjust_proj_x; // shift project position by configuration
                                  // value from runtime manager
      sign.v = v + adjust_proj_y; // shift project position by configuration
                                  // value from runtime manager

      sign.radius = radius;
      sign.x = signalcenter.x(), sign.y = signalcenter.y(),
      sign.z = signalcenter.z();
      sign.hang =
          vmap.vectors[signal.vid].hang; // hang is expressed in [0, 360] degree
      ...
      double signal_angle =
          GetSignalAngleInCameraSystem(vmap.vectors[signal.vid].hang + 180.0f,
  ...
  std::cout << "There are " << signalsInFrame.Signals.size()
            << " signals in range" << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  private_nh.param<std::string>("camera_info_topic", cameraInfo_topic_name,
                                "/camera_info");
  ...
  ros::Subscriber sub_point =
      rosnode.subscribe("vector_map_info/point", SUBSCRIBE_QUEUE_SIZE,
                        &VectorMap::load_points, &vmap);
  ros::Subscriber sub_line =
      rosnode.subscribe("vector_map_info/line", SUBSCRIBE_QUEUE_SIZE,
                        &VectorMap::load_lines, &vmap);
  ros::Subscriber sub_lane =
      rosnode.subscribe("vector_map_info/lane", SUBSCRIBE_QUEUE_SIZE,
                        &VectorMap::load_lanes, &vmap);
  ros::Subscriber sub_vector =
      rosnode.subscribe("vector_map_info/vector", SUBSCRIBE_QUEUE_SIZE,
                        &VectorMap::load_vectors, &vmap);
  ros::Subscriber sub_signal =
      rosnode.subscribe("vector_map_info/signal", SUBSCRIBE_QUEUE_SIZE,
                        &VectorMap::load_signals, &vmap);
  ros::Subscriber sub_whiteline =
      rosnode.subscribe("vector_map_info/white_line", SUBSCRIBE_QUEUE_SIZE,
                        &VectorMap::load_whitelines, &vmap);
  ros::Subscriber sub_dtlane =
      rosnode.subscribe("vector_map_info/dtlane", SUBSCRIBE_QUEUE_SIZE,
                        &VectorMap::load_dtlanes, &vmap);
  ...
  while (vmap.points.empty() || vmap.lines.empty() || vmap.whitelines.empty() ||
         vmap.lanes.empty() || vmap.dtlanes.empty() || vmap.vectors.empty() ||
         vmap.signals.empty()) {
    ros::spinOnce();
    std::cout << ".";
    wait_rate.sleep();
  ...
  ros::Subscriber cameraInfoSubscriber =
      rosnode.subscribe(cameraInfo_topic_name, 100, cameraInfoCallback);
  ros::Subscriber cameraImage =
      rosnode.subscribe(cameraInfo_topic_name, 100, cameraInfoCallback);
  ros::Subscriber adjust_xySubscriber =
      rosnode.subscribe("/config/adjust_xy", 100, adjust_xyCallback);
  ros::Subscriber current_pose_subscriber;
  ros::Subscriber waypoint_subscriber;
  if (g_use_vector_map_server) {
    /* Create subscribers which deliver informations requested by server */
    current_pose_subscriber = rosnode.subscribe(
        "/current_pose", 1, &VectorMapClient::set_pose, &g_vector_map_client);
    waypoint_subscriber = rosnode.subscribe("/final_waypoints", 1,
                                            &VectorMapClient::set_waypoints,
                                            &g_vector_map_client);

    /* Create ros client to use Server-Client communication */
    g_ros_client = rosnode.serviceClient<vector_map_server::GetSignal>(
        "vector_map_server/get_signal");
  }

  ros::Publisher signalPublisher =
      rosnode.advertise<autoware_msgs::Signals>("roi_signal", 100);
