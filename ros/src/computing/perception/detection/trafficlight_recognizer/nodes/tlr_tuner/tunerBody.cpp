...
/*============== Utility function ==============*/
static void colorTrack(const cv::Mat &hsv_img, const int hue, const int sat,
                       const int val, const int hw, const int sw, const int vw,
                       cv::Mat *dst) {
  cv::Mat kernel =
      cv::Mat::ones(5, 5, CV_8U); /* kernel for dilation and erosion*/

  /* create mask image */
  cv::inRange(hsv_img, cv::Scalar(hue - hw, sat - sw, val - vw),
              cv::Scalar(hue + hw, sat + sw, val + vw), *dst);
...
void TunerBody::image_raw_callBack(const sensor_msgs::Image &image_msg) {
  ...
  if (updateImage) {
    ...
    updateImage = false;
    Clicked_point = cv::Point(
        -1, -1); // if image is reloaded, reset clicked point to out of image
  }
...
void TunerBody::launch(void) {
  ...
  ros::Subscriber image_sub =
      n.subscribe(image_topic_name, 1, image_raw_callBack);

  ros::Publisher tunedResult_pub = n.advertise<autoware_msgs::TunedResult>(
      "tuned_result", ADVERTISE_QUEUE_SIZE, ADVERTISE_LATCH);
  ...
  while (ros::ok()) {
    ...
    colorTrack(hsv_img, Selected_set->hue.center, Selected_set->sat.center,
               Selected_set->val.center, Selected_set->hue.range,
               Selected_set->sat.range, Selected_set->val.range, &mask);

    std::vector<std::vector<cv::Point>> contours;
    std::vector<cv::Vec4i> hierarchy;

    cv::Mat mask_clone = mask.clone();
    cv::findContours(mask_clone, contours, hierarchy, CV_RETR_TREE,
    ...
    if (maxIndex != -1) {
      ...
      drawContours(result, std::vector<std::vector<cv::Point>>(1, hull), 0,
                   CV_RGB(220, 30, 20), 3);
    }
    ...
    cv::Mat roi_mask =
        base(cv::Rect(result.cols, 0, mask_colored.cols, mask_colored.rows));
    mask_colored.copyTo(roi_mask);

    cv::imshow(windowName, base);
    cv::waitKey(10);
    // if ( (cv::waitKey(10)) == '\x1b') { /* if 'ESC' key is typed, finish the
    // program */
    //   break;
    // }
...
void TunerBody::saveResult(std::string fileName) {
  ...
  if (Red_set.isUpdated == false) {
    ...
    */
    ...
    Red_set.sat.center =
        ((DEFAULT_SAT_UPPER - DEFAULT_SAT_LOWER) / 2) + DEFAULT_SAT_LOWER;
    Red_set.sat.range = (DEFAULT_SAT_UPPER - DEFAULT_SAT_LOWER) / 2;
    Red_set.val.center =
        ((DEFAULT_VAL_UPPER - DEFAULT_VAL_LOWER) / 2) + DEFAULT_VAL_LOWER;
