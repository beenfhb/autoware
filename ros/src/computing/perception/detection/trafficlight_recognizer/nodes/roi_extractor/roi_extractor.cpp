...
void ROIExtractor::ImageRawCallback(const sensor_msgs::Image &image) {
  ...
  // Save this topic's time stamp so that same image will not be processed more
  // than twice
...
void ROIExtractor::ROISignalCallback(
  ...
  Context::SetContexts(signal_positions, extracted_pos, frame_.rows,
                       frame_.cols);
  ...
  cv::Mat roi = frame_(cv::Rect(signal_positions.at(0).topLeft,
                                signal_positions.at(0).botRight));
  std::string file_name =
      target_directory_ + std::to_string(file_count_) + ".png";
  ...
  // Reject image if its similarity level with previous saved ROI is higher than
  // threshold
  if (k_similarity_threshold_ <
      CalculateSimilarity(roi, previous_saved_frame_)) {
    return;
  }
...
void ROIExtractor::CreateTargetDirectory(std::string base_name) {
  // Extracted ROI's images will be saved in
  // "[base_name]/tlr_TrainingDataSet/Images"
  std::string target_directory_name =
      base_name + "/tlr_TrainingDataSet/Images/";
...
void ROIExtractor::MakeDirectoryTree(const std::string &target,
  ...
  while (separator_end != std::string::npos) {
    std::string sub_directory =
        sub_tree.substr(separator_start, separator_end - separator_start);
...
double ROIExtractor::CalculateSimilarity(const cv::Mat &image1,
                                         const cv::Mat &image2) {
...
int main(int argc, char *argv[]) {
  ...
  std::string target_directory_name =
      std::string(getenv("HOME")) + "/.autoware";
  int minimum_height = 32;
  double similarity_threshold = 0.9;
  private_node_handler.param<std::string>("image_raw_topic", image_topic_name,
                                          "/image_raw");
  private_node_handler.param<std::string>(
      "target_directory", target_directory_name, target_directory_name);
  private_node_handler.param<int>("minimum_height", minimum_height,
                                  32); // The default minimum height is 32
  private_node_handler.param<double>(
  ...
  ros::Subscriber image_subscriber = node_handler.subscribe(
      image_topic_name, 1, &ROIExtractor::ImageRawCallback, &extractor);

  ros::Subscriber roi_signal_subscriber = node_handler.subscribe(
      "/roi_signal", 1, &ROIExtractor::ROISignalCallback, &extractor);
