...
*/
static inline bool IsRange(const double lower, const double upper,
                           const double val) {
  if (lower <= upper) {
...
static void colorExtraction(
    const cv::Mat &src, // input HSV image
    ...
    const double val_lower, const double val_upper) // value thresholds
{
  ...
  *dst = cv::Scalar::all(0);
  ...
  for (int i = 0; i < 256; i++) {
    lut.at<cv::Vec3b>(i)[0] =
        (IsRange(hue_lower, hue_upper, Actual_Hue(i))) ? 255 : 0;
    lut.at<cv::Vec3b>(i)[1] =
        (IsRange(sat_lower, sat_upper, Actual_Sat(i))) ? 255 : 0;
    lut.at<cv::Vec3b>(i)[2] =
        (IsRange(val_lower, val_upper, Actual_Val(i))) ? 255 : 0;
  }
...
static bool checkExtinctionLight(const cv::Mat &src_img,
  ...
  roi_top_left.x = (top_left.x < 0) ? 0
                                    : (src_img.cols < top_left.x) ? src_img.cols
                                                                  : top_left.x;
  roi_top_left.y = (top_left.y < 0) ? 0
                                    : (src_img.rows < top_left.y) ? src_img.rows
                                                                  : top_left.y;
  ...
  cv::Mat kernel = getStructuringElement(
      cv::MORPH_ELLIPSE, cv::Size(2 * anchor + 1, 2 * anchor + 1),
      cv::Point(anchor, anchor));

  cv::Mat topHat_dark;
  morphologyEx(hsv_channel[2], topHat_dark, cv::MORPH_TOPHAT, kernel,
               cv::Point(anchor, anchor), 5);
  ...
  findContours(topHat_dark, dark_contours, dark_hierarchy, CV_RETR_CCOMP,
  ...
  for (unsigned int i = 0; i < dark_contours.size(); i++) {
    ...
    double circleLevel = (IsNearlyZero(perimeter))
                             ? 0.0f
                             : (4.0f * CV_PI * area / pow(perimeter, 2));

    if (std::max(bound.width, bound.height) <
            2 * std::min(bound.width, bound.height) && // dimension ratio
...
static cv::Mat
signalDetect_inROI(const cv::Mat &roi, const cv::Mat &src_img,
                   const double estimatedRadius, const cv::Point roi_topLeft,
                   bool in_turn_signal // if true it will not try to mask by
                                       // using "circularity""
) {
  /* reduce noise */
  ...
  colorExtraction(noiseReduced, &red_mask, thSet.Red.Hue.lower,
                  thSet.Red.Hue.upper, thSet.Red.Sat.lower, thSet.Red.Sat.upper,
                  thSet.Red.Val.lower, thSet.Red.Val.upper);

  cv::Mat yellow_mask(roi.rows, roi.cols, CV_8UC1);
  colorExtraction(noiseReduced, &yellow_mask, thSet.Yellow.Hue.lower,
                  thSet.Yellow.Hue.upper, thSet.Yellow.Sat.lower,
                  thSet.Yellow.Sat.upper, thSet.Yellow.Val.lower,
                  thSet.Yellow.Val.upper);

  cv::Mat green_mask(roi.rows, roi.cols, CV_8UC1);
  colorExtraction(noiseReduced, &green_mask, thSet.Green.Hue.lower,
                  thSet.Green.Hue.upper, thSet.Green.Sat.lower,
                  thSet.Green.Sat.upper, thSet.Green.Val.lower,
                  thSet.Green.Val.upper);
  ...
  findContours(binarized, bright_contours, bright_hierarchy, CV_RETR_CCOMP,
               CV_CHAIN_APPROX_NONE);
  ...
  for (unsigned int i = 0; i < bright_contours.size(); i++) {
    ...
    double area_lower_limit = (3 * sqrt(3)) * pow(estimatedRadius / 3.0, 2) /
                              4; // the area of inscribed triangle of 1/3 circle
    double area_upper_limit =
        pow(estimatedRadius, 2) * M_PI; // the area of the circle
    ...
    if (std::max(bound.width, bound.height) <
      ...
      cnd.circleLevel = (IsNearlyZero(perimeter))
                            ? 0.0f
                            : (4.0 * CV_PI * area / pow(perimeter, 2));
      cnd.isBlacked = false;
      candidates.push_back(cnd);
    }

    drawContours(bright_mask, bright_contours, contours_idx, rangeColor,
                 CV_FILLED, 8, bright_hierarchy, 0);
  ...
  // std::cerr << "before checkExtrinctionLight. candidates: " << candidates_num
  // << std::endl;

  /* decrease candidates by checking existence of turned off light in their
   * neighborhood */
  if (!in_turn_signal &&
      candidates_num > 1) /* if there are multipule candidates */
  {
    for (unsigned int i = 0; i < candidates.size(); i++) {
      /* check wheter this candidate seems to be green lamp */
      cv::Point check_roi_topLeft = cv::Point(
          candidates.at(i).center.x - 2 * estimatedRadius + roi_topLeft.x,
          candidates.at(i).center.y - 2 * estimatedRadius + roi_topLeft.y);
      cv::Point check_roi_botRight = cv::Point(
          candidates.at(i).center.x + 6 * estimatedRadius + roi_topLeft.x,
          candidates.at(i).center.y + 2 * estimatedRadius + roi_topLeft.y);
      bool likeGreen =
          checkExtinctionLight(src_img, check_roi_topLeft, check_roi_botRight,
                               candidates.at(i).center);

      /* check wheter this candidate seems to be yellow lamp */
      check_roi_topLeft = cv::Point(
          candidates.at(i).center.x - 4 * estimatedRadius + roi_topLeft.x,
          candidates.at(i).center.y - 2 * estimatedRadius + roi_topLeft.y);
      check_roi_botRight = cv::Point(
          candidates.at(i).center.x + 4 * estimatedRadius + roi_topLeft.x,
          candidates.at(i).center.y + 2 * estimatedRadius + roi_topLeft.y);
      bool likeYellow =
          checkExtinctionLight(src_img, check_roi_topLeft, check_roi_botRight,
      ...
      if (!likeGreen && !likeYellow &&
          !likeRed) /* this region may not be traffic light */
      {
        candidates_num--;
        drawContours(bright_mask, bright_contours, candidates.at(i).idx, BLACK,
                     CV_FILLED, 8, bright_hierarchy, 0);
  ...
  // std::cerr << "after checkExtrinctionLight. candidates: " << candidates_num
  // << std::endl;

  /* choose one candidate by comparing degree of circularity */
  if (!in_turn_signal &&
      candidates_num > 1) /* if there are still multiple candidates */
  {
    ...
    for (unsigned int i = 0; i < candidates.size(); i++) {
      ...
      drawContours(bright_mask, bright_contours, candidates.at(i).idx,
                   regionColor, CV_FILLED, 8, bright_hierarchy, 0);
    }
  }
...
void TrafficLightDetector::brightnessDetect(const cv::Mat &input) {
  ...
  for (int i = 0; i < static_cast<int>(contexts.size()); i++) {
    ...
    cv::Mat signalMask = signalDetect_inROI(
        roi_HSV, input.clone(), context.lampRadius, context.topLeft,
    ...
    // << yellow_pixNum << ", " << red_pixNum << ") / " << valid_pixNum
    // <<std::endl;
    ...
    if (valid_pixNum > 0) {
      ...
      isYellow_bright =
          (((double)yellow_pixNum / valid_pixNum) > 0.5) ? true : false;
      isGreen_bright =
          (((double)green_pixNum / valid_pixNum) > 0.5) ? true : false;
    } else {
      isRed_bright = false;
    ...
    int currentLightsCode =
        getCurrentLightsCode(isRed_bright, isYellow_bright, isGreen_bright);
    contexts.at(i).lightState =
        determineState(contexts.at(i).lightState, currentLightsCode,
...
double getBrightnessRatioInCircle(const cv::Mat &input, const cv::Point center,
                                  const int radius) {
  ...
  for (int i = center.x - radius; i < center.x + radius; i++) {
    for (int j = center.y - radius; j < center.y + radius; j++) {
      if ((i - center.x) * (i - center.x) + (j - center.y) * (j - center.y) <
          radius * radius) {
  ...
  // std::cout << "(" << center.x << ", " << center.y << ") "<< "  white:" <<
  // whitePoints << " black: " << blackPoints << std::endl;
  return ((double)whitePoints) / (whitePoints + blackPoints);
}

int getCurrentLightsCode(bool display_red, bool display_yellow,
                         bool display_green) {
  return (int)display_red + 2 * ((int)display_yellow) +
         4 * ((int)display_green);
}
...
/*
 *  Attempt to recognize by color tracking in HSV. Detects good only green, but
 * need to play also with S and V parameters range.
 */
void TrafficLightDetector::colorDetect(const cv::Mat &input, cv::Mat &output,
                                       const cv::Rect coords, int Hmin,
                                       int Hmax) {
