...
Context::Context(cv::Point aRedCenter, cv::Point aYellowCenter,
                 cv::Point aGreenCenter, int aLampRadius, cv::Point aTopLeft,
                 cv::Point aBotRight) {
 ...
 */
bool Context::CompareContext(const Context in_context_a,
                             const Context in_context_b) {
...
void Context::SetContexts(
    std::vector<Context> &out_signal_contexts,
  ...
  for (unsigned int i = 0; i < in_lamp_signals_positions->Signals.size(); i++) {
    ...
    tmp_lamp_position.signalId =
        in_lamp_signals_positions->Signals.at(i).signalId;
    ...
    lane_id_vector.push_back(
        tmp_lamp_position
            .linkId); // store lanes ids, to later identify signals contexts
  }

  // get unique lane ids
  std::sort(lane_id_vector.begin(), lane_id_vector.end());
  std::vector<int>::iterator new_end =
      std::unique(lane_id_vector.begin(), lane_id_vector.end());
  lane_id_vector.erase(new_end, lane_id_vector.end());

  std::vector<Context> final_signal_contexts;

  // one traffic signal per lane, check each lane and find the bulb belonging to
  // this lane (this signal Context)
  for (unsigned int ctx_idx = 0; ctx_idx < lane_id_vector.size(); ctx_idx++) {
    ...
    for (lamp_iterator = signals_lamps.begin();
         lamp_iterator < signals_lamps.end(); lamp_iterator++) {
      ...
      if (lamp_iterator->linkId == lane_id_vector.at(ctx_idx) &&
          0 < img_x - radius - 1.5 * radius &&
          img_x + radius + 1.5 * radius < in_image_width &&
          0 < img_y - radius - 1.5 * radius &&
        ...
        case 2: /* GREEN */
          ...
          current_signal_context.greenCenter3d =
              cv::Point3d(map_x, map_y, map_z);
          break;
        case 3: /* YELLOW */
          current_signal_context.yellowCenter = cv::Point(img_x, img_y);
          current_signal_context.yellowCenter3d =
              cv::Point3d(map_x, map_y, map_z);
        ...
        most_left = (most_left > img_x - radius - 1.5 * min_radius)
                        ? img_x - radius - 1.5 * min_radius
                        : most_left;
        most_top = (most_top > img_y - radius - 1.5 * min_radius)
                       ? img_y - radius - 1.5 * min_radius
                       : most_top;
        most_right = (most_right < img_x + radius + 1.5 * min_radius)
                         ? img_x + radius + 1.5 * min_radius
                         : most_right;
        most_bottom = (most_bottom < img_y + radius + 1.5 * min_radius)
                          ? img_y + radius + 1.5 * min_radius
                          : most_bottom;
    ...
    /* search whether this signal has already belonged in
     * detector.out_signal_contexts */
    bool isInserted = false;
    std::vector<int> eraseCandidate;
    for (unsigned int i = 0; i < out_signal_contexts.size(); i++) {
      if (current_signal_context.signalID ==
              out_signal_contexts.at(i).signalID &&
        ...
        final_signal_contexts.back().lightState =
            out_signal_contexts.at(i).lightState;
        final_signal_contexts.back().stateJudgeCount =
            out_signal_contexts.at(i).stateJudgeCount;
        final_signal_contexts.back().newCandidateLightState =
            out_signal_contexts.at(i).newCandidateLightState;
        isInserted = true;
        break;
      }
    }
    ...
    if (isInserted == false && current_signal_context.lampRadius != INT_MAX) {
      final_signal_contexts.push_back(
          current_signal_context); // this current_signal_context is new in
                                   // detector.out_signal_contexts
    }
  ...
  /* sort by lampRadius */
  std::sort(final_signal_contexts.begin(), final_signal_contexts.end(),
            CompareContext);
