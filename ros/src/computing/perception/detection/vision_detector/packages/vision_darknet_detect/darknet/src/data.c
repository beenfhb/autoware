...
matrix load_image_augment_paths(char **paths, int n, int min, int max, int size,
                                float angle, float aspect, float hue,
...
data load_data_seg(int n, char **paths, int m, int w, int h, int classes,
  ...
  for (i = 0; i < n; ++i) {
    ...
    image sized =
        rotate_crop_image(orig, a.rad, a.scale, a.w, a.h, a.dx, a.dy, a.aspect);
    ...
    image mask =
        get_segmentation_image(random_paths[i], orig.w, orig.h, classes);
    // image mask = make_image(orig.w, orig.h, classes+1);
    image sized_m =
        rotate_crop_image(mask, a.rad, a.scale / div, a.w / div, a.h / div,
                          a.dx / div, a.dy / div, a.aspect);
...
data load_data_iseg(int n, char **paths, int m, int w, int h, int classes,
                    int boxes, int coords, int min, int max, float angle,
...
data load_data_region(int n, char **paths, int m, int w, int h, int size,
  ...
  for (i = 0; i < n; ++i) {
    ...
    fill_truth_region(random_paths[i], d.y.vals[i], classes, size, flip, dx, dy,
                      1. / sx, 1. / sy);
...
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes,
  ...
  for (i = 0; i < n; ++i) {
    ...
    float new_ar =
        (orig.w + rand_uniform(-dw, dw)) / (orig.h + rand_uniform(-dh, dh));
...
void *load_thread(void *ptr) {
  ...
  if (a.type == OLD_CLASSIFICATION_DATA) {
    ...
    *a.d = load_data_regression(a.paths, a.n, a.m, a.classes, a.min, a.max,
                                a.size, a.angle, a.aspect, a.hue, a.saturation,
                                a.exposure);
  } else if (a.type == CLASSIFICATION_DATA) {
    *a.d = load_data_augment(
        a.paths, a.n, a.m, a.labels, a.classes, a.hierarchy, a.min, a.max,
        a.size, a.angle, a.aspect, a.hue, a.saturation, a.exposure, a.center);
    ...
    *a.d =
        load_data_detection(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes,
                            a.jitter, a.hue, a.saturation, a.exposure);
...
/*
   data load_data_study(char **paths, int n, int m, char **labels, int k, int
   min, int max, int size, float angle, float aspect, float hue, float
   saturation, float exposure)
   {
   data d = {0};
   ...
   saturation, exposure); d.y = load_labels_paths(paths, n, labels, k); if(m)
   free(paths); return d;
   }
 */
...
data load_data_regression(char **paths, int n, int m, int k, int min, int max,
                          int size, float angle, float aspect, float hue,
