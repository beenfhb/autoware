...
void forward_local_layer(const local_layer l, network net) {
  ...
  for (i = 0; i < l.batch; ++i) {
    ...
    im2col_cpu(input, l.c, l.h, l.w, l.size, l.stride, l.pad, net.workspace);
...
void backward_local_layer_gpu(local_layer l, network net) {
  ...
  gradient_array_gpu(l.output_gpu, l.outputs * l.batch, l.activation,
                     l.delta_gpu);
  for (i = 0; i < l.batch; ++i) {
    axpy_gpu(l.outputs, 1, l.delta_gpu + i * l.outputs, 1, l.bias_updates_gpu,
             1);
  }
...
void update_local_layer_gpu(local_layer l, update_args a) {
  ...
  axpy_gpu(size, learning_rate / batch, l.weight_updates_gpu, 1, l.weights_gpu,
           1);
