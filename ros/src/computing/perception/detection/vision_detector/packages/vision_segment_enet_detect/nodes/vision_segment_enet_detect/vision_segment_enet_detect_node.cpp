...
class ROSENetSegmenterApp {
  ...
  void image_callback(const sensor_msgs::Image &in_image_sensor) {
    ...
    if (!segmented_mat.empty()) {
      ...
      segmented_rosimage.header = in_image_sensor.header;
      segmented_rosimage.encoding = "bgr8";
      segmented_rosimage.image = segmented_mat;

      cv_bridge::CvImage blended_rosimage;
      blended_rosimage.header = in_image_sensor.header;
      blended_rosimage.encoding = "bgr8";
...
public:
  void Run() {
    ...
    enet_segmenter_ = new ENetSegmenter(
        network_definition_file, pretrained_model_file, lookuptable_file);
    ...
    subscriber_image_raw_ = node_handle_.subscribe(
        image_raw_topic_str, 1, &ROSENetSegmenterApp::image_callback, this);

    publisher_image_segmented_ =
        node_handle_.advertise<sensor_msgs::Image>("/image_segmented", 1);
    ROS_INFO("Publishing /image_segmented");
    publisher_image_segmented_blended_ =
        node_handle_.advertise<sensor_msgs::Image>("/image_segmented_blended",
                                                   1);
  ...
  ROSENetSegmenterApp() { enet_segmenter_ = NULL; }
};
