...
void ENetSegmenter::Predict(const cv::Mat &in_image_mat,
  ...
  class_each_row =
      class_each_row.t(); // transpose to make each row with all probabilities
  ...
  cv::resize(out_segmented, out_segmented,
             cv::Size(in_image_mat.cols, in_image_mat.rows));
}

cv::Mat ENetSegmenter::Visualization(cv::Mat in_prediction_map,
                                     std::string in_lookuptable_file) {
