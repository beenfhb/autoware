    ...
    for (int x = 0; x < C_dims[1]; x++) {
      ...
      for (int y = 0; y < C_dims[0]; y++) {
        ...
        for (int xp = 0; xp < width1; xp++) {
          switch (B_dims[0]) {
          case 20:
            val += T_off[19] * B_off[19];
          case 19:
            val += T_off[18] * B_off[18];
          case 18:
            val += T_off[17] * B_off[17];
          case 17:
            val += T_off[16] * B_off[16];
          case 16:
            val += T_off[15] * B_off[15];
          case 15:
            val += T_off[14] * B_off[14];
          case 14:
            val += T_off[13] * B_off[13];
          case 13:
            val += T_off[12] * B_off[12];
          case 12:
            val += T_off[11] * B_off[11];
          case 11:
            val += T_off[10] * B_off[10];
          case 10:
            val += T_off[9] * B_off[9];
          case 9:
            val += T_off[8] * B_off[8];
...
// Output Score
FLOAT **dpm_ttic_cpu_fconvsMT(FLOAT *feat, FLOAT *flfeat, FLOAT **filter,
                              int *sym_info, int start, int end, int *A_SIZE,
                              int **B_SIZE, int *M_size) {
  start = start - 1;
  end = end - 1;
