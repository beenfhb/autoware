...
static Model_info *load_modelinfo(const char *filename) {
  Model_info *MI = (Model_info *)malloc(sizeof(Model_info)); // Model
                                                             // information
  ...
  CUresult res = cuMemHostAlloc((void **)&(MI->def), sizeof(FLOAT) * DefL * 4,
...
static Rootfilters *load_rootfilter(const char *filename) {
  ...
  FLOAT t1, t2, t3;
  FLOAT dummy_t1, dummy_t2, dummy_t3; // variable for dummy scan in order to
  ...
  RF->root_size = (int **)malloc(sizeof(int *) * RF->NoR); // size of root
                                                           // filter
  RF->rootfilter =
      (FLOAT **)malloc(sizeof(FLOAT *) * RF->NoR); // weight of root filter
  RF->rootsym =
      (int *)malloc(sizeof(int) * RF->NoR); // symmetric information of root
...
#ifdef ORIGINAL
    RF->rootfilter[i] =
        (FLOAT *)malloc(sizeof(FLOAT) * NUMB); // weight of root filter
#else
#ifdef SEPARETE_MEM
    res = cuMemHostAlloc((void **)&(RF->rootfilter[i]), sizeof(FLOAT) * NUMB,
                         CU_MEMHOSTALLOC_DEVICEMAP);
    if (res != CUDA_SUCCESS) {
  ...
  FLOAT *dst_root;
  res = cuMemHostAlloc((void **)&dst_root, sum_size_root,
                       CU_MEMHOSTALLOC_DEVICEMAP);
  if (res != CUDA_SUCCESS) {
  ...
  for (int i = 0; i < RF->NoR; i++) {
    ...
    /* adjust the location of file-pointer */
    fscanf(file, FLOAT_SCAN_FMT3, &dummy_t1, &dummy_t2,
           &dummy_t3); // this is dummy scan
...
static Partfilters *load_partfilter(const char *filename) {
  ...
  PF->part_size = (int **)malloc(sizeof(int *) * PF->NoP); // size of part
                                                           // filter
  PF->partfilter =
      (FLOAT **)malloc(sizeof(FLOAT *) * PF->NoP); // weight of part filter
  PF->part_partner =
      (int *)malloc(sizeof(int) * PF->NoP); // symmetric information of part
  PF->part_sym =
      (int *)malloc(sizeof(int) * PF->NoP); // symmetric information of part
...
#else
#ifdef SEPARETE_MEM
    res = cuMemHostAlloc((void **)&(PF->partfilter[i]), sizeof(FLOAT) * NUMB,
                         CU_MEMHOSTALLOC_DEVICEMAP);
    if (res != CUDA_SUCCESS) {
  ...
  FLOAT *dst_part;
  res = cuMemHostAlloc((void **)&dst_part, SUM_SIZE_PART,
                       CU_MEMHOSTALLOC_DEVICEMAP);
  if (res != CUDA_SUCCESS) {
...
GPUModel *dpm_ttic_gpu_load_model(FLOAT ratio, const char *com_csv,
                                  const char *root_csv, const char *part_csv) {
