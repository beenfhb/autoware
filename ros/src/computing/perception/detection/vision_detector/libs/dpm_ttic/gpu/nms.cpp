...
// Quick sort function
static void quickSort(FLOAT *data, int *Order, int first_index,
                      int last_index) {
...
FLOAT *dpm_ttic_gpu_nms(FLOAT *boxes, FLOAT overlap, int *num, GPUModel *MO) {
  ...
  quickSort(scores, sorted_orders, 0,
            NUM - 1); // sort score of detected rectangle

  int *checked = (int *)calloc(
      NUM,
      sizeof(int)); // cheked flags (picked =1,non-checked =0,suppressed =-1)
  int cnum = NUM - 1;
  int cur = 0;
  int pi_num = 0;
