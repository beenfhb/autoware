...
static FLOAT *detect(IplImage *IM, GPUModel *MO, FLOAT thresh, int *D_NUMS,
                     FLOAT *A_SCORE) {
  /* for measurement */
  ...
  FLOAT *boxes = dpm_ttic_gpu_get_boxes(feature, scales, featsize, MO, D_NUMS,
                                        A_SCORE, thresh);
...
RESULT *dpm_ttic_gpu_car_detection(IplImage *IM, GPUModel *MO, FLOAT thresh,
                                   int *D_NUMS, FLOAT *A_SCORE, FLOAT overlap) {
  FLOAT *boxes =
      detect(IM, MO, thresh, D_NUMS, A_SCORE); // detect high-score region
  FLOAT *rects = dpm_ttic_gpu_nms(boxes, overlap, D_NUMS,
                                  MO); // get boundary-rectangles of car
  RESULT *result =
      dpm_ttic_gpu_get_new_rects(IM, MO, rects, D_NUMS); // get current result
