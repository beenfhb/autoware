...
static const FLOAT Hcos[9] = {1.0000,  0.9397,  0.7660,  0.5000, 0.1736,
                              -0.1736, -0.5000, -0.7660, -0.9397};
static const FLOAT Hsin[9] = {0.0000, 0.3420, 0.6428, 0.8660, 0.9848,
...
// resize.cc
extern FLOAT *resize(FLOAT *src, int *sdims, int *odims,
                     FLOAT scale); // resize image
...
// initialize scales
FLOAT *gpu_init_scales(Model_info *MI, IplImage *IM, int X,
                       int Y) // X,Y length of image
{
  ...
  if (MI->ini) {
    ...
    const FLOAT sc =
        pow(2.0, (1 / (double)interval)); // represent down-scale rate
    ...
    for (int kk = 1; kk < numcomponent; kk++) {
      if (MI->rsize[kk * 2] < MRY)
        MRY = MI->rsize[kk * 2];
      if (MI->rsize[kk * 2 + 1] < MRX)
        MRX = MI->rsize[kk * 2 + 1];
    }
  ...
  // return
  FLOAT *scales = (FLOAT *)calloc((max_scale + interval),
                                  sizeof(FLOAT)); // Model information
  return (scales);
}
...
int *gpu_init_featsize(Model_info *MI) {
  ...
  int *featsize = (int *)calloc(
      LofFeat * 2, sizeof(FLOAT)); // feature size information matrix
  return (featsize);
}
...
#ifdef ORIGINAL
static FLOAT *
calc_feature(FLOAT *SRC,  // resized image
  ...
  /* input size */
  const int height =
      ISIZE[0]; //{268,268,134,67,233,117,203,203,177,154,89,203,154,77}
  const int width =
      ISIZE[1]; //{448,112,224,390,195,340,170,296,257,148,340,257,129}
  const int dims[2] = {height, width};

  /* size of Histgrams and Norm calculation space size */
  const int blocks[2] = {
      (int)floor(double(height) / double(sbin) + 0.5),
      (int)floor(double(width) / double(sbin) + 0.5)}; //{67,112}....sbine=4
  ...
  FLOAT *hist = (FLOAT *)calloc(blocks[0] * blocks[1] * 18,
                                sizeof(FLOAT)); // HOG histgram
...
static void calc_feature_byGPU(
    ...
    int *FTSIZE, // size of feature(output)
    int sbin,    // block size desicion element for each filter
    int level, CUdeviceptr hist_dev, CUdeviceptr norm_dev, CUdeviceptr feat_dev,
    CUstream stream) {
  /* rename argument */
  ...
  void *kernel_args_hist[] = {&hist_dev, (void *)&sbin, (void *)&visible[0],
                              (void *)&visible[1], (void *)&level};
  ...
  if ((visible[1] - 1) % thread_num_x != 0)
    block_num_x++;
  if ((visible[0] - 1) % thread_num_y != 0)
    block_num_y++;
...
#ifdef USE_STREAM
  res = cuLaunchKernel(func_calc_hist[0], // call function
  ...
  */
  ...
  /* compute energy in each block by summing over orientations */
  void *kernel_args_norm[] = {&hist_dev, &norm_dev, (void *)&blocks[0],
                              (void *)&blocks[1], (void *)&level};

  /* decide CUDA block shape */
  thread_num_x =
      (blocks[1] < sqrt(max_thread_num)) ? blocks[1] : sqrt(max_thread_num);
  thread_num_y =
      (blocks[0] < sqrt(max_thread_num)) ? blocks[0] : sqrt(max_thread_num);

  block_num_x = blocks[1] / thread_num_x;
  block_num_y = blocks[0] / thread_num_y;
  if (blocks[1] % thread_num_x != 0)
    block_num_x++;
  if (blocks[0] % thread_num_y != 0)
    block_num_y++;
  ...
  /* execute GPU function */
#ifdef USE_STREAM
  res = cuLaunchKernel(func_calc_norm[0], // call function
                       block_num_x,       // gridDimX
                       block_num_y,       // gridDimY
                       1,                 // gridDimZ
                       thread_num_x,      // blockDimX
  ...
  */
  ...
  void *kernel_args_feat[] = {
      &hist_dev,       &norm_dev,          &feat_dev,          (void *)&out[0],
      (void *)&out[1], (void *)&blocks[0], (void *)&blocks[1], (void *)&level};

  /* decide CUDA block shape */
  thread_num_x =
      (out[1] < sqrt(max_thread_num)) ? out[1] : sqrt(max_thread_num);
  thread_num_y =
      (out[0] < sqrt(max_thread_num)) ? out[0] : sqrt(max_thread_num);
  ...
  if (out[1] % thread_num_x != 0 || block_num_x == 0)
    block_num_x++;
  if (out[0] % thread_num_y != 0 || block_num_y == 0)
    block_num_y++;
  ...
  /* execute GPU function */
#ifdef USE_STREAM
  res = cuLaunchKernel(func_calc_feat[0], // call function
                       block_num_x,       // gridDimX
                       block_num_y,       // gridDimY
                       1,                 // gridDimZ
                       thread_num_x,      // blockDimX
...
static void *feat_calc_forGPU(void *thread_arg) {
  ...
  calc_feature_byGPU( // IM,
      ISIZE, FSIZE, sbin, level, hist_dev, norm_dev, feat_dev, stream);
...
static void ini_thread_data(thread_data *TD, FLOAT *IM, int *INSIZE, int sbin,
                            int level) {
  TD->IM = IM;
...
#else
static void ini_thread_data_fouGPU(thread_data_forGPU *TD, int *INSIZE,
                                   int sbin, int level, CUdeviceptr hist_dev,
                                   CUdeviceptr norm_dev, CUdeviceptr feat_dev,
                                   CUstream stream) {
  //  TD->IM       = IM;
  memcpy(TD->ISIZE, INSIZE, sizeof(int) * 3);
  TD->FSIZE[0] = 0;
...
FLOAT **gpu_calc_f_pyramid(IplImage *Image, Model_info *MI, int *FTSIZE,
                           FLOAT *scale) // calculate feature pyramid
{
  /* constant parameters */
  ...
  int org_image_size[3] = {
      Image->height, Image->width,
      Image->nChannels}; // original image size // (元INSIZE)
...
#else
  thread_data_forGPU *td =
      (thread_data_forGPU *)calloc(LEN, sizeof(thread_data_forGPU));
#endif
  pthread_t *ts = (pthread_t *)calloc(LEN, sizeof(pthread_t));

  //  FLOAT **resized_image      = (FLOAT**)calloc(LEN, sizeof(FLOAT*)); //
  //  resized image // (originally RIM_S)
  int *resized_image_size = (int *)calloc(
      LEN * 3, sizeof(int)); // resized image size // (originally RI_S)
  int t_count = 0;
  ...
  dpm_ttic_gpu_calc_resized_image_size(org_image_size, resized_image_size,
                                       interval, sc, max_scale, scale);
  ...
  dpm_ttic_gpu_resize_byGPU(org_image, org_image_size, resized_image_size,
                            interval, LEN, stream);
  ...
  for (int level = 0; level < LEN; level++) {
    ...
    int out_size[3] = {max_i(blocks_inner[0] - 2, 0),
                       max_i(blocks_inner[1] - 2, 0), 27 + 4};
  ...
  res = cuMemAlloc(&hist_ptr_incrementer_dev,
                   LEN * sizeof(unsigned long long int));
  ...
  res = cuMemAlloc(&norm_ptr_incrementer_dev,
                   LEN * sizeof(unsigned long long int));
  ...
  res = cuMemAlloc(&feat_ptr_incrementer_dev,
                   LEN * sizeof(unsigned long long int));
  MY_CUDA_CHECK(res, "cuMemAlloc(feat_ptr_incrementer_dev)");

  /* upload resized image data */
  // unsigned long long int ptr_resized_image_dev = (unsigned long long
  // int)resized_image_dev; for (int level=0; level<LEN; level++)
  //   {
  //     int height_inner = resized_image_size[level*3];
  //     int width_inner  = resized_image_size[level*3 + 1];
  ...
  //     ptr_resized_image_dev += (unsigned long long
  //     int)height_inner*width_inner*depth_inner*sizeof(FLOAT);
  //   }

  /* upload resized image size to GPU */
  res = cuMemcpyHtoD(resized_image_size_dev, resized_image_size,
                     LEN * 3 * sizeof(int));
  ...
  // res = cuMemcpyHtoD(image_idx_incrementer_dev, image_idx_incrementer,
  // LEN*sizeof(int)); MY_CUDA_CHECK(res,
  // "cuMemcpyHtoD(image_idx_incrementer_dev)");

  res = cuMemcpyHtoD(hist_ptr_incrementer_dev, hist_ptr_incrementer,
                     LEN * sizeof(unsigned long long int));
  ...
  res = cuMemcpyHtoD(norm_ptr_incrementer_dev, norm_ptr_incrementer,
                     LEN * sizeof(unsigned long long int));
  ...
  res = cuMemcpyHtoD(feat_ptr_incrementer_dev, feat_ptr_incrementer,
                     LEN * sizeof(unsigned long long int));
  ...
  res = cuModuleGetTexRef(&resized_image_size_texref, module[0],
                          "resized_image_size");
  MY_CUDA_CHECK(res, "cuModuleGetTexRef(resized_image_size)");

  // CUtexref image_idx_incrementer_texref;
  // res = cuModuleGetTexRef(&image_idx_incrementer_texref, module[0],
  // "image_idx_incrementer"); MY_CUDA_CHECK(res,
  // "cuModuleGetTexRef(image_idx_incrementer)");

  CUtexref hist_ptr_incrementer_texref;
  res = cuModuleGetTexRef(&hist_ptr_incrementer_texref, module[0],
                          "hist_ptr_incrementer");
  MY_CUDA_CHECK(res, "cuModuleGetTexRef(hist_ptr_incrementer)");

  CUtexref norm_ptr_incrementer_texref;
  res = cuModuleGetTexRef(&norm_ptr_incrementer_texref, module[0],
                          "norm_ptr_incrementer");
  MY_CUDA_CHECK(res, "cuModuleGetTexRef(norm_ptr_incrementer)");

  CUtexref feat_ptr_incrementer_texref;
  res = cuModuleGetTexRef(&feat_ptr_incrementer_texref, module[0],
                          "feat_ptr_incrementer");
  MY_CUDA_CHECK(res, "cuModuleGetTexRef(feat_ptr_incrementer)");

  /* bind to texture memory on GPU */
  // res = cuTexRefSetAddress(NULL, resized_image_texref, resized_image_dev,
  // sum_size_image); MY_CUDA_CHECK(res,
  // "cuTexRefSetAddress(resized_image_dev)");

  res = cuTexRefSetAddress(NULL, resized_image_size_texref,
                           resized_image_size_dev, LEN * 3 * sizeof(int));
  MY_CUDA_CHECK(res, "cuTexRefSetAddress(resized_image_size_dev)");

  // res = cuTexRefSetAddress(NULL, image_idx_incrementer_texref,
  // image_idx_incrementer_dev, LEN*sizeof(int)); MY_CUDA_CHECK(res,
  // "cuTexRefSetAddress(image_idx_incrementer_dev)");

  res = cuTexRefSetAddress(NULL, hist_ptr_incrementer_texref,
                           hist_ptr_incrementer_dev,
                           LEN * sizeof(unsigned long long int));
  MY_CUDA_CHECK(res, "cuTexRefSetAddress(hist_ptr_incrementer_dev)");

  res = cuTexRefSetAddress(NULL, norm_ptr_incrementer_texref,
                           norm_ptr_incrementer_dev,
                           LEN * sizeof(unsigned long long int));
  MY_CUDA_CHECK(res, "cuTexRefSetAddress(norm_ptr_incrementer_dev)");

  res = cuTexRefSetAddress(NULL, feat_ptr_incrementer_texref,
                           feat_ptr_incrementer_dev,
                           LEN * sizeof(unsigned long long int));
  MY_CUDA_CHECK(res, "cuTexRefSetAddress(feat_ptr_incrementer_dev)");

  /* texture memory configuration */
  // //  res = cuTexRefSetFlags(resized_image_texref,
  // CU_TRSF_NORMALIZED_COORDINATES); res =
  // cuTexRefSetFlags(resized_image_texref, CU_TRSF_READ_AS_INTEGER);
  // MY_CUDA_CHECK(res, "cuTexRefSetFlags(resized_image_texref)");

  //  res = cuTexRefSetFlags(resized_image_size_texref,
  ...
  // //  res = cuTexRefSetFlags(image_idx_incrementer_texref,
  // CU_TRSF_NORMALIZED_COORDINATES); res =
  // cuTexRefSetFlags(image_idx_incrementer_texref, CU_TRSF_READ_AS_INTEGER);
  // MY_CUDA_CHECK(res, "cuTexRefSetFlags(image_idx_incrementer_texref)");

  //  res = cuTexRefSetFlags(hist_ptr_incrementer_texref,
  //  CU_TRSF_NORMALIZED_COORDINATES);
  ...
  //  res = cuTexRefSetFlags(norm_ptr_incrementer_texref,
  //  CU_TRSF_NORMALIZED_COORDINATES);
  ...
  //  res = cuTexRefSetFlags(feat_ptr_incrementer_texref,
  //  CU_TRSF_NORMALIZED_COORDINATES);
  ...
  // #else
  //   {
  //     res = cuTexRefSetFormat(resized_image_texref,
  //     CU_AD_FORMAT_UNSIGNED_INT32, 2); MY_CUDA_CHECK(res,
  //     "cuTexRefSetFormat(resized_image_texref)");
  //   }
  // #endif

  res = cuTexRefSetFormat(resized_image_size_texref, CU_AD_FORMAT_SIGNED_INT32,
                          1);
  MY_CUDA_CHECK(res, "cuTexRefSetFormat(resized_image_size_texref)");

  // res = cuTexRefSetFormat(image_idx_incrementer_texref,
  // CU_AD_FORMAT_SIGNED_INT32, 1); MY_CUDA_CHECK(res,
  // "cuTexRefSetFormat(image_idx_incrementer_texref)");

  res = cuTexRefSetFormat(hist_ptr_incrementer_texref,
                          CU_AD_FORMAT_UNSIGNED_INT32, 2);
  MY_CUDA_CHECK(res, "cuTexRefSetFormat(hist_ptr_incrementer_texref)");

  res = cuTexRefSetFormat(norm_ptr_incrementer_texref,
                          CU_AD_FORMAT_UNSIGNED_INT32, 2);
  MY_CUDA_CHECK(res, "cuTexRefSetFormat(norm_ptr_incrementer_texref)");

  res = cuTexRefSetFormat(feat_ptr_incrementer_texref,
                          CU_AD_FORMAT_UNSIGNED_INT32, 2);
  ...
  for (int ii = 0; ii < interval; ii++) {
    ...
    /* "first" 2x interval */
#ifdef ORIGINAL
    ini_thread_data(&td[t_count], resized_image[ii],
                    resized_image_size + ii * 3, sbin2,
                    ii); // initialize thread
#else
    ini_thread_data_fouGPU(&td[t_count],
                           //                             resized_image[ii],
                           resized_image_size + ii * 3, sbin2, ii, hist_dev,
                           norm_dev, feat_dev,
                           stream[ii]); // initialize thread
#endif

#ifdef ORIGINAL
    if (pthread_create(&ts[t_count], NULL, feat_calc, (void *)&td[t_count]))
#else
    if (pthread_create(&ts[t_count], NULL, feat_calc_forGPU,
                       (void *)&td[t_count]))
#endif
    {
      printf("Error thread\n");
    ...
    /* "second" 1x interval */
#ifdef ORIGINAL
    ini_thread_data(&td[t_count], resized_image[ii + interval],
                    resized_image_size + ii * 3, sbin,
                    ii + interval); // initialize thread
#else
    ini_thread_data_fouGPU(
        &td[t_count],
        //                             resized_image[ii+interval],
        resized_image_size + ii * 3, sbin, ii + interval, hist_dev, norm_dev,
        feat_dev,
        stream[ii + interval]); // initialize thread
#endif
...
#ifdef ORIGINAL
      ini_thread_data(&td[t_count], resized_image[jj + interval],
                      resized_image_size + (jj + interval) * 3, sbin,
                      jj + interval); // initialize thread
#else
      ini_thread_data_fouGPU(
          &td[t_count],
          //                                 resized_image[jj+interval],
          resized_image_size + (jj + interval) * 3, sbin, jj + interval,
          hist_dev, norm_dev, feat_dev,
          stream[jj + interval]); // initialize thread
#endif
