...
/*********************************************************************/
FLOAT *dpm_ttic_gpu_Ipl_to_FLOAT_forGPU(
    IplImage *Input) // get intensity data (FLOAT) of input
{
...
static void upload_org_image_toGPU(FLOAT *org_image, int org_image_size[3]) {
  ...
  res = cuTexRefSetArray(org_image_texref, org_image_dev,
  ...
  /* configuration in order to return border value when outside region access is
   * occur */
...
static void make_image_idx_incrementer(int *resized_image_size, int LEN) {
  CUresult res;
  res = cuMemHostAlloc((void **)&image_idx_incrementer, LEN * sizeof(int),
  ...
  res = cuTexRefSetAddress(NULL, image_idx_incrementer_texref,
                           image_idx_incrementer_dev, LEN * sizeof(int));
...
static void *bilinear_resizing(void *arg) {
  ...
  void *kernel_arg[] = {(void *)&src_height, (void *)&src_width,
                        &resized_image_dev,  (void *)&dst_height,
                        (void *)&dst_width,  (void *)&hfactor,
                        (void *)&wfactor,    (void *)&level};

  /* define CUDA kernel shape */
  int max_thread_num = 0;
  res = cuDeviceGetAttribute(&max_thread_num,
                             CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_BLOCK, dev[0]);
  MY_CUDA_CHECK(res, "cuDeviceGetAttribute()");

  int thread_num_x =
      (dst_width < sqrt(max_thread_num)) ? dst_width : sqrt(max_thread_num);
  int thread_num_y =
      (dst_height < sqrt(max_thread_num)) ? dst_height : sqrt(max_thread_num);

  int block_num_x = dst_width / thread_num_x;
  int block_num_y = dst_height / thread_num_y;
  int block_num_z = nChannels;
  if (dst_width % thread_num_x != 0)
    block_num_x++;
  if (dst_height % thread_num_y != 0)
    block_num_y++;

  int sharedMemBytes = 0;

  /* execute GPU function */
  res = cuLaunchKernel(func_resize[0], // call function
...
void dpm_ttic_gpu_resize_byGPU(FLOAT *org_image, int *org_image_size,
                               int *resized_image_size, int interval, int LEN,
                               CUstream *stream_array) {
  /* pthread handler */
  /* to calculate all resized image, the required number of threads is (LEN -
   * interval) */
  pthread_t *thread = (pthread_t *)calloc(LEN - interval, sizeof(pthread_t));

  /* structure to carry data to pthread function */
  resize_thread_arg *args =
      (resize_thread_arg *)calloc(LEN - interval, sizeof(resize_thread_arg));
  int thread_count = 0;
...
#else
    pthread_create(&thread[thread_count], NULL, bilinear_resizing,
  ...
  /* (interval <= level < 2*interval) use same resize scale as (0 <= level <
   * interval) */
  for (int level = interval; level < 2 * interval; level++) {
    ...
    unsigned long long int src_ptr =
        (unsigned long long int)resized_image_dev +
        (unsigned long long int)(image_idx_incrementer[level - interval] *
                                 sizeof(FLOAT));
    unsigned long long int dst_ptr =
        (unsigned long long int)resized_image_dev +
        (unsigned long long int)(image_idx_incrementer[level] * sizeof(FLOAT));
    int copy_size = resized_image_size[level * 3] *
                    resized_image_size[level * 3 + 1] *
                    resized_image_size[level * 3 + 2] * sizeof(FLOAT);
    res = cuMemcpyDtoDAsync((CUdeviceptr)dst_ptr, (CUdeviceptr)src_ptr,
                            copy_size, stream_array[level]);
  }
...
void dpm_ttic_gpu_calc_resized_image_size(int *org_image_size,
                                          int *resized_image_size, int interval,
                                          FLOAT sc, int max_scale,
                                          FLOAT *scale_array) {
  ...
  for (int ii = 0; ii < interval; ii++) {
    ...
    memcpy(resized_image_size + (ii + interval) * 3,
           resized_image_size + (ii)*3, 3 * sizeof(int));
    ...
    for (int jj = ii + interval; jj < max_scale; jj += interval) {
      resized_image_size[(jj + interval) * 3] =
          (int)((FLOAT)resized_image_size[jj * 3] * extra_scale + 0.5);
      resized_image_size[(jj + interval) * 3 + 1] =
          (int)((FLOAT)resized_image_size[jj * 3 + 1] * extra_scale + 0.5);
      resized_image_size[(jj + interval) * 3 + 2] =
          resized_image_size[jj * 3 + 2];
...
/*************************************************/
void dpm_ttic_gpu_create_resized_image_texref(void) {
  CUresult res;
...
#else
  {
    res = cuModuleGetTexRef(&resized_image_texref, module[0],
                            "resized_image_double");
    MY_CUDA_CHECK(res, "cuModuleGetTexRef(resized_image)");
  }
#endif
  ...
  {
    res =
        cuTexRefSetFormat(resized_image_texref, CU_AD_FORMAT_UNSIGNED_INT32, 2);
    MY_CUDA_CHECK(res, "cuTexRefSetFormat(resized_image_texref)");
  }
#endif
...
/* free GPU memory region allocated in this file */
void dpm_ttic_gpu_cleanup_about_resize(void) {
  CUresult res;
