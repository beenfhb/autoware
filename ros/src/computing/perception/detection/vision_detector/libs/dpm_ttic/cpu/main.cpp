...
DPMTTIC::DPMTTIC(const char *com_csv, const char *root_csv,
                 const char *part_csv) {
  constexpr double RATIO = 1;
  model_ = dpm_ttic_cpu_load_model(RATIO, com_csv, root_csv, part_csv);
}

DPMTTIC::~DPMTTIC() { dpm_ttic_cpu_free_model(model_); }
...
DPMTTICResult DPMTTIC::detect_objects(IplImage *image,
  ...
  RESULT *cars =
      dpm_ttic_cpu_car_detection(image, model_, param.threshold,
