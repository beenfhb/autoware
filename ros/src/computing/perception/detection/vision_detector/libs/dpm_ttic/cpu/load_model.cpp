...
static Rootfilters *load_rootfilter(const char *filename) {
  ...
  for (int ii = 0; ii < RF->NoR; ii++) {
    ...
    int NUMB =
        RF->root_size[ii][0] * RF->root_size[ii][1] * RF->root_size[ii][2];
    RF->rootfilter[ii] =
...
static Partfilters *load_partfilter(const char *filename) {
  ...
  for (int ii = 0; ii < PF->NoP; ii++) {
    ...
    // printf("%f  %f  %f\n",t1,t2,t3);
    int NUMB =
        PF->part_size[ii][0] * PF->part_size[ii][1] * PF->part_size[ii][2];
    PF->partfilter[ii] =
        (FLOAT *)malloc(sizeof(FLOAT) * NUMB); // weight of root filter
    for (int jj = 0; jj < NUMB; jj++) {
      fscanf(file, FLOAT_SCAN_FMT, &t1);
...
MODEL *dpm_ttic_cpu_load_model(FLOAT ratio, const char *com_csv,
