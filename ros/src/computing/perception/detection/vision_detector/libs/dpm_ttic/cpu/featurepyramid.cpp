...
// initialize scales
FLOAT *dpm_ttic_cpu_ini_scales(Model_info *MI, IplImage *IM, int X,
...
static FLOAT *calc_feature(FLOAT *SRC, int *ISIZE, int *FTSIZE, int sbin) {
  ...
  const int OUT_SIZE[3] = {max_i(blocks[0] - 2, 0), max_i(blocks[1] - 2, 0),
                           27 + 4};            //{65,110,31}.....
  ...
  // feature(Output)
  FLOAT *feat =
      (FLOAT *)calloc(OUT_SIZE[0] * OUT_SIZE[1] * OUT_SIZE[2], sizeof(FLOAT));
    ...
    for (int y = 1; y < vis_R[0]; y++) {
      ...
      if (flagX) {
        if (iyp >= 0) {
          *(Htemp + ixp_b + iyp) +=
              vy1 *
              vx1Xv; // 1-少数をxyでかけたものにエッジ強度の2乗をかけたもの
  ...
  for (int x = 0; x < OUT_SIZE[1]; x++) {
    ...
    FLOAT *pt = Norm + BA;
    FLOAT nc1 =
        1.0 / sqrt(*pt + *(pt + 1) + *(pt + blocks[0]) + *(pt + BX) + eps);
    pt = Norm + BB;
    FLOAT nc3 =
        1.0 / sqrt(*pt + *(pt + 1) + *(pt + blocks[0]) + *(pt + BX) + eps);
...
static FLOAT *
Ipl_to_FLOAT(IplImage *Input) // get intensity data (FLOAT) of input
{
...
FLOAT **dpm_ttic_cpu_calc_f_pyramid(IplImage *Image, Model_info *MI,
                                    int *FTSIZE,
                                    FLOAT *scale) // calculate feature pyramid
{
  ...
  for (int ii = 0; ii < interval; ii++) {
    ...
    //"first" 2x interval
    ini_thread_data(&td[t_count], RIM_S[ii], RISIZE, sbin2,
    ...
    *(scale + ii) = st * 2; // save scale
    ...
    ini_thread_data(&td[t_count], RIM_S[ii + interval], RISIZE, sbin,
    ...
    *(scale + ii + interval) = st; // save scale
    ...
    for (int jj = ii + interval; jj < max_scale; jj += interval) {
      ...
      ini_thread_data(&td[t_count], RIM_S[jj + interval], RISIZE, sbin,
  ...
  for (int ss = 0; ss < interval; ss++)
    s_free(RIM_S[ss]);
  for (int ss = interval * 2; ss < LEN; ss++)
    s_free(RIM_S[ss]);
  s_free(RI_S);
  s_free(RIM_S);
