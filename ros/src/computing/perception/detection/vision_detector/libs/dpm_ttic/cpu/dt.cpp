...
static inline void dt_helper(FLOAT *src, FLOAT *dst, int *ptr, int step, int s1,
                             int s2, int d1, int d2, FLOAT a, FLOAT b) {
  if (d2 >= d1) {
    int d = (d1 + d2) >> 1;
    int ds = d * step;
    int s = s1;
    ...
    for (int p = s1 + 1; p <= s2; p++) {
      int t1 = d - s;
      int t2 = d - p;
      if (src_ss + a * t1 * t1 + b * t1 >
          *(src + p * step) + a * t2 * t2 + b * t2) {
        s = p;
        src_ss = *(src + s * step);
      }
    }
...
// sub function of dt
static void dt1d(FLOAT *src, FLOAT *dst, int *ptr, int step, int n, FLOAT a,
                 FLOAT b) {
  dt_helper(src, dst, ptr, step, 0, n - 1, 0, n - 1, a, b);
}

// Decide best filter position by dynamic programing
FLOAT *dpm_ttic_cpu_dt(FLOAT *vals, FLOAT ax, FLOAT bx, FLOAT ay, FLOAT by,
                       int *dims, int *Ix, int *Iy) {
