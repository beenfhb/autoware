...
static int *get_gmpc(FLOAT *score, FLOAT thresh, int *ssize,
                     int *good_matched) {
...
static FLOAT *rootbox(int x, int y, FLOAT scale, int pad_x, int pad_y,
                      int *root_size) {
...
static FLOAT *partbox(int x, int y, int ax, int ay, FLOAT scale, int padx,
                      int pady, int *psize, int *lx, int *ly, int *ssize) {
  FLOAT *Out = (FLOAT *)malloc(sizeof(FLOAT) * 4);
  int probex = (x - 1) * 2 + ax;
  int probey = (y - 1) * 2 + ay;
...
static void calc_a_score_GPU(FLOAT *ac_score, FLOAT **score, int *ssize_start,
                             Model_info *MI, FLOAT scale, int *size_score_array,
                             int NoC) {
  CUresult res;
  ...
  res = cuMemHostAlloc((void **)&RY_array, NoC * sizeof(int),
  ...
  res = cuMemHostAlloc((void **)&RX_array, NoC * sizeof(int),
                       CU_MEMHOSTALLOC_DEVICEMAP);
  if (res != CUDA_SUCCESS) {
  ...
  void *kernel_args[] = {(void *)&IWID,   (void *)&IHEI,   (void *)&scale,
                         (void *)&padx_n, (void *)&pady_n, &RX_dev,
                         &RY_dev,         &ac_score_dev,   &score_dev,
                         &ssize_dev,      (void *)&NoC,    &size_score_dev};
  ...
  if (IWID % thread_num_x != 0)
    block_num_x++;
  if (IHEI % thread_num_y != 0)
    block_num_y++;

  gettimeofday(&tv_kernel_start, nullptr);
  /* launch GPU kernel */
  res = cuLaunchKernel(func_calc_a_score[0], // call function
...
// detect boundary box
FLOAT *dpm_ttic_gpu_get_boxes(FLOAT **features, FLOAT *scales,
                              int *feature_size, GPUModel *MO,
                              int *detected_count, FLOAT *acc_score,
                              FLOAT thresh) {
  // constant parameters
  ...
  FLOAT **Tboxes = (FLOAT **)calloc(
      LofFeat, sizeof(FLOAT *)); // box coordinate information(Temp)
  ...
  FLOAT *dst_feat;
  res = cuMemHostAlloc((void **)&dst_feat, SUM_SIZE_feat,
                       CU_MEMHOSTALLOC_DEVICEMAP);
  if (res != CUDA_SUCCESS) {
  ...
  ///////level
  for (int level = interval; level < L_MAX;
       level++) // feature's loop(A's loop) 1level 1picture
  {
    if (feature_size[level * 2] + 2 * pady < MO->MI->max_Y ||
        (feature_size[level * 2 + 1] + 2 * padx < MO->MI->max_X)) {
  ...
  rootmatch = fconvsMT_GPU(featp2, SUM_SIZE_feat, rootfilter, rootsym, 1, NoR,
                           new_PADsize, RF_size, rm_size_array, L_MAX, interval,
                           feature_size, padx, pady, MO->MI->max_X,
                           MO->MI->max_Y, ROOT);
  ...
  if (NoP > 0) {
    ...
    partmatch = fconvsMT_GPU(featp2, SUM_SIZE_feat, partfilter, part_sym, 1,
                             NoP, new_PADsize, part_size, pm_size_array, L_MAX,
                             interval, feature_size, padx, pady, MO->MI->max_X,
                             MO->MI->max_Y, PART);
  ...
  for (int level = interval; level < L_MAX; level++) {
    ...
    for (int j = 0; j < NoC; j++) {
      ...
      /* root score + offset */
      RL_array[L][j] =
          rm_size_array[level][j * 2] *
          rm_size_array[level][j * 2 + 1]; // length of root-matching
  ...
  if (NoP > 0) {
    ...
    int **sub_dst_DID_4 =
        (int **)malloc(NoC * (L_MAX - interval) * sizeof(int *));
    int *dst_DID_4;
    res = cuMemHostAlloc((void **)&dst_DID_4, tmp_array_size,
                         CU_MEMHOSTALLOC_DEVICEMAP);
    if (res != CUDA_SUCCESS) {
    ...
    int **sub_dst_PIDX =
        (int **)malloc(NoC * (L_MAX - interval) * sizeof(int *));
    int *dst_PIDX;
    res = cuMemHostAlloc((void **)&dst_PIDX, tmp_array_size,
                         CU_MEMHOSTALLOC_DEVICEMAP);
    if (res != CUDA_SUCCESS) {
    ...
    for (int level = interval; level < L_MAX; level++) {
      ...
      for (int j = 0; j < NoC; j++) {
        for (int k = 0; k < numpart[j]; k++) {
          ...
          int PSSIZE[2] = {
              pm_size_array[L][PIDX_array[L][j][k] * 2],
    ...
    FLOAT ****M_array = dt_GPU(Ix_array,       // int ****Ix_array
      ...
      for (int j = 0; j < NoC; j++) {
        for (int k = 0; k < numpart[j]; k++) {
          int PSSIZE[2] = {
              pm_size_array[L][PIDX_array[L][j][k] * 2],
              pm_size_array[L][PIDX_array[L][j][k] * 2 + 1]}; // Size of C
          int R_S[2] = {rm_size_array[level][j * 2],
                        rm_size_array[level][j * 2 + 1]};

          dpm_ttic_add_part_calculation(SCORE_array[L][j], M_array[L][j][k],
                                        R_S, PSSIZE, ax_array[L][j][k],
                                        ay_array[L][j][k]);
        }
      }
    }
       ...
       level++) // feature's loop(A's loop) 1level 1picture
  {
    ...
    calc_a_score_GPU(acc_score,            // FLOAT *ac_score
    ...
    for (int j = 0; j < NoC; j++) {
      ...
      for (int k = 0; k < GMN; k++) {
        ...
        for (int pp = 0; pp < numpart[j]; pp++) {
          ...
          int Isize[2] = {pm_size_array[L][MO->MI->pidx[j][pp] * 2],
                          pm_size_array[L][MO->MI->pidx[j][pp] * 2 + 1]};

          /* calculate part box coordinate */
          FLOAT *PB = partbox(x, y, ax_array[L][j][pp], ay_array[L][j][pp],
                              scale, padx, pady, PBSIZE, Ix_array[L][j][pp],
                              Iy_array[L][j][pp], Isize);
  ...
  FLOAT *boxes = (FLOAT *)calloc(
      detected_boxes * GL, sizeof(FLOAT)); // box coordinate information(Temp)
