...
static FLOAT *detect(IplImage *IM, MODEL *MO, FLOAT thresh, int *D_NUMS,
                     FLOAT *A_SCORE) {
  ...
  FLOAT *boxes = dpm_ttic_cpu_get_boxes(feature, scales, featsize, MO, D_NUMS,
                                        A_SCORE, thresh);
...
RESULT *dpm_ttic_cpu_car_detection(IplImage *image, MODEL *model, FLOAT thresh,
                                   int *D_NUMS, FLOAT *A_SCORE, FLOAT overlap) {
  FLOAT *boxes =
      detect(image, model, thresh, D_NUMS, A_SCORE); // detect high-score region
  FLOAT *rects = dpm_ttic_cpu_nms(boxes, overlap, D_NUMS,
                                  model); // get boundary-rectangles of car
  RESULT *result = dpm_ttic_cpu_get_new_rects(image, model, rects,
                                              D_NUMS); // get current result
