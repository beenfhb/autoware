...
class LkTracker {
  ...
  void GetRectFromPoints(std::vector<cv::Point2f> in_corners_points,
                         cv::Rect &out_boundingbox);
  void ArrowedLine(cv::Mat &in_image, cv::Point in_point1, cv::Point in_point2,
                   const cv::Scalar &in_color, int in_thickness = 1,
                   int in_line_type = 8, int in_shift = 0,
                   double in_tip_length = 0.1);
...
public:
  ...
  LkTracker(int in_id, float in_min_height, float in_max_height,
            float in_range);
