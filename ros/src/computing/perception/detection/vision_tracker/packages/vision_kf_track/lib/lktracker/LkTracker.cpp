...
LkTracker::LkTracker(int in_id, float in_min_height, float in_max_height,
                     float in_range) {
  ...
  term_criteria_ =
      cv::TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, // type
                       criteria_max_iteration_, // max iteration count
                       criteria_epsilon_        // epsilon
      );
  sub_pixel_window_size_ =
      cv::Size(corner_subwindow_size_, corner_subwindow_size_);
...
ObjectDetection LkTracker::GetTrackedObject() { return current_rect_; }
void LkTracker::ArrowedLine(cv::Mat &in_image, cv::Point in_point1,
                            cv::Point in_point2, const cv::Scalar &in_color,
                            int in_thickness, int in_line_type, int in_shift,
                            double in_tip_length) {
  const double tipSize =
      cv::norm(in_point1 - in_point2) *
      in_tip_length; // Factor to normalize the size of the tip depending on the
                     // length of the arrow
  cv::line(in_image, in_point1, in_point2, in_color, in_thickness, in_line_type,
           in_shift);

  const double angle = atan2((double)in_point1.y - in_point2.y,
                             (double)in_point1.x - in_point2.x);
  cv::Point p(cvRound(in_point2.x + tipSize * cos(angle + CV_PI / 4)),
              cvRound(in_point2.y + tipSize * sin(angle + CV_PI / 4)));

  cv::line(in_image, p, in_point2, in_color, in_thickness, in_line_type,
           in_shift);
...
/*void OrbFeatures(cv::Mat in_image)
{
        ...
        cv::Mat descriptors;
        cv::Mat training_descriptors(1, extractor.descriptorSize(),
extractor.descriptorType()); extractor.compute(in_image, keypoints,
descriptors); training_descriptors.push_back(descriptors);
...
unsigned int LkTracker::GetRemainingLifespan() { return lifespan_; }

void LkTracker::NullifyLifespan() { lifespan_ = 0; }

unsigned long int LkTracker::GetFrameCount() { return frame_count_; }

cv::Mat LkTracker::Track(cv::Mat in_image, ObjectDetection in_detection,
                         bool in_update) {
  cv::Mat gray_image;
  ...
  if (in_update && in_detection.rect.width > 0) {
    ...
    if (matched_detection_.rect.x < 0)
      matched_detection_.rect.x = 0;
    if (matched_detection_.rect.y < 0)
      matched_detection_.rect.y = 0;
    if (matched_detection_.rect.x + matched_detection_.rect.width >
        in_image.cols)
      matched_detection_.rect.width = in_image.cols - matched_detection_.rect.x;
    if (matched_detection_.rect.y + matched_detection_.rect.height >
        in_image.rows)
      matched_detection_.rect.height =
          in_image.rows - matched_detection_.rect.y;
  ...
  if ((in_update || prev_image_.empty()) &&
      (matched_detection_.rect.width > 0 &&
       matched_detection_.rect.height > 0)) // add as new object
  {
    cv::goodFeaturesToTrack(
        gray_image,       // input to extract corners
    ...
    if (current_points_.size() <= 0) {
      ObjectDetection tmp_det;
      tmp_det.rect = cv::Rect(0, 0, 0, 0);
      tmp_det.score = 0;
      current_rect_ = tmp_det;
      return in_image;
    }
    cv::cornerSubPix(gray_image, current_points_, sub_pixel_window_size_,
                     cv::Size(-1, -1), term_criteria_);
    // frame_count_ = 0;
    current_centroid_x_ = 0;
    current_centroid_y_ = 0;
    // current_points_.push_back(cv::Point(matched_detection_.x,
    // matched_detection_.y));
    ...
    // std::cout << "CENTROID" << current_centroid_x_ <<","<<
    // current_centroid_y_<< std::endl << std::endl;
  ...
  } else if (!prev_points_.empty()) // try to match current object
  {
    ...
    cv::calcOpticalFlowPyrLK(prev_image_,     // previous image frame
                             ...
                             status, err, window_size_, 3, term_criteria_, 0,
                             0.001);
    std::size_t i = 0, k = 0;
    ...
    for (i = 0, k = 0; i < prev_points_.size(); i++) {
      ...
      cv::Point2f p, q;
      p.x = (int)prev_points_[i].x;
      p.y = (int)prev_points_[i].y;
      q.x = (int)current_points_[i].x;
      q.y = (int)current_points_[i].y;
  ...
  cv::kmeans(valid_points, 2, labels,
  ...
  cv::Point centroid(current_centroid_x_ / valid_points.size(),
                     current_centroid_y_ / valid_points.size());
  ...
  for (std::size_t i = 0; i < valid_points.size(); i++) {
    ...
    if (cv::norm(valid_points[i] - center1) <
            matched_detection_.rect.width * 0.8 &&
        cv::norm(valid_points[i] - center2) <
            matched_detection_.rect.width *
                0.8) // distance between point and cluster centroid
    {
  ...
  if (cv::norm(center2 - center1) >
      matched_detection_.rect.width * 0.75) // if centroids are too far keep
                                            // only the one with the most points
  {
  ...
  GetRectFromPoints(final_points, current_rect_.rect);
  ...
  if (prev_points_.size() > 0) {
    cv::Point center_point =
        cv::Point(current_rect_.rect.x + current_rect_.rect.width / 2,
                  current_rect_.rect.y + current_rect_.rect.height / 2);
    ...
    // ArrowedLine(in_image, center_point, direction_point, cv::Scalar(0,0,255),
    // 2);
...
void LkTracker::GetRectFromPoints(std::vector<cv::Point2f> in_corners_points,
                                  cv::Rect &out_boundingbox) {

  if (in_corners_points.empty()) {
    return;
  }

  int min_x = in_corners_points[0].x, min_y = in_corners_points[0].y,
      max_x = in_corners_points[0].x, max_y = in_corners_points[0].y;
