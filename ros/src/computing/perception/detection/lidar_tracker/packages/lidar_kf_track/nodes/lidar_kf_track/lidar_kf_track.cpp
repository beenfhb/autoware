...
KFLidarTracker::KFLidarTracker(float in_time_delta,
                               ...
                               bool pose_estimation, size_t maximum_track_id)
    : time_delta_(in_time_delta),
      ...
      maximum_trace_length_(maximum_trace_length), next_track_id_(0),
      pose_estimation_(false), maximum_track_id_(maximum_track_id) {}

void KFLidarTracker::CreatePolygonFromPoints(
    const geometry_msgs::Polygon &in_points, boost_polygon &out_polygon) {
  std::vector<boost_point_xy> hull_detection_points;

  for (size_t k = 0; k < in_points.points.size() / 2; k++) {
    hull_detection_points.push_back(
        boost_point_xy(in_points.points[k].x, in_points.points[k].y));
  }
...
void KFLidarTracker::Update(
  ...
  if (num_tracks == 0) {
    ...
    for (size_t i = 0; i < num_detections; ++i) {
      tracks_.push_back(CTrack(in_cloud_cluster_array.clusters[i], time_delta_,
  ...
  // else
  {
    // std::cout << "Trying to match " << num_tracks << " tracks with " <<
    ...
    for (size_t i = 0; i < num_detections; i++) {
      ...
      CreatePolygonFromPoints(
          in_cloud_cluster_array.clusters[i].convex_hull.polygon,
          hull_detection_polygon);
      detections_areas[i] = boost::geometry::area(hull_detection_polygon);

      for (size_t j = 0; j < num_tracks; j++) {
        // float current_distance =
        // tracks_[j].CalculateDistance(cv::Point2f(in_cloud_cluster_array.clusters[i].centroid_point.point.x,
        // in_cloud_cluster_array.clusters[i].centroid_point.point.y));
        float current_distance = sqrt(
            pow(tracks_[j].GetCluster().centroid_point.point.x -
                    in_cloud_cluster_array.clusters[i].centroid_point.point.x,
                2) +
            pow(tracks_[j].GetCluster().centroid_point.point.y -
                    in_cloud_cluster_array.clusters[i].centroid_point.point.y,
                2));

        // tracker polygon
        boost_polygon hull_track_polygon;
        CreatePolygonFromPoints(tracks_[j].GetCluster().convex_hull.polygon,
                                hull_track_polygon);

        // if(current_distance < current_distance_threshold)
        if (!boost::geometry::disjoint(hull_detection_polygon,
                                       hull_track_polygon) ||
            (current_distance <
             current_distance_threshold)) { // assign the closest detection or
                                            // overlapping
          current_distance_threshold = current_distance;
          track_assignments[j] = i; // assign detection i to track j
          track_assignments_vector[j].push_back(
              i); // add current detection as a match
          detections_assignments.push_back(
              j); ///////////////////////////////////////
        }
      }
    }
    ...
    for (size_t i = 0; i < num_tracks; i++) {
      if (track_assignments[i] >= 0) // if this track was assigned, update
                                     // kalman filter, reset remaining life
      {
        // keep oldest
        tracks_[i].skipped_frames = 0;

        // join all assigned detections to update the tracker
        /*autoware_msgs::CloudClusterPtr summed_cloud_cluster(new
        autoware_msgs::CloudCluster()); pcl::PointCloud<pcl::PointXYZ>::Ptr
        summed_cloud_ptr(new pcl::PointCloud<pcl::PointXYZ>); for (size_t j = 0;
        j < track_assignments_vector[i].size(); j++)
        {
                pcl::PointCloud<pcl::PointXYZ>::Ptr current_cloud_ptr(new
        pcl::PointCloud<pcl::PointXYZ>);
                pcl::fromROSMsg(in_cloud_cluster_array.clusters[track_assignments_vector[i][j]].cloud,
        *current_cloud_ptr); *summed_cloud_ptr += *current_cloud_ptr;
        }
        ...
        merged_cluster->SetCloud(summed_cloud_ptr, indices,
        in_cloud_cluster_array.clusters[track_assignments[i]].header, i,255,
        255, 255, "", pose_estimation_);

        merged_cluster->ToROSMessage(in_cloud_cluster_array.clusters[track_assignments[i]].header,
        *summed_cloud_cluster);*/

        tracks_[i].Update(
            in_cloud_cluster_array
                .clusters[track_assignments[i]], //*summed_cloud_cluster,
            true, maximum_trace_length_);
    ...
    for (size_t i = 0; i < num_detections; ++i) {
      std::vector<size_t>::iterator it =
          find(detections_assignments.begin(), detections_assignments.end(), i);
      if (it ==
          detections_assignments.end()) // if detection not found in the already
                                        // assigned ones, add new tracker
      {
        tracks_.push_back(CTrack(in_cloud_cluster_array.clusters[i],
                                 time_delta_, acceleration_noise_magnitude_,
                                 next_track_id_++));
    ...
    // finally check trackers among them. Remove previously merged objects,
    // causing ID duplication
    for (size_t i = 0; i < tracks_.size(); i++) {
      for (size_t j = 0; j < tracks_.size(); j++) {
        if (i != j && (tracks_[i].GetCluster().centroid_point.point.x ==
                           tracks_[j].GetCluster().centroid_point.point.x &&
                       tracks_[i].GetCluster().centroid_point.point.y ==
                           tracks_[j].GetCluster().centroid_point.point.y &&
                       tracks_[i].GetCluster().centroid_point.point.z ==
                           tracks_[j].GetCluster().centroid_point.point.z)) {
...
void KFLidarTracker::CheckTrackerMerge(
    size_t in_tracker_id, std::vector<CTrack> &in_trackers,
  ...
  for (size_t i = 0; i < in_trackers.size(); i++) {
    if (i != in_tracker_id && !in_out_visited_trackers[i]) {
      double distance = sqrt(
          pow(in_trackers[in_tracker_id].GetCluster().centroid_point.point.x -
                  in_trackers[i].GetCluster().centroid_point.point.x,
              2) +
          pow(in_trackers[in_tracker_id].GetCluster().centroid_point.point.y -
                  in_trackers[i].GetCluster().centroid_point.point.y,
              2));
      boost_polygon in_tracker_poly;
      CreatePolygonFromPoints(
          in_trackers[in_tracker_id].GetCluster().convex_hull.polygon,
          in_tracker_poly);
      in_trackers[in_tracker_id].area = boost::geometry::area(in_tracker_poly);

      boost_polygon current_tracker_poly;
      CreatePolygonFromPoints(in_trackers[i].GetCluster().convex_hull.polygon,
                              current_tracker_poly);
      ...
      if (!boost::geometry::disjoint(in_tracker_poly, current_tracker_poly) ||
        ...
        CheckTrackerMerge(i, in_trackers, in_out_visited_trackers,
...
void KFLidarTracker::MergeTrackers(std::vector<CTrack> &in_trackers,
...
void KFLidarTracker::CheckAllTrackersForMerge(
    std::vector<CTrack> &out_trackers) {
  ...
  for (size_t i = 0; i < tracks_.size(); i++) {
    if (!visited_trackers[i]) {
      ...
      CheckTrackerMerge(i, tracks_, visited_trackers, merge_indices,
                        tracker_merging_threshold_);
      MergeTrackers(tracks_, out_trackers, merge_indices, current_index++,
                    merged_trackers);
    }
  }
  for (size_t i = 0; i < tracks_.size(); i++) {
        ...
        if (!tracks_.empty())
        {
                ...
                case CentersDist:
                        for (size_t i = 0; i < tracks_.size(); i++)
                        {
                                ...
                                {
                                        cost_matrix[i + j * N] =
tracks_[i].CalculateDistance(cv::Point2f(in_cloud_cluster_array.clusters[j].centroid_point.point.x,
in_cloud_cluster_array.clusters[j].centroid_point.point.y));
                                }
                        }
                        break;
                ...
                case RectsDist:
                        for (size_t i = 0; i < tracks_.size(); i++)
                        {
                                ...
                                {
                                        cost_matrix[i + j * N] =
tracks_[i].CalculateDistance(
cv::Rect_<float>(in_cloud_cluster_array.clusters[i].centroid_point.point.x -
in_cloud_cluster_array.clusters[i].bounding_box.dimensions.x/2,
                ...
                // -----------------------------------
                // Solving assignment problem (tracks and predictions of Kalman
filter)
                // -----------------------------------
                AssignmentProblemSolver APS;
                std::cout << "Hungarian Algorithm Start"<< std::endl;
                APS.Solve(cost_matrix, N, M, assignment,
AssignmentProblemSolver::optimal); std::cout << "Hungarian Algorithm End"<<
std::endl;
                // -----------------------------------
                ...
                for (size_t i = 0; i < assignment.size(); i++)
                {
                        if (assignment[i] != -1)
                        {
                                if (cost_matrix[i + assignment[i] * N] >
distance_threshold_)
                                {
                                        assignment[i] = -1;
                                        tracks_[i].skipped_frames = 1;
                                        std::cout << "Existing track Not matched
" << i << " Distance:" << cost_matrix[i + assignment[i] * N] << std::endl;
                                }
                        }
                        else
                        {
                                // If track have no assigned detect, then
increment skipped frames counter. tracks_[i].skipped_frames++; std::cout <<
"Existing track Not matched " << i <<", dying " << tracks_[i].skipped_frames <<
std::endl;
                        }
                }
...
maximum_allowed_skipped_frames_)
        ...
        for (size_t i = 0; i < detections_num; ++i)
        {
                if (find(assignment.begin(), assignment.end(), i) ==
assignment.end())
                {
                        std::cout << "New object, Not matched " << i <<
std::endl; tracks_.push_back(CTrack(in_cloud_cluster_array.clusters[i],
                                                                        time_delta_,
                                                                        acceleration_noise_magnitude_,
                                                                        next_track_id_++)
                                                        );
...
correct.
                ...
                if (assignment[i] != -1) // If we have assigned detect, then
update using its coordinates,
                {
                        tracks_[i].skipped_frames = 0;
                        std::cout << "Matched. Kalman Update on " << i <<
std::endl; tracks_[i].Update(	in_cloud_cluster_array.clusters[i], true,
                                                                maximum_trace_length_);
                }
                else				     // if not continue using
predictions
                {
                        std::cout << "Not Matched. Kalman Update on " << i <<
std::endl; tracks_[i].Update(autoware_msgs::CloudCluster(), false,
maximum_trace_length_);
                }
        }
