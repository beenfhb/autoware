...
KFLidarTrackNode::KFLidarTrackNode()
    : node_handle_("~"), pose_estimation_(false) {
  cloud_clusters_sub_ =
      node_handle_.subscribe("/cloud_clusters_class", 10,
                             &KFLidarTrackNode::CloudClustersCallback, this);
  pub_detected_objects_ =
      node_handle_.advertise<autoware_msgs::DetectedObjectArray>(
          "/detected_objects", 10);
  pub_jsk_tracked_objects_ =
      node_handle_.advertise<jsk_recognition_msgs::BoundingBoxArray>(
          "/bounding_boxes_tracked", 1);
  pub_jsk_hulls_ = node_handle_.advertise<jsk_recognition_msgs::PolygonArray>(
      "/cluster_hulls_tracked", 1);
  pub_jsk_pictograms_ =
      node_handle_.advertise<jsk_rviz_plugins::PictogramArray>(
          "/cluster_ids_tracked", 1);

  node_handle_.param("distance_matching_threshold",
                     distance_matching_threshold_, 1.5);
  ROS_INFO("distance_matching_threshold: %f",
           distance_matching_threshold_); // distance threshold to match objects
                                          // between scans
  node_handle_.param("tracker_merging_threshold", tracker_merging_threshold_,
                     1.0);
  ROS_INFO("tracker_merging_threshold: %f",
           tracker_merging_threshold_); // distance threshold to match objects
                                        // between scans
  node_handle_.param("pose_estimation", pose_estimation_, false);
  ROS_INFO("pose_estimation: %d",
           pose_estimation_); // whether or not to estimate pose
  ...
  ROS_INFO("maximum_track_id: %d",
           maximum_track_id_); // frames to keep an object

  tracker_ptr = boost::shared_ptr<KFLidarTracker>(new KFLidarTracker(
      0.2f,                         // dt
...
void KFLidarTrackNode::CloudClustersCallback(
    const autoware_msgs::CloudClusterArray::Ptr &in_cloud_cluster_array_ptr) {
  ...
  for (unsigned int i = 0; i < tracker_ptr->tracks_.size(); i++) {
    ...
    tracked_pictogram.pose.position.x =
        tracker_ptr->tracks_[i].GetCluster().max_point.point.x;
    tracked_pictogram.pose.position.y =
        tracker_ptr->tracks_[i].GetCluster().max_point.point.y;
    tracked_pictogram.pose.position.z =
        tracker_ptr->tracks_[i].GetCluster().max_point.point.z;
    ...
    tracked_pictogram.character =
        std::to_string(tracker_ptr->tracks_[i].track_id);
  ...
  // autoware_msgs::DetectedObjectArray detected_objects;
  // detected_objects.header = in_cloud_cluster_array_ptr->header;
  // for (auto i = in_cloud_cluster_array_ptr->clusters.begin(); i !=
  // in_cloud_cluster_array_ptr->clusters.end(); i++)
  //{
