...
TKalmanFilter::TKalmanFilter(cv::Point2f pt, float dt, float Accel_noise_mag) {
  ...
  // But we can guess, the range of acceleration values thich can be achieved by
  // tracked object.
  ...
  kalman.transitionMatrix = (cv::Mat_<float>(4, 4) << 1, 0, deltatime, 0, 0, 1,
                             0, deltatime, 0, 0, 1, 0, 0, 0, 0, 1);
  ...
  kalman.processNoiseCov =
      (cv::Mat_<float>(4, 4) << pow(deltatime, 4.0) / 4.0, 0,
       pow(deltatime, 3.0) / 2.0, 0, 0, pow(deltatime, 4.0) / 4.0, 0,
       pow(deltatime, 3.0) / 2.0, pow(deltatime, 3.0) / 2.0, 0,
       pow(deltatime, 2.0), 0, 0, pow(deltatime, 3.0) / 2.0, 0,
       pow(deltatime, 2.0));
