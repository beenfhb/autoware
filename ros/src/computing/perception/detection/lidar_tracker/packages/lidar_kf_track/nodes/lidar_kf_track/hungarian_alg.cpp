...
float AssignmentProblemSolver::Solve(const std::vector<float> &distMatrixIn,
...
void AssignmentProblemSolver::assignmentoptimal(
    std::vector<int> &assignment, float &cost,
    const std::vector<float> &distMatrixIn, size_t nOfRows, size_t nOfColumns) {
  ...
  std::vector<float> distMatrix(
      nOfElements,
      0.0); // float* distMatrix = (float *)malloc(nOfElements * sizeof(float));
  ...
  std::vector<bool> coveredColumns(
      nOfColumns,
      false); // bool* coveredColumns = (bool*)calloc(nOfColumns, sizeof(bool));
  std::vector<bool> coveredRows(
      nOfRows,
      false); // bool* coveredRows = (bool*)calloc(nOfRows, sizeof(bool));
  std::vector<bool> starMatrix(
      nOfElements,
      false); // bool* starMatrix = (bool*)calloc(nOfElements, sizeof(bool));
  std::vector<bool> primeMatrix(
      nOfElements,
      false); // bool* primeMatrix = (bool*)calloc(nOfElements, sizeof(bool));
  std::vector<bool> newStarMatrix(
      nOfElements, false); // bool* newStarMatrix = (bool*)calloc(nOfElements,
                           // sizeof(bool)); /* used in step4 */
  ...
  step2b(assignment, distMatrix, starMatrix, newStarMatrix, primeMatrix,
         coveredColumns, coveredRows, nOfRows, nOfColumns,
         (nOfRows <= nOfColumns) ? nOfRows : nOfColumns);
...
void AssignmentProblemSolver::buildassignmentvector(
    std::vector<int> &assignment, std::vector<bool> &starMatrix, size_t nOfRows,
    size_t nOfColumns) {
  for (size_t row = 0; row < nOfRows; row++) {
    for (size_t col = 0; col < nOfColumns; col++) {
...
void AssignmentProblemSolver::computeassignmentcost(
    const std::vector<int> &assignment, float &cost,
    const std::vector<float> &distMatrixIn, size_t nOfRows) {
  for (size_t row = 0; row < nOfRows; row++) {
...
void AssignmentProblemSolver::assignmentsuboptimal2(
  ...
  const size_t nOfElements = nOfRows * nOfColumns;
  std::vector<float> distMatrix(
      nOfElements, 0.0); //= (float*)malloc(nOfElements * sizeof(float));
  for (size_t n = 0; n < nOfElements; n++) {
...
// --------------------------------------------------------------------------
// Computes a suboptimal solution. Good for cases with many forbidden
// assignments.
// --------------------------------------------------------------------------
void AssignmentProblemSolver::assignmentsuboptimal1(
  ...
  /* allocate memory */
  std::vector<int> nOfValidObservations(
      nOfRows,
      0); // int* nOfValidObservations = (int *)calloc(nOfRows, sizeof(int));
  std::vector<int> nOfValidTracks(
      nOfColumns,
      0); // int* nOfValidTracks = (int *)calloc(nOfColumns, sizeof(int));
  ...
  if (infiniteValueFound) {
    ...
    while (repeatSteps) {
      ...
      /* step 1: reject assignments of multiply validated tracks to singly
      ...
      /* step 2: reject assignments of multiply validated observations to singly
       * validated tracks */
      if (nOfColumns > 1) {
        for (size_t row = 0; row < nOfRows; row++) {
          bool singleValidationFound = false;
    ...
    /* for each multiply validated track that validates only with singly
     * validated  */
    ...
    // for each multiply validated observation that validates only with singly
    // validated  track, choose the track with minimum distance
    for (size_t col = 0; col < nOfColumns; col++) {
      if (nOfValidTracks[col] > 1) {
        bool allSinglyValidated = true;
        float minValue = std::numeric_limits<float>::max();
