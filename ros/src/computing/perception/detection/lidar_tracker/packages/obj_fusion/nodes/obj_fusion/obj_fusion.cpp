...
enum ObjLabel { Car, Person, Unknown };
...
void fusion_cb(const autoware_msgs::ObjLabel::ConstPtr &obj_label_msg,
               const autoware_msgs::CloudClusterArray::ConstPtr
                   &in_cloud_cluster_array_ptr) {
  tf::StampedTransform tform;
  tf::TransformListener tflistener;
  try {
    ros::Time latest_time = ros::Time(0);
    tflistener.waitForTransform("/map", "/velodyne", latest_time,
                                ros::Duration(10));
  ...
  if (centroids.empty() || obj_label.reprojected_positions.empty() ||
  ...
  for (unsigned int i = 0; i < obj_label.obj_id.size(); ++i) {
    ...
    for (unsigned int j = 0; j < centroids.size(); ++j) {
      double distance = euclid_distance(obj_label.reprojected_positions.at(i),
                                        centroids.at(j));
  ...
  for (unsigned int i = 0; i < obj_label.obj_id.size(); ++i) {
    ...
    if (use_vmap) {
      ...
      ROS_INFO("pos x=%f y=%f z=%f", get_lane.request.pose.pose.position.x,
      ...
      if (vmap_server.call(get_lane)) {
        for (const auto &lane : get_lane.response.objects.data) {
          ...
          if ((mx - fp.ly) * (mx - fp.ly) + (my - fp.bx) * (my - fp.bx) <
              vmap_threshold) {
            fixed_rotation = true;
            ...
            ROS_INFO(" %d roll=%f pitch=%f yaw=%f", rot_n * 90, roll, pitch,
                     yaw);
            break;
          }
        }
...
int main(int argc, char *argv[]) {
  ...
  typedef message_filters::sync_policies::ApproximateTime<
      autoware_msgs::ObjLabel, autoware_msgs::CloudClusterArray>
      SyncPolicy;
  message_filters::Subscriber<autoware_msgs::ObjLabel> obj_label_sub(
      n, "obj_label", SUBSCRIBE_QUEUE_SIZE);
  message_filters::Subscriber<autoware_msgs::CloudClusterArray>
      cluster_centroids_sub(n, "/cloud_clusters", SUBSCRIBE_QUEUE_SIZE);
  ...
  marker_array_pub =
      n.advertise<visualization_msgs::MarkerArray>("obj_pose_arrow", 1, true);
  vmap_server = n.serviceClient<vector_map_server::GetLane>(
      "/vector_map_server/get_lane");
