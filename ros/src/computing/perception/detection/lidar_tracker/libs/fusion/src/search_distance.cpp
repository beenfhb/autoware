...
float getMedian(const std::vector<float> &candidates) {
  ...
  if (num_candidates % 2 == 0) {
    median = (sorted_candidates.at(num_candidates / 2 - 1) +
              sorted_candidates.at(num_candidates / 2)) /
             2;
  } else {
...
float getMode(const std::vector<float> &candidates) {
  ...
  /*
    ...
    key (first) of map = bin (category) of histgram (bin means integer distance
    of candidates here) value (second) of map = candidate's real value which
