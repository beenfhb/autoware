...
public:
  /* \brief Constructor. Creates a Cluster object using the specified points in
   * a PointCloud \param[in] in_origin_cloud_ptr 	Origin PointCloud
   * \param[in] in_cluster_indices 	Indices of the Origin Pointcloud to create
   * the Cluster \param[in] in_id 				ID of the
   * cluster \param[in] in_r 					Amount of Red
   * [0-255] \param[in] in_g 					Amount of Green
   * [0-255] \param[in] in_b 					Amount of Blue
   * [0-255]
   * \param[in] in_label 				Label to identify this cluster
   * (optional) \param[in] in_estimate_pose		Flag to enable Pose
  ...
  void SetCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_origin_cloud_ptr,
                ...
                std_msgs::Header in_ros_header, int in_id, int in_r, int in_g,
                int in_b, std::string in_label, bool in_estimate_pose);

  /* \brief Returns the autoware_msgs::CloudCluster message associated to this
   * Cluster */
  ...
  /* \brief Returns the pointer to the PointCloud containing the points in this
   * Cluster */
  ...
  /* \brief Returns the angle in radians of the BoundingBox. 0 if pose
  ...
  /* \brief Returns a pointer to a PointCloud object containing the merged
   * points between current Cluster and the specified PointCloud \param[in]
   * in_cloud_ptr 	Origin PointCloud
   * */
  pcl::PointCloud<pcl::PointXYZ>::Ptr
  JoinCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr);

  /* \brief Calculates and returns a pointer to the FPFH Descriptor of this
   * cluster
   *
   */
  std::vector<float> GetFpfhDescriptor(const unsigned int &in_ompnum_threads,
