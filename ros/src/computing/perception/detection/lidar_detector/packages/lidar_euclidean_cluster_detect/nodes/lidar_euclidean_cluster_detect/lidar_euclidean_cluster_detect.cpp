...
tf::StampedTransform findTransform(const std::string &in_target_frame,
  ...
  try {
    ...
    _vectormap_transform_listener->lookupTransform(
...
bool checkPointInGrid(const grid_map::GridMap &in_grid_map,
  ...
  double cv_x = (in_grid_map.getLength().y() - origin_y_offset - in_point.y) /
                in_grid_map.getResolution();
  double cv_y = (in_grid_map.getLength().x() - origin_x_offset - in_point.x) /
                in_grid_map.getResolution();

  // check coords are inside the gridmap
  if (cv_x < 0 || cv_x > in_grid_image.cols || cv_y < 0 ||
      cv_y > in_grid_image.rows) {
    return false;
  }
...
void publishCloudClusters(const ros::Publisher *in_publisher,
  ...
  if (in_target_frame != in_header.frame_id) {
    ...
    for (auto i = in_clusters.clusters.begin(); i != in_clusters.clusters.end();
      ...
      try {
        _transform_listener->lookupTransform(in_target_frame,
                                             _velodyne_header.frame_id,
                                             ros::Time(), *_transform);
        pcl_ros::transformPointCloud(in_target_frame, *_transform, i->cloud,
                                     cluster_transformed.cloud);
        _transform_listener->transformPoint(in_target_frame, ros::Time(),
...
clusterAndColorGpu(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr,
  ...
  for (auto it = cluster_indices.begin(); it != cluster_indices.end(); it++) {
    ...
    cluster->SetCloud(in_cloud_ptr, it->points_in_cluster, _velodyne_header, k,
                      (int)_colors[k].val[0], (int)_colors[k].val[1],
...
void mergeClusters(const std::vector<ClusterPtr> &in_clusters,
  ...
  if (sum_cloud.points.size() > 0) {
    ...
    merged_cluster->SetCloud(
        mono_cloud.makeShared(), indices, _velodyne_header, current_index,
  ...
  if (!_use_multiple_thres) {
    ...
    if (_use_gpu) {
      all_clusters = clusterAndColorGpu(cloud_ptr, out_cloud_ptr,
                                        in_out_centroids, _clustering_distance);
    } else {
  ...
  if (_use_vector_map) {
    if (_wayarea_gridmap.exists(_gridmap_layer)) {
      ...
      grid_map::GridMapCvConverter::toImage<unsigned char, 1>(
          _wayarea_gridmap, _gridmap_layer, CV_8UC1, _grid_min_value,
          _grid_max_value, grid_image);
      ...
      for (unsigned int i = 0; i < final_clusters.size(); i++) {
        ...
        if (_wayarea_gridmap.getFrameId() != _velodyne_header.frame_id) {
          tf::StampedTransform grid_sensor_tf = findTransform(
              _wayarea_gridmap.getFrameId(), _velodyne_header.frame_id);
          final_centroid_point =
              transformPoint(original_centroid_point, grid_sensor_tf);
        } else {
          final_centroid_point = original_centroid_point;
        }

        bool point_in_grid = checkPointInGrid(_wayarea_gridmap, grid_image,
                                              final_centroid_point);
        final_clusters[i]->SetValidity(point_in_grid);
      }
      // timer.stop();
      // std::cout << "vectormap filtering took " << timer.getTimeMilli() << "
      // ms to check " << final_clusters.size() << std::endl;
    } else {
      ROS_INFO("%s layer not contained in the OccupancyGrid",
               _gridmap_layer.c_str());
    }
  }
...
void velodyne_callback(
  ...
  if (!_using_sensor_cloud) {
    ...
    clipCloud(downsampled_cloud_ptr, clipped_cloud_ptr, _clip_min_height,
              _clip_max_height);

    if (_keep_lanes)
      keepLanePoints(clipped_cloud_ptr, inlanes_cloud_ptr,
                     _keep_lane_left_distance, _keep_lane_right_distance);
    else
    ...
    publishCentroids(&_centroid_pub, centroids, _output_frame,
                     _velodyne_header);

    cloud_clusters.header = _velodyne_header;

    publishCloudClusters(&_pub_clusters_message, cloud_clusters, _output_frame,
                         _velodyne_header);
...
int main(int argc, char **argv) {
  ...
  _pub_cluster_cloud =
      h.advertise<sensor_msgs::PointCloud2>("/points_cluster", 1);
  _pub_ground_cloud =
      h.advertise<sensor_msgs::PointCloud2>("/points_ground", 1);
  _centroid_pub =
      h.advertise<autoware_msgs::Centroids>("/cluster_centroids", 1);

  _pub_points_lanes_cloud =
      h.advertise<sensor_msgs::PointCloud2>("/points_lanes", 1);
  _pub_clusters_message = h.advertise<autoware_msgs::CloudClusterArray>(
      "/detection/lidar_detector/cloud_clusters", 1);
  _pub_detected_objects = h.advertise<autoware_msgs::DetectedObjectArray>(
      "/detection/lidar_detector/objects", 1);

  _pub_grid_map =
      h.advertise<grid_map_msgs::GridMap>("grid_map_wayarea", 1, true);
  ...
  private_nh.param("clustering_distances", str_distances,
                   std::string("[0.5,1.1,1.6,2.1,2.6]"));
  ROS_INFO("clustering_distances: %s", str_distances.c_str());
  private_nh.param("clustering_ranges", str_ranges,
  ...
  private_nh.param<std::string>("wayarea_gridmap_topic", gridmap_topic,
                                "grid_map_wayarea");
  ROS_INFO("wayarea_gridmap_topic: %s", gridmap_topic.c_str());
  private_nh.param<std::string>("wayarea_gridmap_layer", _gridmap_layer,
                                "wayarea");
  ROS_INFO("wayarea_gridmap_layer: %s", _gridmap_layer.c_str());
  private_nh.param<int>("wayarea_no_road_value", _gridmap_no_road_value,
                        _grid_max_value);
  ROS_INFO("wayarea_no_road_value: %ds", _gridmap_no_road_value);
  ros::Subscriber wayarea_sub =
      h.subscribe(gridmap_topic, 1, wayarea_gridmap_callback);
