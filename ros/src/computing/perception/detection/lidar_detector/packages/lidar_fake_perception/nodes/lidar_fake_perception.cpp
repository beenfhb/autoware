...
LidarFakePerception::LidarFakePerception() : nh_(), private_nh_("~") {
  ...
  private_nh_.param<double>("object_angular_velocity", object_angular_velocity_,
                            0.0);
  ...
  object_initial_pose_sub_ =
      nh_.subscribe("/move_base_simple/goal", 1,
                    &LidarFakePerception::objectInitialPoseCallback, this);
  real_objects_sub_ = nh_.subscribe(
      "/detected_objects", 1, &LidarFakePerception::objectsCallback, this);
  real_points_sub_ = nh_.subscribe("/points_raw", 1,
                                   &LidarFakePerception::pointsCallback, this);
  fake_twist_sub_ = nh_.subscribe("/fake_twist", 1,
                                  &LidarFakePerception::twistCallback, this);

  fake_objects_pub_ = nh_.advertise<autoware_msgs::DetectedObjectArray>(
      "/detection/fake_perception/objects", 1);
...
void LidarFakePerception::objectInitialPoseCallback(
...
void LidarFakePerception::objectsCallback(
    const autoware_msgs::DetectedObjectArray &msg) {
  real_objects_ = msg;
}
...
void LidarFakePerception::updateFakes() {
  ...
  if (real_objects_.objects.size() != 0) {
    ...
    std::copy(real_objects_.objects.begin(), real_objects_.objects.end(),
              fake_objects_.objects.begin());
    fake_object_id_ =
        (real_objects_.objects.end() - 1)->id + 1; // using incremented id
  }
...
bool LidarFakePerception::updateFakeObject() {
  ...
  if (object_lifetime_ > 0) {
    if ((ros::Time::now() - fake_object_initial_time_).toSec() >
        object_lifetime_) {
      return false;
    }
  }
  ...
  try {
    tf_listener_.waitForTransform(global_frame_, object_frame_, ros::Time(0),
                                  ros::Duration(1.0));
    tf_listener_.lookupTransform(global_frame_, object_frame_, ros::Time(0),
                                 global2local);
  ...
  fake_object_pose_.setOrigin(tf::Vector3(
      objpos.x(), objpos.y(), object_z_offset_ + object_height_ / 2.));
  ...
  fake_object_.acceleration =
      geometry_msgs::Twist(); // NOTE: by constant velocity model
  tf::poseTFToMsg(global2local.inverse() * fake_object_pose_,
                  fake_object_.pose); // local

  return true;
}

void LidarFakePerception::updatePose(const double &dt,
                                     const geometry_msgs::Twist &twist,
                                     tf::Transform &tf) {
  // update pose by constant velocity model
  tf::Transform dpose;
  tf::Quaternion dquat;
  dpose.setOrigin(tf::Vector3(twist.linear.x * dt, twist.linear.y * dt,
                              twist.linear.z * dt));
  dquat.setRPY(twist.angular.x * dt, twist.angular.y * dt,
               twist.angular.z * dt);
...
void LidarFakePerception::convertObjectToPoints(
    const autoware_msgs::DetectedObject &obj, PointCloudT &points) {
  points.clear();
