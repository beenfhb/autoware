...
private:
  ...
  // publisher
  ros::Publisher
      fake_objects_pub_; // output objects, input and fake object are merged
  ros::Publisher
      fake_points_pub_; // output points, input and fake points are merged
  ...
  ros::Subscriber
      fake_twist_sub_; // input twist command, controlling fake object
  ...
  double
      object_velocity_; // constant velocity instead of subscribed twist [m/s]
  double object_angular_velocity_; // constant angular velocity instead of
                                   // subscribed twist [rad/s]

  // param, object meta-info
  double
      object_intensity_;   // constant intensity value of fake points, 0-255 [-]
  double object_lifetime_; // fake object lifetime (NOTE: when this is negative
                           // value, lifetime is inifinity) [s]
  double object_points_space_; // fake points space [m]
  std::string object_label_;   // fake object label (e.g. tracking state)
  std::string object_frame_;   // fake object frame_id (NOTE: not affected to
                               // input object)
  ...
  void updatePose(const double &dt, const geometry_msgs::Twist &twist,
                  tf::Transform &tf);
  void updateFakePoints();
  void convertObjectToPoints(const autoware_msgs::DetectedObject &obj,
                             PointCloudT &points);
  void publishFakes();
};
