...
public:
  ImageViewHandler(size_t w, size_t h)
      : linked_view_handler(0), rview_default(-0.5, w - 0.5, h - 0.5, 0 - 0.5),
        rview_max(-0.5, w - 0.5, -0.5, h - 0.5), rview(rview_default),
        target(rview), use_nn(false) {}
  ...
  void ScreenToImage(Viewport &v, int xpix, int ypix, float &ximg,
                     float &yimg) {
    ximg = rview.x.min + rview.x.Size() * (xpix - v.l) / (float)v.w;
    yimg = rview.y.min + rview.y.Size() * (ypix - v.b) / (float)v.h;
  }

  bool UseNN() const { return use_nn; }

  pangolin::XYRangef &GetViewToRender() { return rview; }

  pangolin::XYRangef &GetView() { return target; }

  pangolin::XYRangef &GetDefaultView() { return rview_default; }

  pangolin::XYRangef &GetSelection() { return selection; }
  ...
  void Mouse(View &view, pangolin::MouseButton button, int x, int y,
             bool pressed, int button_state) PANGOLIN_OVERRIDE {
    XYRangef &sel =
        linked_view_handler ? linked_view_handler->selection : selection;
  ...
  void MouseMotion(View &view, int x, int y,
                   int button_state) PANGOLIN_OVERRIDE {
    XYRangef &sel =
        linked_view_handler ? linked_view_handler->selection : selection;
    ...
    if (button_state == MouseButtonLeft) {
      ...
      Special(view, InputSpecialScroll, x, y, d[0], -d[1], 0.0f, 0.0f,
              button_state);
    }
  ...
  void PassiveMouseMotion(View &, int x, int y,
                          int button_state) PANGOLIN_OVERRIDE {}

  void Special(View &view, pangolin::InputSpecial inType, float x, float y,
    ...
    if (inType == InputSpecialScroll) {
      ...
      const float df[2] = {is[0] * d[0] / (float)view.v.w,
                           is[1] * d[1] / (float)view.v.h};
...
protected:
  ...
  void AdjustScale() {
    ...
    if (tv.target.x.AbsSize() > tv.rview_max.x.AbsSize())
      tv.target.x.Scale(tv.rview_max.x.AbsSize() / tv.target.x.AbsSize(),
                        tv.target.x.Mid());
    if (tv.target.y.AbsSize() > tv.rview_max.y.AbsSize())
      tv.target.y.Scale(tv.rview_max.y.AbsSize() / tv.target.y.AbsSize(),
                        tv.target.y.Mid());
  }

  void AdjustTranslation() {
    ImageViewHandler &tv = linked_view_handler ? *linked_view_handler : *this;
    if (tv.target.x.max > tv.rview_max.x.max)
      tv.target.x -= tv.target.x.max - tv.rview_max.x.max;
    if (tv.target.x.min < tv.rview_max.x.min)
      tv.target.x -= tv.target.x.min - tv.rview_max.x.min;
    if (tv.target.y.min > tv.rview_max.y.max)
      tv.target.y -= tv.target.y.min - tv.rview_max.y.max;
    if (tv.target.y.max < tv.rview_max.y.min)
      tv.target.y -= tv.target.y.max - tv.rview_max.y.min;
  }
