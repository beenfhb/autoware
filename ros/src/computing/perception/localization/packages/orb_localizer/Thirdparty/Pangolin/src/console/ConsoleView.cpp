...
inline Colour ParseJson(const json::value &val) {
  return Colour(val.contains("r") ? val["r"].get<double>() : 0.0,
...
ConsoleView::ConsoleView(ConsoleInterpreter *interpreter)
    : interpreter(interpreter), font(GlFont::I()), hiding(false), bottom(1.0f),
      background_colour(0.2f, 0.0f, 0.0f, 0.6f), animation_speed(0.2) {
  SetHandler(this);
  ...
  Var<Colour>::Attach("pango.console.colours.Cmd",
                      line_colours[ConsoleLineTypeCmd]);
  Var<Colour>::Attach("pango.console.colours.CmdOptions",
                      line_colours[ConsoleLineTypeCmdOptions]);
  Var<Colour>::Attach("pango.console.colours.Stdout",
                      line_colours[ConsoleLineTypeStdout]);
  Var<Colour>::Attach("pango.console.colours.Stderr",
                      line_colours[ConsoleLineTypeStderr]);
  Var<Colour>::Attach("pango.console.colours.Output",
                      line_colours[ConsoleLineTypeOutput]);
  Var<Colour>::Attach("pango.console.colours.Help",
                      line_colours[ConsoleLineTypeHelp]);
...
ConsoleView::~ConsoleView() { delete interpreter; }
...
void ConsoleView::ToggleShow() { Show(!IsShown()); }
...
void ConsoleView::Render() {
  ...
  GLfloat verts[] = {0.0f,         (GLfloat)v.h, (GLfloat)v.w, (GLfloat)v.h,
                     (GLfloat)v.w, bottom * v.h, 0.0f,         bottom * v.h};
...
void ConsoleView::Keyboard(View &, unsigned char key, int x, int y,
                           bool pressed) {
  ...
  if (pressed) {
    ...
    if (key == '`') {
      ...
      txt =
          font.Text("%s", txt.Text().substr(0, txt.Text().size() - 1).c_str());
      edited = true;
...
ConsoleView::Line *ConsoleView::GetLine(int id, ConsoleLineType line_type,
                                        const std::string &prefix) {
  int match = 0;
  for (Line &l : line_buffer) {
    if (l.linetype == line_type &&
        l.text.Text().substr(0, prefix.size()) == prefix) {
      if (id == match) {
        return &l;
      } else {
        ++match;
