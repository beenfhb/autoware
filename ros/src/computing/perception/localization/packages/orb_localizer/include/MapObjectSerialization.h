...
/*
 * createIdList functions convert list of map objects (in list, set, vector and
 * map) into list of ID createObjectList functions do their reverse
 */
...
set<MapObject *> createObjectList(const set<idtype> &mapIdList) {
  ...
  for (set<idtype>::const_iterator it = mapIdList.begin();
       it != mapIdList.end(); it++) {
    try {
...
vector<idtype> createIdList(const vector<MapObject *> &mapObjectList) {
  vector<idtype> idList =
      vector<idtype>(mapObjectList.size(), static_cast<idtype>(-1));

  int p = 0;
  for (typename vector<MapObject *>::const_iterator it = mapObjectList.begin();
       it != mapObjectList.end(); it++) {
    MapObject *obj = *it;
    if (obj != NULL)
      idList[p] = obj->mnId;
    p++;
...
vector<MapObject *> createObjectList(const vector<idtype> &mapIdList) {
  vector<MapObject *> objVect =
      vector<MapObject *>(mapIdList.size(), static_cast<MapObject *>(NULL));

  int p = 0;
  for (vector<idtype>::const_iterator it = mapIdList.begin();
       it != mapIdList.end(); it++) {
    MapObject *obj;
    try {
...
list<MapObject *> createObjectList(const list<idtype> &mapIdList) {
  ...
  for (list<idtype>::const_iterator it = mapIdList.begin();
       it != mapIdList.end(); it++) {
    try {
      MapObject *obj = MapObject::objectListLookup.at(*it);
...
template <typename T> bool debugSerialization(const T &src) {
  stringstream ssOut;
...
void save(Archive &ar, const ORB_SLAM2::KeyFrame &keyframe,
          const unsigned int version) {
  ar &keyframe.mnId &keyframe.mnFrameId &keyframe.mTimeStamp &keyframe
      .mnGridCols &keyframe.mnGridRows &keyframe.mfGridElementWidthInv &keyframe
      .mfGridElementHeightInv &keyframe.mnTrackReferenceForFrame &keyframe
      .mnFuseTargetForKF &keyframe.mnBALocalForKF &keyframe
      .mnBAFixedForKF &keyframe.mnLoopQuery &keyframe.mnLoopWords &keyframe
      .mLoopScore &keyframe.mnRelocQuery &keyframe.mnRelocWords &keyframe
      .mRelocScore;
  ar &keyframe.mTcwGBA &keyframe.mTcwBefGBA &keyframe.mnBAGlobalForKF;
  ar &keyframe.fx &keyframe.fy &keyframe.cx &keyframe.cy &keyframe
      .invfx &keyframe.invfy &keyframe.mbf &keyframe.mb &keyframe.mThDepth;
  ar &keyframe.N;
  ar &keyframe.mvKeys &keyframe.mvKeysUn &keyframe.mvuRight &keyframe
      .mvDepth &keyframe.mDescriptors;
  ar &keyframe.mBowVec &keyframe.mFeatVec;
  ar &keyframe.mTcp;
  ar &keyframe.mnScaleLevels &keyframe.mfScaleFactor &keyframe
      .mfLogScaleFactor &keyframe.mvScaleFactors &keyframe
      .mvLevelSigma2 &keyframe.mvInvLevelSigma2;
  ar &keyframe.mnMinX &keyframe.mnMinY &keyframe.mnMaxX &keyframe
      .mnMaxY &keyframe.mK;
  ...
  ar &keyframe.Tcw &keyframe.Twc &keyframe.Ow &keyframe.Cw;
  ...
  map<idtype, int> imConnectedKeyFrameWeights =
      createIdList(keyframe.mConnectedKeyFrameWeights);
  ar &imConnectedKeyFrameWeights;

  vector<idtype> vmvpOrderedConnectedKeyFrames =
      createIdList(keyframe.mvpOrderedConnectedKeyFrames);
  ...
  ar &keyframe.mbNotErase &keyframe.mbToBeErased &keyframe.mbBad &keyframe
      .mHalfBaseline;
...
// XXX: Need to modify code for ORB_SLAM2
template <class Archive>
void load(Archive &ar, ORB_SLAM2::KeyFrame &keyframe,
          const unsigned int version) {
  ar &keyframe.mnId &keyframe.mnFrameId &keyframe.mTimeStamp &keyframe
      .mnGridCols &keyframe.mnGridRows &keyframe.mfGridElementWidthInv &keyframe
      .mfGridElementHeightInv &keyframe.mnTrackReferenceForFrame &keyframe
...
void save(Archive &ar, const MapPoint &mapPoint, const unsigned int version) {
  ar &mapPoint.mnId &mapPoint.mnFirstKFid &mapPoint.mnFirstFrame &mapPoint
      .nObs &mapPoint.mTrackProjX &mapPoint.mTrackProjY &mapPoint.mTrackProjXR;
  //	ar & mapPoint.mbTrackInView;
  ar &mapPoint.mnTrackScaleLevel &mapPoint.mTrackViewCos &mapPoint
      .mnTrackReferenceForFrame &mapPoint.mnLastFrameSeen &mapPoint
      .mnBALocalForKF &mapPoint.mnFuseCandidateForKF &mapPoint
      .mnLoopPointForKF &mapPoint.mnCorrectedByKF &mapPoint
      .mnCorrectedReference;
  ar &mapPoint.mPosGBA &mapPoint.mnBAGlobalForKF;
  ...
  ar &mapPoint.mNormalVector &mapPoint.mDescriptor;

  int refKFId = (mapPoint.mpRefKF == NULL ? -1 : mapPoint.mpRefKF->mnId);
  ar &refKFId;

  ar &mapPoint.mnVisible &mapPoint.mnFound &mapPoint.mbBad;

  int _mpReplaced =
      (mapPoint.mpReplaced == NULL ? -1 : mapPoint.mpReplaced->mnId);
  ar &_mpReplaced;

  ar &mapPoint.mfMinDistance &mapPoint.mfMaxDistance;
}
...
void load(Archive &ar, MapPoint &mapPoint, const unsigned int version) {
  ...
  // XXX: Should we refactor the above part of this function? it looks the same
  // as saving part
  MapPoint::mpReplacement[mapPoint.mnId] = _mpReplaced;
  mapPoint.mObservations = createObjectList<KeyFrame>(kfObservation);
  mapPoint.mpRefKF =
      (refKFId == -1 ? NULL : KeyFrame::objectListLookup[refKFId]);
  MapPoint::objectListLookup[mapPoint.mnId] = &mapPoint;
}

template <class Archive>
void save(Archive &ar, const KeyFrameDatabase &kfdb,
          const unsigned int version) {
