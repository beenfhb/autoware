...
public:
  ...
  void ChangeCalibration(const double fx, const double fy, const double cx,
                         const double cy);
  ...
  // XXX: Stub
  KeyFrame *getNearestKeyFrame() { return mpLastKeyFrame; }

  inline void setFps(int f) { mMaxFrames = f; }
...
public:
  ...
  bool trackingIsGood() const {
    return mLastProcessedState == ORB_SLAM2::Tracking::OK and
...
protected:
  ...
  int
      ...
      imageWorkHeight,
      // ROI
      ROIx0, ROIy0, ROIwidth, ROIheight;
};
