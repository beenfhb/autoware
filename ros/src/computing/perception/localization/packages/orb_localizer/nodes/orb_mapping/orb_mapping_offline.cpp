...
void interruptHandler(int s) { interruptStop = true; }
...
void loadData(const string &dataDir, imagePose &dataset) {
  ...
  while (!feof(posefd)) {
    ...
    float row[8];
    float timestamp;
    int p = fscanf(posefd, "%f %f %f %f %f %f %f %f", &row[0], &row[1], &row[2],
...
int main(int argc, char *argv[]) {
  ...
  mapperOpts.add_options()("help", "produce help messages")(
  ...
  if (mapperOptMap.count("help") or !mapperOptMap.count("config") or
      !mapperOptMap.count("mapfile") or !mapperOptMap.count("datadir")) {
  ...
  recomputeNewCameraParameter(
      ...
      (double)SLAM.fsSettings["Camera.cy"], fx2, fy2, cx2, cy2, 1920, 1440,
  ...
  for (int i = startNum; i < stopNum - 2; i++) {
    ...
    cv::resize(
        image, image,
        cv::Size((int)SLAM.fsSettings["Camera.WorkingResolution.Width"],
                 (int)SLAM.fsSettings["Camera.WorkingResolution.Height"]));
    image = image(cv::Rect((int)SLAM.fsSettings["Camera.ROI.x0"],
...

    {
      ...
      //			cout << ORB_SLAM2::KeyFrame::extEgoOrientation
      //<< endl;
    }
