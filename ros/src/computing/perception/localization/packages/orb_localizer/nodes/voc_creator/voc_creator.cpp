...
const int orbLevels = 8, initFast = 20, minFast = 5;

const int imgWidth = 800, imgHeight = 600;

const int vocabHeight = 10, vocabLevel = 6;

class VocCreator {
public:
  VocCreator(ros::NodeHandle &_nodeHandler)
      : nodeHandler(_nodeHandler), imageBuf(nodeHandler),
        orbVocabulary(vocabHeight, vocabLevel) {
    int numOrbFeatures;
    nodeHandler.getParam("number_extraction", numOrbFeatures);
    orbExtractor = new ORBextractor(numOrbFeatures, orbScaleFactor, orbLevels,
                                    initFast, minFast);
    ...
    imageSub =
        imageBuf.subscribe(imageTopic, 1, &VocCreator::imageCallback, this);
    cv::namedWindow("XYZ");
  }
  ...
  void appendVocabulary(cv::Mat &image) {
    ...
    // convert
    vector<cv::Mat> frameDescriptor =
        ORB_SLAM2::Converter::toDescriptorVector(fDescriptors);
