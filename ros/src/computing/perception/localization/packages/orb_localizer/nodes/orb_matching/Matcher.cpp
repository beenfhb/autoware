...
tf::Transform rotateAxes(const tf::Transform &cPose, const float roll = 0,
                         const float pitch = 0, const float yaw = 0) {
...
void debugKeyframeSelection() {
  ...
  for (auto &kf : keyframeSelectorDebug) {
    ...
    float dirx = mtkf[0][2], diry = mtkf[1][2], dirz = mtkf[2][2];
...
Matcher::Matcher(ros::NodeHandle &nh, bool _doPublish)
    : rosnode(nh), lastImageTimestamp(0), gotFirstFrame(false),
      lastKeyframeId(0), cpuTimeDebug(0), doPublish(_doPublish),
  ...
  SLAMSystem = new ORB_SLAM2::System(orbGenericVocabFile, configFile,
                                     ORB_SLAM2::System::MONOCULAR, false,
                                     mapPath, ORB_SLAM2::System::LOCALIZATION);
  ...
  externalFrameMoving = (string)SLAMSystem->fsSettings["Localization.frame2"] +
                        ros::this_node::getName();
  offsetKeyframe =
      SLAMSystem->fsSettings["ExternalLocalization.OffsetKeyframes"];

  cout << "TF: From " << externalFrameFixed << " to " << externalFrameMoving
       << endl;

  string imageTopic;
  rosnode.getParam("image_topic", imageTopic);
  imageSub =
      imageBuf->subscribe(imageTopic, 1, &Matcher::imageCallback, this, th);
  ...
  keyframeDebugger =
      rosnode.advertise<visualization_msgs::MarkerArray>("keyframeDebug", 1);
}
...
void Matcher::imageCallback(const sensor_msgs::ImageConstPtr &msg) {
  ...
  if (gotFirstFrame == false) {
    ...
    recomputeNewCameraParameter(
        ...
        (double)SLAMSystem->fsSettings["Camera.cy"], fx2, fy2, cx2, cy2,
  ...
  cv::resize(
      image, image,
      cv::Size((int)SLAMSystem->fsSettings["Camera.WorkingResolution.Width"],
               (int)SLAMSystem->fsSettings["Camera.WorkingResolution.Height"]));
  image = image(cv::Rect((int)SLAMSystem->fsSettings["Camera.ROI.x0"],
  ...
  try {
    if (SLAMSystem->getTracker()->trackingIsGood()) {
      ...
      mTfBr->sendTransform(
          tf::StampedTransform(currentOrbPose, ros::Time(lastImageTimestamp),
                               "/ORB_SLAM/World", "/ORB_SLAM/Camera"));

      //		 Here, we use offset of external localization from the
      //keyframe
...
void Matcher::publishPose(const tf::Transform *pose) {
  if (doPublish == false)
    return;
  ...
  if (pose == NULL) {
    gpose.pose.position.x = gpose.pose.position.y = gpose.pose.position.z =
        1e15;
    ...
    mTfBr->sendTransform(
        tf::StampedTransform(*pose, ros::Time(lastImageTimestamp),
                             externalFrameFixed, externalFrameMoving));

    for (int i = 0; i < 36; i++)
      gvpose.pose.covariance[i] = 0;
  }
...
Eigen::Quaternionf make_quaternion(const float &x, const float &y,
...
tf::Transform Matcher::localizeByReference(const tf::Transform &tfOrb) {
  tf::Matrix3x3 orient(tfOrb.getRotation());
  ORB_SLAM2::KeyFrame *kfNear = SLAMSystem->getMap()->getNearestKeyFrame(
      make_vector3(tfOrb.getOrigin()), make_quaternion(tfOrb.getRotation()),
...
tf::Transform Matcher::localizeByReference(
    const tf::Transform &orbOriginalResult, const tf::Transform &orbMap,
    const tf::Transform &orbMapOffset, const tf::Transform &realMapPose,
    const tf::Transform &realMapOffset) {
  double offDistO =
      cv::norm(tfToCv(orbMap.getOrigin()) - tfToCv(orbMapOffset.getOrigin()));
  double offDistE = cv::norm(tfToCv(realMapPose.getOrigin()) -
...
tf::Transform Matcher::localizeByReference(const tf::Transform &tfOrb,
  ...
  ORB_SLAM2::KeyFrame *kOffset =
      SLAMSystem->getMap()->offsetKeyframe(kf, offsetKeyframe);
  if (kOffset == NULL)
