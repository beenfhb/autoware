...
class pFrame {
public:
  pFrame(cv::Mat &inImage) : imageBgr(inImage) {
    cv::Mat imageHsv;
...
#define CDF_MEDIUM_TOLERANCE 0.1
  bool needEqualize() {
    ...
    return (ccdf.at<float>(127) < 0.5 - CDF_MEDIUM_TOLERANCE or
            ccdf.at<float>(127) > 0.5 + CDF_MEDIUM_TOLERANCE);
  }
  ...
  static cv::Mat cdf(const cv::Mat &scImage) {
    ...
    const float *ranges[] = {range};
    //	uint8_t
    cv::calcHist(&scImage, 1, 0, cv::Mat(), hist, 1, &histSize, ranges, true,
                 false);
    // cumulative sum
...
void imageCallback(const sensor_msgs::ImageConstPtr &msg) {
  ...
  //	sensor_msgs::ImagePtr imageMsg = cv_bridge::CvImage(std_msgs::Header(),
  //"bgr8", pcImage.outputRgbFromHsv()).toImageMsg();
  ...
  //"mono8", pcImage).toImageMsg(); 	mImageResult.publish(imagePc);
  //	cv::cvtColor(cv_ptr->image, image, code);
}
...
int main(int argc, char *argv[]) {
  ...
  mImageHandler = mTrans->subscribe("/camera/image_raw", 1, imageCallback,
