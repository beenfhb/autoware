...
inline double nrand(double stdDev) {
  return stdDev * sqrt(-2.0 * log(frandom())) * cos(2.0 * M_PI * frandom());
}
 ...
 */
template <class State, class Observation, class MotionCtrl> class VehicleBase {
public:
  virtual State initializeParticleState() const = 0;
  virtual State motionModel(const State &vstate,
                            const MotionCtrl &ctrl) const = 0;
  virtual double
  measurementModel(const State &state,
                   const vector<Observation> &observations) const = 0;
};

template <class State> class Particle {
  // friend class ParticleFilter<S, class _obs, class _ctrl>;
public:
  inline Particle() : weight(0) {}
...
template <class State, class Observation, class MotionCtrl>
class ParticleFilter {
public:
  ParticleFilter(int numPart, VehicleBase<State, Observation, MotionCtrl> &vh)
      : vehicle(vh), numberOfParticle(numPart), counter(0) {

    particleList.resize(numberOfParticle);
    //		std::fill(particleList.begin(), particleList.end(),
    //Particle<state>(stateInitializer));
    srand(time(0));
  }
  ...
  inline void update(const MotionCtrl &control,
                     const vector<Observation> &observationList) {
    // Prediction
    //#pragma omp parallel
  ...
  inline void getStates(vector<State *> &statePtrList) {
    // First version: just return all state particles
    for (int i = 0; i < particleList.size(); i++) {
      Particle<State> &pt = particleList[i];
      statePtrList[i] = &(pt.current);
    }

    // Second version: return only effective sample size
  }

  int getNumberOfParticles() const { return numberOfParticle; }

  Particle<State> &getParticle(int num) { return particleList[num]; }

  vector<Particle<State>> &getParticleList() const { return particleList; }
