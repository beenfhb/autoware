...
void QuaternionToRPY(const double &qx, const double &qy, const double &qz,
                     const double &qw, double &roll, double &pitch,
                     double &yaw) {
...
MotionControl
MotionControl::fromOdometryMsg(const nav_msgs::OdometryConstPtr &msg) {
...
FusionOdometry::FusionOdometry(ros::NodeHandle &nh, const string &odomTopic)
    : rosnode(nh), filter(NumberOfParticles, *this), prevCount(0), prevTime(0),
      positionFound(false), tfPub(new tf::TransformBroadcaster) {
  odomSub = rosnode.subscribe(odomTopic, 100, &FusionOdometry::callback, this);
  pfDebugPub =
      rosnode.advertise<visualization_msgs::Marker>("fusion_odometry", 1);
  pfStates.resize(filter.getNumberOfParticles());
}

VehicleState FusionOdometry::initializeParticleState() const {
  VehicleState cst;
  ...
  QuaternionToRPY(
      currentPose.pose.pose.orientation.x, currentPose.pose.pose.orientation.y,
...
VehicleState FusionOdometry::motionModel(const VehicleState &vstate,
                                         const MotionControl &ctrl) const {
  VehicleState nstate;
...
#define VELOCITY_THRESHOLD 0.08
  ...
  QuaternionToRPY(lastGoodPose.pose.pose.orientation.x,
                  lastGoodPose.pose.pose.orientation.y,
                  lastGoodPose.pose.pose.orientation.z,
                  lastGoodPose.pose.pose.orientation.w, roll, pitch, yaw);
  nstate.roll = roll;
  nstate.pitch = pitch;
...
double FusionOdometry::measurementModel(
    const VehicleState &vs,
    const vector<geometry_msgs::PoseWithCovarianceStamped> &observations)
    const {
#define DISTANCE_THRESHOLD 2.0
  ...
  for (const auto &pose : observations) {
    ...
    double wo, xt = pose.pose.pose.orientation.x,
               yt = pose.pose.pose.orientation.y, xs = vs.position.x(),
               ys = vs.position.y();

    double distX = xt - xs, distY = yt - ys,
...
void FusionOdometry::callback(const nav_msgs::OdometryConstPtr &msg) {
...
void FusionOdometry::publish() {
  filter.getStates(pfStates);
  VehicleState pfAvg = FusionOdometry::getAverage(pfStates);
  tf::Transform cPose = pfAvg.toTf();
  tfPub->sendTransform(
      tf::StampedTransform(cPose, currentPose.header.stamp, "world", "vision"));
...
FusionOdometry::getAverage(const vector<VehicleState *> &pfStates) {
  ...
  avg = std::accumulate(
      pfStates.begin(), pfStates.end(), pInit,
