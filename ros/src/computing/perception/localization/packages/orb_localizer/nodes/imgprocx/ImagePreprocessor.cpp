...
ImagePreprocessor::ImagePreprocessor(ProcessMode m,
                                     ...
                                     ros::NodeHandle &nh)
    : node(nh), pMode(m), iAlpha(0.5), rgbImageBuf(3) {
  imgsub = node.subscribe(subscribedImageTopic, 1,
                          &ImagePreprocessor::messageCallback, this);
...
void ImagePreprocessor::messageCallback(const sensor_msgs::ImageConstPtr &msg) {
  ...
  sensor_msgs::ImagePtr imagePc =
      cv_bridge::CvImage(std_msgs::Header(), isMono ? "mono8" : "bgr8",
                         resultImage)
          .toImageMsg();
...
cv::Mat ImagePreprocessor::cdf(cv::Mat &grayImage, cv::Mat mask) {
  ...
  float range[] = {0, 255};
  const float *ranges[] = {range};
  cv::calcHist(&grayImage, 1, 0, cv::Mat(), hist, 1, &histSize, ranges, true,
               false);
  // cumulative sum
...
cv::Mat ImagePreprocessor::setGamma(cv::Mat &grayImage, const float gamma,
                                    bool LUT_only) {
  cv::Mat grayOut;
...
cv::Mat ImagePreprocessor::autoAdjustGammaRGB(cv::Mat &rgbImg,
...
cv::Mat ImagePreprocessor::autoAdjustGammaMono(cv::Mat &grayImg, float *gamma,
                                               cv::Mat mask) {
...
cv::Mat ImagePreprocessor::toIlluminatiInvariant(const cv::Mat &rgbImage,
                                                 const float alpha) {
  ...
  for (it = rgbImage.begin<cv::Vec3b>(), end = rgbImage.end<cv::Vec3b>();
       it != end; ++it) {
    //		cv::Vec3b &curPixel = *it;
    float fb = (*it)[0] / 255.0, fg = (*it)[1] / 255.0, fr = (*it)[2] / 255.0;
