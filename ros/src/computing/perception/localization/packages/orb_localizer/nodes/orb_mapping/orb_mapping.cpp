...
public:
  ORB_Mapper(ORB_SLAM2::System &pSL, ros::NodeHandle &nh, int fps_ = 10)
      : SLAMSystem(pSL), rosnode(nh), extListener(NULL),
        lastImageTimestamp(0.0), doStop(false), fps(fps_),
        gotFirstFrame(false) {
    // Image Subscription
    ...
    //		imageSub =
    //imageBuf->subscribe((string)SLAMSystem.fsSettings["Camera.topic"], 1,
    //&ORB_Mapper::imageCallback, this, th);

    string groundTruthPose;
    rosnode.getParam("external_localization_topic", groundTruthPose);
    poseSub =
        rosnode.subscribe(groundTruthPose, 2, &ORB_Mapper::poseCallback, this);
  }
  ...
  void poseCallback(const geometry_msgs::PoseStampedConstPtr &cpose) {
    //		printf ("%f %f %f\n", cpose->pose.position.x, cpose->pose.position.y,
    //cpose->pose.position.z);
    ...
    ORB_SLAM2::KeyFrame::extEgoOrientation.at<double>(0) =
        cpose->pose.orientation.x,
    ORB_SLAM2::KeyFrame::extEgoOrientation.at<double>(1) =
        cpose->pose.orientation.y,
    ORB_SLAM2::KeyFrame::extEgoOrientation.at<double>(2) =
        cpose->pose.orientation.z,
    ORB_SLAM2::KeyFrame::extEgoOrientation.at<double>(3) =
        cpose->pose.orientation.w;
  }
...
int main(int argc, char *argv[]) {
  ...
  ORB_SLAM2::System SLAM(orbVocabFile, configFile, ORB_SLAM2::System::MONOCULAR,
                         true, mapPath, System::MAPPING);

  ORB_Mapper Mapper(SLAM, nodeHandler);

  string imageTopic;
  nodeHandler.getParam("image_topic", imageTopic);
  Mapper.imageSub = Mapper.imageBuf->subscribe(
      imageTopic, 100, &ORB_Mapper::imageCallback, &Mapper, Mapper.th);
