...
inline bool isInvalidPose(const geometry_msgs::PoseWithCovarianceStamped &p) {
  return (p.pose.pose.position.x >= 1e6 and p.pose.pose.position.y >= 1e6 and
          p.pose.pose.position.z >= 1e6);
}
...
struct VehicleState {
  ...
  VehicleState()
      : position(Eigen::Vector3d::Zero()), yaw(0.0), pitch(0.0), roll(0.0) {}
...
struct MotionControl {
  double velocity, yawrate, timestamp, timediff;
  MotionControl() : velocity(0.0), yawrate(0.0), timestamp(-1), timediff(0) {}
  static MotionControl fromOdometryMsg(const nav_msgs::OdometryConstPtr &msg);
};

class FusionOdometry
    : public PF::VehicleBase<VehicleState,
...
public:
  ...
  VehicleState motionModel(const VehicleState &s,
                           const MotionControl &ct) const;
  double measurementModel(const VehicleState &vs,
                          const vector<geometry_msgs::PoseWithCovarianceStamped>
                              &observations) const;
...
protected:
  ...
  PF::ParticleFilter<VehicleState, geometry_msgs::PoseWithCovarianceStamped,
                     MotionControl>
      filter;
