...
void equalizeImageHistogramFromMask(cv::Mat &input, cv::Mat &output,
                                    cv::Mat &mask) {
...
void tfToCV(const tf::Transform &src, cv::Mat &position, cv::Mat &orientation) {
  ...
  position.at<double>(0) = p.x(), position.at<double>(1) = p.y(),
  position.at<double>(2) = p.z();

  orientation = cv::Mat(4, 1, CV_64F);
  tf::Quaternion otn = src.getRotation();
  orientation.at<double>(0) = otn.x(), orientation.at<double>(1) = otn.y(),
  orientation.at<double>(2) = otn.z(), orientation.at<double>(3) = otn.w();
}
...
void recomputeNewCameraParameter(
    ...
    // New
    double &fx2, double &fy2, double &cx2, double &cy2, int width1, int height1,
...
void tf2positiondirection(const tf::Transform &pose,
                          float positiondirection[6]) {
  // position
  positiondirection[0] = pose.getOrigin().x();
  positiondirection[1] = pose.getOrigin().y();
  positiondirection[2] = pose.getOrigin().z();
  float fdirx = pose.getRotation().x(), fdiry = pose.getRotation().y(),
        fdirz = pose.getRotation().z(), fdirnorm;
...
tf::Transform getKeyFrameExtPose(const KeyFrame *kf) {
  ...
  if (kf->extPosition.empty() or kf->extOrientation.empty()) {
    ...
    Ext.setOrigin(tf::Vector3(kf->extPosition.at<double>(0),
                              kf->extPosition.at<double>(1),
                              kf->extPosition.at<double>(2)));
    Ext.setRotation(tf::Quaternion(
        kf->extOrientation.at<double>(0), kf->extOrientation.at<double>(1),
        kf->extOrientation.at<double>(2), kf->extOrientation.at<double>(3)));
  }
  return Ext;
}
