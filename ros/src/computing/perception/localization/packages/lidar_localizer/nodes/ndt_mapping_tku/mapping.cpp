...
// pcl::PointCloud<velodyne_pointcloud::PointXYZIR>::ConstPtr& msg)
void points_callback(const pcl::PointCloud<pcl::PointXYZI>::ConstPtr &msg) {
  ...
  if ((int)prev_points.points.size() > 0) {
    //  tf_listener->waitForTransform("/world", header.frame_id, header.stamp,
    //  ros::Duration(1));
    // pcl_ros::transformPointCloud("/world", *msg, pcl_out, *tf_listener);
    try {
      /*    tf_listener->waitForTransform("/japan_7", header.frame_id,
      header.stamp, ros::Duration(1)); pcl_ros::transformPointCloud("/japan_7",
      *msg, pcl_out, *tf_listener);
      */
      //    tf_listener->waitForTransform("/world", header.frame_id,
      //    header.stamp, ros::Duration(1));
      tf_listener->waitForTransform(
          "map", "base_link", prev_time /*header.stamp*/, ros::Duration(1));
      tf_listener->lookupTransform("map", "base_link", prev_time, transform);
      /*pcl_ros::transformPointCloud("world", prev_time, prev_points,
       * "ndt_frame", pcl_out, *tf_listener);
       */
    ...
    if (count % 5 == 0) {
      ...
      for (int i = 0; i < (int)pcl_out.points.size(); i++) {
        ...
        //    pcl_out.points[i].y, pcl_out.points[i].x, pcl_out.points[i].z,
        //    pcl_out.points[i].intensity);
        ofs << pcl_out.points[i].x << "," << pcl_out.points[i].y << ","
...
int main(int argc, char **argv) {
  ...
  velodyne_pub =
      n.advertise<pcl::PointCloud<pcl::PointXYZI>>("velodyne_points/world", 1);
