...
static ros::Publisher predict_pose_diff_pub;
static ros::Publisher predict_pose_diff_yaw_pub;
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    */
    ...
    const double predict_pose_diff = std::sqrt(std::pow(current_pose.x-predict_pose_for_ndt.x, 2.0)+std::pow(current_pose.y-predict_pose_for_ndt.y, 2.0)+std::pow(current_pose.z-predict_pose_for_ndt.z, 2.0));
    std_msgs::Float32 predict_pose_diff_msg;
    predict_pose_diff_msg.data = predict_pose_diff;
    predict_pose_diff_pub.publish(predict_pose_diff_msg);

    const double predict_pose_diff_yaw = std::fabs(calcDiffForRadian(current_pose.yaw, predict_pose_for_ndt.yaw));
    std_msgs::Float32 predict_pose_diff_yaw_msg;
    predict_pose_diff_yaw_msg.data = predict_pose_diff_yaw;
    predict_pose_diff_yaw_pub.publish(predict_pose_diff_yaw_msg);

    double outlier_ratio_ = 0.55;
    double gauss_c1 = 10 * (1 - outlier_ratio_);
    double gauss_c2 = outlier_ratio_ / pow (ndt_res, 3);
    double gauss_d3 = -log (gauss_c2);
    double gauss_d1_ = -log ( gauss_c1 + gauss_c2 ) - gauss_d3;
    double trans_probability2 = trans_probability / -gauss_d1_;
    ...
    std::cout << "Transformation Probability: " << trans_probability2 << std::endl;
    std::cout << "gauss_d1_: " << gauss_d1_ << std::endl;
...
int main(int argc, char** argv)
{
  ...
  predict_pose_diff_pub = nh.advertise<std_msgs::Float32>("/predict_pose_diff", 10);
  predict_pose_diff_yaw_pub = nh.advertise<std_msgs::Float32>("/predict_pose_diff_yaw", 10);
