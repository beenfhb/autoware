...
void points_callback(const pcl::PointCloud<pcl::PointXYZI>::ConstPtr &input) {
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr transformed_input(
...
*/
    ...
    q.setRPY(3.141592, 0.05, -0.05);
    tf::Transform tf_baselink_to_localizer(q, v);
    pcl_ros::transformPointCloud(*input, *transformed_input,
                                 tf_baselink_to_localizer);
  }

  tf::StampedTransform transform;
  if (input->size() > 0) {
    try {
      tf_listener->waitForTransform(PARENT_FRAME, CHILD_FRAME, header.stamp,
                                    ros::Duration(1));
      tf_listener->lookupTransform(PARENT_FRAME, CHILD_FRAME, header.stamp,
                                   transform);
    ...
    if (CHILD_FRAME == "velodyne" &&
        POINTS_TOPIC == "hokuyo_3d/hokuyo_cloud2") {
      for (int i = 0; i < (int)transformed_input->size(); i++) {
        tf::Point pt(transformed_input->points[i].x,
                     transformed_input->points[i].y,
                     transformed_input->points[i].z);
      ...
      for (int i = 0; i < (int)input->size(); i++) {
        tf::Point pt(input->points[i].x, input->points[i].y,
                     input->points[i].z);
        tf::Point pt_world = transform * pt;
        pcl::PointXYZI wp;
        double distance = pt.x() * pt.x() + pt.y() * pt.y() + pt.z() * pt.z();
    ...
    filename = OUTPUT_DIR + PARENT_FRAME + "-" + CHILD_FRAME + "_" + lidar +
               "_" + std::to_string(map_id) + ".csv";
    ...
    std::cout << "Wrote " << pcl_out.size() << " points to " << filename << "."
              << std::endl;
    added_scan_num++;
