...
CUDAH int Matrix::rows() const { return rows_; }

CUDAH int Matrix::cols() const { return cols_; }

CUDAH int Matrix::offset() const { return offset_; }

CUDAH double *Matrix::buffer() const { return buffer_; }
...
CUDAH double &Matrix::operator()(int index) { return buffer_[index * offset_]; }
...
CUDAH bool Matrix::inverse(Matrix &output) {
  ...
  if (rows_ == 3) {
    double det =
        at(0, 0) * at(1, 1) * at(2, 2) + at(0, 1) * at(1, 2) * at(2, 0) +
