...
public:
   ...
   * centroids and the input point are less than radius. Results of the search
   * are stored into valid_points, starting_voxel_id, and voxel_id. Valid
   * points: the function return one or more voxels for these points. Other
   * points are considered as invalid. Valid voxels: voxels returned from the
   * search. The number of valid points is stored in valid_points_num. The total
   * number of valid voxels. If the query of both point A and point B return
   * voxel X, then the number of valid voxels is 2, not 1. */
  void radiusSearch(float *qx, float *qy, float *qz, int points_num,
                    float radius, int max_nn, int **valid_points,
                    int **starting_voxel_id, int **voxel_id,
  ...
  void nearestNeighborSearch(float *trans_x, float *trans_y, float *trans_z,
                             int point_num, int *valid_distance,
                             double *min_distance, float max_range);
...
private:
  ...
  template <typename T = int> void ExclusiveScan(T *input, int ele_num, T *sum);

  /* A wrapper for exclusive scan using thrust.
   * Output sum is not required. */
  template <typename T = int> void ExclusiveScan(T *input, int ele_num);
