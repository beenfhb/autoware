...
private:
  CUDAH void computeOrthogonalComplement(MatrixDevice w, MatrixDevice u,
                                         MatrixDevice v);
...
CUDAH SymmetricEigensolver3x3::SymmetricEigensolver3x3(
    const SymmetricEigensolver3x3 &other) {
...
CUDAH void SymmetricEigensolver3x3::computeEigenvector01(int tid) {
  if (norm_[tid] > 0.0) {
    MatrixDevice evec0(3, 1, offset_ * 3,
                       eigenvectors_ + tid + i02_[tid] * offset_);

    // row0 is r0xr1, row1 is r0xr2, row2 is r1xr2
    MatrixDevice rxr(3, 3, offset_, buffer_ + 3 * 3 * offset_ + tid);

    double d0 =
        rxr(0, 0) * rxr(0, 0) + rxr(0, 1) * rxr(0, 1) * rxr(0, 2) * rxr(0, 2);
    double d1 =
        rxr(1, 0) * rxr(1, 0) + rxr(1, 1) * rxr(1, 1) * rxr(1, 2) * rxr(1, 2);
    double d2 =
        rxr(2, 0) * rxr(2, 0) + rxr(2, 1) * rxr(2, 1) * rxr(2, 2) * rxr(2, 2);
...
CUDAH void SymmetricEigensolver3x3::multiply(MatrixDevice u, double mult,
                                             MatrixDevice output) {
  output(0) = u(0) * mult;
  output(1) = u(1) * mult;
  output(2) = u(2) * mult;
...
CUDAH void SymmetricEigensolver3x3::subtract(MatrixDevice u, MatrixDevice v,
                                             MatrixDevice output) {
  output(0) = u(0) - v(0);
  output(1) = u(1) - v(1);
  output(2) = u(2) - v(2);
...
CUDAH void SymmetricEigensolver3x3::divide(MatrixDevice u, double div,
                                           MatrixDevice output) {
  output(0) = u(0) / div;
  output(1) = u(1) / div;
  output(2) = u(2) / div;
...
CUDAH void SymmetricEigensolver3x3::cross(MatrixDevice u, MatrixDevice v,
                                          MatrixDevice out) {
...
CUDAH void SymmetricEigensolver3x3::computeOrthogonalComplement(
    MatrixDevice w, MatrixDevice u, MatrixDevice v) {
  bool c = (fabs(w(0)) > fabs(w(1)));

  double inv_length = (c) ? (1.0 / sqrt(w(0) * w(0) + w(2) * w(2)))
                          : (1.0 / sqrt(w(1) * w(1) + w(2) * w(2)));
...
CUDAH void SymmetricEigensolver3x3::computeEigenvector2(int tid) {
  if (norm_[tid] > 0.0) {
    MatrixDevice evec0(3, 1, offset_ * 3,
                       eigenvectors_ + tid + i02_[tid] * offset_);
    MatrixDevice evec1(3, 1, offset_ * 3, eigenvectors_ + tid + offset_);
    MatrixDevice evec2(3, 1, offset_ * 3,
                       eigenvectors_ + tid + i02_[tid + offset_] * offset_);
