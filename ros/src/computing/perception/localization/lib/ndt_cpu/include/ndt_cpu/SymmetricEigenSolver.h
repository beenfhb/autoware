...
private:
  void computeEigenvector0(double a00, double a01, double a02, double a11,
                           double a12, double a22, int i0);

  void computeEigenvector1(double a00, double a01, double a02, double a11,
                           double a12, double a22, int i0, int i1);

  void computeEigenvector2(int i0, int i1, int i2);

  void computeOrthogonalComplement(Eigen::Vector3d &w, Eigen::Vector3d &u,
                                   Eigen::Vector3d &v);
...
SymmetricEigensolver3x3::SymmetricEigensolver3x3(
    const Eigen::Matrix3d input_matrix) {
  input_ = input_matrix;
  evecs_.setZero();
  evals_.setZero();
}
...
Eigen::Vector3d SymmetricEigensolver3x3::eigenvalues() { return evals_; }
...
Eigen::Vector3d SymmetricEigensolver3x3::cross(Eigen::Vector3d u,
                                               Eigen::Vector3d v) {
  Eigen::Vector3d out;
