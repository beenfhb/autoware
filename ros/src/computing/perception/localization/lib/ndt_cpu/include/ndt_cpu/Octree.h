...
/* The octree is built on top of a voxel grid to fasten the nearest neighbor
 * search */
namespace cpu {
template <typename PointSourceType> class Octree {
public:
  Octree();

  /* Input is a vector of boundaries and ptsum of the voxel grid
   * Those boundaries is needed since the number of actually occupied voxels may
   * be much smaller than reserved number of voxels */
  void setInput(std::vector<Eigen::Vector3i> occupied_voxels,
                typename pcl::PointCloud<PointSourceType>::Ptr point_cloud);

  void update(std::vector<Eigen::Vector3i> new_voxels,
...
private:
  ...
  // Convert from 3d indexes and level of the tree node to the actual index in
  // the array
  int index2id(int idx, int idy, int idz, int level);
  int index2id(int idx, int idy, int idz, OctreeLevelBoundaries bounds,
               OctreeLevelDim dims);

  // Convert from the index in the array to the 3d indexes of the tree node
  Eigen::Vector3i id2index(int id, int level);
  Eigen::Vector3i id2index(int id, OctreeLevelBoundaries bounds,
                           OctreeLevelDim dims);
  ...
  void
  updateOctreeContent(std::vector<Eigen::Vector3i> new_voxels,
                      typename pcl::PointCloud<PointSourceType>::Ptr new_cloud);
  ...
  void goUp(Eigen::Matrix<int, 4, 1> tree_node, PointSourceType q,
            double &min_range, int &current_nn_voxel);

  void goDown(Eigen::Matrix<int, 4, 1> tree_node, PointSourceType q,
              double &min_range, int &current_nn_voxel);
