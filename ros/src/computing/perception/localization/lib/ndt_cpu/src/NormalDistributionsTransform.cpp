...
NormalDistributionsTransform<PointSourceType,
                             PointTargetType>::NormalDistributionsTransform() {
...
void NormalDistributionsTransform<
    PointSourceType, PointTargetType>::setStepSize(double step_size) {
  step_size_ = step_size;
}

template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<
    PointSourceType, PointTargetType>::setResolution(float resolution) {
  resolution_ = resolution;
}

template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<
    PointSourceType, PointTargetType>::setOutlierRatio(double olr) {
  outlier_ratio_ = olr;
}

template <typename PointSourceType, typename PointTargetType>
double
NormalDistributionsTransform<PointSourceType, PointTargetType>::getStepSize()
    const {
  return step_size_;
}

template <typename PointSourceType, typename PointTargetType>
float NormalDistributionsTransform<PointSourceType,
                                   PointTargetType>::getResolution() const {
  return resolution_;
}

template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<PointSourceType,
                                    PointTargetType>::getOutlierRatio() const {
  return outlier_ratio_;
}

template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<
    PointSourceType, PointTargetType>::getTransformationProbability() const {
  return trans_probability_;
}

template <typename PointSourceType, typename PointTargetType>
int NormalDistributionsTransform<PointSourceType,
                                 PointTargetType>::getRealIterations() {
  return real_iterations_;
}

template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<
    PointSourceType, PointTargetType>::auxilaryFunction_PsiMT(double a,
                                                              double f_a,
                                                              double f_0,
                                                              double g_0,
                                                              double mu) {
...
template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<
    PointSourceType, PointTargetType>::auxilaryFunction_dPsiMT(double g_a,
                                                               double g_0,
                                                               double mu) {
  return (g_a - mu * g_0);
}
...
template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<PointSourceType, PointTargetType>::
    setInputTarget(typename pcl::PointCloud<PointTargetType>::Ptr input) {
...
void NormalDistributionsTransform<PointSourceType, PointTargetType>::
  ...
  if (source_cloud_->points.size() > 0) {
    trans_probability_ =
        score / static_cast<double>(source_cloud_->points.size());
  }
}

template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<PointSourceType, PointTargetType>::
  ...
  for (int idx = 0; idx < source_cloud_->points.size(); idx++) {
    ...
    for (int i = 0; i < neighbor_ids.size(); i++) {
      ...
      score +=
          updateDerivatives(score_gradient, hessian, point_gradient,
...
template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<PointSourceType, PointTargetType>::
    computeAngleDerivatives(Eigen::Matrix<double, 6, 1> pose,
                            bool compute_hessian) {
...
template <typename PointSourceType, typename PointTargetType>
double
NormalDistributionsTransform<PointSourceType, PointTargetType>::getFitnessScore(
    double max_range) {
...
template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<PointSourceType, PointTargetType>::
    updateVoxelGrid(typename pcl::PointCloud<PointTargetType>::Ptr new_cloud) {
  // Update voxel grid
