...
VoxelGrid<PointSourceType>::VoxelGrid()
    : voxel_num_(0), max_x_(FLT_MIN), max_y_(FLT_MIN), max_z_(FLT_MIN),
      min_x_(FLT_MAX), min_y_(FLT_MAX), min_z_(FLT_MAX), voxel_x_(0),
      voxel_y_(0), voxel_z_(0), max_b_x_(0), max_b_y_(0), max_b_z_(0),
      min_b_x_(0), min_b_y_(0), min_b_z_(0), vgrid_x_(0), vgrid_y_(0),
      vgrid_z_(0), min_points_per_voxel_(6), real_max_bx_(INT_MIN),
      real_max_by_(INT_MIN), real_max_bz_(INT_MIN), real_min_bx_(INT_MAX),
      real_min_by_(INT_MAX), real_min_bz_(INT_MAX) {
  centroid_.reset();
  icovariance_.reset();
  points_id_.reset();
...
int VoxelGrid<PointSourceType>::roundUp(int input, int factor) {
  return (input < 0) ? -((-input) / factor) * factor
                     : ((input + factor - 1) / factor) * factor;
}

template <typename PointSourceType>
int VoxelGrid<PointSourceType>::roundDown(int input, int factor) {
  return (input < 0) ? -((-input + factor - 1) / factor) * factor
                     : (input / factor) * factor;
}
...
template <typename PointSourceType>
Eigen::Matrix3d
VoxelGrid<PointSourceType>::getInverseCovariance(int voxel_id) const {
  return (*icovariance_)[voxel_id];
}

template <typename PointSourceType>
void VoxelGrid<PointSourceType>::setLeafSize(float voxel_x, float voxel_y,
                                             float voxel_z) {
  voxel_x_ = voxel_x;
  voxel_y_ = voxel_y;
  voxel_z_ = voxel_z;
...
template <typename PointSourceType>
int VoxelGrid<PointSourceType>::voxelId(PointSourceType p, float voxel_x,
...
template <typename PointSourceType>
int VoxelGrid<PointSourceType>::voxelId(int idx, int idy, int idz, int min_b_x,
                                        int min_b_y, int min_b_z, int size_x,
                                        int size_y, int size_z) {
  return (idx - min_b_x) + (idy - min_b_y) * size_x +
         (idz - min_b_z) * size_x * size_y;
}
  ...
  for (int idx = real_min_bx_; idx <= real_max_bx_; idx++)
    for (int idy = real_min_by_; idy <= real_max_by_; idy++)
      for (int idz = real_min_bz_; idz <= real_max_bz_; idz++) {
        int i = voxelId(idx, idy, idz, min_b_x_, min_b_y_, min_b_z_, vgrid_x_,
                        vgrid_y_, vgrid_z_);
        ...
        if (ipoint_num >= min_points_per_voxel_) {
          covariance =
              ((*tmp_cov_)[i] - 2.0 * (pt_sum * (*centroid_)[i].transpose())) /
                  point_num +
              (*centroid_)[i] * (*centroid_)[i].transpose();
...
template <typename PointSourceType>
void VoxelGrid<PointSourceType>::setInput(
    typename pcl::PointCloud<PointSourceType>::Ptr input_cloud) {
...
template <typename PointSourceType>
void VoxelGrid<PointSourceType>::findBoundaries(
    typename pcl::PointCloud<PointSourceType>::Ptr input_cloud, float &max_x,
    float &max_y, float &max_z, float &min_x, float &min_y, float &min_z) {
...
template <typename PointSourceType>
void VoxelGrid<PointSourceType>::radiusSearch(PointSourceType p, float radius,
                                              std::vector<int> &voxel_ids,
                                              int max_nn) {
  float t_x = p.x;
  float t_y = p.y;
  float t_z = p.z;
  ...
  for (int idx = min_id_x; idx <= max_id_x && nn < max_nn; idx++) {
    for (int idy = min_id_y; idy <= max_id_y && nn < max_nn; idy++) {
      for (int idz = min_id_z; idz <= max_id_z && nn < max_nn; idz++) {
        int vid = voxelId(idx, idy, idz, min_b_x_, min_b_y_, min_b_z_, vgrid_x_,
                          vgrid_y_, vgrid_z_);
...
template <typename PointSourceType>
int VoxelGrid<PointSourceType>::nearestVoxel(
    PointSourceType query_point, Eigen::Matrix<float, 6, 1> boundaries,
    float max_range) {
  ...
  for (int i = lower_x; i <= upper_x; i++) {
    for (int j = lower_y; j <= upper_y; j++) {
      for (int k = lower_z; k <= upper_z; k++) {
        int vid = voxelId(i, j, k, min_b_x_, min_b_y_, min_b_z_, vgrid_x_,
                          vgrid_y_, vgrid_z_);
        Eigen::Vector3d c = (*centroid_)[vid];

        if ((*points_id_)[vid].size() > 0) {
          double cur_dist =
              sqrt((qx - c(0)) * (qx - c(0)) + (qy - c(1)) * (qy - c(1)) +
                   (qz - c(2)) * (qz - c(2)));
...
template <typename PointSourceType>
double VoxelGrid<PointSourceType>::nearestNeighborDistance(PointSourceType q,
                                                           float max_range) {
  ...
  double min_dist =
      sqrt((q.x - c(0)) * (q.x - c(0)) + (q.y - c(1)) * (q.y - c(1)) +
           (q.z - c(2)) * (q.z - c(2)));
...
template <typename PointSourceType>
void VoxelGrid<PointSourceType>::updateBoundaries(float max_x, float max_y,
                                                  float max_z, float min_x,
  ...
  if (new_max_x > max_x_ || new_max_y > max_y_ || new_max_z > max_z_ ||
    ...
    if (max_b_x > max_b_x_ || max_b_y > max_b_y_ || max_b_z > max_b_z_ ||
      ...
      boost::shared_ptr<std::vector<Eigen::Matrix3d>> old_icovariance =
          icovariance_;
      boost::shared_ptr<std::vector<std::vector<int>>> old_points_id =
          points_id_;
      boost::shared_ptr<std::vector<int>> old_points_per_voxel =
          points_per_voxel_;

      boost::shared_ptr<std::vector<Eigen::Vector3d>> old_tmp_centroid =
          tmp_centroid_;
      ...
      icovariance_ =
          boost::make_shared<std::vector<Eigen::Matrix3d>>(voxel_num);
      ...
      tmp_centroid_ =
          boost::make_shared<std::vector<Eigen::Vector3d>>(voxel_num);
      ...
      for (int idx = real_min_bx_; idx <= real_max_bx_; idx++) {
        for (int idy = real_min_by_; idy <= real_max_by_; idy++) {
          for (int idz = real_min_bz_; idz <= real_max_bz_; idz++) {
            old_id = voxelId(idx, idy, idz, min_b_x_, min_b_y_, min_b_z_,
                             vgrid_x_, vgrid_y_, vgrid_z_);
            new_id = voxelId(idx, idy, idz, min_b_x, min_b_y, min_b_z, vgrid_x,
                             vgrid_y, vgrid_z);
...
template <typename PointSourceType>
void VoxelGrid<PointSourceType>::update(
    typename pcl::PointCloud<PointSourceType>::Ptr new_cloud) {
  ...
  findBoundaries(new_cloud, new_max_x, new_max_y, new_max_z, new_min_x,
                 new_min_y, new_min_z);
   ...
   */
  updateBoundaries(new_max_x, new_max_y, new_max_z, new_min_x, new_min_y,
                   new_min_z);
...
template <typename PointSourceType>
void VoxelGrid<PointSourceType>::updateVoxelContent(
    typename pcl::PointCloud<PointSourceType>::Ptr new_cloud) {
  ...
  for (int i = 0; i < new_cloud->points.size(); i++) {
    ...
    int vid = voxelId(vx, vy, vz, min_b_x_, min_b_y_, min_b_z_, vgrid_x_,
                      vgrid_y_, vgrid_z_);
    ...
    if (ipoint_num >= min_points_per_voxel_) {
      Eigen::Matrix3d covariance;
      covariance =
          ((*tmp_cov_)[vid] - 2.0 * (pt_sum * (*centroid_)[vid].transpose())) /
              point_num +
          (*centroid_)[vid] * (*centroid_)[vid].transpose();
      covariance *= (point_num - 1.0) / point_num;
