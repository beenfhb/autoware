...
template <typename PointSourceType> Octree<PointSourceType>::Octree() {
  leaf_x_ = 4; // 16;
  leaf_y_ = 4; // 16;
  leaf_z_ = 2; // 4;
...
template <typename PointSourceType>
void Octree<PointSourceType>::setInput(
    std::vector<Eigen::Vector3i> occupied_voxels,
    typename pcl::PointCloud<PointSourceType>::Ptr point_cloud) {
  octree_.reset();
  reserved_size_.reset();
  dimension_.reset();
  occupancy_check_.reset();
  ...
  occupancy_check_ =
      boost::make_shared<std::vector<std::vector<unsigned int>>>();
  ...
  std::vector<unsigned int> occupancy0(
      static_cast<int>((node_number + sizeof(unsigned int) * 8 - 1) /
  ...
  while (node_number > 8) {
    ...
    std::vector<unsigned int> new_occupancy_check(
        static_cast<int>((node_number + sizeof(unsigned int) * 8 - 1) /
                         (sizeof(unsigned int) * 8)),
        0);
  ...
  for (int i = 0; i < occupied_voxels.size(); i++) {
    ...
    if (!isOccupied(nid, 0)) {
      ...
      current_node.centroid =
          current_node.centroid * current_node.point_num + new_node;
...
template <typename PointSourceType>
bool Octree<PointSourceType>::isOccupied(std::vector<unsigned int> occupancy,
                                         int node_id) {
  int val_loc = node_id / (sizeof(int) * 8);
  int bit_loc = node_id % (sizeof(int) * 8);
...
template <typename PointSourceType>
void Octree<PointSourceType>::setOccupied(std::vector<unsigned int> &occupancy,
                                          int node_id) {
  int val_loc = node_id / (sizeof(int) * 8);
  int bit_loc = node_id % (sizeof(int) * 8);
      ...
      for (int k = cbounds.lower_z; k <= cbounds.upper_z; k++) {
        ...
        if (isOccupied(cid, level - 1)) {
          if (isOccupied(pid, level)) {
            ...
            pnode.centroid = pnode.centroid * pnode.point_num +
                             cnode.centroid * cnode.point_num;
...
template <typename PointSourceType>
void Octree<PointSourceType>::update(
    std::vector<Eigen::Vector3i> new_voxels,
...
template <typename PointSourceType>
void Octree<PointSourceType>::updateBoundaries(
    std::vector<Eigen::Vector3i> new_voxels) {
  ...
  if (dst_bounds.lower_x < src_bounds.lower_x ||
    ...
    // Backup old octree
    boost::shared_ptr<std::vector<std::vector<OctreeNode>>> old_octree =
        octree_;
    boost::shared_ptr<std::vector<std::vector<unsigned int>>>
        old_occupancy_check = occupancy_check_;
    boost::shared_ptr<std::vector<OctreeLevelBoundaries>> old_reserved_size =
        reserved_size_;
    ...
    std::vector<unsigned int> new_occupy0(
        (node_number + sizeof(unsigned int) * 8 - 1) /
            (sizeof(unsigned int) * 8),
        0);
    ...
    while (node_number > 8) {
      ...
      std::vector<OctreeNode> new_level(node_number);
      std::vector<unsigned int> new_occupancy(
          static_cast<int>((node_number + sizeof(unsigned int) * 8 - 1) /
                           (sizeof(unsigned int) * 8)),
          0);
        ...
        if (stat32 != 0) {
          for (int j = 0; j < sizeof(unsigned int) * 8; j++) {
            if ((stat32 & (1 << j)) == (1 << j)) {
              ...
              int dst_id =
                  index2id(vid(0), vid(1), vid(2), dst_bounds, dst_dim);
...
int Octree<PointSourceType>::index2id(int idx, int idy, int idz, int level) {
  ...
  return ((idx - bound.lower_x) + (idy - bound.lower_y) * dim.x +
          (idz - bound.lower_z) * dim.x * dim.y);
}

template <typename PointSourceType>
int Octree<PointSourceType>::index2id(int idx, int idy, int idz,
                                      OctreeLevelBoundaries bounds,
                                      OctreeLevelDim dim) {
  return ((idx - bounds.lower_x) + (idy - bounds.lower_y) * dim.x +
          (idz - bounds.lower_z) * dim.x * dim.y);
}
...
template <typename PointSourceType>
Eigen::Vector3i Octree<PointSourceType>::id2index(int id,
                                                  OctreeLevelBoundaries bounds,
                                                  OctreeLevelDim dims) {
  Eigen::Vector3i output;
...
template <typename PointSourceType>
void Octree<PointSourceType>::updateOctreeContent(
    std::vector<Eigen::Vector3i> new_voxels,
    typename pcl::PointCloud<PointSourceType>::Ptr new_cloud) {
  for (int i = 0; i < new_voxels.size(); i++) {
    Eigen::Vector3i vid = new_voxels[i];
...
Octree<PointSourceType>::nearestOctreeNode(PointSourceType q) {
...
template <typename PointSourceType>
void Octree<PointSourceType>::initRange(PointSourceType q, double &min_range,
                                        int &nn_id) {
    ...
    for (int j = lower_y; j <= upper_y; j++) {
      for (int k = lower_z; k <= upper_z; k++) {
        ...
        if (isOccupied(node_id, 0)) {
          ...
          double cur_dist =
              sqrt((c(0) - q.x) * (c(0) - q.x) + (c(1) - q.y) * (c(1) - q.y) +
                   (c(2) - q.z) * (c(2) - q.z));
...
// Check all childrens
template <typename PointSourceType>
void Octree<PointSourceType>::goDown(Eigen::Matrix<int, 4, 1> node,
                                     PointSourceType q, double &min_range,
                                     int &current_nn_voxel) {
  ...
  int upper_x =
      (node(0) * 2 + 1 > bounds.upper_x) ? bounds.upper_x : node(0) * 2 + 1;

  int lower_y = (node(1) * 2 < bounds.lower_y) ? bounds.lower_y : node(1) * 2;
  int upper_y =
      (node(1) * 2 + 1 > bounds.upper_y) ? bounds.upper_y : node(1) * 2 + 1;

  int lower_z = (node(2) * 2 < bounds.lower_z) ? bounds.lower_z : node(2) * 2;
  int upper_z =
      (node(2) * 2 + 1 > bounds.upper_z) ? bounds.upper_z : node(2) * 2 + 1;
...
template <typename PointSourceType>
void Octree<PointSourceType>::goUp(Eigen::Matrix<int, 4, 1> node,
                                   PointSourceType q, double &min_range,
                                   int &current_nn_voxel) {
