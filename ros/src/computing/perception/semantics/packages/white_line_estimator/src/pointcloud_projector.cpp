...
PointCloudProjector::PointCloudProjector(double min_area,double max_area)
{
    min_area_ = min_area;
    max_area_ = max_area;
...
void PointCloudProjector::setCameraInfo(sensor_msgs::CameraInfo info)
{
...
void PointCloudProjector::setProjectionMatrix(autoware_msgs::ProjectionMatrix proj_matrix)
{
    std::lock_guard<std::mutex> lock(mtx_);
...
cv::Mat PointCloudProjector::getGroundConvexHull(std::vector<ProjectedPoint> ground_points,cv::Size size)
{
...
boost::optional<std::vector<ProjectedPoint> > PointCloudProjector::project(cv::Mat image,const sensor_msgs::PointCloud2ConstPtr& pointcloud_msg,
...
std::vector<ProjectedPoint> PointCloudProjector::projectPointCloudToImage(sensor_msgs::PointCloud2 point_cloud,cv::Size size)
{
