...
double DecisionMakerNode::calcPosesAngleDiffN(const geometry_msgs::Pose &p_from,
                                              const geometry_msgs::Pose &p_to) {
  // convert to [-pi : pi]
  return getPoseAngle(p_from) - getPoseAngle(p_to);
}

double DecisionMakerNode::calcPosesAngleDiff(const geometry_msgs::Pose &p_from,
                                             const geometry_msgs::Pose &p_to) {
  // convert to [-pi : pi]
...
double DecisionMakerNode::calcIntersectWayAngle(
    const autoware_msgs::Lane &laneinArea) {
  double diff = 0.0;
...
bool DecisionMakerNode::isLocalizationConvergence(double _x, double _y,
