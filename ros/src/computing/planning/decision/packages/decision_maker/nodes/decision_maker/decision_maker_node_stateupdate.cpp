...
void DecisionMakerNode::setupStateCallback(void) {
  ...
  ctx->setCallbackInFunc(
      state_machine::DRIVE_BEHAVIOR_STOPLINE_PLAN_STATE,
  ...
  // obstacle avoidance
  ctx->setCallbackUpdateFunc(
      state_machine::DRIVE_BEHAVIOR_OBSTACLE_AVOIDANCE_STATE,
...
void DecisionMakerNode::createShiftLane(void) {
  ...
  autoware_msgs::LaneArray shift_lanes = current_shifted_lane_array_ =
      current_based_lane_array_;
  if (!current_shifted_lane_array_.lanes.empty()) {
    ...
    for (auto &lane : shift_lanes.lanes) {
      ...
      for (auto &wp : lane.waypoints) {
        ...
        wp.change_flag = current_based_lane_array_.lanes.at(lane_idx)
                             .waypoints.at(wp_idx++)
                             .change_flag;
      }
      lane_idx++;
  ...
  try {
    for (auto it = begin(current_shifted_lane_array_.lanes);
         it != end(current_shifted_lane_array_.lanes) &&
         it_shift != end(shift_lanes.lanes);) {
      for (auto &wp : it->waypoints) {
        wp.change_flag = isRightShift ? 1 : 2;
      }
      it = current_shifted_lane_array_.lanes.insert(it + insert_lane_idx_offset,
                                                    *(it_shift++));
      it++;
      if (!isRightShift) {
        it++;
...
void DecisionMakerNode::updateLaneWaypointsArray(void) {
  ...
  for (auto &lane : current_shifted_lane_array_.lanes) {
    for (auto &wp : lane.waypoints) {
      ...
      if (CurrentStoplineTarget_.gid - 2 <= wp.gid &&
          wp.gid <= CurrentStoplineTarget_.gid + 2) {
        wp.wpstate.stopline_state = 0;
      }
    }
  }
    ...
    for (size_t wp_idx = 0; wp_idx < lane.waypoints.size() - 1; wp_idx++) {
      for (auto &stopline : stoplines) {
        ...
        if (amathutils::isIntersectLine(
                ...
                lane.waypoints.at(wp_idx + 1).pose.pose.position.y, bp.x, bp.y,
                fp.x, fp.y)) {
...
void DecisionMakerNode::updateStateStop(int status) {
  ...
  if (status) {
    if (current_velocity_ == 0.0 && !foundOtherVehicleForIntersectionStop_ &&
        !timerflag) {
