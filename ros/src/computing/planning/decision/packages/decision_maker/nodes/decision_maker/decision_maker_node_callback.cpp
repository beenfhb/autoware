...
void DecisionMakerNode::callbackFromCurrentPose(
    const geometry_msgs::PoseStamped &msg) {
  geometry_msgs::PoseStamped _pose = current_pose_ = msg;
  bool initLocalizationFlag =
      ctx->isCurrentState(state_machine::INITIAL_LOCATEVEHICLE_STATE);
  if (initLocalizationFlag &&
      isLocalizationConvergence(_pose.pose.position.x, _pose.pose.position.y,
                                _pose.pose.position.z, _pose.pose.orientation.x,
...
void DecisionMakerNode::callbackFromSimPose(
    const geometry_msgs::PoseStamped &msg) {
...
void DecisionMakerNode::callbackFromStateCmd(const std_msgs::Int32 &msg) {
  ROS_INFO("Received forcing state changing request: %llx",
           1ULL << (uint64_t)msg.data);
...
void DecisionMakerNode::callbackFromLaneChangeFlag(const std_msgs::Int32 &msg) {
  if (msg.data == enumToInteger<E_ChangeFlags>(E_ChangeFlags::LEFT) &&
      ctx->isCurrentState(
          state_machine::DRIVE_BEHAVIOR_ACCEPT_LANECHANGE_STATE)) {
    ctx->disableCurrentState(
        state_machine::DRIVE_BEHAVIOR_LANECHANGE_RIGHT_STATE);
...
void DecisionMakerNode::callbackFromConfig(
    const autoware_config_msgs::ConfigDecisionMaker &msg) {
...
void DecisionMakerNode::callbackFromLightColor(
    const ros::MessageEvent<autoware_msgs::TrafficLight const> &event) {
  ...
  if (!isManualLight) { // && topic.find("manage") == std::string::npos){
    ...
    if (current_traffic_light_ == state_machine::E_RED ||
        current_traffic_light_ == state_machine::E_YELLOW) {
      ctx->setCurrentState(
          state_machine::DRIVE_BEHAVIOR_TRAFFICLIGHT_RED_STATE);
      ctx->disableCurrentState(
          state_machine::DRIVE_BEHAVIOR_TRAFFICLIGHT_GREEN_STATE);
    } else {
      ctx->setCurrentState(
          state_machine::DRIVE_BEHAVIOR_TRAFFICLIGHT_GREEN_STATE);
      ctx->disableCurrentState(
...
void DecisionMakerNode::callbackFromObjectDetector(
    const autoware_msgs::CloudClusterArray &msg) {
  ...
  if (ctx->isCurrentState(state_machine::DRIVE_STATE)) {
    if (msg.clusters.size()) {
      ...
      for (const auto cluster : msg.clusters) {
        ...
        tflistener_baselink.transformPose(cluster.header.frame_id,
                                          cluster.header.stamp, cluster_pose,
                                          "base_link", baselink_pose);

        if (detectionArea_.x1 * param_detection_area_rate_ >=
                baselink_pose.pose.position.x &&
            baselink_pose.pose.position.x >=
                detectionArea_.x2 * param_detection_area_rate_ &&
            detectionArea_.y1 * param_detection_area_rate_ >=
                baselink_pose.pose.position.y &&
            baselink_pose.pose.position.y >=
                detectionArea_.y2 * param_detection_area_rate_) {
  ...
  if (l_detection_flag ||
      (ros::Time::now().toSec() - setFlagTime) >= 1.0 /*1.0sec*/) {
    foundOtherVehicleForIntersectionStop_ = l_detection_flag;
  }
}

void DecisionMakerNode::callbackFromPointsRaw(
    const sensor_msgs::PointCloud2::ConstPtr &msg) {
  if (ctx->setCurrentState(state_machine::INITIAL_LOCATEVEHICLE_STATE))
    Subs["points_raw"].shutdown();
}

void DecisionMakerNode::insertPointWithinCrossRoad(
    const std::vector<CrossRoadArea> &_intersects,
    ...
    for (auto &wp : lane.waypoints) {
      ...
      for (auto &area : intersects) {
        if (CrossRoadArea::isInsideArea(&area, pp)) {
          // area's
          if (area.insideLanes.empty() ||
              wp.gid != area.insideLanes.back().waypoints.back().gid + 1) {
...
void DecisionMakerNode::setWaypointState(autoware_msgs::LaneArray &lane_array) {
  ...
  for (auto &area : intersects) {
    for (auto &laneinArea : area.insideLanes) {
      ...
      int angle_deg =
          ((int)std::floor(calcIntersectWayAngle(laneinArea))); // normalized
      int steering_state;
        ...
        if (getDistance(bp.x, bp.y,
          ...
          if (amathutils::isIntersectLine(
            ...
            if (amathutils::isPointLeftFromLine(
              ...
              lane.waypoints.at(wp_idx).wpstate.stopline_state =
...
// for based waypoint
void DecisionMakerNode::callbackFromLaneWaypoint(
    const autoware_msgs::LaneArray &msg) {
  ...
  current_controlled_lane_array_ = current_shifted_lane_array_ =
      current_based_lane_array_; // controlled path
...
void DecisionMakerNode::callbackFromFinalWaypoint(
  ...
  if (!ctx->isCurrentState(state_machine::DRIVE_STATE)) {
    std::cerr << "State is not DRIVE_STATE[" << ctx->getCurrentStateName()
              << "]" << std::endl;
    return;
  }
  // cached
  ...
  size_t idx = param_stopline_target_waypoint_ +
               (current_velocity_ * param_stopline_target_ratio_);
  ...
  for (size_t i = (previous_idx > idx) ? idx : previous_idx; i <= idx; i++) {
    if (i < current_finalwaypoints_.waypoints.size()) {
      if (current_finalwaypoints_.waypoints.at(i).wpstate.stopline_state ==
  ...
  if (ctx->isCurrentState(
    ...
    for (size_t i = idx; i > 0; i--) {
      _TargetStateFlag = getStateFlags(
          current_finalwaypoints_.waypoints.at(i).wpstate.steering_state);
...
void DecisionMakerNode::callbackFromTwistCmd(
...
void DecisionMakerNode::callbackFromClosestWaypoint(
    const std_msgs::Int32 &msg) {
  closest_waypoint_ = msg.data;
}

void DecisionMakerNode::callbackFromVectorMapArea(
    const vector_map_msgs::AreaArray &msg) {
  initVectorMap();
}
void DecisionMakerNode::callbackFromVectorMapPoint(
    const vector_map_msgs::PointArray &msg) {
  initVectorMap();
}
void DecisionMakerNode::callbackFromVectorMapLine(
    const vector_map_msgs::LineArray &msg) {
  initVectorMap();
}
void DecisionMakerNode::callbackFromVectorMapCrossRoad(
    const vector_map_msgs::CrossRoadArray &msg) {
  initVectorMap();
}

void DecisionMakerNode::callbackFromCurrentVelocity(
    const geometry_msgs::TwistStamped &msg) {
