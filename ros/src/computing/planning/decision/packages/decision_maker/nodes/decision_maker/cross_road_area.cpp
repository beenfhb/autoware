...
CrossRoadArea *
CrossRoadArea::findClosestCrossRoad(const autoware_msgs::Lane &_finalwaypoints,
  ...
  if (!_finalwaypoints.waypoints.empty()) {
    _pa.x =
        _finalwaypoints.waypoints[TARGET_WAYPOINTS_NUM].pose.pose.position.x;
    _pa.y =
        _finalwaypoints.waypoints[TARGET_WAYPOINTS_NUM].pose.pose.position.y;
    _pa.z = 0.0;
  }
...
std::vector<geometry_msgs::Point> convhull(const CrossRoadArea *_TargetArea) {
  ...
  size_t l = 0;
  for (auto i = begin(_TargetArea->points); i != end(_TargetArea->points);
       i++) {
  ...
  for (auto p = begin(_TargetArea->points); p != end(_TargetArea->points);
       p++) {
...
bool CrossRoadArea::isInsideArea(const CrossRoadArea *_TargetArea,
