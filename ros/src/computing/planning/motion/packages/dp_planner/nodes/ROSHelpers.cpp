...
void ROSHelpers::ConvertFromPlannerHToAutowareVisualizePathFormat(
  ...
  for (unsigned int il = 0; il < paths.size(); il++) {
    for (unsigned int i = 0; i < paths.at(il).size(); i++) {
      ...
      if (i == localPlanner.m_iSafeTrajectory &&
          il == localPlanner.m_iCurrentTotalPathId) {
        lane_waypoint_marker.color.r = 1.0;
...
void ROSHelpers::ConvertFromAutowareCloudClusterObstaclesToPlannerH(
  ...
  for (unsigned int i = 0; i < clusters.clusters.size(); i++) {
    ...
    PlannerHNS::DetectedObject obj;
    obj.center.pos =
        PlannerHNS::GPSPoint(clusters.clusters.at(i).centroid_point.point.x,
    ...
    //		if(size <= 0.25 || size >= 5 || distance_y > 20.0 || distance_x
    //> 20.0) 			continue;

    //		if(distance_y > 10.0 || distance_x > 10.0)
    //			continue;

    if (distance_x <= car_info.length / 1.5 &&
        distance_y <= car_info.width / 1.5) // don't detect yourself
      continue;

    nOrPoints += point_cloud.points.size();
    nPoints += obj.contour.size();
    // std::cout << " Distance_X: " << distance_x << ", " << " Distance_Y: " <<
    // distance_y << ", " << " Size: " << size << std::endl;
...
PlannerHNS::SHIFT_POS ROSHelpers::ConvertShiftFromAutowareToPlannerH(
    const PlannerXNS::AUTOWARE_SHIFT_POS &shift) {
...
PlannerXNS::AUTOWARE_SHIFT_POS ROSHelpers::ConvertShiftFromPlannerHToAutoware(
