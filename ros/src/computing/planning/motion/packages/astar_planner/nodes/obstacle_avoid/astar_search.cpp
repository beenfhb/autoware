...
void AstarSearch::createStateUpdateTableLocal(int angle_size) {
  ...
  for (int i = 0; i < angle_size; i++) {
    ...
    std::cos(-1.0 * M_PI_2 + robot_angle +
    descretized_angle);
    nu.shift_y     = left_circle_center_y + minimum_turning_radius_ *
    std::sin(-1.0 * M_PI_2 + robot_angle +
    descretized_angle);
...
bool AstarSearch::search() {
  ...
  while (!openlist_.empty()) {
    ...
    for (const auto &state : state_update_table_[sn.index_theta]) {
      ...
      double next_hc =
          nodes_[next.index_y][next.index_x]
                [0].hc; // wavefront or distance transform heuristic
