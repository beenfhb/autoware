...
SearchInfo::SearchInfo()
    : map_set_(false), start_set_(false), goal_set_(false), path_set_(false),
      closest_waypoint_index_(-1), obstacle_waypoint_index_(-1),
      start_waypoint_index_(-1), goal_waypoint_index_(-1), state_(""),
  ...
  private_nh_.param<double>("avoid_velocity_limit_mps",
                            avoid_velocity_limit_mps_, 4.166);
...
double SearchInfo::calcPathLength(const autoware_msgs::Lane &lane,
  ...
  for (int i = start_waypoint_index; i < goal_waypoint_index; i++) {
    ...
    dist_sum += astar_planner::calcDistance(p1.position.x, p1.position.y,
...
void SearchInfo::mapCallback(const nav_msgs::OccupancyGridConstPtr &msg) {
  ...
  geometry_msgs::Pose ogm_in_map =
      astar_planner::transformPose(map_.info.origin, map2ogm_frame);
...
void SearchInfo::currentPoseCallback(
    const geometry_msgs::PoseStampedConstPtr &msg) {
  current_pose_ = *msg;
  health_checker_ptr_->NODE_ACTIVATE();
  health_checker_ptr_->CHECK_RATE(
      "topic_ndt_pose_subscribe_rate_slow_in_obstacle_avoid", 8.0, 4.0, 2.0,
      "topic ndt_pose subscribe rate in obstacle_avoid node is slow.");
  if (closest_waypoint_index_ != -1 && path_set_) {
    autoware_msgs::Waypoint closest_waypoint =
        subscribed_waypoints_.waypoints[closest_waypoint_index_];
    double dist = std::sqrt(std::pow(closest_waypoint.pose.pose.position.x -
                                         current_pose_.pose.position.x,
                                     2) +
                            std::pow(closest_waypoint.pose.pose.position.y -
                                         current_pose_.pose.position.y,
                                     2));
    health_checker_ptr_->CHECK_MAX_VALUE(
        "closest_waypoint_distance", dist, 0.5, 1.0, 2.0,
        "distance between closest_waypoint and current_pose is too long.");
  }
  return;
}

void SearchInfo::currentVelocityCallback(
...
void SearchInfo::goalCallback(const geometry_msgs::PoseStampedConstPtr &msg) {
  ...
  try {
    tf_listener_.lookupTransform(map_frame, goal_frame, ros::Time(0),
                                 map2world);
  } catch (tf::TransformException ex) {
    ROS_ERROR("%s", ex.what());
    return;
  ...
  goal_pose_local_.pose =
      astar_planner::transformPose(goal_pose_global_.pose, ogm2map_);
  ...
  try {
    tf_listener_.lookupTransform(map_frame_, start_frame, ros::Time(0),
                                 map2start_frame);
  } catch (tf::TransformException ex) {
    ROS_ERROR("%s", ex.what());
    return;
  ...
  // Set start pose
  start_pose_global_.pose =
      astar_planner::transformPose(current_pose_.pose, map2start_frame);
  start_pose_global_.header = current_pose_.header;
  start_pose_local_.pose =
      astar_planner::transformPose(start_pose_global_.pose, ogm2map_);
...
void SearchInfo::obstacleWaypointCallback(const std_msgs::Int32ConstPtr &msg) {
  ...
  if (msg->data < 0 || closest_waypoint_index_ < 0 ||
      current_waypoints_.waypoints.empty()) {
    return;
  }
  ...
  if (obstacle_waypoint_index_ >=
          prev_obstacle_waypoint_index - same_obstacle_threshold &&
  ...
  // Handle out of range
  if (goal_waypoint_index_ >=
      static_cast<int>(getCurrentWaypoints().waypoints.size()))
    goal_waypoint_index_ = getCurrentWaypoints().waypoints.size() - 1;

  double original_path_length = calcPathLength(
      current_waypoints_, start_waypoint_index_, goal_waypoint_index_);
  ...
  relative_transit_pose.orientation =
      current_waypoints_.waypoints[obstacle_waypoint_index_]
          .pose.pose.orientation;
  tf::Pose obstacle_pose_tf;
  tf::poseMsgToTF(
      current_waypoints_.waypoints[obstacle_waypoint_index_].pose.pose,
      obstacle_pose_tf);

  transit_pose_global_.pose =
      astar_planner::transformPose(relative_transit_pose, obstacle_pose_tf);
  transit_pose_local_.pose =
      astar_planner::transformPose(transit_pose_global_.pose, ogm2map_);
