...
#include "../velocity_set/velocity_set_info.h"
#include "../velocity_set/velocity_set_path.h"
                           ...
                           const double deceleration_range, const ros::Publisher& detection_range_pub)
{
  ...
  waypoint_marker_decelerate.type = visualization_msgs::Marker::SPHERE_LIST;
  ...
  crosswalk_marker.lifetime = ros::Duration(0.5);
  ...
  for (const auto &cross_walk : crosswalk_manager.getRelevantCrossWalks())
  {
    auto vertices = cross_walk.getVertices();
    crosswalk_marker.points.push_back(vertices.at(2));
    crosswalk_marker.points.push_back(vertices.at(1));
    crosswalk_marker.points.push_back(vertices.at(0));

    crosswalk_marker.points.push_back(vertices.at(0));
    crosswalk_marker.points.push_back(vertices.at(3));
    crosswalk_marker.points.push_back(vertices.at(2));
  }

  // publish marker
  if(crosswalk_marker.points.size() > 0) {
...
EControl crossWalkDetection(const pcl::PointCloud<pcl::PointXYZ>& points, const std::vector<CrossWalkHandler>& crosswalks,
                            ...
                            ObstaclePoints* obstacle_points)
{
  ...
  for (const auto& c : crosswalks)
  {
    auto detection_area = c.getVertices();
    std::vector<geometry_msgs::Point> relative_detection_area = calcRelativeCoordinates(detection_area, localizer_pose.pose);
    ...
    for (const auto& p : points)
    {
      ...
      point_temp.y = p.y;
      point_temp.z = p.z;
      if (isWithinArea(point_temp,relative_detection_area))
      {
        stop_count++;
                       ...
                       const int wpidx_detection_result_by_other_nodes)
{
  ...
  *obstacle_type = EObstacleType::NONE;
  ...
  for (int i = closest_waypoint; i < closest_waypoint + STOP_SEARCH_DISTANCE; i++)
  {
    ...
    std::vector<CrossWalkHandler> relevant_crosswalks = crosswalk_manager.getRelevantCrossWalks(i);
    if (!relevant_crosswalks.empty())
    {
                         ...
                         int* obstacle_waypoint, ObstaclePoints* obstacle_points)
{
  ...
  int stop_obstacle_waypoint =
    detectStopObstacle(points, closest_waypoint, lane, crosswalk_manager, vs_info.getStopRange(),
...
{
  ObstaclePoints obstacle_points;
  EControl detection_result = pointsDetection(vs_info.getPoints(), closest_waypoint, lane, crosswalk_manager, vs_info,
                                              obstacle_waypoint, &obstacle_points);
  displayDetectionRange(lane, crosswalk_manager, closest_waypoint, detection_result, *obstacle_waypoint, vs_info.getStopRange(),
...
int main(int argc, char** argv)
{
  ...
  // class
  CrossWalkManager crosswalk_manager;
  crosswalk_manager.setMultipleDetectionFlag(enable_multiple_crosswalk_detection);
