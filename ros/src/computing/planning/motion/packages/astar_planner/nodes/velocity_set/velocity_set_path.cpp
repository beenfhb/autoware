...
VelocitySetPath::VelocitySetPath() : set_path_(false), current_vel_(0) {
...
void VelocitySetPath::setTemporalWaypoints(
    int temporal_waypoints_size, int closest_waypoint,
  ...
  for (int i = 0; i < temporal_waypoints_size; i++) {
    ...
    temporal_waypoints_.waypoints.push_back(
        new_waypoints_.waypoints[closest_waypoint + i]);
  }

  return;
}

void VelocitySetPath::changeWaypointsForDeceleration(double deceleration,
  ...
  for (int index = obstacle_waypoint + extra; index >= closest_waypoint;
       index--) {
    ...
    // v = sqrt( (v0)^2 + 2ax )
    double changed_vel =
        std::sqrt(square_vel_min +
                  2.0 * deceleration * calcInterval(index, obstacle_waypoint));
...
void VelocitySetPath::avoidSuddenAcceleration(double deceleration,
                                              int closest_waypoint) {
  ...
  for (int i = 0;; i++) {
    ...
    // v = root((v0)^2 + 2ax)
    double changed_vel =
        std::sqrt(square_current_vel +
                  2 * deceleration *
                      calcInterval(closest_waypoint, closest_waypoint + i)) +
        velocity_offset_;

    // Don't exceed original velocity
    if (changed_vel >
        new_waypoints_.waypoints[closest_waypoint + i].twist.twist.linear.x)
      return;
...
void VelocitySetPath::avoidSuddenDeceleration(double velocity_change_limit,
                                              double deceleration,
                                              int closest_waypoint) {
  if (closest_waypoint < 0)
    return;
  ...
  // not avoid braking
  if (current_vel_ -
          new_waypoints_.waypoints[closest_waypoint].twist.twist.linear.x <
      velocity_change_limit)
    return;

  // std::cout << "avoid sudden braking!" << std::endl;

  double square_vel = (current_vel_ - velocity_change_limit) *
                      (current_vel_ - velocity_change_limit);
  for (int i = 0;; i++) {
    if (!checkWaypoint(closest_waypoint + i, __FUNCTION__))
      return;
    ...
    double changed_vel =
        square_vel -
        2 * deceleration * calcInterval(closest_waypoint, closest_waypoint + i);

    if (changed_vel < 0)
      break;

    new_waypoints_.waypoints[closest_waypoint + i].twist.twist.linear.x =
        std::sqrt(changed_vel);
  }
}

void VelocitySetPath::changeWaypointsForStopping(int stop_waypoint,
                                                 int obstacle_waypoint,
                                                 int closest_waypoint,
                                                 double deceleration) {
  if (closest_waypoint < 0)
  ...
  for (int index = stop_waypoint; index >= closest_waypoint; index--) {
    ...
    // v = (v0)^2 + 2ax, and v0 = 0
    double changed_vel =
        std::sqrt(2.0 * deceleration * calcInterval(index, stop_waypoint));
...
double VelocitySetPath::calcInterval(const int begin, const int end) const {
  // check index
  if (begin < 0 || begin >= getPrevWaypointsSize() || end < 0 ||
      end >= getPrevWaypointsSize()) {
...
void VelocitySetPath::resetFlag() { set_path_ = false; }

void VelocitySetPath::waypointsCallback(
...
void VelocitySetPath::currentVelocityCallback(
