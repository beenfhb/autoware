...
bool HevCnt::Process() { return true; }

bool HevCnt::Stop() { return true; }
...
void HevCnt::GetBrakeInf(BrakeInf *brake) {
  ...
  brake->brakeLamp = _brakeInf.brakeLamp;
  brake->blinkerLeft = _brakeInf.blinkerLeft;
  brake->blinkerRight = _brakeInf.blinkerRight;
...
void HevCnt::UpdateDriveState(REP_DRIVE_INFO_INDEX index) {
  ...
  case REP_GAS_PEDAL:
    _hevCnt->GetGasStroke((int &)_drvInf.actualPedalStr,
                          (int &)_drvInf.targetPedalStr,
  ...
  case REP_WHEEL_VELOCITY_F:
    _hevCnt->GetWheelVelocF((float &)_otherInf.velocFrFromP,
                            (float &)_otherInf.velocFlFromP);
    break;
  case REP_WHEEL_VELOCITY_R:
    _hevCnt->GetWheelVelocR((float &)_otherInf.velocRrFromP,
                            (float &)_otherInf.velocRlFromP);
    break;
  case REP_BRAKE_PEDAL:
    _hevCnt->GetBrakeStroke((int &)_brakeInf.actualPedalStr,
                            (int &)_brakeInf.targetPedalStr,
                            (int &)_brakeInf.inputPedalStr);
    break;
  case REP_BRAKE_PEDAL_FROMOBD:
    _hevCnt->GetBrakeStrokeFromOBD((float &)_otherInf.brkPedalStrFromP,
                                   (bool &)_brakeInf.pressed);
    break;
  case REP_SHIFT_POS:
    _hevCnt->GetShiftMode((int &)_drvInf.actualShift,
                          (int &)_drvInf.targetShift,
                          (int &)_drvInf.inputShift);
    break;
  ...
  case REP_DTC_STATUS: //_hevCnt->GetDtcStatus((char&)_otherInf.dtcData1,
    ...
    //(char&)_otherInf.dtcData4, (char&)_otherInf.dtcData5,
    //(char&)_otherInf.dtcData6, (char&)_otherInf.dtcData7,
    //(char&)_otherInf.dtcData8);
    break;
  case REP_BRAKE_STATUS:
    _hevCnt->GetBrakeStatus((unsigned char &)_brakeInf.brakeLamp,
                            (unsigned char &)_brakeInf.blinkerLeft,
                            (unsigned char &)_brakeInf.blinkerRight,
                            (unsigned char &)_brakeInf.brakeMode);
    break;
  default:
    printf("\n");
    break;
...
void HevCnt::UpdateBattState(REP_BATT_INFO_INDEX index) {
  switch (index) {
  case REP_BATT_INFO:
    _hevCnt->GetBattInfo((float &)_battInf.soc, (int &)_battInf.max_temp,
...
void HevCnt::ReceiveVersion(char c0, char c1, char c2, char c3, char c4,
...
void HevCnt::rcvTime(char *date) {
  getDate();
  sprintf(date, "%d/%d/%d/%d:%d:%d.%ld", _s_time->tm_year + 1900,
          _s_time->tm_mon + 1, _s_time->tm_mday, _s_time->tm_hour,
          _s_time->tm_min, _s_time->tm_sec, _getTime.tv_usec);
}

void HevCnt::SetSeat(int seat) { _sensInf.seat = seat; }

void HevCnt::SetOfz(int index, int val) { _sensInf.ofz[index] = val; }

void HevCnt::SetCnts(int index) { _sensInf.cntS[index] += 1; }

int HevCnt::GetSeat() { return _sensInf.seat; }

int HevCnt::GetOfz(int index) { return _sensInf.ofz[index]; }

int HevCnt::GetCnts(int index) { return _sensInf.cntS[index]; }

float HevCnt::GetDist() { return _sensInf.distance; }

void HevCnt::SetStrMode(int mode) { _hevCnt->SetStrMode(mode); }

void HevCnt::SetStrCMode(int cmode) { _hevCnt->SetStrControlMode(cmode); }

void HevCnt::SetStrOMOde(int omode) { _hevCnt->SetStrOverrideMode(omode); }
...
void HevCnt::SetStrServo(int servo) { _hevCnt->SetStrServo(servo); }

void HevCnt::SetDrvMode(int mode) { _hevCnt->SetDrvMode(mode); }

void HevCnt::SetDrvCMode(int cmode) { _hevCnt->SetDrvControlMode(cmode); }

void HevCnt::SetDrvOMode(int omode) { _hevCnt->SetDrvOverrideMode(omode); }

void HevCnt::SetDrvStroke(int stroke) { _hevCnt->SetGasStroke(stroke); }
...
void HevCnt::SetDrvShiftMode(int shift) { _hevCnt->SetShiftMode(shift); }

void HevCnt::SetDrvServo(int servo) { _hevCnt->SetDrvServo(servo); }

void HevCnt::SetBrakeStroke(int stroke) { _hevCnt->SetBrakeStroke(stroke); }

void HevCnt::SetBrakeLamp(unsigned char lamp) { _hevCnt->SetBrakeLamp(lamp); }

void HevCnt::SetBlinkerLeft(unsigned char blink_left) {
  _hevCnt->SetBlinkerLeft(blink_left);
}

void HevCnt::SetBlinkerRight(unsigned char blink_right) {
  _hevCnt->SetBlinkerRight(blink_right);
}

void HevCnt::SetBrakeMode(unsigned char mode) { _hevCnt->SetBrakeMode(mode); }

void HevCnt::SetControlGain(int index, int gain) {
  _hevCnt->SetControlGain(index, gain);
}

void HevCnt::SndDiagReq(HEV_ECU kind) { _hevCnt->GetDiag(kind); }

void HevCnt::SndDiagClear(HEV_ECU kind) { _hevCnt->ClearDiag(kind); }

void HevCnt::SndErrReq() { _hevCnt->ReadErrorStatusReq(); }

void HevCnt::SndErrClear() { _hevCnt->ClearCntDiag(); }

void HevCnt::SndVersionReq() { _hevCnt->ReadVersionReq(); }
...
void HevCnt::SaveConfig() { _hevCnt->SaveControlGain(); }
