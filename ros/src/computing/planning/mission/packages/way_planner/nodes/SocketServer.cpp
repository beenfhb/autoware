...
int HMISocketServer::InitSocket(int port_send, int port_receive) {
  ...
  if (pthread_create(&sock_thread_tid_send, nullptr,
                     &HMISocketServer::ThreadMainSend, this) != 0) {
  ...
  int ret_r = bind(m_Socket_receive, (struct sockaddr *)&addr_receive,
                   sizeof(addr_receive));
  if (ret_r == -1) {
    std::perror("bind");
    return -1;
  ...
  if (pthread_create(&sock_thread_tid_receive, nullptr,
                     &HMISocketServer::ThreadMainReceive, this) != 0) {
    std::perror("pthread_create");
  }
...
void *HMISocketServer::ThreadMainSend(void *pSock) {
  ...
  while (!pS->m_bExitMainLoop) {
    ...
    client_sock =
        accept(pS->m_Socket_send, reinterpret_cast<sockaddr *>(&client), &len);
    if (client_sock == -1) {
      std::perror("accept");
      usleep(500);
      continue;
...
void *HMISocketServer::ThreadMainReceive(void *pSock) {
  ...
  while (!pS->m_bExitMainLoop) {
    ...
    client_sock = accept(pS->m_Socket_receive,
