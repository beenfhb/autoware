...
void ROSHelpers::ConvertFromPlannerHPointsToAutowarePathFormat(
...
void ROSHelpers::ConvertFromPlannerHToAutowarePathFormat(
  ...
  for (unsigned int i = 0; i < path.size(); i++) {
    ...
    // std::cout << "PathID: " << i << ", LID:" << path.at(i).laneId << ",
    // LeftLaneID: " <<  path.at(i).LeftLaneId << ", RightLaneID: " <<
    // path.at(i).RightLaneId << std::endl;

    for (unsigned int iaction = 0; iaction < path.at(i).actionCost.size();
         iaction++) {
      if (path.at(i).actionCost.at(iaction).first ==
          PlannerHNS::RIGHT_TURN_ACTION)
        wp.dtlane.dir = 1;
      else if (path.at(i).actionCost.at(iaction).first ==
...
void ROSHelpers::ConvertFromRoadNetworkToAutowareVisualizeMapFormat(
  ...
  for (unsigned int i = 0; i < map.roadSegments.size(); i++)
    for (unsigned int j = 0; j < map.roadSegments.at(i).Lanes.size(); j++)
      ROSHelpers::ConvertFromPlannerHToAutowarePathFormat(
          map.roadSegments.at(i).Lanes.at(j).points, map_lane_array);
  ...
  for (unsigned int i = 0; i < map_lane_array.lanes.size(); i++) {
    ...
    for (unsigned int j = 0; j < map_lane_array.lanes.at(i).waypoints.size();
         j++) {
  ...
  ROSHelpers::createGlobalLaneArrayOrientationMarker(map_lane_array,
                                                     markerArray);
  total_color.r = 0.99;
  total_color.g = 0.99;
  total_color.b = 0.99;
  ...
  for (unsigned int i = 0; i < map.stopLines.size(); i++) {
    ...
    ROSHelpers::ConvertFromPlannerHPointsToAutowarePathFormat(
        map.stopLines.at(i).points, lane_array_2);

    stop_waypoint_marker.points.clear();
    stop_waypoint_marker.id = count;
    for (unsigned int j = 0; j < lane_array_2.lanes.size(); j++) {

      for (unsigned int k = 0; k < lane_array_2.lanes.at(j).waypoints.size();
           k++) {
        geometry_msgs::Point point;
...
void ROSHelpers::ConvertFromPlannerHToAutowareVisualizePathFormat(
  ...
  for (unsigned int i = 0; i < globalPaths.size(); i++) {
    ...
    //			  dir_marker.pose.orientation =
    //tf::createQuaternionMsgFromYaw(globalPaths.at(i).at(j).pos.a);
  ...
  if (globalPaths.size() > 0) {
    ...
    for (unsigned int i = close_index + 1; i < globalPaths.at(0).size(); i++) {
      ...
      if (d > min_distance && globalPaths.at(0).at(i).pFronts.size() > 1) {
        for (unsigned int j = 0; j < globalPaths.at(0).at(i).pFronts.size();
             j++) {
          PlannerHNS::WayPoint *wp = globalPaths.at(0).at(i).pFronts.at(j);

          bool bFound = false;
          for (unsigned int ib = 0; ib < branches.size(); ib++) {
            if (wp->actionCost.size() > 0 &&
                branches.at(ib)->actionCost.size() > 0 &&
                branches.at(ib)->actionCost.at(0).first ==
                    wp->actionCost.at(0).first) {
              bFound = true;
              break;
            }
          }

          if (wp->actionCost.size() > 0 && !bFound &&
              closest_wp.laneId != wp->laneId)
