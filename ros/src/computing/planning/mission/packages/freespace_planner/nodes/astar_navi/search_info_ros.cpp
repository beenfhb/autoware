...
SearchInfo::SearchInfo()
    : map_set_(false), start_set_(false), goal_set_(false) {}
...
void SearchInfo::mapCallback(const nav_msgs::OccupancyGridConstPtr &msg) {
  ...
  geometry_msgs::Pose ogm_in_map =
      astar::transformPose(map_.info.origin, map2ogm_frame);
...
void SearchInfo::startCallback(
  ...
  start_pose_local_.pose =
      astar::transformPose(start_pose_global_.pose, ogm2map_);
...
void SearchInfo::goalCallback(const geometry_msgs::PoseStampedConstPtr &msg) {
  ...
  try {
    tf_listener_.lookupTransform(global_frame, goal_frame, ros::Time(0),
                                 map2world);
  } catch (tf::TransformException ex) {
    ROS_ERROR("%s", ex.what());
    return;
  ...
  goal_pose_local_.pose =
      astar::transformPose(goal_pose_global_.pose, ogm2map_);
