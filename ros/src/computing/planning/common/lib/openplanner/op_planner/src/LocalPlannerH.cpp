...
/// \brief OpenPlanner's local planing functions combines in one process, used
/// in simulation vehicle and OpenPlanner old implementation like dp_planner
/// node. \author Hatem Darweesh \date Dec 14, 2016
...
void LocalPlannerH::Init(const ControllerParams &ctrlParams,
...
void LocalPlannerH::InitBehaviorStates() {
  ...
  m_pMissionCompleteState = new MissionAccomplishedState(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), 0);
  m_pGoalState =
      new GoalState(m_pStopState->m_pParams, m_pStopState->GetCalcParams(),
                    m_pMissionCompleteState);
  m_pGoToGoalState = new ForwardState(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoalState);
  m_pWaitState = new WaitState(m_pStopState->m_pParams,
                               m_pStopState->GetCalcParams(), m_pGoToGoalState);
  m_pInitState = new InitState(m_pStopState->m_pParams,
                               m_pStopState->GetCalcParams(), m_pGoToGoalState);
  m_pFollowState = new FollowState(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoToGoalState);
  m_pAvoidObstacleState = new SwerveState(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoToGoalState);
  m_pTrafficLightStopState = new TrafficLightStopState(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoToGoalState);
  m_pTrafficLightWaitState = new TrafficLightWaitState(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoToGoalState);
  m_pStopSignWaitState = new StopSignWaitState(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoToGoalState);
  m_pStopSignStopState = new StopSignStopState(m_pStopState->m_pParams,
                                               m_pStopState->GetCalcParams(),
...
void LocalPlannerH::FirstLocalizeMe(const WayPoint &initCarPos) {
  ...
  m_OdometryState.pos.x =
      initCarPos.pos.x + (m_CarInfo.wheel_base / 2.0 * cos(initCarPos.pos.a));
  m_OdometryState.pos.y =
      initCarPos.pos.y + (m_CarInfo.wheel_base / 2.0 * sin(initCarPos.pos.a));
}
...
void LocalPlannerH::LocalizeMe(const double &dt) {
  ...
  if (m_CurrentShift == SHIFT_POS_DD) {
    ...
    m_OdometryState.pos.a +=
        m_CurrentVelocity * dt * tan(m_CurrentSteering) / m_CarInfo.wheel_base;
  ...
  m_OdometryState.pos.a =
      atan2(sin(m_OdometryState.pos.a), cos(m_OdometryState.pos.a));
  m_OdometryState.pos.a = UtilityH::FixNegativeAngle(m_OdometryState.pos.a);

  state.pos.a = m_OdometryState.pos.a;
  state.pos.x =
      m_OdometryState.pos.x - (m_CurrentVelocity * dt * (m_CarInfo.wheel_base) *
                               cos(m_OdometryState.pos.a));
  state.pos.y = m_OdometryState.pos.y -
                (m_CurrentVelocity * dt * (m_CarInfo.wheel_base / 2.0) *
                 sin(m_OdometryState.pos.a));
}

void LocalPlannerH::UpdateState(const PlannerHNS::VehicleState &state,
                                const bool &bUseDelay) {
  if (!bUseDelay) {
    ...
    //				 << ", Diff: " << diff
    //				 << ", inc: " << inc << std::endl;
    if (UtilityH::GetTimeDiffNow(m_SteerDelayTimer) >
...
void LocalPlannerH::AddAndTransformContourPoints(
    const PlannerHNS::DetectedObject &obj,
    std::vector<PlannerHNS::WayPoint> &contourPoints) {
  contourPoints.clear();
...
void LocalPlannerH::TransformPoint(const PlannerHNS::WayPoint &refPose,
...
bool LocalPlannerH::GetNextTrafficLight(
    const int &prevTrafficLightId,
  ...
  for (unsigned int i = 0; i < trafficLights.size(); i++) {
    double d = hypot(trafficLights.at(i).pos.y - state.pos.y,
                     trafficLights.at(i).pos.x - state.pos.x);
    if (d <= trafficLights.at(i).stoppingDistance) {
      double a_diff = UtilityH::AngleBetweenTwoAnglesPositive(
          UtilityH::FixNegativeAngle(trafficLights.at(i).pos.a),
          UtilityH::FixNegativeAngle(state.pos.a));

      if (a_diff < M_PI_2 && trafficLights.at(i).id != prevTrafficLightId) {
        // std::cout << "Detected Light, ID = " << trafficLights.at(i).id << ",
        // Distance = " << d << ", Angle = " << trafficLights.at(i).pos.a*RAD2DEG
        // << ", Car Heading = " << state.pos.a*RAD2DEG << ", Diff = " <<
...
void LocalPlannerH::CalculateImportantParameterForDecisionMaking(
    const PlannerHNS::VehicleState &car_state, const int &goalID,
    const bool &bEmergencyStop, const vector<TrafficLight> &detectedLights,
    const TrajectoryCost &bestTrajectory) {
  PreCalculatedConditions *pValues = m_pCurrentBehaviorState->GetCalcParams();

  double critical_long_front_distance = m_CarInfo.wheel_base / 2.0 +
                                        m_CarInfo.length / 2.0 +
                                        m_params.verticalSafetyDistance;

  pValues->minStoppingDistance =
      -pow(car_state.speed, 2) / (m_CarInfo.max_deceleration);

  pValues->iCentralTrajectory =
      m_pCurrentBehaviorState->m_pParams->rollOutNumber / 2;
  ...
  if (bestTrajectory.lane_index >= 0)
    ...
    PlannerHNS::PlanningHelpers::GetRelativeInfoRange(
        m_TotalPath, state,
        m_params.rollOutDensity * m_params.rollOutNumber / 2.0 + 0.1, info);
  ...
  if (m_TotalPath.size() > 0)
    distanceToClosestStopLine =
        PlanningHelpers::GetDistanceToClosestStopLineAndCheck(
            m_TotalPath.at(pValues->iCurrSafeLane), state, 0, stopLineID,
            stopSignID, trafficLightID) -
        critical_long_front_distance;

  if (distanceToClosestStopLine > 0 &&
      distanceToClosestStopLine < pValues->minStoppingDistance) {
    ...
    // std::cout << "LP => D: " << pValues->distanceToStop() << ", PrevSignID: "
    // << pValues->prevTrafficLightID << ", CurrSignID: " <<
    // pValues->currentTrafficLightID << ", Green: " << bGreenTrafficLight <<
    // endl;
  }

  // 	cout << "Distance To Closest: " << distanceToClosestStopLine << ", Stop
  // LineID: " << stopLineID << ", Stop SignID: " << stopSignID << ", TFID: " <<
  // trafficLightID << endl;
  ...
  // cout << "Distances: " << pValues->stoppingDistances.size() << ", Distance
  // To Stop : " << pValues->distanceToStop << endl;
}

void LocalPlannerH::UpdateCurrentLane(PlannerHNS::RoadNetwork &map,
  ...
  if (!pPathLane) {
    cout << "Performance Alert: Can't Find Lane Information in Global Path, "
            "Searching the Map :( "
         << endl;
    pMapLane =
        MappingHelpers::GetClosestLaneFromMap(state, map, search_distance);
  }
...
void LocalPlannerH::SimulateOdoPosition(
    const double &dt, const PlannerHNS::VehicleState &vehicleState) {
  SetSimulatedTargetOdometryReadings(vehicleState.speed, vehicleState.steer,
                                     vehicleState.shift);
...
bool LocalPlannerH::NoWayTest(const double &min_distance,
                              const int &iGlobalPathIndex) {
  if (m_TotalPath.size() == 0)
    return false;

  PlannerHNS::RelativeInfo info;
  PlanningHelpers::GetRelativeInfo(m_TotalPath.at(iGlobalPathIndex), state,
                                   info);

  double d = 0;
  for (unsigned int i = info.iFront;
       i < m_TotalPath.at(iGlobalPathIndex).size() - 1; i++) {
    d += hypot(m_TotalPath.at(iGlobalPathIndex).at(i + 1).pos.y -
                   m_TotalPath.at(iGlobalPathIndex).at(i).pos.y,
               m_TotalPath.at(iGlobalPathIndex).at(i + 1).pos.x -
                   m_TotalPath.at(iGlobalPathIndex).at(i).pos.x);
    if (d > min_distance)
      return false;
  }
...
bool LocalPlannerH::SelectSafeTrajectoryAndSpeedProfile(
    const PlannerHNS::VehicleState &vehicleState) {
  PlannerHNS::PreCalculatedConditions *preCalcPrams =
      m_pCurrentBehaviorState->GetCalcParams();

  bool bNewTrajectory = false;

  if (m_TotalPath.size() > 0) {
    int currIndex = PlannerHNS::PlanningHelpers::GetClosestNextPointIndexFast(
        m_Path, state);
    ...
    if (m_RollOuts.size() == 0 || currIndex > index_limit ||
      ...
      planner.GenerateRunoffTrajectory(
          m_TotalPath, state,
      ...
      if (preCalcPrams->iPrevSafeLane >= 0 &&
          ...
          preCalcPrams->iPrevSafeTrajectory <
              (int)m_RollOuts.at(preCalcPrams->iPrevSafeLane).size()) {
        m_Path = m_RollOuts.at(preCalcPrams->iPrevSafeLane)
                     .at(preCalcPrams->iPrevSafeTrajectory);
...
PlannerHNS::BehaviorState LocalPlannerH::GenerateBehaviorState(
    const PlannerHNS::VehicleState &vehicleState) {
  PlannerHNS::PreCalculatedConditions *preCalcPrams =
      m_pCurrentBehaviorState->GetCalcParams();
  ...
  double average_braking_distance =
      -pow(vehicleState.speed, 2) / (m_CarInfo.max_deceleration) +
      m_params.additionalBrakingDistance;

  if (average_braking_distance < 15)
    average_braking_distance = 15;

  currentBehavior.indicator = PlanningHelpers::GetIndicatorsFromPath(
      m_Path, state, average_braking_distance);

  return currentBehavior;
}

// double LocalPlannerH::UpdateVelocityDirectlyToTrajectory(const BehaviorState&
// beh, const VehicleState& CurrStatus, const double& dt)
// {
//	RelativeInfo info, total_info;
//	PlanningHelpers::GetRelativeInfo(m_TotalPath.at(m_iCurrentTotalPathId),
//state, total_info); 	PlanningHelpers::GetRelativeInfo(m_Path, state, info);
//	double average_braking_distance = -pow(CurrStatus.speed,
//2)/(m_CarInfo.max_deceleration); 	double max_velocity	=
//PlannerHNS::PlanningHelpers::GetVelocityAhead(m_TotalPath.at(m_iCurrentTotalPathId),
//total_info, average_braking_distance);
//
...
//	}
//	else if(beh.state == TRAFFIC_LIGHT_STOP_STATE || beh.state ==
//STOP_SIGN_STOP_STATE || beh.state == STOP_SIGN_WAIT_STATE || beh.state ==
//TRAFFIC_LIGHT_WAIT_STATE)
//	{
//		PlanningHelpers::GetFollowPointOnTrajectory(m_Path, info,
//beh.stopDistance - critical_long_front_distance, point_index);
//
...
//	{
//		double targe_acceleration = -pow(CurrStatus.speed,
//2)/(2.0*(beh.followDistance - critical_long_front_distance));
//		if(targe_acceleration <= 0 &&  targe_acceleration >
//m_CarInfo.max_deceleration/2.0)
//		{
//			double target_velocity = (targe_acceleration * dt) +
//CurrStatus.speed; 			for(unsigned int i = 0; i < m_Path.size(); i++)
//			{
//				if(m_pCurrentBehaviorState->GetCalcParams()->iCurrSafeTrajectory
...
//			}
//
//			//cout << "Accelerate -> Target V: " << target_velocity <<
//", Brake D: " <<  average_braking_distance << ", Acceleration: " <<
//targe_acceleration << endl;
//		}
//		else
//		{
//			WayPoint pursuite_point =
//PlanningHelpers::GetFollowPointOnTrajectory(m_Path, info, beh.followDistance -
//critical_long_front_distance, point_index); 			double inc = CurrStatus.speed; 			int
//iRange = point_index - info.iBack;
//
//			if(iRange > 0)
//				inc = inc / (double)iRange;
//			else
...
//				 if(i < m_Path.size() && i >= 0)
//				 {
//					 target_velocity = target_velocity < 0 ? 0 :
//target_velocity;
//					 if(m_pCurrentBehaviorState->GetCalcParams()->iCurrSafeTrajectory
//== m_pCurrentBehaviorState->GetCalcParams()->iCentralTrajectory)
//						 m_Path.at(i).v =
//target_velocity; 					 else 						 m_Path.at(i).v = target_velocity*AVOIDANCE_SPEED_FACTOR;
//				 }
//
//				 target_velocity -= inc;
//			}
//
//			//cout << "Decelerate -> Target V: " << target_velocity <<
//", Brake D: " <<  average_braking_distance << ", Start I" << info.iBack <<
//endl;
//		}
//
//	}
//	else if(beh.state == FORWARD_STATE || beh.state ==
...
double LocalPlannerH::UpdateVelocityDirectlyToTrajectory(
    const BehaviorState &beh, const VehicleState &CurrStatus,
    const double &dt) {
  ...
  PlanningHelpers::GetRelativeInfo(
      m_TotalOriginalPath.at(m_iCurrentTotalPathId), state, total_info);
  PlanningHelpers::GetRelativeInfo(m_Path, state, info);
  double average_braking_distance =
      -pow(CurrStatus.speed, 2) / (m_CarInfo.max_deceleration) +
      m_params.additionalBrakingDistance;
  double max_velocity = PlannerHNS::PlanningHelpers::GetVelocityAhead(
      m_TotalOriginalPath.at(m_iCurrentTotalPathId), total_info,
      m_PrevBrakingWayPoint, average_braking_distance);
  ...
  if (m_Path.size() <= 5) {
    ...
    for (unsigned int i = 0; i < m_Path.size(); i++)
      m_Path.at(i).v = target_velocity;
  } else if (beh.state == TRAFFIC_LIGHT_STOP_STATE ||
             beh.state == STOP_SIGN_STOP_STATE ||
             beh.state == STOP_SIGN_WAIT_STATE ||
             beh.state == TRAFFIC_LIGHT_WAIT_STATE) {
    PlanningHelpers::GetFollowPointOnTrajectory(
        m_Path, info, beh.stopDistance - critical_long_front_distance,
        point_index);
    ...
    double targe_acceleration =
        -pow(CurrStatus.speed, 2) /
        (2.0 * (beh.followDistance - critical_long_front_distance));
    if (targe_acceleration <= 0 &&
        targe_acceleration > m_CarInfo.max_deceleration / 2.0) {
      ...
      return desiredVelocity;
      // cout << "Accelerate -> Target V: " << target_velocity << ", Brake D: "
      // <<  average_braking_distance << ", Acceleration: " << targe_acceleration
      // << endl;
    } else {
      WayPoint pursuite_point = PlanningHelpers::GetFollowPointOnTrajectory(
          m_Path, info, beh.followDistance - critical_long_front_distance,
          point_index);
      ...
      return desiredVelocity;
      // cout << "Decelerate -> Target V: " << target_velocity << ", Brake D: "
      // <<  average_braking_distance << ", Start I" << info.iBack << endl;
    }

  } else if (beh.state == FORWARD_STATE ||
             beh.state == OBSTACLE_AVOIDANCE_STATE) {
    double target_velocity = max_velocity;
    if (m_pCurrentBehaviorState->GetCalcParams()->iCurrSafeTrajectory !=
        m_pCurrentBehaviorState->GetCalcParams()->iCentralTrajectory)
      target_velocity *= AVOIDANCE_SPEED_FACTOR;
...
void LocalPlannerH::ExtractHorizonAndCalculateRecommendedSpeed() {
  if (m_pCurrentBehaviorState->GetCalcParams()->bNewGlobalPath &&
      m_TotalOriginalPath.size() > 0) {
    m_PrevBrakingWayPoint = 0;
    PlanningHelpers::FixPathDensity(
        m_TotalOriginalPath.at(m_iCurrentTotalPathId),
        m_pCurrentBehaviorState->m_pParams->pathDensity);
    PlanningHelpers::SmoothPath(m_TotalOriginalPath.at(m_iCurrentTotalPathId),
                                0.49, 0.25, 0.05);

    PlanningHelpers::GenerateRecommendedSpeed(
        m_TotalOriginalPath.at(m_iCurrentTotalPathId),
        m_CarInfo.max_speed_forward,
        m_pCurrentBehaviorState->m_pParams->speedProfileFactor);
    m_TotalOriginalPath.at(m_iCurrentTotalPathId)
        .at(m_TotalOriginalPath.at(m_iCurrentTotalPathId).size() - 1)
        .v = 0;
  }
  ...
  for (unsigned int i = 0; i < m_TotalOriginalPath.size(); i++) {
    ...
    PlanningHelpers::ExtractPartFromPointToDistanceFast(
        m_TotalOriginalPath.at(i), state,
...
PlannerHNS::BehaviorState LocalPlannerH::DoOneStep(
    const double &dt, const PlannerHNS::VehicleState &vehicleState,
    const std::vector<PlannerHNS::DetectedObject> &obj_list, const int &goalID,
    PlannerHNS::RoadNetwork &map, const bool &bEmergencyStop,
    const std::vector<TrafficLight> &trafficLight, const bool &bLive) {
  PlannerHNS::BehaviorState beh;

  m_params.minFollowingDistance =
      m_InitialFollowingDistance + vehicleState.speed * 1.5;
  ...
  TrajectoryCost tc = m_TrajectoryCostsCalculatotor.DoOneStep(
      m_RollOuts, m_TotalPath, state,
      m_pCurrentBehaviorState->GetCalcParams()->iCurrSafeTrajectory,
      m_pCurrentBehaviorState->GetCalcParams()->iCurrSafeLane,
      *m_pCurrentBehaviorState->m_pParams, m_CarInfo, vehicleState,
      m_PredictedTrajectoryObstacles);
