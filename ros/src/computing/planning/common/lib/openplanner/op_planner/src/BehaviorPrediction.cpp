...
/// \brief Predict detected vehicles's possible trajectories, these trajectories
/// extracted from the vector map. \author Hatem Darweesh \date Jul 6, 2017
...
void BehaviorPrediction::FilterObservations(
    const std::vector<DetectedObject> &obj_list, RoadNetwork &map,
...
void BehaviorPrediction::DoOneStep(const std::vector<DetectedObject> &obj_list,
...
void BehaviorPrediction::CalculateCollisionTimes(const double &minSpeed) {
  for (unsigned int i = 0; i < m_ParticleInfo_II.size(); i++) {
    for (unsigned int j = 0;
         j < m_ParticleInfo_II.at(i)->obj.predTrajectories.size(); j++) {
      PlannerHNS::PlanningHelpers::PredictConstantTimeCostForTrajectory(
          m_ParticleInfo_II.at(i)->obj.predTrajectories.at(j),
          m_ParticleInfo_II.at(i)->obj.center, minSpeed, m_PredictionDistance);
      //			PlannerHNS::PlanningHelpers::CalcAngleAndCost(m_PredictedObjects.at(i).predTrajectories.at(j));
    }
  }
}

void BehaviorPrediction::ExtractTrajectoriesFromMap(
    const std::vector<DetectedObject> &curr_obj_list, RoadNetwork &map,
    std::vector<ObjParticles *> &old_obj_list) {
  PlannerH planner;
...
void BehaviorPrediction::CalPredictionTimeForObject(ObjParticles *pCarPart) {
  if (pCarPart->obj.center.v > 0)
    pCarPart->m_PredictionTime =
        (MIN_PREDICTION_TIME + 1.5 * pCarPart->obj.center.v) /
        pCarPart->obj.center.v;
  else
    pCarPart->m_PredictionTime = MIN_PREDICTION_TIME;
}

void BehaviorPrediction::PredictCurrentTrajectory(RoadNetwork &map,
                                                  ObjParticles *pCarPart) {
  ...
  if (pCarPart->obj.bDirection && pCarPart->obj.bVelocity) {
    ...
    pCarPart->obj.pClosestWaypoints =
        MappingHelpers::GetClosestWaypointsListFromMap(
            fake_pose, map, m_MaxLaneDetectionDistance,
            pCarPart->obj.bDirection);
    planner.PredictTrajectoriesUsingDP(
        fake_pose, pCarPart->obj.pClosestWaypoints, m_PredictionDistance,
        pCarPart->obj.predTrajectories, m_bGenerateBranches,
        pCarPart->obj.bDirection);
  } else {
    bool bLocalDirectionSearch = false;
    pCarPart->obj.pClosestWaypoints =
        MappingHelpers::GetClosestWaypointsListFromMap(
            pCarPart->obj.center, map, m_MaxLaneDetectionDistance,
            pCarPart->obj.bDirection);
    ...
    planner.PredictTrajectoriesUsingDP(
        pCarPart->obj.center, pCarPart->obj.pClosestWaypoints,
        m_PredictionDistance, pCarPart->obj.predTrajectories,
        m_bGenerateBranches, pCarPart->obj.bDirection);
  }

  for (unsigned int t = 0; t < pCarPart->obj.predTrajectories.size(); t++) {
    //		std::ostringstream path_name;
    //		path_name <<
    //"/home/hatem/autoware_openplanner_logs/TempPredLog/"; 		path_name << t ;
    //		path_name << "_";
    //		PlanningHelpers::GenerateRecommendedSpeed(pCarPart->obj.predTrajectories.at(t),
    //10, 1.0); 		PlannerHNS::PlanningHelpers::WritePathToFile(path_name.str(),
    //pCarPart->obj.predTrajectories.at(t));
    if (pCarPart->obj.predTrajectories.at(t).size() > 0)
      pCarPart->obj.predTrajectories.at(t).at(0).collisionCost = 0;
  }
}

void BehaviorPrediction::ParticleFilterSteps(
    std::vector<ObjParticles *> &part_info) {
...
int BehaviorPrediction::FromIndicatorToNumber(
    const PlannerHNS::LIGHT_INDICATOR &indi) {
...
PlannerHNS::LIGHT_INDICATOR
BehaviorPrediction::FromNumbertoIndicator(const int &num) {
  if (num == 0)
...
double
BehaviorPrediction::CalcIndicatorWeight(PlannerHNS::LIGHT_INDICATOR p_ind,
                                        PlannerHNS::LIGHT_INDICATOR obj_ind) {
  if ((obj_ind == PlannerHNS::INDICATOR_LEFT ||
       obj_ind == PlannerHNS::INDICATOR_RIGHT ||
       obj_ind == PlannerHNS::INDICATOR_NONE) &&
      p_ind == obj_ind)
    return 0.99;
  else
    return 0.01;
...
void BehaviorPrediction::CalOnePartWeight(ObjParticles *pParts, Particle &p) {
  ...
  // p.pose_w = exp(-(0.5*pow((p.pose.pos.x -
  // pParts->obj.center.pos.x),2)/(2*MEASURE_POSE_ERROR*MEASURE_POSE_ERROR)+
  // pow((p.pose.pos.y -
  // pParts->obj.center.pos.y),2)/(2*MEASURE_POSE_ERROR*MEASURE_POSE_ERROR)));
  p.pose_w = 1.0 / hypot(0.5 * (p.pose.pos.y - pParts->obj.center.pos.y),
                         0.5 * (p.pose.pos.x - pParts->obj.center.pos.x));
  // p.dir_w  =
  // exp(-(pow(fabs(UtilityHNS::UtilityH::AngleBetweenTwoAnglesPositive(p.pose.pos.a,
  // pParts->obj.center.pos.a)),2)/(2*MEASURE_ANGLE_ERROR*MEASURE_ANGLE_ERROR)));
  p.dir_w = M_PI_2 - fabs(UtilityHNS::UtilityH::AngleBetweenTwoAnglesPositive(
                         p.pose.pos.a, pParts->obj.center.pos.a));
  p.vel_w = exp(-(pow((p.vel - pParts->obj.center.v), 2) /
                  (2 * MEASURE_VEL_ERROR * MEASURE_VEL_ERROR)));
  // p.vel_w = fabs(p.vel - pParts->obj.center.v);
  p.ind_w = CalcIndicatorWeight(FromNumbertoIndicator(p.indicator),
                                pParts->obj.indicator_state);
  p.ind_w -= p.ind_w * MEASURE_IND_ERROR;
  p.acl_w = CalcAccelerationWeight(p.acc, pParts->obj.acceleration_desc);

  // std::cout << p.beh << "|" << p.vel_w <<"|" <<p.vel << "|" <<
  // pParts->obj.center.v;
  ...
  p.w_raw = p.pose_w * POSE_FACTOR + p.dir_w * DIRECTION_FACTOR +
            p.vel_w * VELOCITY_FACTOR + p.ind_w * INDICATOR_FACTOR +
            p.acl_w * ACCELERATE_FACTOR;
...
void BehaviorPrediction::NormalizeOnePartWeight(ObjParticles *pParts,
                                                Particle &p) {
  if (p.bDeleted)
    return;
  ...
  p.w = p.pose_w * POSE_FACTOR + p.dir_w * DIRECTION_FACTOR +
        p.vel_w * VELOCITY_FACTOR + p.ind_w * INDICATOR_FACTOR +
        p.acl_w * ACCELERATE_FACTOR;
...
void BehaviorPrediction::CalculateWeights(ObjParticles *pParts) {
  ...
  // std::cout << "Befor Normalize: Max: " <<  pParts->acl_w_max << ", Min: " <<
  // pParts->acl_w_min << std::endl; std::cout << std::endl;

  // if((pParts->m_TrajectoryTracker.size() > 1 && pParts->min_w_raw < 0.5) ||
  // pParts->max_w_raw == 0 || fabs(pParts->max_w_raw - pParts->min_w_raw) < 0.1
  // )
  if ((pParts->max_w_raw == 0 ||
       fabs(pParts->max_w_raw - pParts->min_w_raw) < 0.1 ||
       pParts->min_w_raw > 0.5) &&
      pParts->m_TrajectoryTracker.size() > 1)
    m_bCanDecide = false;
  else
    m_bCanDecide = true;
...
void BehaviorPrediction::CalculateAveragesAndProbabilities(
    ObjParticles *pParts) {
...
void BehaviorPrediction::RemoveWeakParticles(ObjParticles *pParts) {
  ...
  double critical_val =
      pParts->min_w + (pParts->max_w - pParts->min_w) * KEEP_PERCENTAGE;
  //	std::cout << "Behavior Weights
  //------------------------------------------------ " << std::endl; 	std::cout
  //<< "Max Raw: " << pParts->max_w_raw << ", Min Raw: " << pParts->min_w_raw <<
  //std::endl; 	std::cout << "Keep Percent Value: " << critical_val << std::endl;
  //
  ...
  for (int i = 0; i < pParts->m_AllParticles.size(); i++) {
    ...
    double d = hypot(
        pParts->obj.center.pos.y - pParts->m_AllParticles.at(i)->pose.pos.y,
        pParts->obj.center.pos.x - pParts->m_AllParticles.at(i)->pose.pos.x);

    if (pParts->m_AllParticles.at(i)->w < critical_val ||
        d > m_PredictionDistance) {
      pParts->m_AllParticles.at(i)->pTraj->DeleteParticle(
          *(pParts->m_AllParticles.at(i)),
          pParts->m_AllParticles.at(i)->original_index);
...
void BehaviorPrediction::FindBest(ObjParticles *pParts) {
  ...
  for (unsigned int t = 1; t < pParts->m_TrajectoryTracker.size(); t++) {
    if (pParts->m_TrajectoryTracker.at(t)->best_p >
        pParts->best_beh_track->best_p) {
  ...
  if (m_bDebugOut) {
    std::cout
        << "Behavior Prob ------------------------------------------------ : "
        << pParts->m_TrajectoryTracker.size() << std::endl;
    std::cout << "Raw  Weights: Max: " << pParts->max_w_raw
              << ", Min: " << pParts->min_w_raw << std::endl;
    std::cout << "Norm Weights: Max: " << pParts->max_w
              << ", Min: " << pParts->min_w << std::endl;
  }
  ...
  if (m_bCanDecide && pParts->best_beh_track != nullptr) {
    ...
    if (m_bDebugOut)
      std::cout << "Trajectory (" << pParts->i_best_track
                << "), P: " << pParts->best_beh_track->best_p << " , Beh ("
                << pParts->best_beh_track->best_beh << ", " << str_beh << ")"
                << std::endl;

    if (pParts->best_beh_track->index < pParts->obj.predTrajectories.size())
      pParts->obj.predTrajectories.at(pParts->best_beh_track->index)
          .at(0)
          .collisionCost = 1;

  } else {
    if (m_bDebugOut)
      std::cout << "Trajectory (" << -1 << "), P: " << 0 << " , Beh (" << -1
                << ", "
                << "Can't Decide"
                << ")" << std::endl;
  }
  ...
  if (m_bDebugOut) {
    for (unsigned int t = 0; t < pParts->m_TrajectoryTracker.size(); t++) {
      if (pParts->m_TrajectoryTracker.at(t)->nAliveForward > 0)
        std::cout << t << ", Forward Particles:"
                  << pParts->m_TrajectoryTracker.at(t)->nAliveForward
                  << std::endl;
      if (pParts->m_TrajectoryTracker.at(t)->nAliveStop > 0)
        std::cout << t << ", Stoping Particles:"
                  << pParts->m_TrajectoryTracker.at(t)->nAliveStop << std::endl;
    }
...
void BehaviorPrediction::SamplesFreshParticles(ObjParticles *pParts) {
  ...
  //	for(unsigned int t=0; t < pParts->m_TrajectoryTracker.size(); t++)
  //	{
  //		PlanningHelpers::FixPathDensity(pParts->m_TrajectoryTracker.at(t)->trajectory,
  //0.5);
  ...
  for (unsigned int t = 0; t < pParts->m_TrajectoryTracker.size(); t++) {
    ...
    PlanningHelpers::GetRelativeInfo(
        pParts->m_TrajectoryTracker.at(t)->trajectory, pParts->obj.center,
        info);
    unsigned int point_index = 0;
    p.pose = PlanningHelpers::GetFollowPointOnTrajectory(
        pParts->m_TrajectoryTracker.at(t)->trajectory, info,
        PREDICTION_DISTANCE_PERCENTAGE * m_PredictionDistance, point_index);

    if (pParts->m_TrajectoryTracker.at(t)->beh ==
            PlannerHNS::BEH_FORWARD_STATE &&
        pParts->m_TrajectoryTracker.at(t)->nAliveForward < BEH_PARTICLES_NUM) {
      p.beh = PlannerHNS::BEH_FORWARD_STATE;
      int nPs =
          BEH_PARTICLES_NUM - pParts->m_TrajectoryTracker.at(t)->nAliveForward;
    ...
    if (ENABLE_STOP_BEHAVIOR_GEN == 1 &&
        pParts->m_TrajectoryTracker.at(t)->nAliveStop < BEH_PARTICLES_NUM) {
      p.beh = PlannerHNS::BEH_STOPPING_STATE;
      int nPs =
          BEH_PARTICLES_NUM - pParts->m_TrajectoryTracker.at(t)->nAliveStop;
...
void BehaviorPrediction::CollectParticles(ObjParticles *pParts) {
  ...
  for (unsigned int t = 0; t < pParts->m_TrajectoryTracker.size(); t++) {
    for (unsigned int i = 0;
         i < pParts->m_TrajectoryTracker.at(t)->m_ForwardPart.size(); i++) {
      pParts->m_TrajectoryTracker.at(t)->m_ForwardPart.at(i).original_index = i;
      pParts->m_AllParticles.push_back(
          &pParts->m_TrajectoryTracker.at(t)->m_ForwardPart.at(i));
    }

    for (unsigned int i = 0;
         i < pParts->m_TrajectoryTracker.at(t)->m_StopPart.size(); i++) {
      pParts->m_TrajectoryTracker.at(t)->m_StopPart.at(i).original_index = i;
      pParts->m_AllParticles.push_back(
          &pParts->m_TrajectoryTracker.at(t)->m_StopPart.at(i));
    }
  }
}
...
void BehaviorPrediction::MoveParticles(ObjParticles *pParts) {
  ...
  for (unsigned int t = 0; t < pParts->m_TrajectoryTracker.size(); t++) {
    PlanningHelpers::GenerateRecommendedSpeed(
        ...
        carInfo.max_speed_forward, 1.0);
  }

  //	std::cout << "Motion Status------ " << std::endl;
  //	if(pParts->obj.acceleration_desc == 1)
  //		std::cout << "Acceleration: " << pParts->obj.acceleration_raw << ",
  //Accelerate: " << pParts->obj.acceleration_desc << std::endl; 	else
  //if(pParts->obj.acceleration_desc == -1) 		std::cout << "Acceleration: " <<
  //pParts->obj.acceleration_raw << ", Braking   : " <<
  //pParts->obj.acceleration_desc << std::endl; 	else 		std::cout << "Acceleration:
  //" << pParts->obj.acceleration_raw << ", Cruising  : " <<
  //pParts->obj.acceleration_desc << std::endl;
  ...
  for (unsigned int i = 0; i < pParts->m_AllParticles.size(); i++) {
    ...
    if (USE_OPEN_PLANNER_MOVE == 0) {
      ...
      curr_part_info = decision_make.MoveStepSimple(
          dt, p->pose, p->pTraj->trajectory, carInfo);
      ...
      //			if(curr_behavior.state ==
      //PlannerHNS::STOPPING_STATE && p->beh == PlannerHNS::BEH_YIELDING_STATE)
      //				p->vel += 1;
      //			else if(p->beh ==
      //PlannerHNS::BEH_YIELDING_STATE) 				p->vel = p->vel/2.0; 			else
      //if(curr_behavior.state != PlannerHNS::STOPPING_STATE && p->beh ==
      //PlannerHNS::BEH_STOPPING_STATE) 				p->vel += 1; 			else if(p->beh ==
      //PlannerHNS::BEH_STOPPING_STATE) 				p->vel = 0;

      //			if(curr_behavior.state !=
      //PlannerHNS::STOPPING_STATE && p->beh == PlannerHNS::BEH_STOPPING_STATE)
      //				p->vel += 1;
      ...
      curr_behavior =
          decision_make.MoveStep(dt, p->pose, p->pTraj->trajectory, carInfo);
      p->acc = UtilityHNS::UtilityH::GetSign(curr_behavior.maxVelocity -
                                             p->vel_prev_big);
      ...
      if (curr_behavior.state == PlannerHNS::STOPPING_STATE &&
          p->beh == PlannerHNS::BEH_YIELDING_STATE)
        p->vel += 1;
      else if (p->beh == PlannerHNS::BEH_YIELDING_STATE)
        p->vel = p->vel / 2.0;
      else if (curr_behavior.state != PlannerHNS::STOPPING_STATE &&
               p->beh == PlannerHNS::BEH_STOPPING_STATE)
        p->vel += 1;
