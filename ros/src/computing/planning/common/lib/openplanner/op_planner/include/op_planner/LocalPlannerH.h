...
public:
  ...
  std::string carId;
  Lane *pLane;
  double m_SimulationSteeringDelayFactor; // second , time that every degree
                                          // change in the steering wheel takes
  ...
  void SetSimulatedTargetOdometryReadings(const double &velocity_d,
                                          const double &steering_d,
  ...
  double GetSimulatedVelocity() { return m_CurrentVelocity; }

  double GetSimulatedSteering() { return m_CurrentSteering; }

  double GetSimulatedShift() { return m_CurrentShift; }
...
public:
  ...
  void Init(const ControllerParams &ctrlParams, const PlanningParams &params,
  ...
  void CalculateImportantParameterForDecisionMaking(
      const VehicleState &car_state, const int &goalID,
      const bool &bEmergencyStop, const vector<TrafficLight> &detectedLights,
      const TrajectoryCost &bestTrajectory);

  BehaviorState DoOneStep(const double &dt, const VehicleState &state,
                          const std::vector<DetectedObject> &obj_list,
                          const int &goalID, RoadNetwork &map,
...
private:
  ...
  //	bool CalculateObstacleCosts(RoadNetwork& map, const VehicleState&
  //vstatus, const std::vector<DetectedObject>& obj_list);
  //
  ...
  bool GetNextTrafficLight(const int &prevTrafficLightId,
  ...
  void AddAndTransformContourPoints(const DetectedObject &obj,
                                    std::vector<WayPoint> &contourPoints);
  double UpdateVelocityDirectlyToTrajectory(const BehaviorState &beh,
                                            const VehicleState &CurrStatus,
                                            const double &dt);
