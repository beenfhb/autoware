...
/// \brief Helper functions for mapping operation such as (load and initialize
/// vector maps , convert map from one format to another, .. ) \author Hatem
/// Darweesh \date Jul 2, 2016
...
GPSPoint MappingHelpers::GetTransformationOrigin(const int &bToyotaCityMap) {
  ...
  //		return GPSPoint(14805.945, 84680.211, -39.59, 0); // for moriyama
  //map 	else
  return GPSPoint();
...
int MappingHelpers::GetLaneIdByWaypointId(const int &id,
...
int MappingHelpers::ReplaceMyID(
    int &id, const std::vector<std::pair<int, int>> &rep_list) {
...
void MappingHelpers::ConstructRoadNetworkFromROSMessage(
    const std::vector<UtilityHNS::AisanLanesFileReader::AisanLane> &lanes_data,
    const std::vector<UtilityHNS::AisanPointsFileReader::AisanPoints>
        &points_data,
    const std::vector<UtilityHNS::AisanCenterLinesFileReader::AisanCenterLine>
        &dt_data,
    const std::vector<
    ...
    const std::vector<UtilityHNS::AisanStopLineFileReader::AisanStopLine>
        &stop_line_data,
    const std::vector<UtilityHNS::AisanSignalFileReader::AisanSignal>
        &signal_data,
    const std::vector<UtilityHNS::AisanVectorFileReader::AisanVector>
        &vector_data,
    const std::vector<UtilityHNS::AisanCurbFileReader::AisanCurb> &curb_data,
    const std::vector<UtilityHNS::AisanRoadEdgeFileReader::AisanRoadEdge>
        &roadedge_data,
    const std::vector<UtilityHNS::AisanWayareaFileReader::AisanWayarea>
        &wayarea_data,
    const std::vector<UtilityHNS::AisanCrossWalkFileReader::AisanCrossWalk>
        &crosswalk_data,
  ...
  for (unsigned int l = 0; l < lanes_data.size(); l++) {
    ...
    if (l + 1 < lanes_data.size()) {
      ...
      if (curr_lane_point.FLID == next_lane_point.LnID &&
          curr_lane_point.DID == next_lane_point.DID) {
        ...
        if (curr_lane_point.BLID2 != 0) {
          if (next_lane_point.BLID2 == 0)
            next_lane_point.BLID2 = curr_lane_point.BLID2;
          else if (next_lane_point.BLID3 == 0)
            next_lane_point.BLID3 = curr_lane_point.BLID2;
          else if (next_lane_point.BLID4 == 0)
            next_lane_point.BLID4 = curr_lane_point.BLID2;
        }

        if (curr_lane_point.BLID3 != 0) {
          if (next_lane_point.BLID2 == 0)
            next_lane_point.BLID2 = curr_lane_point.BLID3;
          else if (next_lane_point.BLID3 == 0)
            next_lane_point.BLID3 = curr_lane_point.BLID3;
          else if (next_lane_point.BLID4 == 0)
            next_lane_point.BLID4 = curr_lane_point.BLID3;
        }

        if (curr_lane_point.BLID3 != 0) {
          if (next_lane_point.BLID2 == 0)
            next_lane_point.BLID2 = curr_lane_point.BLID4;
          else if (next_lane_point.BLID3 == 0)
            next_lane_point.BLID3 = curr_lane_point.BLID4;
          else if (next_lane_point.BLID4 == 0)
            next_lane_point.BLID4 = curr_lane_point.BLID4;
        }

        if (curr_lane_point.FLID2 != 0) {
          if (next_lane_point.FLID2 == 0)
            next_lane_point.FLID2 = curr_lane_point.FLID2;
          else if (next_lane_point.FLID3 == 0)
            next_lane_point.FLID3 = curr_lane_point.FLID2;
          else if (next_lane_point.FLID4 == 0)
            next_lane_point.FLID4 = curr_lane_point.FLID2;
        }

        if (curr_lane_point.FLID3 != 0) {
          if (next_lane_point.FLID2 == 0)
            next_lane_point.FLID2 = curr_lane_point.FLID3;
          else if (next_lane_point.FLID3 == 0)
            next_lane_point.FLID3 = curr_lane_point.FLID3;
          else if (next_lane_point.FLID4 == 0)
            next_lane_point.FLID4 = curr_lane_point.FLID3;
        }

        if (curr_lane_point.FLID3 != 0) {
          if (next_lane_point.FLID2 == 0)
            next_lane_point.FLID2 = curr_lane_point.FLID4;
          else if (next_lane_point.FLID3 == 0)
            next_lane_point.FLID3 = curr_lane_point.FLID4;
          else if (next_lane_point.FLID4 == 0)
            next_lane_point.FLID4 = curr_lane_point.FLID4;
        }

        if (prev_lane_point.FLID == curr_lane_point.LnID)
          prev_lane_point.FLID = next_lane_point.LnID;

        id_replace_list.push_back(
            make_pair(curr_lane_point.LnID, next_lane_point.LnID));
    ...
    WayPoint wp;
    bool bFound =
        GetWayPoint(curr_lane_point.LnID, lane_obj.id, curr_lane_point.RefVel,
                    curr_lane_point.DID, dt_data, points_data, origin, wp);

    wp.originalMapID = curr_lane_point.originalMapID;

    if (curr_lane_point.LaneDir == 'L') {
      wp.actionCost.push_back(
          make_pair(LEFT_TURN_ACTION, LEFT_INITIAL_TURNS_COST));
      // std::cout << " Left Lane : " << curr_lane_point.LnID << std::endl ;
    } else if (curr_lane_point.LaneDir == 'R') {
      wp.actionCost.push_back(
          make_pair(RIGHT_TURN_ACTION, RIGHT_INITIAL_TURNS_COST));
    ...
    // if(prev_lane_point.DID == curr_lane_point.DID && curr_lane_point.LnID ==
    // prev_lane_point.FLID)
    //		if(prevWayPoint.pos.x == wp.pos.x && prevWayPoint.pos.y ==
    //wp.pos.y)
    //		{
    //			//if((prev_lane_point.FLID2 != 0 && curr_lane_point.FLID2 != 0) ||
    //(prev_lane_point.FLID3 != 0 && curr_lane_point.FLID3 != 0) ||
    //(prev_lane_point.FLID4 != 0 && curr_lane_point.FLID4 != 0))
    //			{
    //				cout << "Prev WP, LnID: " << prev_lane_point.LnID << ",BLID: " <<
    //prev_lane_point.BLID << ",FLID: " << prev_lane_point.FLID << ",DID: " <<
    //prev_lane_point.DID
    //						<< ", Begin: " << prev_lane_point.BLID2 << "," <<
    //prev_lane_point.BLID3 << "," << prev_lane_point.BLID4
    //						<< ", End: " << prev_lane_point.FLID2 << "," <<
    //prev_lane_point.FLID3 << "," << prev_lane_point.FLID4 << ": " <<
    //prev_lane_point.LaneDir <<   endl; 				cout << "Curr WP, LnID: " <<
    //curr_lane_point.LnID << ",BLID: " << curr_lane_point.BLID << ",FLID: " <<
    //curr_lane_point.FLID << ",DID: " << curr_lane_point.DID
    //						<< ", Begin: " << curr_lane_point.BLID2 <<  "," <<
    //curr_lane_point.BLID3 <<  "," << curr_lane_point.BLID4
    //						<< ", End: " << curr_lane_point.FLID2 <<  ","
    //<<curr_lane_point.FLID3 <<  "," << curr_lane_point.FLID4 <<   ": " <<
    //curr_lane_point.LaneDir << endl << endl;
    //			}
    //		}
  ...
  // delete first two lanes !!!!! Don't know why , you don't know why ! , these
  // two line cost you a lot .. ! why why , works for toyota map , but not with
  // moriyama
  if (bSpecialFlag) {
    if (roadLanes.size() > 0)
  ...
  // For each lane, the previous code set the fromId as the id of the last
  // waypoint of the previos lane. here we fix that by finding from each fromID
  // the corresponding point and replace the fromId by the LaneID associated with
  // that point.
  for (unsigned int l = 0; l < roadLanes.size(); l++) {
    for (unsigned int fp = 0; fp < roadLanes.at(l).fromIds.size(); fp++) {
      roadLanes.at(l).fromIds.at(fp) =
          GetLaneIdByWaypointId(roadLanes.at(l).fromIds.at(fp), roadLanes);
    }

    for (unsigned int tp = 0; tp < roadLanes.at(l).toIds.size(); tp++) {
      roadLanes.at(l).toIds.at(tp) =
          GetLaneIdByWaypointId(roadLanes.at(l).toIds.at(tp), roadLanes);
    }
  ...
  for (unsigned int rs = 0; rs < map.roadSegments.size(); rs++) {
    ...
    for (unsigned int i = 0; i < map.roadSegments.at(rs).Lanes.size(); i++) {
      ...
      for (unsigned int j = 0; j < pL->fromIds.size(); j++) {
        for (unsigned int l = 0; l < map.roadSegments.at(rs).Lanes.size();
             l++) {
      ...
      for (unsigned int j = 0; j < pL->points.size(); j++) {
        if (pL->points.at(j).actionCost.size() > 0) {
          if (pL->points.at(j).actionCost.at(0).first == LEFT_TURN_ACTION) {
            AssignActionCostToLane(pL, LEFT_TURN_ACTION,
                                   LEFT_INITIAL_TURNS_COST);
            break;
          } else if (pL->points.at(j).actionCost.at(0).first ==
                     RIGHT_TURN_ACTION) {
            AssignActionCostToLane(pL, RIGHT_TURN_ACTION,
                                   RIGHT_INITIAL_TURNS_COST);
            break;
          }
        }
      }
  ...
  if (bFindCurbsAndWayArea) {
    ...
    // Wayarea
    ExtractWayArea(area_data, wayarea_data, line_data, points_data, origin,
                   map);
  }
...
void MappingHelpers::AssignActionCostToLane(Lane *pL, ACTION_TYPE action,
                                            double cost) {
  for (unsigned int j = 0; j < pL->points.size(); j++) {
  ...
  for (unsigned int rs = 0; rs < map.roadSegments.size(); rs++) {
    for (unsigned int i = 0; i < map.roadSegments.at(rs).Lanes.size(); i++) {
      for (unsigned int p = 0;
           p < map.roadSegments.at(rs).Lanes.at(i).points.size(); p++) {
...
WayPoint *MappingHelpers::FindWaypointV2(const int &id, const int &l_id,
                                         RoadNetwork &map) {
  for (unsigned int rs = 0; rs < map.roadSegments.size(); rs++) {
    for (unsigned int i = 0; i < map.roadSegments.at(rs).Lanes.size(); i++) {
...
void MappingHelpers::ConstructRoadNetworkFromDataFiles(
    const std::string vectoMapPath, RoadNetwork &map, const bool &bZeroOrigin) {
  /**
  ...
  if (points_data.size() == 0) {
    std::cout
        << std::endl
        << "## Alert Can't Read Points Data from vector map files in path: "
        << vectoMapPath << std::endl;
    return;
  }
  ...
  if (nodes_data.size() > 0 && bSpecialMap == 0) {
    ConstructRoadNetworkFromROSMessageV2(
        lanes_data, points_data, dt_data, intersection_data, area_data,
        line_data, stop_line_data, signal_data, vector_data, curb_data,
        roadedge_data, way_area_data, crosswalk_data, nodes_data, conn_data,
        &lanes, &points, &nodes, &lines, GetTransformationOrigin(0), map,
        false);
  } else {
...
bool MappingHelpers::GetWayPoint(
    const int &id, const int &laneID, const double &refVel, const int &did,
    const std::vector<UtilityHNS::AisanCenterLinesFileReader::AisanCenterLine>
        &dtpoints,
    ...
    if (dtpoints.at(dtp).DID == did) {
      for (unsigned int p = 0; p < points.size(); p++) {
        if (dtpoints.at(dtp).PID == points.at(p).PID) {
          ...
          wp.v = refVel;
          //					double integ_part =
          //points.at(p).L; 					double deg = trunc(points.at(p).L); 					double min =
          //trunc((points.at(p).L - deg) * 100.0) / 60.0; 					double sec =
          //modf((points.at(p).L - deg) * 100.0, &integ_part)/36.0; 					double L =
          //deg + min + sec;
          //
          //					deg = trunc(points.at(p).B);
          //					min = trunc((points.at(p).B - deg) * 100.0)
          /// 60.0; 					sec = modf((points.at(p).B - deg) * 100.0,
          //&integ_part)/36.0; 					double B = deg + min + sec;

          wp.pos =
              GPSPoint(points.at(p).Ly + origin.x, points.at(p).Bx + origin.y,
                       points.at(p).H + origin.z, dtpoints.at(dtp).Dir);
...
void MappingHelpers::LoadKML(const std::string &kmlFile, RoadNetwork &map) {
  ...
  try {
    ...
    cout << "KML Custom Reader Error, Can't Load .kml File, path is: "
         << kmlFile << endl;
    cout << e.what() << endl;
    return;
  }
  ...
  for (unsigned int rs = 0; rs < map.roadSegments.size(); rs++) {
    for (unsigned int i = 0; i < map.roadSegments.at(rs).Lanes.size(); i++) {
      for (unsigned int itl = 0; itl < map.trafficLights.size(); itl++) {
        if (map.trafficLights.at(itl).CheckLane(
                map.roadSegments.at(rs).Lanes.at(i).id)) {
          map.trafficLights.at(itl).pLanes.push_back(
              &map.roadSegments.at(rs).Lanes.at(i));
          map.roadSegments.at(rs).Lanes.at(i).trafficlights.push_back(
              map.trafficLights.at(itl));
        }
      }

      for (unsigned int isl = 0; isl < map.stopLines.size(); isl++) {
        if (map.stopLines.at(isl).laneId ==
            map.roadSegments.at(rs).Lanes.at(i).id) {
          map.stopLines.at(isl).pLane = &map.roadSegments.at(rs).Lanes.at(i);
          map.roadSegments.at(rs).Lanes.at(i).stopLines.push_back(
              map.stopLines.at(isl));
          WayPoint wp((map.stopLines.at(isl).points.at(0).x +
                       map.stopLines.at(isl).points.at(1).x) /
                          2.0,
                      (map.stopLines.at(isl).points.at(0).y +
                       map.stopLines.at(isl).points.at(1).y) /
                          2.0,
                      (map.stopLines.at(isl).points.at(0).z +
                       map.stopLines.at(isl).points.at(1).z) /
                          2.0,
                      (map.stopLines.at(isl).points.at(0).a +
                       map.stopLines.at(isl).points.at(1).a) /
                          2.0);
          map.roadSegments.at(rs)
              .Lanes.at(i)
              .points
              .at(PlanningHelpers::GetClosestNextPointIndexFast(
                  map.roadSegments.at(rs).Lanes.at(i).points, wp))
              .stopLineID = map.stopLines.at(isl).id;
        }
      }
    }
  }
  ...
  cout << "Map loaded from kml file with (" << laneLinksList.size()
       << ") lanes, First Point ( " << GetFirstWaypoint(map).pos.ToString()
       << ")" << endl;
}
...
TiXmlElement *MappingHelpers::GetDataFolder(const string &folderName,
                                            TiXmlElement *pMainElem) {
  if (!pMainElem)
    return nullptr;
...
WayPoint *
MappingHelpers::GetClosestWaypointFromMap(const WayPoint &pos, RoadNetwork &map,
...
vector<WayPoint *> MappingHelpers::GetClosestWaypointsListFromMap(
    const WayPoint &pos, RoadNetwork &map, const double &distance,
    const bool bDirectionBased) {
  vector<Lane *> pLanes =
      GetClosestLanesListFromMap(pos, map, distance, bDirectionBased);
  ...
  for (unsigned int i = 0; i < pLanes.size(); i++) {
    if (pLanes.at(i)) {
      int closest_index = PlanningHelpers::GetClosestNextPointIndexFast(
          pLanes.at(i)->points, pos);
...
WayPoint *MappingHelpers::GetClosestBackWaypointFromMap(const WayPoint &pos,
                                                        RoadNetwork &map) {
  double distance_to_nearest_lane = 1;
  Lane *pLane = 0;
  while (distance_to_nearest_lane < 100 && pLane == 0) {
...
std::vector<Lane *>
MappingHelpers::GetClosestLanesFast(const WayPoint &center, RoadNetwork &map,
  ...
  for (unsigned int j = 0; j < map.roadSegments.size(); j++) {
    for (unsigned int k = 0; k < map.roadSegments.at(j).Lanes.size(); k++) {
      ...
      int index =
          PlanningHelpers::GetClosestNextPointIndexFast(pL->points, center);

      if (index < 0 || index >= pL->points.size())
        continue;

      double d = hypot(pL->points.at(index).pos.y - center.pos.y,
                       pL->points.at(index).pos.x - center.pos.x);
      if (d <= distance)
...
Lane *MappingHelpers::GetClosestLaneFromMap(const WayPoint &pos,
                                            RoadNetwork &map,
                                            const double &distance,
                                            const bool bDirectionBased) {
  ...
  for (unsigned int j = 0; j < map.roadSegments.size(); j++) {
    for (unsigned int k = 0; k < map.roadSegments.at(j).Lanes.size(); k++) {
      ...
      d = 0;
      min_d = DBL_MAX;
      for (unsigned int pindex = 0;
           pindex < map.roadSegments.at(j).Lanes.at(k).points.size();
           pindex++) {

        d = distance2points(
            map.roadSegments.at(j).Lanes.at(k).points.at(pindex).pos, pos.pos);
        if (d < min_d)
          min_d = d;
      }

      if (min_d < distance)
        laneLinksList.push_back(
            make_pair(min_d, &map.roadSegments.at(j).Lanes.at(k)));
    }
  }
  ...
  for (unsigned int i = 0; i < laneLinksList.size(); i++) {
    RelativeInfo info;
    PlanningHelpers::GetRelativeInfo(laneLinksList.at(i).second->points, pos,
                                     info);

    if (info.perp_distance == 0 && laneLinksList.at(i).first != 0)
      continue;

    if (bDirectionBased && fabs(info.perp_distance) < min_d &&
        fabs(info.angle_diff) < 45) {
...
vector<Lane *> MappingHelpers::GetClosestLanesListFromMap(
    const WayPoint &pos, RoadNetwork &map, const double &distance,
    const bool bDirectionBased) {
  vector<pair<double, Lane *>> laneLinksList;
  double d = 0;
  ...
  for (unsigned int i = 0; i < laneLinksList.size(); i++) {
    ...
    if (bDirectionBased && fabs(info.perp_distance) < distance &&
        fabs(info.angle_diff) < 30) {
...
Lane *MappingHelpers::GetClosestLaneFromMapDirectionBased(
    const WayPoint &pos, RoadNetwork &map, const double &distance) {
  ...
  for (unsigned int j = 0; j < map.roadSegments.size(); j++) {
    for (unsigned int k = 0; k < map.roadSegments.at(j).Lanes.size(); k++) {
      ...
      if (min_d < distance)
        laneLinksList.push_back(make_pair(
            min_d, &map.roadSegments.at(j).Lanes.at(k).points.at(min_i)));
    }
  }
  ...
  for (unsigned int i = 0; i < laneLinksList.size(); i++) {
    RelativeInfo info;
    PlanningHelpers::GetRelativeInfo(laneLinksList.at(i).second->pLane->points,
                                     pos, info);
    if (info.perp_distance == 0 && laneLinksList.at(i).first != 0)
      continue;

    a_diff = UtilityH::AngleBetweenTwoAnglesPositive(
        laneLinksList.at(i).second->pos.a, pos.pos.a);
...
std::vector<Lane *> MappingHelpers::GetClosestMultipleLanesFromMap(
    const WayPoint &pos, RoadNetwork &map, const double &distance) {
  vector<Lane *> lanesList;
  double d = 0;
  double a_diff = 0;
    ...
    for (unsigned int k = 0; k < map.roadSegments.at(j).Lanes.size(); k++) {
      for (unsigned int pindex = 0;
        ...
        d = distance2points(
            map.roadSegments.at(j).Lanes.at(k).points.at(pindex).pos, pos.pos);
        a_diff = UtilityH::AngleBetweenTwoAnglesPositive(
            map.roadSegments.at(j).Lanes.at(k).points.at(pindex).pos.a,
            pos.pos.a);
...
WayPoint *MappingHelpers::GetLastWaypoint(RoadNetwork &map) {
  if (map.roadSegments.size() > 0 &&
      map.roadSegments.at(map.roadSegments.size() - 1).Lanes.size() > 0) {
    std::vector<Lane> *lanes =
        &map.roadSegments.at(map.roadSegments.size() - 1).Lanes;
    if (lanes->at(lanes->size() - 1).points.size() > 0)
      return &lanes->at(lanes->size() - 1)
                  .points.at(lanes->at(lanes->size() - 1).points.size() - 1);
  }

  return nullptr;
}

void MappingHelpers::GetUniqueNextLanes(const Lane *l,
                                        const vector<Lane *> &traversed_lanes,
...
Lane *MappingHelpers::GetLaneFromPath(const WayPoint &currPos,
                                      const std::vector<WayPoint> &currPath) {
  if (currPath.size() < 1)
    return nullptr;

  int closest_index =
      PlanningHelpers::GetClosestNextPointIndexFast(currPath, currPos);
...
std::vector<Curb> MappingHelpers::GetCurbsList(TiXmlElement *pElem) {
  ...
  for (; pE; pE = pE->NextSiblingElement()) {
    ...
    if (pNameXml) {
      ...
      TiXmlElement *pPoints =
          pE->FirstChildElement("LineString")->FirstChildElement("coordinates");
...
std::vector<TrafficSign>
MappingHelpers::GetTrafficSignsList(TiXmlElement *pElem) {
...
MappingHelpers::GetTrafficLightsList(TiXmlElement *pElem) {
  ...
  for (; pE; pE = pE->NextSiblingElement()) {
    ...
    if (pNameXml) {
      ...
      TiXmlElement *pPoints =
          pE->FirstChildElement("Point")->FirstChildElement("coordinates");
...
vector<WayPoint> MappingHelpers::GetCenterLaneData(TiXmlElement *pElem,
                                                   const int &currLaneID) {
  ...
  if (pV) {
    ...
    TiXmlElement *pInfoEl =
        pElem->FirstChildElement("Folder")->FirstChildElement("description");
    ...
    if (gps_points.size() == add_info_list.size()) {
      for (unsigned int i = 0; i < gps_points.size(); i++) {
        gps_points.at(i).id =
            GetIDsFromPrefix(add_info_list.at(i), "WPID", "AC").at(0);
        gps_points.at(i).actionCost.push_back(
            GetActionPairFromPrefix(add_info_list.at(i), "AC", "From"));
        gps_points.at(i).fromIds =
            GetIDsFromPrefix(add_info_list.at(i), "From", "To");
        gps_points.at(i).toIds =
            GetIDsFromPrefix(add_info_list.at(i), "To", "Lid");
        ...
        vector<double> dnums =
            GetDoubleFromPrefix(add_info_list.at(i), "Vel", "Dir");
        if (dnums.size() > 0)
...
vector<WayPoint> MappingHelpers::GetCenterLaneDataVer0(TiXmlElement *pElem,
                                                       const int &currLaneID) {
  vector<WayPoint> gps_points;
  ...
  if (pV) {
    ...
    if (gps_points.size() == add_info_list.size()) {
      for (unsigned int i = 0; i < gps_points.size(); i++) {
        gps_points.at(i).id =
            GetIDsFromPrefix(add_info_list.at(i), "WPID", "C").at(0);
        gps_points.at(i).fromIds =
            GetIDsFromPrefix(add_info_list.at(i), "From", "To");
        gps_points.at(i).toIds =
            GetIDsFromPrefix(add_info_list.at(i), "To", "Vel");
        gps_points.at(i).v =
            GetDoubleFromPrefix(add_info_list.at(i), "Vel", "Dir").at(0);
        gps_points.at(i).pos.a = gps_points.at(i).pos.dir =
            GetDoubleFromPrefix(add_info_list.at(i), "Dir", "").at(0);
        //				if(currLaneID == 11115)
        //				{
        //
        //					pair<ACTION_TYPE, double>
        //act_cost; 						act_cost.first = FORWARD_ACTION; 						act_cost.second = 100;
...
vector<int> MappingHelpers::GetIDsFromPrefix(const string &str,
                                             const string &prefix,
...
vector<double> MappingHelpers::GetDoubleFromPrefix(const string &str,
                                                   const string &prefix,
                                                   const string &postfix) {
  int index1 = str.find(prefix) + prefix.size();
  int index2 = str.find(postfix, index1);
...
pair<ACTION_TYPE, double>
MappingHelpers::GetActionPairFromPrefix(const string &str, const string &prefix,
                                        const string &postfix) {
  int index1 = str.find(prefix) + prefix.size();
  int index2 = str.find(postfix, index1);
  if (index2 < 0 || postfix.size() == 0)
...
vector<string> MappingHelpers::SplitString(const string &str,
                                           const string &token) {
  ...
  for (unsigned int rs = 0; rs < map.roadSegments.size(); rs++) {
    ...
    for (unsigned int i = 0; i < map.roadSegments.at(rs).Lanes.size(); i++) {
      ...
      for (unsigned int rs_2 = 0; rs_2 < map.roadSegments.size(); rs_2++) {
        for (unsigned int i2 = 0; i2 < map.roadSegments.at(rs_2).Lanes.size();
             i2++) {
          int iCenter1 = pL->points.size() / 2;
          WayPoint wp_1 = pL->points.at(iCenter1);
          int iCenter2 = PlanningHelpers::GetClosestNextPointIndexFast(
              map.roadSegments.at(rs_2).Lanes.at(i2).points, wp_1);
          WayPoint closest_p =
              map.roadSegments.at(rs_2).Lanes.at(i2).points.at(iCenter2);
          double mid_a1 = wp_1.pos.a;
          double mid_a2 = closest_p.pos.a;
          double angle_diff =
              UtilityH::AngleBetweenTwoAnglesPositive(mid_a1, mid_a2);
          double distance = distance2points(wp_1.pos, closest_p.pos);

          if (pL->id != map.roadSegments.at(rs_2).Lanes.at(i2).id &&
              angle_diff < 0.05 && distance < 3.5 && distance > 2.5) {
            double perp_distance = DBL_MAX;
            if (pL->points.size() > 2 &&
                map.roadSegments.at(rs_2).Lanes.at(i2).points.size() > 2) {
              RelativeInfo info;
              PlanningHelpers::GetRelativeInfo(pL->points, closest_p, info);
              perp_distance = info.perp_distance;
              // perp_distance =
              // PlanningHelpers::GetPerpDistanceToVectorSimple(pL->points.at(iCenter1-1),
              // pL->points.at(iCenter1+1), closest_p);
            }

            if (perp_distance > 1.0 && perp_distance < 10.0) {
              pL->pRightLane = &map.roadSegments.at(rs_2).Lanes.at(i2);
              for (unsigned int i_internal = 0; i_internal < pL->points.size();
                   i_internal++) {
                if (i_internal <
                    map.roadSegments.at(rs_2).Lanes.at(i2).points.size()) {
                  pL->points.at(i_internal).RightPointId =
                      map.roadSegments.at(rs_2).Lanes.at(i2).id;
                  pL->points.at(i_internal).pRight =
                      &map.roadSegments.at(rs_2).Lanes.at(i2).points.at(
                          i_internal);
                  //									map.roadSegments.at(rs_2).Lanes.at(i2).points.at(i_internal).pLeft
                  //= &pL->points.at(i_internal);
                }
              }
              ...
              for (unsigned int i_internal = 0; i_internal < pL->points.size();
                ...
                if (i_internal <
                    map.roadSegments.at(rs_2).Lanes.at(i2).points.size()) {
                  pL->points.at(i_internal).LeftPointId =
                      map.roadSegments.at(rs_2).Lanes.at(i2).id;
                  pL->points.at(i_internal).pLeft =
                      &map.roadSegments.at(rs_2).Lanes.at(i2).points.at(
                          i_internal);
                  //									map.roadSegments.at(rs_2).Lanes.at(i2).points.at(i_internal).pRight
                  //= &pL->points.at(i_internal);
                }
              }
            }
...
void MappingHelpers::ExtractSignalData(
    const std::vector<UtilityHNS::AisanSignalFileReader::AisanSignal>
        &signal_data,
    const std::vector<UtilityHNS::AisanVectorFileReader::AisanVector>
        &vector_data,
        ...
        if (signal_data.at(is).VID == vector_data.at(iv).VID) {
          for (unsigned int ip = 0; ip < points_data.size(); ip++) {
            if (vector_data.at(iv).PID == points_data.at(ip).PID) {
              tl.pos = GPSPoint(points_data.at(ip).Ly + origin.x,
                                points_data.at(ip).Bx + origin.y,
                                points_data.at(ip).H + origin.z,
                                vector_data.at(iv).Hang * DEG2RAD);
              break;
            }
          }
        }
...
void MappingHelpers::ExtractStopLinesData(
    const std::vector<UtilityHNS::AisanStopLineFileReader::AisanStopLine>
        &stop_line_data,
    const std::vector<UtilityHNS::AisanLinesFileReader::AisanLine> &line_data,
    const std::vector<UtilityHNS::AisanPointsFileReader::AisanPoints>
      ...
      if (stop_line_data.at(ist).LID == line_data.at(il).LID) {
        ...
        for (unsigned int ip = 0; ip < points_data.size(); ip++) {
          if (points_data.at(ip).PID == s_id ||
              points_data.at(ip).PID == e_id) {
            sl.points.push_back(GPSPoint(points_data.at(ip).Ly + origin.x,
                                         points_data.at(ip).Bx + origin.y,
                                         points_data.at(ip).H + origin.z, 0));
          }
        }
      }
    }
...
void MappingHelpers::ExtractCurbData(
    const std::vector<UtilityHNS::AisanCurbFileReader::AisanCurb> &curb_data,
    const std::vector<UtilityHNS::AisanLinesFileReader::AisanLine> &line_data,
    const std::vector<UtilityHNS::AisanPointsFileReader::AisanPoints>
        &points_data,
  ...
  for (unsigned int ic = 0; ic < curb_data.size(); ic++) {
    ...
    for (unsigned int il = 0; il < line_data.size(); il++) {
      if (curb_data.at(ic).LID == line_data.at(il).LID) {
        ...
        for (unsigned int ip = 0; ip < points_data.size(); ip++) {
          if (points_data.at(ip).PID == s_id) {
            c.points.push_back(GPSPoint(points_data.at(ip).Ly + origin.x,
                                        points_data.at(ip).Bx + origin.y,
                                        points_data.at(ip).H + origin.z, 0));
            WayPoint wp;
...
void MappingHelpers::ExtractWayArea(
    const std::vector<UtilityHNS::AisanAreasFileReader::AisanArea> &area_data,
    const std::vector<UtilityHNS::AisanWayareaFileReader::AisanWayarea>
        &wayarea_data,
    const std::vector<UtilityHNS::AisanLinesFileReader::AisanLine> &line_data,
          ...
          if (line_data.at(il).LID >= s_id && line_data.at(il).LID <= e_id) {
            for (unsigned int ip = 0; ip < points_data.size(); ip++) {
              if (points_data.at(ip).PID == line_data.at(il).BPID) {
                GPSPoint p(points_data.at(ip).Ly + origin.x,
                           points_data.at(ip).Bx + origin.y,
                           points_data.at(ip).H + origin.z, 0);
      ...
      for (unsigned int p = 0; p < pLane->points.size(); p++) {
        ...
        if (p + 1 == pLane->points.size()) // Last Point in Lane
        {
          for (unsigned int j = 0; j < pLane->toLanes.size(); j++) {
            // cout << "Link, Next Lane: " << pWP->laneId << ", WP: " << pWP->id
            // << " To WP: " << pWP->toIds.at(j) << endl;
            pWP->pFronts.push_back(&pLane->toLanes.at(j)->points.at(0));
          }
        } else {
          if (pWP->toIds.size() > 1) {
            cout << "Error Error Erro ! Lane: " << pWP->laneId
                 << ", Point: " << pWP->originalMapID << endl;
          } else {
            //	cout << "Link, Same Lane: " << pWP->laneId << ", WP: " <<
            //pWP->id << " To WP: " <<
            //map.roadSegments.at(rs).Lanes.at(i).points.at(p+1).id << endl;
      ...
      for (unsigned int p = 0;
        ...
        for (unsigned int isl = 0; isl < map.stopLines.size(); isl++) {
          ...
          if (map.stopLines.at(isl).linkID == pWP->id) {
            ...
            for (unsigned int itl = 0; itl < map.trafficLights.size(); itl++) {
              if (map.trafficLights.at(itl).id ==
                  map.stopLines.at(isl).trafficLightID) {
...
void MappingHelpers::LinkTrafficLightsAndStopLinesConData(
    const std::vector<UtilityHNS::AisanDataConnFileReader::DataConn> &conn_data,
  ...
  for (unsigned int rs = 0; rs < map.roadSegments.size(); rs++) {
    for (unsigned int i = 0; i < map.roadSegments.at(rs).Lanes.size(); i++) {
      ...
      for (unsigned int ic = 0; ic < conn_data.size(); ic++) {
        UtilityHNS::AisanDataConnFileReader::DataConn data_conn =
            conn_data.at(ic);
        ...
        if (map.roadSegments.at(rs).Lanes.at(i).id == data_conn.LID) {
          for (unsigned int itl = 0; itl < map.trafficLights.size(); itl++) {
            if (map.trafficLights.at(itl).id == data_conn.SID) {
              map.trafficLights.at(itl).laneIds.push_back(
                  map.roadSegments.at(rs).Lanes.at(i).id);
              map.trafficLights.at(itl).pLanes.push_back(
                  &map.roadSegments.at(rs).Lanes.at(i));
              map.roadSegments.at(rs).Lanes.at(i).trafficlights.push_back(
          ...
          for (unsigned int isl = 0; isl < map.stopLines.size(); isl++) {
            if (map.stopLines.at(isl).id == data_conn.SLID) {
              map.stopLines.at(isl).laneId =
                  map.roadSegments.at(rs).Lanes.at(i).id;
              map.stopLines.at(isl).pLane =
                  &map.roadSegments.at(rs).Lanes.at(i);
...
void MappingHelpers::UpdateMapWithOccupancyGrid(
    OccupancyToGridMap &map_info, const std::vector<int> &data,
    RoadNetwork &map, std::vector<WayPoint *> &updated_list) {
  PlannerHNS::Mat3 rotationMat(-map_info.center.pos.a);
  PlannerHNS::Mat3 translationMat(-map_info.center.pos.x,
                                  -map_info.center.pos.y);
  updated_list.clear();
  ...
  for (unsigned int rs = 0; rs < map.roadSegments.size(); rs++) {
    for (unsigned int i = 0; i < map.roadSegments.at(rs).Lanes.size(); i++) {
      for (unsigned int p = 0;
        ...
        int cell_value = 0;
        if (map_info.GetCellIndexFromPoint(relative_point, data, cell_value) ==
            true) {
          if (cell_value == 0) {
            bool bFound = false;
            for (unsigned int i_action = 0; i_action < pWP->actionCost.size();
                 i_action++) {
...
void MappingHelpers::ConstructRoadNetworkFromROSMessageV2(
    const std::vector<UtilityHNS::AisanLanesFileReader::AisanLane> &lanes_data,
    const std::vector<UtilityHNS::AisanPointsFileReader::AisanPoints>
        &points_data,
    const std::vector<UtilityHNS::AisanCenterLinesFileReader::AisanCenterLine>
    ...
    UtilityHNS::AisanLinesFileReader *pLinedata, const GPSPoint &origin,
    RoadNetwork &map, const bool &bSpecialFlag,
    const bool &bFindLaneChangeLanes, const bool &bFindCurbsAndWayArea) {
  vector<Lane> roadLanes;
  ...
  cout << " >> Map loaded from data with " << roadLanes.size() << " lanes"
       << endl;
}

bool MappingHelpers::GetPointFromDataList(
    UtilityHNS::AisanPointsFileReader *pPointsData, const int &pid,
    WayPoint &out_wp) {
  if (pPointsData == nullptr)
    return false;
...
int MappingHelpers::GetBeginPointIdFromLaneNo(
    UtilityHNS::AisanLanesFileReader *pLaneData,
    UtilityHNS::AisanPointsFileReader *pPointsData,
...
int MappingHelpers::GetEndPointIdFromLaneNo(
    UtilityHNS::AisanLanesFileReader *pLaneData,
    UtilityHNS::AisanPointsFileReader *pPointsData,
    UtilityHNS::AisanNodesFileReader *pNodesData, const int &LnID) {
  UtilityHNS::AisanLanesFileReader::AisanLane *pL = nullptr;
...
bool MappingHelpers::IsStartLanePoint(
    UtilityHNS::AisanLanesFileReader *pLaneData,
    UtilityHNS::AisanLanesFileReader::AisanLane *pL) {
  ...
  UtilityHNS::AisanLanesFileReader::AisanLane *pPrevL =
      pLaneData->GetDataRowById(pL->BLID);
  if (pPrevL == nullptr || pPrevL->FLID2 > 0 || pPrevL->FLID3 > 0 ||
      pPrevL->FLID4 > 0 || pPrevL->JCT > 0)
    return true;

  return false;
}

bool MappingHelpers::IsEndLanePoint(
    UtilityHNS::AisanLanesFileReader *pLaneData,
    UtilityHNS::AisanLanesFileReader::AisanLane *pL) {
  if (pL->FLID2 > 0 || pL->FLID3 > 0 || pL->FLID4 > 0)
    return true;

  UtilityHNS::AisanLanesFileReader::AisanLane *pNextL =
      pLaneData->GetDataRowById(pL->FLID);

  return IsStartLanePoint(pLaneData, pNextL);
}

void MappingHelpers::GetLanesStartPoints(
    UtilityHNS::AisanLanesFileReader *pLaneData,
  ...
  for (unsigned int il = 0; il < pLaneData->m_data_list.size(); il++) {
    ...
    if (DEBUG_MAP_PARSING) {
      if (IsStartLanePoint(pLaneData, pL) && IsEndLanePoint(pLaneData, pL))
        cout << " :( :( :( Start And End in the same time !! " << pL->LnID
             << endl;
    }
  }
}

void MappingHelpers::ConnectLanes(UtilityHNS::AisanLanesFileReader *pLaneData,
                                  std::vector<PlannerHNS::Lane> &lanes) {
  for (unsigned int il = 0; il < lanes.size(); il++) {
    WayPoint fp = lanes.at(il).points.at(0);
    UtilityHNS::AisanLanesFileReader::AisanLane *pFirstL =
        pLaneData->GetDataRowById(fp.originalMapID);
    if (pFirstL != nullptr) {
      if (pFirstL->BLID > 0)
    ...
    UtilityHNS::AisanLanesFileReader::AisanLane *pEndL =
        pLaneData->GetDataRowById(ep.originalMapID);
    if (pEndL != nullptr) {
      if (pEndL->FLID > 0)
...
void MappingHelpers::GetLanePoints(
    UtilityHNS::AisanLanesFileReader *pLaneData,
    UtilityHNS::AisanPointsFileReader *pPointsData,
  ...
  while (!bStart) {
    ...
    if (next_lnid == 0)
      ...
      bStart =
          IsStartLanePoint(pLaneData, pLaneData->GetDataRowById(next_lnid));

    //		if(_lnid == 1267 ) //|| _lnid == 1268 || _lnid == 1269 || _lnid ==
    //958) 			out_lane.id = lnID;

    if (out_lane.points.size() == 0) {
      WayPoint wp1, wp2;
      GetPointFromDataList(pPointsData,
                           pNodesData->GetDataRowById(pL->BNID)->PID, wp1);
      wp1.v = pL->RefVel == 0 ? DEFAULT_REF_VELOCITY : pL->RefVel;

      wp1.id = pL->BNID;

      if (pL->BLID != 0) {
        _rID = GetBeginPointIdFromLaneNo(pLaneData, pPointsData, pNodesData,
                                         pL->BLID);
        if (_rID > 0)
      ...
      GetPointFromDataList(pPointsData,
                           pNodesData->GetDataRowById(pL->FNID)->PID, wp2);
      wp2.v = pL->RefVel == 0 ? DEFAULT_REF_VELOCITY : pL->RefVel;
      wp2.id = pL->FNID;

      if (bStart && pL->FLID != 0) {
        _rID = GetEndPointIdFromLaneNo(pLaneData, pPointsData, pNodesData,
                                       pL->FLID);
        if (_rID > 0)
      ...
      if (pL->LaneDir == 'L') {
        wp1.actionCost.push_back(
            make_pair(LEFT_TURN_ACTION, LEFT_INITIAL_TURNS_COST));
        wp2.actionCost.push_back(
            make_pair(LEFT_TURN_ACTION, LEFT_INITIAL_TURNS_COST));
      ...
      WayPoint wp;
      GetPointFromDataList(pPointsData,
                           pNodesData->GetDataRowById(pL->FNID)->PID, wp);
  ...
  for (unsigned int il = 0; il < lanes.size(); il++) {
    for (int ip = 1; ip < lanes.at(il).points.size(); ip++) {
      ...
      if (d == 0) {
        ...
        if (DEBUG_MAP_PARSING)
          cout << "Fixed Redundant Points for Lane:" << lanes.at(il).id
               << ", Current: " << ip
               << ", Size: " << lanes.at(il).points.size() << endl;
      }
    }
  }
}
...
void MappingHelpers::FixTwoPointsLane(Lane &l) {
  if (l.points.size() == 2) {
    ...
    cout << "## WOW Lane " << l.id << " With Size (" << l.points.size() << ") "
         << endl;
  }
}
    ...
    if (lanes.at(il).fromIds.size() == 0) {
      ...
      for (int l = 0; l < sp_lanes.size(); l++) {
        if (pFL->id != sp_lanes.at(l).id) {
          ...
          WayPoint lastofother =
              sp_lanes.at(l).points.at(sp_lanes.at(l).points.size() - 1);
          PlanningHelpers::GetRelativeInfoLimited(sp_lanes.at(l).points,
                                                  pFL->points.at(0), info, 0);
          double back_distance =
              hypot(lastofother.pos.y - pFL->points.at(0).pos.y,
                    lastofother.pos.x - pFL->points.at(0).pos.x);
          bool bCloseFromBack = false;
          if ((info.bAfter == true && back_distance < 15.0) ||
              info.bAfter == false)
            bCloseFromBack = true;

          if (fabs(info.perp_distance) < 2 &&
              fabs(info.perp_distance) < closest_d && info.bBefore == false &&
              bCloseFromBack) {
      ...
      if (pL != nullptr && pFL != nullptr) {
        if (closest_info.iFront == pL->points.size() - 1) {
          ...
          pL->points.at(closest_info.iFront)
              .toIds.push_back(pFL->points.at(0).id);

          pFL->points.at(0).fromIds.push_back(
              pL->points.at(closest_info.iFront).id);
          pFL->fromIds.push_back(pL->id);
          bAtleastOneChange = true;
          if (DEBUG_MAP_PARSING) {
            cout << "Closest Next Lane For: " << pFL->id << " , Is:" << pL->id
                 << ", Distance=" << fabs(closest_info.perp_distance)
                 << ", Size: " << pL->points.size()
                 << ", back_index: " << closest_info.iBack
                 << ", front_index: " << closest_info.iFront
                 << ", Direct: " << closest_info.angle_diff << endl;
          ...
          front_half.points.insert(front_half.points.begin(),
                                   pL->points.begin() + closest_info.iFront,
                                   pL->points.end());
          ...
          back_half.points.insert(back_half.points.begin(), pL->points.begin(),
                                  pL->points.begin() + closest_info.iFront);
          ...
          WayPoint *last_from_back =
              &back_half.points.at(back_half.points.size() - 1);
          ...
          if (front_half.points.size() > 1 && back_half.points.size() > 1) {
            if (DEBUG_MAP_PARSING)
              cout << "Split this one Nicely! first_half_size: "
                   << front_half.points.size()
                   << ", second_hald_size: " << back_half.points.size() << endl;
        ...
        if (DEBUG_MAP_PARSING)
          cout << "=> Can't find Before Lanes For:  " << lanes.at(il).id
               << endl;
      }
    }
  }
    ...
    if (lanes.at(il).toIds.size() == 0) {
      ...
      for (int l = 0; l < sp_lanes.size(); l++) {
        if (pBL->id != sp_lanes.at(l).id) {
          ...
          PlanningHelpers::GetRelativeInfoLimited(sp_lanes.at(l).points,
                                                  last_point, info, 0);
          double front_distance = hypot(firstofother.pos.y - last_point.pos.y,
                                        firstofother.pos.x - last_point.pos.x);
          bool bCloseFromFront = false;
          if ((info.bBefore == true && front_distance < 15.0) ||
              info.bBefore == false)
            bCloseFromFront = true;

          if (fabs(info.perp_distance) < 2 &&
              fabs(info.perp_distance) < closest_d && info.bAfter == false &&
              bCloseFromFront) {
            closest_d = fabs(info.perp_distance);
            pL = &sp_lanes.at(l);
            closest_info = info;
      ...
      if (pL != nullptr && pBL != nullptr) {
        if (closest_info.iFront == 0) {
          ...
          pL->points.at(closest_info.iFront)
              .fromIds.push_back(pBL->points.at(pBL->points.size() - 1).id);

          pBL->points.at(pBL->points.size() - 1)
              .toIds.push_back(pL->points.at(closest_info.iFront).id);
          pBL->toIds.push_back(pL->id);

          bAtleastOneChange = true;
          if (DEBUG_MAP_PARSING) {
            cout << "Closest Back Lane For: " << pBL->id << " , Is:" << pL->id
                 << ", Distance=" << fabs(closest_info.perp_distance)
                 << ", Size: " << pL->points.size()
                 << ", back_index: " << closest_info.iBack
                 << ", front_index: " << closest_info.iFront
...
void MappingHelpers::ExtractCurbDataV2(
    const std::vector<UtilityHNS::AisanCurbFileReader::AisanCurb> &curb_data,
    UtilityHNS::AisanLinesFileReader *pLinedata,
    UtilityHNS::AisanPointsFileReader *pPointsData, const GPSPoint &origin,
    RoadNetwork &map) {
  for (unsigned int ic = 0; ic < curb_data.size(); ic++) {
    Curb c;
    c.id = curb_data.at(ic).ID;
    ...
    for (unsigned int il = 0; il < pLinedata->m_data_list.size(); il++) {
      if (curb_data.at(ic).LID == pLinedata->m_data_list.at(il).LID) {
        ...
        AisanPointsFileReader::AisanPoints *pP =
            pPointsData->GetDataRowById(s_id);
        if (pP != nullptr) {
          c.points.push_back(GPSPoint(pP->Ly + origin.x, pP->Bx + origin.y,
                                      pP->H + origin.z, 0));
          WayPoint wp;
          wp.pos = c.points.at(0);
          Lane *pLane = GetClosestLaneFromMap(wp, map, 5);
...
void MappingHelpers::ExtractSignalDataV2(
    const std::vector<UtilityHNS::AisanSignalFileReader::AisanSignal>
        &signal_data,
    const std::vector<UtilityHNS::AisanVectorFileReader::AisanVector>
        &vector_data,
    UtilityHNS::AisanPointsFileReader *pPointData, const GPSPoint &origin,
    RoadNetwork &map) {
  for (unsigned int is = 0; is < signal_data.size(); is++) {
    // if(signal_data.at(is).Type == 2)
    {
      ...
      for (unsigned int iv = 0; iv < vector_data.size(); iv++) {
        if (signal_data.at(is).VID == vector_data.at(iv).VID) {
          AisanPointsFileReader::AisanPoints *pP =
              pPointData->GetDataRowById(vector_data.at(iv).PID);
          if (pP != nullptr) {
            tl.pos =
                GPSPoint(pP->Ly + origin.x, pP->Bx + origin.y, pP->H + origin.z,
                         vector_data.at(iv).Hang * DEG2RAD);
          }
        }
      }
...
void MappingHelpers::ExtractStopLinesDataV2(
    const std::vector<UtilityHNS::AisanStopLineFileReader::AisanStopLine>
        &stop_line_data,
    UtilityHNS::AisanLinesFileReader *pLineData,
    UtilityHNS::AisanPointsFileReader *pPointData, const GPSPoint &origin,
  ...
  for (unsigned int ist = 0; ist < stop_line_data.size(); ist++) {
    ...
    AisanLinesFileReader::AisanLine *pLine =
        pLineData->GetDataRowById(stop_line_data.at(ist).LID);
    if (pLine != nullptr) {
      UtilityHNS::AisanPointsFileReader::AisanPoints *pStart =
          pPointData->GetDataRowById(pLine->BPID);
      UtilityHNS::AisanPointsFileReader::AisanPoints *pEnd =
          pPointData->GetDataRowById(pLine->FPID);
      if (pStart != nullptr)
        sl.points.push_back(GPSPoint(pStart->Ly + origin.x,
                                     pStart->Bx + origin.y,
                                     pStart->H + origin.z, 0));

      if (pEnd != nullptr)
        sl.points.push_back(GPSPoint(pEnd->Ly + origin.x, pEnd->Bx + origin.y,
                                     pEnd->H + origin.z, 0));
    }
...
void MappingHelpers::FindAdjacentLanesV2(RoadNetwork &map) {
  ...
  for (unsigned int rs = 0; rs < map.roadSegments.size(); rs++) {
    for (unsigned int i = 0; i < map.roadSegments.at(rs).Lanes.size(); i++) {
      Lane *pL = &map.roadSegments.at(rs).Lanes.at(i);
      for (unsigned int i2 = 0; i2 < map.roadSegments.at(rs).Lanes.size();
           i2++) {
        ...
        for (unsigned int p = 0; p < pL->points.size(); p++) {
          ...
          if (!info.bAfter && !info.bBefore && fabs(info.perp_distance) > 1.2 &&
              ...
              UtilityH::AngleBetweenTwoAnglesPositive(info.perp_point.pos.a,
                                                      pWP->pos.a) < 0.06) {
