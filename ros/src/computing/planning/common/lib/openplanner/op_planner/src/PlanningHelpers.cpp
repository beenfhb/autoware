...
std::vector<std::pair<GPSPoint, GPSPoint>>
    PlanningHelpers::m_TestingClosestPoint;

PlanningHelpers::PlanningHelpers() {}

PlanningHelpers::~PlanningHelpers() {}

bool PlanningHelpers::GetRelativeInfoRange(
    const std::vector<std::vector<WayPoint>> &trajectories, const WayPoint &p,
    const double &searchDistance, RelativeInfo &info) {
  ...
  for (unsigned int i = 0; i < trajectories.size(); i++) {
    ...
    double angle_diff = UtilityH::AngleBetweenTwoAnglesPositive(
                            info_item.perp_point.pos.a, p.pos.a) *
                        RAD2DEG;
    if (angle_diff < 75) {
  ...
  for (unsigned int i = 0; i < infos.size(); i++) {
    if (searchDistance > 0) {
      double laneChangeCost = trajectories.at(infos.at(i).iGlobalPath)
                                  .at(infos.at(i).iFront)
                                  .laneChangeCost;
      if (fabs(infos.at(i).perp_distance) < searchDistance &&
          laneChangeCost < minCost) {
        min_index = i;
...
bool PlanningHelpers::GetRelativeInfo(const std::vector<WayPoint> &trajectory,
                                      const WayPoint &p, RelativeInfo &info,
                                      const int &prevIndex) {
  ...
  if (trajectory.size() == 2) {
    ...
    p1 = WayPoint((trajectory.at(0).pos.x + trajectory.at(1).pos.x) / 2.0,
                  ...
                  (trajectory.at(0).pos.z + trajectory.at(1).pos.z) / 2.0,
                  trajectory.at(0).pos.a);
    info.iFront = 1;
    info.iBack = 0;
  } else {
    info.iFront = GetClosestNextPointIndexFast(trajectory, p, prevIndex);
    //		WayPoint p2 = p;
    //		int old_index = GetClosestNextPointIndex(trajectory, p2,
    //prevIndex); 		if(old_index != info.iFront) 			cout << " Alert Alert !!!! fast:
    //" << info.iFront << ", slow: " << old_index  << endl;
  ...
  info.perp_point.pos.y =
      info.perp_distance; // perp distance between the car and the trajectory
  ...
  info.angle_diff =
      UtilityH::AngleBetweenTwoAnglesPositive(p1.pos.a, p.pos.a) * RAD2DEG;

  return true;
}

bool PlanningHelpers::GetRelativeInfoLimited(
    const std::vector<WayPoint> &trajectory, const WayPoint &p,
    RelativeInfo &info, const int &prevIndex) {
  if (trajectory.size() < 2)
    return false;
  ...
  if (trajectory.size() == 2) {
    ...
    if (info.iFront == 0) {
      ...
      p0 = _trajectory.at(info.iFront - 1);
      p1 = WayPoint((p0.pos.x + _trajectory.at(info.iFront).pos.x) / 2.0,
                    (p0.pos.y + _trajectory.at(info.iFront).pos.y) / 2.0,
                    (p0.pos.z + _trajectory.at(info.iFront).pos.z) / 2.0,
                    p0.pos.a);
    }
    ...
    info.perp_point.pos.x = 0; // on the same y axis of the car
    info.perp_point.pos.y =
        info.perp_distance; // perp distance between the car and the _trajectory
    ...
    if (info.iFront == 0) {
      ...
      double angle_befor_last = UtilityH::FixNegativeAngle(
          atan2(_trajectory.at(s - 2).pos.y - _trajectory.at(s - 1).pos.y,
                _trajectory.at(s - 2).pos.x - _trajectory.at(s - 1).pos.x));
      double angle_from_perp = UtilityH::FixNegativeAngle(
          atan2(info.perp_point.pos.y - _trajectory.at(s - 1).pos.y,
                info.perp_point.pos.x - _trajectory.at(s - 1).pos.x));
      double diff_last_perp = UtilityH::AngleBetweenTwoAnglesPositive(
    ...
    if (info.iFront == 0) {
      ...
      double angle_befor_last = UtilityH::FixNegativeAngle(
          atan2(trajectory.at(s - 2).pos.y - trajectory.at(s - 1).pos.y,
                trajectory.at(s - 2).pos.x - trajectory.at(s - 1).pos.x));
      double angle_from_perp = UtilityH::FixNegativeAngle(
          atan2(info.perp_point.pos.y - trajectory.at(s - 1).pos.y,
                info.perp_point.pos.x - trajectory.at(s - 1).pos.x));
      double diff_last_perp = UtilityH::AngleBetweenTwoAnglesPositive(
          angle_befor_last, angle_from_perp);
      info.after_angle = diff_last_perp;
      if (diff_last_perp > M_PI_2) {
...
bool PlanningHelpers::GetThreePointsInfo(const WayPoint &p0, const WayPoint &p1,
                                         const WayPoint &p2, WayPoint &perp_p,
                                         double &long_d, double lat_d) {

  if (p0.pos.x == p1.pos.x && p0.pos.y == p1.pos.y)
    return false;
  if (p0.pos.x == p2.pos.x && p0.pos.y == p2.pos.y)
    return false;
...
WayPoint PlanningHelpers::GetFollowPointOnTrajectory(
    const std::vector<WayPoint> &trajectory, const RelativeInfo &init_p,
    const double &distance, unsigned int &point_index) {
  ...
  if (init_p.iBack == 0 && init_p.iBack == init_p.iFront &&
      init_p.from_back_distance > distance) {
    follow_point = trajectory.at(init_p.iFront);
    follow_point.pos.x =
        init_p.perp_point.pos.x + distance * cos(follow_point.pos.a);
    follow_point.pos.y =
        init_p.perp_point.pos.y + distance * sin(follow_point.pos.a);
  }
  // condition 2, if far after the last point on the trajectory
    ...
    while (local_i < (int)trajectory.size() - 1 && d < distance) {
      local_i++;
      d += hypot(
          trajectory.at(local_i).pos.y - trajectory.at(local_i - 1).pos.y,
          trajectory.at(local_i).pos.x - trajectory.at(local_i - 1).pos.x);
    }
...
double PlanningHelpers::GetExactDistanceOnTrajectory(
    const std::vector<WayPoint> &trajectory, const RelativeInfo &p1,
  ...
  if (p2.iFront == p1.iFront && p2.iBack == p1.iBack) {
    ...
    for (int i = p1.iFront; i < p2.iBack; i++)
      d_on_path += hypot(trajectory.at(i + 1).pos.y - trajectory.at(i).pos.y,
                         trajectory.at(i + 1).pos.x - trajectory.at(i).pos.x);
...
int PlanningHelpers::GetClosestNextPointIndex_obsolete(
    const vector<WayPoint> &trajectory, const WayPoint &p,
    const int &prevIndex) {
  if (trajectory.size() == 0 || prevIndex < 0)
    return 0;
  ...
  //	cout << ", Minimum After: " << min_index << ", Big O: " <<
  //trajectory.size() << endl;

  return min_index;
}

int PlanningHelpers::GetClosestNextPointIndexFastV2(
    const vector<WayPoint> &trajectory, const WayPoint &p,
    const int &prevIndex) {
  ...
  double d_to_zero =
      hypot(p.pos.y - trajectory[0].pos.y, p.pos.x - trajectory[0].pos.x);
  double d_to_size = hypot(trajectory[size - 1].pos.y - p.pos.y,
                           trajectory[size - 1].pos.x - p.pos.x);
  ...
  if (iStart > 0 && iStart < size - 1 &&
      GetThreePointsInfo(trajectory.at(0), trajectory.at(iStart), p, perp_p,
                         long_d, lat_d)) {
    //		m_TestingClosestPoint.push_back(make_pair(trajectory.at(0).pos,
    //p.pos));
    //		m_TestingClosestPoint.push_back(make_pair(trajectory.at(iStart).pos,
    //p.pos));
  ...
  if (iEnd >= 0 && iEnd < size - 2 &&
      GetThreePointsInfo(trajectory.at(size - 1), trajectory.at(iEnd), p,
                         perp_p, long_d, lat_d)) {
    //		m_TestingClosestPoint.push_back(make_pair(trajectory.at(size-1).pos,
    //p.pos));
    //		m_TestingClosestPoint.push_back(make_pair(trajectory.at(iEnd).pos,
    //p.pos));
  ...
  if (iStart < size)
    d_from_start = hypot(trajectory[iStart].pos.y - p.pos.y,
                         trajectory[iStart].pos.x - p.pos.x);

  double d_from_end = d_to_size;
  if (iEnd >= 0)
    d_from_end = hypot(trajectory[iEnd].pos.y - p.pos.y,
                       trajectory[iEnd].pos.x - p.pos.x);
  ...
  //	m_TestingClosestPoint.push_back(make_pair(trajectory.at(min_index).pos,
  //p.pos));

  //	cout << ", Minimum After: " << min_index << ", Big O: " << ncout <<
  //endl; 	cout << "d_zero: " << d_to_zero << ", d_start: " << d_from_start <<
  //endl; 	cout << "d_size: " << d_to_size << ", d_end: " << d_from_end << endl;
  return min_index;
}

int PlanningHelpers::GetClosestNextPointIndexFast(
    const vector<WayPoint> &trajectory, const WayPoint &p,
    const int &prevIndex) {
  int size = (int)trajectory.size();
  ...
  for (int i = 0; i < size; i += skip) {
    if (i + skip / 2 < size)
      d = (distance2pointsSqr(trajectory[i].pos, p.pos) +
           distance2pointsSqr(trajectory[i + skip / 2].pos, p.pos)) /
          2.0;
    else
      d = distance2pointsSqr(trajectory[i].pos, p.pos);
    if (d < minD) {
  ...
  if ((size - skip / 2 - 1) > 0)
    d = (distance2pointsSqr(trajectory[size - 1].pos, p.pos) +
         distance2pointsSqr(trajectory[size - skip / 2 - 1].pos, p.pos)) /
        2.0;
  else
    d = distance2pointsSqr(trajectory[size - 1].pos, p.pos);
...
int PlanningHelpers::GetClosestNextPointIndexDirectionFast(
    const vector<WayPoint> &trajectory, const WayPoint &p,
    const int &prevIndex) {
  int size = (int)trajectory.size();
  ...
  for (unsigned int i = prevIndex; i < size; i++) {
    d = distance2pointsSqr(trajectory[i].pos, p.pos);
    double angle_diff =
        UtilityH::AngleBetweenTwoAnglesPositive(trajectory[i].pos.a, p.pos.a) *
        RAD2DEG;
...
int PlanningHelpers::GetClosestPointIndex_obsolete(
    const vector<WayPoint> &trajectory, const WayPoint &p,
    const int &prevIndex) {
  if (trajectory.size() == 0 || prevIndex < 0)
    return 0;
...
WayPoint PlanningHelpers::GetPerpendicularOnTrajectory_obsolete(
    const vector<WayPoint> &trajectory, const WayPoint &p, double &distance,
    const int &prevIndex) {
  if (trajectory.size() < 2)
    return p;
  ...
  if (trajectory.size() == 2) {
    ...
    int next_index =
        GetClosestNextPointIndex_obsolete(trajectory, p, prevIndex);
    ...
    if (next_index == 0) {
      ...
      p1 = WayPoint((p0.pos.x + p2.pos.x) / 2.0, (p0.pos.y + p2.pos.y) / 2.0,
                    (p0.pos.z + p2.pos.z) / 2.0, p0.pos.a);
    }
  }
...
double PlanningHelpers::GetPerpDistanceToTrajectorySimple_obsolete(
    const vector<WayPoint> &trajectory, const WayPoint &p,
    const int &prevIndex) {
...
double PlanningHelpers::GetPerpDistanceToVectorSimple_obsolete(
    const WayPoint &point1, const WayPoint &point2, const WayPoint &pose) {
...
WayPoint PlanningHelpers::GetNextPointOnTrajectory_obsolete(
    const vector<WayPoint> &trajectory, const double &distance,
    const int &currIndex) {
...
double PlanningHelpers::GetDistanceOnTrajectory_obsolete(
    const std::vector<WayPoint> &path, const int &start_index,
    const WayPoint &p) {
  ...
  double padding_distance =
      distance2points(path.at(end_point_index).pos, p.pos);
...
bool PlanningHelpers::CompareTrajectories(const std::vector<WayPoint> &path1,
                                          const std::vector<WayPoint> &path2) {
  if (path1.size() != path2.size())
    return false;

  for (unsigned int i = 0; i < path1.size(); i++) {
    if (path1.at(i).v != path2.at(i).v ||
        path1.at(i).pos.x != path2.at(i).pos.x ||
        path1.at(i).pos.y != path2.at(i).pos.y ||
        path1.at(i).pos.alt != path2.at(i).pos.alt ||
        path1.at(i).pos.lon != path2.at(i).pos.lon)
      return false;
  }
...
double PlanningHelpers::GetDistanceToClosestStopLineAndCheck(
    const std::vector<WayPoint> &path, const WayPoint &p,
...
void PlanningHelpers::CreateManualBranchFromTwoPoints(
    WayPoint &p1, WayPoint &p2, const double &distance,
    const DIRECTION_TYPE &direction, std::vector<WayPoint> &path) {
...
void PlanningHelpers::CreateManualBranch(std::vector<WayPoint> &path,
                                         const int &degree,
...
void PlanningHelpers::FixPathDensity(vector<WayPoint> &path,
  ...
  for (unsigned int si = 0, ei = 1; ei < path.size();) {
    d += hypot(path.at(ei).pos.x - path.at(ei - 1).pos.x,
               path.at(ei).pos.y - path.at(ei - 1).pos.y) +
         remaining;
    a = atan2(path.at(ei).pos.y - path.at(si).pos.y,
              path.at(ei).pos.x - path.at(si).pos.x);
...
void PlanningHelpers::SmoothPath(vector<WayPoint> &path, double weight_data,
  ...
  while (change >= tolerance) {
    ...
    for (int i = 1; i < size - 1; i++) {
      //			if (smoothPath_out[i].pos.a != smoothPath_out[i
      //- 1].pos.a) 				continue;

      xtemp = smoothPath_out[i].pos.x;
      ytemp = smoothPath_out[i].pos.y;

      smoothPath_out[i].pos.x +=
          weight_data * (path_in[i].pos.x - smoothPath_out[i].pos.x);
      smoothPath_out[i].pos.y +=
          weight_data * (path_in[i].pos.y - smoothPath_out[i].pos.y);
...
void PlanningHelpers::PredictConstantTimeCostForTrajectory(
    ...
    const PlannerHNS::WayPoint &currPose, const double &minVelocity,
  ...
  for (unsigned int i = info.iFront; i < path.size(); i++) {
    total_distance += hypot(path.at(i).pos.x - path.at(i - 1).pos.x,
                            path.at(i).pos.y - path.at(i - 1).pos.y);
...
void PlanningHelpers::FixAngleOnly(std::vector<WayPoint> &path) {
  ...
  path[0].pos.a = UtilityH::FixNegativeAngle(
      atan2(path[1].pos.y - path[0].pos.y, path[1].pos.x - path[0].pos.x));

  for (int j = 1; j < path.size() - 1; j++)
    path[j].pos.a = UtilityH::FixNegativeAngle(atan2(
        path[j + 1].pos.y - path[j].pos.y, path[j + 1].pos.x - path[j].pos.x));
  ...
  for (int j = 0; j < path.size() - 1; j++) {
    if (path.at(j).pos.x == path.at(j + 1).pos.x &&
        path.at(j).pos.y == path.at(j + 1).pos.y)
...
double PlanningHelpers::CalcAngleAndCost(vector<WayPoint> &path,
  ...
  for (int j = 1; j < path.size() - 1; j++) {
    path[j].pos.a = UtilityH::FixNegativeAngle(atan2(
        path[j + 1].pos.y - path[j].pos.y, path[j + 1].pos.x - path[j].pos.x));
    path[j].cost =
        path[j - 1].cost + distance2points(path[j - 1].pos, path[j].pos);
  }
...
double
PlanningHelpers::CalcAngleAndCostAndCurvatureAnd2D(vector<WayPoint> &path,
                                                   const double &lastCost) {
  if (path.size() < 2)
    return -1;
  ...
  for (unsigned int j = 1; j < path.size() - 1; j++) {
    ...
    path[j].pos.a = atan2(path[j + 1].pos.y - path[j].pos.y,
...
double PlanningHelpers::CalcCircle(const GPSPoint &pt1, const GPSPoint &pt2,
                                   const GPSPoint &pt3, GPSPoint &center) {
  ...
  center.x = (aSlope * bSlope * (pt1.y - pt3.y) + bSlope * (pt1.x + pt2.x) -
              aSlope * (pt2.x + pt3.x)) /
             (2.0 * (bSlope - aSlope));
  center.y = -1.0 * (center.x - (pt1.x + pt2.x) / 2.0) / aSlope +
             (pt1.y + pt2.y) / 2.0;

  return distance2points(center, pt1);
}

void PlanningHelpers::ExtractPartFromPointToDistance(
    const vector<WayPoint> &originalPath, const WayPoint &pos,
    const double &minDistance, const double &pathDensity,
    vector<WayPoint> &extractedPath, const double &SmoothDataWeight,
    const double &SmoothWeight, const double &SmoothTolerance) {
  extractedPath.clear();
  unsigned int close_index =
      GetClosestNextPointIndexDirectionFast(originalPath, pos);
  //	int i_slow = GetClosestNextPointIndexDirection(originalPath, pos);
  //	if(close_index != i_slow)
  //		cout << "Aler Alert !!! fast: " << close_index << ", slow: " <<
  //i_slow  << endl; vector<WayPoint> tempPath;
  double d_limit = 0;
  if (close_index >= 2)
    close_index -= 2;
  else
  ...
  for (unsigned int i = close_index; i < originalPath.size(); i++) {
    ...
    if (i > 0)
      d_limit += hypot(originalPath.at(i).pos.y - originalPath.at(i - 1).pos.y,
                       originalPath.at(i).pos.x - originalPath.at(i - 1).pos.x);
  ...
  if (extractedPath.size() < 2) {
    cout << endl
         << "### Planner Z . Extracted Rollout Path is too Small, Size = "
...
void PlanningHelpers::ExtractPartFromPointToDistanceDirectionFast(
    const vector<WayPoint> &originalPath, const WayPoint &pos,
    const double &minDistance, const double &pathDensity,
    vector<WayPoint> &extractedPath) {
  ...
  for (int i = close_index; i >= 0; i--) {
    ...
    if (i < originalPath.size())
      d += hypot(originalPath.at(i).pos.y - originalPath.at(i + 1).pos.y,
                 originalPath.at(i).pos.x - originalPath.at(i + 1).pos.x);
    if (d > 10)
      break;
  }
...
void PlanningHelpers::ExtractPartFromPointToDistanceFast(
    const vector<WayPoint> &originalPath, const WayPoint &pos,
    const double &minDistance, const double &pathDensity,
    vector<WayPoint> &extractedPath, const double &SmoothDataWeight,
    const double &SmoothWeight, const double &SmoothTolerance) {
...
void PlanningHelpers::CalculateRollInTrajectories(
    const WayPoint &carPos, const double &speed,
    const vector<WayPoint> &originalCenter, int &start_index, int &end_index,
    vector<double> &end_laterals, vector<vector<WayPoint>> &rollInPaths,
    const double &max_roll_distance, const double &maxSpeed,
    const double &carTipMargin, const double &rollInMargin,
    const double &rollInSpeedFactor, const double &pathDensity,
    const double &rollOutDensity, const int &rollOutNumber,
  ...
  for (unsigned int i = close_index; i < originalCenter.size() - 1; i++) {
    if (i > 0)
      remaining_distance +=
          distance2points(originalCenter[i].pos, originalCenter[i + 1].pos);
  }

  double initial_roll_in_distance = info.perp_distance; // GetPerpDistanceToTrajectorySimple(originalCenter,
                                                        // carPos, close_index);
  ...
  //		WayPoint np = GetPerpendicularOnTrajectory_obsolete(originalCenter,
  //carPos, dummyd, closest_for_each_iteration); 		np.pos = carPos.pos;
  //
  ...
  //			np = GetPerpendicularOnTrajectory_obsolete(originalCenter, p,
  //dummyd, closest_for_each_iteration); 			np.pos = p.pos;
  //			RollOutStratPath.push_back(np);
  //		}
  //
  //		initial_roll_in_distance =
  //GetPerpDistanceToTrajectorySimple_obsolete(originalCenter,
  //RollOutStratPath.at(RollOutStratPath.size()-1), close_index);
  //	}
  ///***   -------------------------------- ***///
  ...
  for (unsigned int j = start_index; j < smoothing_start_index; j++) {
    ...
    for (unsigned int i = 0; i < rollOutNumber + 1; i++) {
      p.pos.x = originalCenter.at(j).pos.x -
                initial_roll_in_distance * cos(p.pos.a + M_PI_2);
      p.pos.y = originalCenter.at(j).pos.y -
                initial_roll_in_distance * sin(p.pos.a + M_PI_2);
  ...
  for (unsigned int j = smoothing_start_index; j < end_index; j++) {
    ...
    for (unsigned int i = 0; i < rollOutNumber + 1; i++) {
      ...
      p.pos.x = originalCenter.at(j).pos.x -
                initial_roll_in_distance * cos(p.pos.a + M_PI_2) -
                d * cos(p.pos.a + M_PI_2);
      p.pos.y = originalCenter.at(j).pos.y -
                initial_roll_in_distance * sin(p.pos.a + M_PI_2) -
                d * sin(p.pos.a + M_PI_2);
      if (i != centralTrajectoryIndex)
        p.v = original_speed * LANE_CHANGE_SPEED_FACTOR;
      else
        p.v = original_speed;
  ...
  for (unsigned int i = 0; i < rollOutNumber + 1; i++)
    rollInPaths.at(i).insert(rollInPaths.at(i).begin(),
                             execluded_from_smoothing.at(i).begin(),
                             execluded_from_smoothing.at(i).end());
  ...
  //		for(unsigned int i=0; i< rollOutNumber+1 ; i++)
  //		{
  //			unsigned int cut_index =
  //GetClosestNextPointIndex(rollInPaths.at(i),
  //RollOutStratPath.at(RollOutStratPath.size()-1));
  //			rollInPaths.at(i).erase(rollInPaths.at(i).begin(),
  //rollInPaths.at(i).begin()+cut_index);
  //			rollInPaths.at(i).insert(rollInPaths.at(i).begin(),
  //RollOutStratPath.begin(), RollOutStratPath.end());
  //		}
  //	}
  ///***   -------------------------------- ***///
  ...
  for (unsigned int j = smoothing_end_index; j < originalCenter.size(); j++) {
    if (j > 0)
      d_limit += distance2points(originalCenter.at(j).pos,
                                 originalCenter.at(j - 1).pos);
  ...
  for (unsigned int i = 0; i < rollOutNumber + 1; i++) {
    SmoothPath(rollInPaths.at(i), SmoothDataWeight, SmoothWeight,
               SmoothTolerance);
  }
...
std::vector<int>
PlanningHelpers::GetUniqueLeftRightIds(const std::vector<WayPoint> &path) {
...
void PlanningHelpers::SmoothSpeedProfiles(vector<WayPoint> &path_in,
                                          double weight_data,
  ...
  while (change >= tolerance) {
    ...
    for (int i = 1; i < size - 1; i++) {
      ...
      newpath[i].v += weight_smooth * (newpath[i - 1].v + newpath[i + 1].v -
                                       (2.0 * newpath[i].v));
...
void PlanningHelpers::SmoothCurvatureProfiles(vector<WayPoint> &path_in,
                                              double weight_data,
                                              double weight_smooth,
                                              double tolerance) {
  if (path_in.size() <= 1)
  ...
  while (change >= tolerance) {
    ...
    for (int i = 1; i < size - 1; i++) {
      ...
      newpath[i].cost +=
          weight_smooth *
          (newpath[i - 1].cost + newpath[i + 1].cost - (2.0 * newpath[i].cost));
...
void PlanningHelpers::SmoothWayPointsDirections(vector<WayPoint> &path_in,
                                                double weight_data,
                                                double weight_smooth,
                                                double tolerance) {
  ...
  while (change >= tolerance) {
    ...
    for (int i = 1; i < size - 1; i++) {
      ...
      newpath[i].pos.a +=
          weight_smooth * (newpath[i - 1].pos.a + newpath[i + 1].pos.a -
                           (2.0 * newpath[i].pos.a));
...
void PlanningHelpers::GenerateRecommendedSpeed(
    vector<WayPoint> &path, const double &max_speed,
    const double &speedProfileFactor) {
  CalcAngleAndCostAndCurvatureAnd2D(path);
  ...
  for (unsigned int i = 0; i < path.size(); i++) {
    ...
    double local_max = (path.at(i).v >= 0 && max_speed > path.at(i).v)
                           ? path.at(i).v
                           : max_speed;
...
WayPoint *PlanningHelpers::BuildPlanningSearchTreeV2(
    WayPoint *pStart, const WayPoint &goalPos, const vector<int> &globalPath,
    const double &DistanceLimit, const bool &bEnableLaneChange,
  ...
  *wp = *pStart;
  ...
  while (nextLeafToTrace.size() > 0) {
    ...
    double angle_to_goal = UtilityH::AngleBetweenTwoAnglesPositive(
        UtilityH::FixNegativeAngle(pH->pos.a),
        UtilityH::FixNegativeAngle(goalPos.pos.a));
    if (distance_to_goal <= 0.1 && angle_to_goal < M_PI_4) {
      cout << "Goal Found, LaneID: " << pH->laneId
      ...
      if (pH->pLeft && !CheckLaneExits(all_cells_to_delete, pH->pLeft->pLane) &&
          !CheckNodeExits(all_cells_to_delete, pH->pLeft) &&
          bEnableLaneChange &&
          before_change_distance > LANE_CHANGE_MIN_DISTANCE) {
        wp = new WayPoint();
        *wp = *pH->pLeft;
      ...
      if (pH->pRight &&
          !CheckLaneExits(all_cells_to_delete, pH->pRight->pLane) &&
          !CheckNodeExits(all_cells_to_delete, pH->pRight) &&
          bEnableLaneChange &&
          before_change_distance > LANE_CHANGE_MIN_DISTANCE) {
        wp = new WayPoint();
        *wp = *pH->pRight;
      ...
      for (unsigned int i = 0; i < pH->pFronts.size(); i++) {
        if (CheckLaneIdExits(globalPath, pH->pLane) && pH->pFronts.at(i) &&
            !CheckNodeExits(all_cells_to_delete, pH->pFronts.at(i))) {
          wp = new WayPoint();
          *wp = *pH->pFronts.at(i);
    ...
    if (distance > DistanceLimit && globalPath.size() == 0) {
      // if(!pGoalCell)
      cout << "Goal Not Found, LaneID: " << pH->laneId
...
WayPoint *PlanningHelpers::BuildPlanningSearchTreeStraight(
    WayPoint *pStart, const double &DistanceLimit,
    vector<WayPoint *> &all_cells_to_delete) {
  if (!pStart)
    return NULL;
  ...
  *wp = *pStart;
  ...
  while (nextLeafToTrace.size() > 0) {
    ...
    for (unsigned int i = 0; i < pH->pFronts.size(); i++) {
      if (pH->pFronts.at(i) &&
        ...
        *wp = *pH->pFronts.at(i);
        ...
        //wp->actionCost.size(); a++)
        //				{
        //					//if(wp->actionCost.at(a).first ==
        //FORWARD_ACTION) 						d += wp->actionCost.at(a).second;
        //				}
    ...
    //		if(pH->pLeft && !CheckLaneExits(all_cells_to_delete,
    //pH->pLeft->pLane))
    //		{
    //			wp = new WayPoint();
    //			*wp = *pH->pLeft;
    //			double d = hypot(wp->pos.y - pH->pos.y, wp->pos.x -
    //pH->pos.x);
    //
    //			for(unsigned int a = 0; a < wp->actionCost.size(); a++)
    //			{
    //				//if(wp->actionCost.at(a).first ==
    //LEFT_TURN_ACTION) 					d += wp->actionCost.at(a).second;
    //			}
    //
    ...
    //		}
    //
    //		if(pH->pRight && !CheckLaneExits(all_cells_to_delete,
    //pH->pRight->pLane))
    //		{
    //			wp = new WayPoint();
    //			*wp = *pH->pRight;
    ...
    //			{
    //				//if(wp->actionCost.at(a).first ==
    //RIGHT_TURN_ACTION) 					d += wp->actionCost.at(a).second;
    //			}
    //
    //			wp->cost = pH->cost + d + LANE_CHANGE_COST;
    //			wp->pLeft = pH;
...
int PlanningHelpers::PredictiveIgnorIdsDP(
    WayPoint *pStart, const double &DistanceLimit,
...
bool PlanningHelpers::CheckLaneIdExits(const std::vector<int> &lanes,
                                       const Lane *pL) {
  if (lanes.size() == 0)
    return true;
...
WayPoint *PlanningHelpers::CheckLaneExits(const vector<WayPoint *> &nodes,
                                          const Lane *pL) {
  if (nodes.size() == 0)
    return nullptr;
...
WayPoint *PlanningHelpers::CheckNodeExits(const vector<WayPoint *> &nodes,
                                          const WayPoint *pL) {
  if (nodes.size() == 0)
    return nullptr;
...
WayPoint *PlanningHelpers::CreateLaneHeadCell(Lane *pLane, WayPoint *pLeft,
                                              WayPoint *pRight,
                                              WayPoint *pBack) {
  if (!pLane)
    return nullptr;
...
double PlanningHelpers::GetLanePoints(Lane *l,
                                      ...
                                      const double &prevCost,
                                      vector<WayPoint> &points) {
  if (l == NULL || minDistance <= 0)
    return 0;
...
WayPoint *PlanningHelpers::GetMinCostCell(const vector<WayPoint *> &cells,
                                          const vector<int> &globalPathIds) {
  if (cells.size() == 1) {
    //		for(unsigned int j = 0; j < cells.at(0)->actionCost.size(); j++)
    //			cout << "Cost (" << cells.at(0)->laneId << ") of going : " <<
    //cells.at(0)->actionCost.at(j).first << ", is : " <<
    //cells.at(0)->actionCost.at(j).second << endl;
    return cells.at(0);
  }
...
void PlanningHelpers::ExtractPlanAlernatives(
    const std::vector<WayPoint> &singlePath,
    std::vector<std::vector<WayPoint>> &allPaths) {
  ...
  for (unsigned int i = 1; i < singlePath.size(); i++) {
    if (singlePath.at(i).bDir != FORWARD_DIR && singlePath.at(i).pLane &&
        singlePath.at(i).pFronts.size() > 0) {
      ...
      PlanningHelpers::GetRelativeInfo(start_point.pLane->points, start_point,
                                       start_info);
      vector<WayPoint *> local_cell_to_delete;
      PlannerHNS::WayPoint *pStart =
          &start_point.pLane->points.at(start_info.iFront);
      WayPoint *pLaneCell = PlanningHelpers::BuildPlanningSearchTreeStraight(
          pStart, BACKUP_STRAIGHT_PLAN_DISTANCE, local_cell_to_delete);
      if (pLaneCell) {
        vector<WayPoint> straight_path;
        vector<vector<WayPoint>> tempCurrentForwardPathss;
        vector<int> globalPathIds;
        PlanningHelpers::TraversePathTreeBackwards(pLaneCell, pStart,
                                                   globalPathIds, straight_path,
                                                   tempCurrentForwardPathss);
    ...
    if (bStartSkip) {
      d += hypot(singlePath.at(i).pos.y - singlePath.at(i - 1).pos.y,
                 singlePath.at(i).pos.x - singlePath.at(i - 1).pos.x);
      if (d > skip_distance) {
        d = 0;
        bStartSkip = false;
      }
...
void PlanningHelpers::TraversePathTreeBackwards(
    WayPoint *pHead, WayPoint *pStartWP, const vector<int> &globalPathIds,
    vector<WayPoint> &localPath,
    std::vector<std::vector<WayPoint>> &localPaths) {
  if (pHead != NULL && pHead->id != pStartWP->id) {
    if (pHead->pBacks.size() > 0) {
      localPaths.push_back(localPath);
      TraversePathTreeBackwards(GetMinCostCell(pHead->pBacks, globalPathIds),
                                pStartWP, globalPathIds, localPath, localPaths);
      ...
      TraversePathTreeBackwards(pHead->pLeft, pStartWP, globalPathIds,
      ...
      TraversePathTreeBackwards(pHead->pRight, pStartWP, globalPathIds,
                                localPath, localPaths);
...
ACTION_TYPE PlanningHelpers::GetBranchingDirection(WayPoint &currWP,
                                                   WayPoint &nextWP) {
  ...
  //	angle = 0;
  //
  //	for(unsigned int i=0; i < nextWP.pLane->points.size() && counter < 10;
  //i++, counter++)
  //	{
  ...
  //	{
  //		t = RIGHT_TURN_ACTION;
  //		cout << "Right Lane, Average Angle = " << angle*RAD2DEG << ", Circle
  //Angle = " << circle_angle*RAD2DEG << ", currAngle = " <<
  //currWP.pos.a*RAD2DEG << endl;
  //	}
  //	else if( currWP.pos.a - circle_angle < (-7.5*DEG2RAD))
  //	{
  //		t = LEFT_TURN_ACTION;
  //		cout << "Left Lane, Average Angle = " << angle*RAD2DEG << ", Circle
  //Angle = " << circle_angle*RAD2DEG << ", currAngle = " <<
  //currWP.pos.a*RAD2DEG << endl;
  //	}
...
void PlanningHelpers::CalcContourPointsForDetectedObjects(
    const WayPoint &currPose, vector<DetectedObject> &obj_list,
    const double &filterDistance) {
...
double
PlanningHelpers::GetVelocityAhead(const std::vector<WayPoint> &path,
                                  const RelativeInfo &info, int &prev_index,
                                  const double &reasonable_brake_distance) {
  if (path.size() == 0)
    return 0;
...
void PlanningHelpers::WritePathToFile(const string &fileName,
                                      const vector<WayPoint> &path) {
  DataRW dataFile;
  ...
  for (unsigned int i = 0; i < path.size(); i++) {
    ostringstream strwp;
    strwp << path.at(i).laneId << "," << path.at(i).id << ","
          << path.at(i).pos.x << "," << path.at(i).pos.y << ","
...
LIGHT_INDICATOR
PlanningHelpers::GetIndicatorsFromPath(const std::vector<WayPoint> &path,
                                       const WayPoint &pose,
                                       const double &seachDistance) {
  if (path.size() < 2)
  ...
  for (unsigned int i = info.iFront; i < path.size() - 2; i++) {
    ...
    total_d += hypot(path.at(i + 1).pos.y - path.at(i).pos.y,
                     path.at(i + 1).pos.x - path.at(i).pos.x);
...
PlannerHNS::WayPoint
PlanningHelpers::GetRealCenter(const PlannerHNS::WayPoint &currState,
