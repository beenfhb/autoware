...
public:
  ...
  MapItem(int ID, GPSPoint start, GPSPoint end, double curvature, double width,
          double length, std::string objName) {
    id = ID;
    sp = start;
    ep = end;
    c = curvature;
...
public:
  ...
  Obstacle(int ID, GPSPoint start, GPSPoint end, double curvature, double width,
           double length, OBSTACLE_TYPE type, std::string fileName)
...
public:
  ...
  DrivableArea(int ID, GPSPoint start, GPSPoint end, double curvature,
               double width, double length, DRIVABLE_TYPE type,
               std::string fileName)
      : MapItem(ID, start, end, curvature, width, length, fileName) {
    t = type;
  }
...
public:
  ...
  double minDistanceToAvoid; // should be smaller than minFollowingDistance and
                             // larger than maxDistanceToAvoid
...
public:
  ...
  int bOutsideControl; // 0 waiting, 1 start, 2 Green Traffic Light, 3 Red
                       // Traffic Light, 5 Emergency Stop
  ...
  std::string ToStringHeader() {
    return "Time:General>>:currentVelocity:distanceToStop:minStoppingDistance:"
           "bStartBehaviorGenerator:bGoalReached:"
...
public:
  ...
  std::string ToString() {
    ...
    for (unsigned int i = 0; i < lateral_costs.size(); i++) {
      str << " - (" << lateral_costs.at(i).first << ", "
          << lateral_costs.at(i).second << ")";
    }
...
public:
  ...
  OccupancyToGridMap(const int &_width, const int &_length, const double &_res,
                     const WayPoint &_center) {
    width = _width;
    length = _length;
    res = _res;
    center = _center;
  ...
  bool GetCellIndexFromPoint(const GPSPoint &p, const std::vector<int> &data,
                             int &_cell) {
