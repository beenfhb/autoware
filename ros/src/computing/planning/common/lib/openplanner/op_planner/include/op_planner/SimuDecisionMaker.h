...
public:
  ...
  void UpdateState(const PlannerHNS::VehicleState &state,
                   const bool &bUseDelay = false);
  void GenerateLocalRollOuts();

  bool SelectSafeTrajectory();
  PlannerHNS::VehicleState
  LocalizeStep(const double &dt, const PlannerHNS::VehicleState &desiredStatus);

  PlannerHNS::BehaviorState
  DoOneStep(const double &dt, const PlannerHNS::VehicleState &vehicleState,
