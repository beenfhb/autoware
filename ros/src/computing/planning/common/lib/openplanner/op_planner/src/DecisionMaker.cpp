...
/// \brief Initialize behaviors state machine, and calculate required parameters
/// for the state machine transition conditions \author Hatem Darweesh \date Dec
/// 14, 2016
...
void DecisionMaker::Init(const ControllerParams &ctrlParams,
...
void DecisionMaker::InitBehaviorStates() {
  ...
  m_pMissionCompleteState = new MissionAccomplishedStateII(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), 0);
  m_pGoalState =
      new GoalStateII(m_pStopState->m_pParams, m_pStopState->GetCalcParams(),
                      m_pMissionCompleteState);
  m_pGoToGoalState = new ForwardStateII(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoalState);
  m_pInitState = new InitStateII(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoToGoalState);

  m_pFollowState = new FollowStateII(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoToGoalState);
  m_pAvoidObstacleState = new SwerveStateII(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoToGoalState);
  m_pStopSignWaitState = new StopSignWaitStateII(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoToGoalState);
  m_pStopSignStopState = new StopSignStopStateII(m_pStopState->m_pParams,
                                                 m_pStopState->GetCalcParams(),
                                                 m_pStopSignWaitState);

  m_pTrafficLightWaitState = new TrafficLightWaitStateII(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoToGoalState);
  m_pTrafficLightStopState = new TrafficLightStopStateII(
      m_pStopState->m_pParams, m_pStopState->GetCalcParams(), m_pGoToGoalState);
...
bool DecisionMaker::GetNextTrafficLight(
  ...
  for (unsigned int i = 0; i < trafficLights.size(); i++) {
    ...
    if (d <= trafficLights.at(i).stoppingDistance) {
      double a_diff = UtilityHNS::UtilityH::AngleBetweenTwoAnglesPositive(
          UtilityHNS::UtilityH::FixNegativeAngle(trafficLights.at(i).pos.a),
...
void DecisionMaker::CalculateImportantParameterForDecisionMaking(
  ...
  if (bestTrajectory.index >= 0 &&
      bestTrajectory.index < (int)m_RollOuts.size())
  ...
  if (ReachEndOfGlobalPath(pValues->minStoppingDistance +
                               critical_long_front_distance,
                           pValues->iCurrSafeLane))
  ...
  distanceToClosestStopLine =
      PlanningHelpers::GetDistanceToClosestStopLineAndCheck(
          m_TotalPath.at(pValues->iCurrSafeLane), state,
          m_params.giveUpDistance, stopLineID, stopSignID, trafficLightID) -
      critical_long_front_distance;

  // std::cout << "StopLineID" << stopLineID << ", StopSignID: " << stopSignID
  // << ", TrafficLightID: " << trafficLightID << ", Distance: " <<
  // distanceToClosestStopLine << ", MinStopDistance: " <<
  // pValues->minStoppingDistance << std::endl;

  if (distanceToClosestStopLine > m_params.giveUpDistance &&
      distanceToClosestStopLine < (pValues->minStoppingDistance + 1.0)) {
  ...
  // std::cout << "Distance To Closest: " << distanceToClosestStopLine << ",
  // Stop LineID: " << stopLineID << ", Stop SignID: " << stopSignID << ", TFID:
  // " << trafficLightID << std::endl;
...
void DecisionMaker::UpdateCurrentLane(const double &search_distance) {
  ...
  pPathLane = MappingHelpers::GetLaneFromPath(
      state, m_TotalPath.at(m_iCurrentTotalPathId));
  if (!pPathLane) {
    std::cout << "Performance Alert: Can't Find Lane Information in Global "
                 "Path, Searching the Map :( "
              << std::endl;
    pMapLane =
        MappingHelpers::GetClosestLaneFromMap(state, m_Map, search_distance);
  }
...
bool DecisionMaker::ReachEndOfGlobalPath(const double &min_distance,
...
void DecisionMaker::SetNewGlobalPath(
    const std::vector<std::vector<WayPoint>> &globalPath) {
  if (m_pCurrentBehaviorState) {
...
bool DecisionMaker::SelectSafeTrajectory() {
  ...
  int currIndex =
      PlannerHNS::PlanningHelpers::GetClosestNextPointIndexFast(m_Path, state);
  int index_limit = 0;
  if (index_limit <= 0)
    index_limit = m_Path.size() / 2.0;
  if (currIndex > index_limit || preCalcPrams->bRePlan ||
      preCalcPrams->bNewGlobalPath) {
    std::cout << "New Local Plan !! " << currIndex << ", "
              << preCalcPrams->bRePlan << ", " << preCalcPrams->bNewGlobalPath
              << ", " << m_TotalOriginalPath.at(0).size()
              << ", PrevLocal: " << m_Path.size();
...
PlannerHNS::BehaviorState DecisionMaker::GenerateBehaviorState(
  ...
  if (preCalcPrams->iPrevSafeTrajectory < 0 ||
      preCalcPrams->iPrevSafeTrajectory >= m_RollOuts.size())
...
double DecisionMaker::UpdateVelocityDirectlyToTrajectory(
  ...
  double max_velocity = PlannerHNS::PlanningHelpers::GetVelocityAhead(
      ...
      total_info.iBack, average_braking_distance);

  unsigned int point_index = 0;
  double critical_long_front_distance = m_CarInfo.length / 2.0;

  if (beh.state == TRAFFIC_LIGHT_STOP_STATE ||
    ...
    //		std::cout << "Stopping : e=" << e << ", desiredPID=" <<
    //desiredVelocity << ", PID: " << m_pidStopping.ToString() << std::endl;
    ...
    if ((desiredVelocity < 0.1 && desiredVelocity > -0.1) ||
        beh.followDistance <= 0) // use only effective velocities
      desiredVelocity = 0;

    // std::cout << "Acc: V: " << desiredVelocity << ", Accel: " <<
    // deceleration_critical<< std::endl;
    ...
    // std::cout << "Target Velocity: " << desiredVelocity << ", Change
    // Slowdown: " << bSlowBecauseChange  << std::endl;
...
DecisionMaker::DoOneStep(const double &dt, const PlannerHNS::WayPoint currPose,
                         const PlannerHNS::VehicleState &vehicleState,
                         const int &goalID,
  ...
  for (unsigned int i = 0; i < m_TotalOriginalPath.size(); i++) {
    ...
    PlannerHNS::PlanningHelpers::ExtractPartFromPointToDistanceDirectionFast(
        m_TotalOriginalPath.at(i), state, m_params.horizonDistance,
  ...
  // std::cout << "Eval_i: " << tc.index << ", Curr_i: " <<
  // m_pCurrentBehaviorState->GetCalcParams()->iCurrSafeTrajectory << ", Prev_i:
  // " << m_pCurrentBehaviorState->GetCalcParams()->iPrevSafeTrajectory <<
  // std::endl;
