...
public:
  ...
  TrajectoryTracker(std::vector<PlannerHNS::WayPoint> &path,
                    const unsigned int &_index) {

    if (path.size() > 0) {
      beh = path.at(0).beh_state;
      // std::cout << "New Path: Beh: " << beh << ", index: " << _index << ",
      // LaneID_0: " << path.at(0).laneId << ", LaneID_1: "<< path.at(1).laneId
      // << std::endl;
    }
  ...
  void UpdatePathAndIndex(std::vector<PlannerHNS::WayPoint> &_path,
                          const unsigned int &_index) {
    if (_path.size() == 0)
      return;
  ...
  double
  CalcMatchingPercentage(const std::vector<PlannerHNS::WayPoint> &_path) {
    if (_path.size() == 0)
      return 0;

    if (beh != _path.at(0).beh_state)
      return 0;
    ...
    for (unsigned int i = 0; i < _path.size(); i++) {
      if (i < trajectory.size()) {
        rms_val += hypot(_path.at(i).pos.y - trajectory.at(i).pos.y,
                         _path.at(i).pos.y - trajectory.at(i).pos.y);
      }
    }
    ...
    if (_ids.size() == ids.size() && ids.size() == nEqualities &&
        rms_error < 5.0) // perfect match
      return 1;

    WayPoint curr_last_pose = _path.at(_path.size() - 1);
    double nMatch = (double)nIds / (double)nCount;
    double _d = hypot(path_last_pose.pos.y - curr_last_pose.pos.y,
                      path_last_pose.pos.x - curr_last_pose.pos.x);
    ...
    double _a_diff = UtilityHNS::UtilityH::AngleBetweenTwoAnglesPositive(
        path_last_pose.pos.a, curr_last_pose.pos.a);
  ...
  void DeleteParticle(const Particle &p, const int &_i) {
    if (p.beh == PlannerHNS::BEH_STOPPING_STATE &&
        nAliveStop > BEH_MIN_PARTICLE_NUM) {
      m_StopPart.erase(m_StopPart.begin() + _i);
      nAliveStop--;
    } else if (p.beh == PlannerHNS::BEH_YIELDING_STATE &&
               nAliveYield > BEH_MIN_PARTICLE_NUM) {
      m_YieldPart.erase(m_YieldPart.begin() + _i);
      nAliveYield--;
    } else if (p.beh == PlannerHNS::BEH_FORWARD_STATE &&
               nAliveForward > BEH_MIN_PARTICLE_NUM) {
      m_ForwardPart.erase(m_ForwardPart.begin() + _i);
      nAliveForward--;
    } else if (p.beh == PlannerHNS::BEH_BRANCH_LEFT_STATE &&
               nAliveLeft > BEH_MIN_PARTICLE_NUM) {
      m_LeftPart.erase(m_LeftPart.begin() + _i);
      nAliveLeft--;
    } else if (p.beh == PlannerHNS::BEH_BRANCH_RIGHT_STATE &&
               nAliveRight > BEH_MIN_PARTICLE_NUM) {
...
public:
  ...
  //		{
  //			if(m_TrajectoryTracker.at(i)->best_p >
  //best_beh_track->best_p)
  //			{
  ...
  void DeleteFromList(std::vector<TrajectoryTracker *> &delete_me_track,
                      const TrajectoryTracker *track) {
  ...
  void MatchWithMax(std::vector<LLP> &matching_list,
                    std::vector<TrajectoryTracker *> &delete_me_track,
                    std::vector<TrajectoryTracker *> &check_list) {
    ...
    while (matching_list.size() > 0) {
      ...
      f.pTrack->UpdatePathAndIndex(obj.predTrajectories.at(f.new_index),
                                   f.new_index);
      ...
      for (int i = 0; i < matching_list.size(); i++) {
        if (matching_list.at(i).new_index == f.new_index ||
            matching_list.at(i).pTrack == f.pTrack) {
  ...
  void MatchTrajectories() {
    ...
    for (unsigned int t = 0; t < obj.predTrajectories.size(); t++) {
      ...
      for (int i = 0; i < m_TrajectoryTracker.size(); i++) {
        ...
        double vMatch =
            pTracker->CalcMatchingPercentage(obj.predTrajectories.at(t));
        if (vMatch == 1.0) // perfect match
        {
          ...
          i--;
          break;
        } else if (vMatch > 0.5) // any matching less than 50%, the trajectory
      ...
      if (!bMatched) {
        m_TrajectoryTracker_temp.push_back(
            new TrajectoryTracker(obj.predTrajectories.at(t), t));
      }
    }
...
public:
  ...
  void DoOneStep(const std::vector<DetectedObject> &obj_list,
                 const WayPoint &currPose, const double &minSpeed,
                 const double &maxDeceleration, RoadNetwork &map);
...
protected:
  // int GetTrajectoryPredictedDirection(const std::vector<WayPoint>& path,
  // const PlannerHNS::WayPoint& pose, const double& pred_distance);
  ...
  double CalcIndicatorWeight(PlannerHNS::LIGHT_INDICATOR p_ind,
  ...
  void FilterObservations(const std::vector<DetectedObject> &obj_list,
                          RoadNetwork &map,
                          std::vector<DetectedObject> &filtered_list);
  void ExtractTrajectoriesFromMap(const std::vector<DetectedObject> &obj_list,
                                  RoadNetwork &map,
                                  std::vector<ObjParticles *> &old_list);
  ...
  static bool sort_trajectories(const std::pair<int, double> &p1,
...
public:
  // move to CPP later
  void DeleteFromList(std::vector<ObjParticles *> &delete_me,
                      const ObjParticles *pElement) {
