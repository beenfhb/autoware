...
void SimuDecisionMaker::SetSimulatedTargetOdometryReadings(
    const double &velocity_d, const double &steering_d,
    const SHIFT_POS &shift_d) {
...
void SimuDecisionMaker::LocalizeMe(const double &dt) {
  ...
  m_OdometryState.pos.a =
      UtilityHNS::UtilityH::FixNegativeAngle(m_OdometryState.pos.a);
...
void SimuDecisionMaker::UpdateState(const PlannerHNS::VehicleState &state,
  ...
  if (!bUseDelay) {
    ...
    if (UtilityHNS::UtilityH::GetTimeDiffNow(m_SteerDelayTimer) >
...
void SimuDecisionMaker::GenerateLocalRollOuts() {
  ...
  _planner.GenerateRunoffTrajectory(
      m_TotalPath, state, m_params.enableLaneChange, state.v,
      m_params.microPlanDistance, m_params.maxSpeed, m_params.minSpeed,
      m_params.carTipMargin, m_params.rollInMargin, m_params.rollInSpeedFactor,
      m_params.pathDensity, m_params.rollOutDensity, m_params.rollOutNumber,
      m_params.smoothingDataWeight, m_params.smoothingSmoothWeight,
      m_params.smoothingToleranceError, m_params.speedProfileFactor,
      m_params.enableHeadingSmoothing, -1, -1, _roll_outs, sampledPoints_debug);
  ...
  for (unsigned int i = 0; i < _roll_outs.size(); i++) {
    for (unsigned int j = 0; j < _roll_outs.at(i).size(); j++) {
      PlannerHNS::PlanningHelpers::PredictConstantTimeCostForTrajectory(
          _roll_outs.at(i).at(j), state, m_params.minSpeed,
...
bool SimuDecisionMaker::SelectSafeTrajectory() {
  ...
  if (!preCalcPrams || m_TotalPath.size() == 0)
...
SimuDecisionMaker::LocalizeStep(const double &dt,
                                const PlannerHNS::VehicleState &desiredStatus) {
  SetSimulatedTargetOdometryReadings(desiredStatus.speed, desiredStatus.steer,
                                     desiredStatus.shift);
...
PlannerHNS::BehaviorState SimuDecisionMaker::DoOneStep(
    const double &dt, const PlannerHNS::VehicleState &vehicleState,
    const int &goalID, const std::vector<TrafficLight> &trafficLight,
  ...
  PlannerHNS::TrajectoryCost tc = m_TrajectoryCostsCalculator.DoOneStepStatic(
      m_RollOuts, m_TotalPath.at(m_iCurrentTotalPathId), state, m_params,
      m_CarInfo, vehicleState, objects);

  // std::cout << "Detected Objects Distance: " << tc.closest_obj_distance << ",
  // N RollOuts: " << m_RollOuts.size() << std::endl;
