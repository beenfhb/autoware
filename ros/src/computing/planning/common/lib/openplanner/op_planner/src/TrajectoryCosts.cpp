...
/// evaluation for dp_planner \author Hatem Darweesh \date Dec 14, 2016
...
TrajectoryCost TrajectoryCosts::DoOneStep(
...
void TrajectoryCosts::CalculateLateralAndLongitudinalCosts(
...
void TrajectoryCosts::NormalizeCosts(vector<TrajectoryCost> &trajectoryCosts) {
  ...
  for (unsigned int ic = 0; ic < trajectoryCosts.size(); ic++) {
    ...
    trajectoryCosts.at(ic).lane_change_cost =
        m_WeightLaneChange * trajectoryCosts.at(ic).lane_change_cost;
    trajectoryCosts.at(ic).lateral_cost =
    ...
    trajectoryCosts.at(ic).cost =
        m_WeightPriority * trajectoryCosts.at(ic).priority_cost / 5.0 +
...
vector<TrajectoryCost> TrajectoryCosts::CalculatePriorityAndLaneChangeCosts(
...
void TrajectoryCosts::CalculateTransitionCosts(
 ...
 */
bool TrajectoryCosts::ValidateRollOutsInput(
    const vector<vector<vector<WayPoint>>> &rollOuts) {
  if (rollOuts.size() == 0)
    return false;
