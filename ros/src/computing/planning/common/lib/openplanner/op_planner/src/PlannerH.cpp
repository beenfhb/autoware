...
void PlannerH::GenerateRunoffTrajectory(
    const std::vector<std::vector<WayPoint>> &referencePaths,
    const WayPoint &carPos, const bool &bEnableLaneChange, const double &speed,
    const double &microPlanDistance, const double &maxSpeed,
    const double &minSpeed, const double &carTipMargin,
    const double &rollInMargin, const double &rollInSpeedFactor,
    const double &pathDensity, const double &rollOutDensity,
    const int &rollOutNumber, const double &SmoothDataWeight,
    const double &SmoothWeight, const double &SmoothTolerance,
    const double &speedProfileFactor, const bool &bHeadingSmooth,
  ...
  for (unsigned int i = 0; i < referencePaths.size(); i++) {
    ...
    if (referencePaths.at(i).size() > 0) {
      PlanningHelpers::CalculateRollInTrajectories(
          carPos, speed, referencePaths.at(i), s_index, e_index, e_distances,
          local_rollOutPaths, microPlanDistance, maxSpeed, carTipMargin,
          rollInMargin, rollInSpeedFactor, pathDensity, rollOutDensity,
          rollOutNumber, SmoothDataWeight, SmoothWeight, SmoothTolerance,
          bHeadingSmooth, sampledPoints_debug);
    } else {
...
double PlannerH::PlanUsingDPRandom(const WayPoint &start,
  ...
  PlannerHNS::WayPoint *pStart =
      PlannerHNS::MappingHelpers::GetClosestBackWaypointFromMap(start, map);

  if (!pStart) {
    GPSPoint sp = start.pos;
    cout << endl
         << "Error: PlannerH -> Can't Find Global Waypoint Nodes in the Map "
            "for Start ("
         << sp.ToString() << ")" << endl;
    return 0;
  }
  ...
  if (!pStart->pLane) {
    cout << endl
         << "Error: PlannerH -> Null Lane, Start (" << pStart->pLane << ")"
         << endl;
    return 0;
  }
  ...
  if (start_info.perp_distance > START_POINT_MAX_DISTANCE) {
    GPSPoint sp = start.pos;
    cout << endl
         << "Error: PlannerH -> Start Distance to Lane is: "
         << start_info.perp_distance << ", Pose: " << sp.ToString()
         << ", LanePose:" << start_info.perp_point.pos.ToString()
         << ", LaneID: " << pStart->pLane->id
         << " -> Check origin and vector map. " << endl;
    return 0;
  }

  vector<WayPoint *> local_cell_to_delete;
  WayPoint *pLaneCell = 0;
  pLaneCell = PlanningHelpers::BuildPlanningSearchTreeStraight(
      pStart, maxPlanningDistance, local_cell_to_delete);

  if (!pLaneCell) {
    cout << endl
         << "PlannerH -> Plan (B) Failed, Sorry we Don't have plan (C) This is "
            "the END."
         << endl;
    return 0;
  }
  ...
  PlanningHelpers::TraversePathTreeBackwards(pLaneCell, pStart, globalPath,
                                             path, tempCurrentForwardPathss);
  cout << endl
       << "Info: PlannerH -> Plan (B) Path With Size (" << (int)path.size()
       << "), MultiPaths No(" << paths.size() << ") Extraction Time : " << endl;
...
double PlannerH::PlanUsingDP(const WayPoint &start, const WayPoint &goalPos,
                             const double &maxPlanningDistance,
  ...
  PlannerHNS::WayPoint *pStart =
      PlannerHNS::MappingHelpers::GetClosestWaypointFromMap(start, map);
  PlannerHNS::WayPoint *pGoal =
      PlannerHNS::MappingHelpers::GetClosestWaypointFromMap(goalPos, map);
  ...
  if (!pStart || !pGoal) {
    ...
    cout << endl
         << "Error: PlannerH -> Can't Find Global Waypoint Nodes in the Map "
            "for Start ("
         << sp.ToString() << ") and Goal (" << gp.ToString() << ")" << endl;
    return 0;
  }
  ...
  if (fabs(goal_info.perp_distance) > GOAL_POINT_MAX_DISTANCE) {
    if (fabs(start_info.perp_distance) > 20) {
      GPSPoint gp = goalPos.pos;
      cout << endl
           << "Error: PlannerH -> Goal Distance to Lane is: "
           << goal_info.perp_distance << ", Pose: " << gp.ToString()
           << ", LanePose:" << goal_info.perp_point.pos.ToString()
           << ", LaneID: " << pGoal->pLane->id
           << " -> Check origin and vector map. " << endl;
      return 0;
    } else {
      WayPoint wp = *pGoal;
  ...
  if (all_cell_to_delete)
    pLaneCell = PlanningHelpers::BuildPlanningSearchTreeV2(
        pStart, *pGoal, globalPath, maxPlanningDistance, bEnableLaneChange,
        *all_cell_to_delete);
  else
    pLaneCell = PlanningHelpers::BuildPlanningSearchTreeV2(
        pStart, *pGoal, globalPath, maxPlanningDistance, bEnableLaneChange,
        local_cell_to_delete);
  ...
  if (!pLaneCell) {
    ...
    if (all_cell_to_delete)
      pLaneCell = PlanningHelpers::BuildPlanningSearchTreeStraight(
          pStart, BACKUP_STRAIGHT_PLAN_DISTANCE, *all_cell_to_delete);
    else
      pLaneCell = PlanningHelpers::BuildPlanningSearchTreeStraight(
    ...
    if (!pLaneCell) {
      bPlan = 'Z';
      cout << endl
           << "PlannerH -> Plan (B) Failed, Sorry we Don't have plan (C) This "
              "is the END."
           << endl;
      return 0;
    }
  ...
  // attach start path to beginning of all paths, but goal path to only the path
  // connected to the goal path.
  for (unsigned int i = 0; i < paths.size(); i++) {
    paths.at(i).insert(paths.at(i).begin(), start_path.begin(),
                       start_path.end());
    if (paths.at(i).size() > 0) {
      // if(hypot(paths.at(i).at(paths.at(i).size()-1).pos.y-goal_info.perp_point.pos.y,
      // paths.at(i).at(paths.at(i).size()-1).pos.x-goal_info.perp_point.pos.x)
      // < 1.5)
      {

        if (paths.at(i).size() > 0 && goal_path.size() > 0) {
          goal_path.insert(goal_path.begin(), paths.at(i).end() - 5,
                           paths.at(i).end());
          ...
          paths.at(i).insert(paths.at(i).end(), goal_path.begin(),
                             goal_path.end());
        }
      }
    }
  ...
  cout << endl
       << "Info: PlannerH -> Plan (" << bPlan << ") Path With Size ("
       << (int)path.size() << "), MultiPaths No(" << paths.size()
       << ") Extraction Time : " << endl;
...
double PlannerH::PredictPlanUsingDP(PlannerHNS::Lane *l, const WayPoint &start,
                                    const double &maxPlanningDistance,
                                    std::vector<std::vector<WayPoint>> &paths) {
  if (!l) {
  ...
  if (distance2points(closest_p.pos, carPos.pos) > 8) {
    cout << endl
         << "Err: PredictiveDP PlannerH -> Distance to Lane is: "
         << distance2points(closest_p.pos, carPos.pos)
         << ", Pose: " << carPos.pos.ToString()
         << ", LanePose:" << closest_p.pos.ToString() << ", LaneID: " << l->id
         << " -> Check origin and vector map. " << endl;
    return 0;
  }

  vector<WayPoint *> pLaneCells;
  int nPaths = PlanningHelpers::PredictiveDP(pStartWP, maxPlanningDistance,
                                             all_cell_to_delete, pLaneCells);
  ...
  for (unsigned int i = 0; i < pLaneCells.size(); i++) {
    ...
    PlanningHelpers::TraversePathTreeBackwards(
        pLaneCells.at(i), pStartWP, globalPath, path, tempCurrentForwardPathss);
    if (path.size() > 0)
...
double PlannerH::PredictTrajectoriesUsingDP(
    const WayPoint &startPose, std::vector<WayPoint *> closestWPs,
    const double &maxPlanningDistance,
    std::vector<std::vector<WayPoint>> &paths, const bool &bFindBranches,
    const bool bDirectionBased, const bool pathDensity) {
  vector<vector<WayPoint>> tempCurrentForwardPathss;
  vector<WayPoint *> all_cell_to_delete;
  vector<int> globalPath;
  ...
  for (unsigned int j = 0; j < closestWPs.size(); j++) {
    pLaneCells.clear();
    int nPaths = PlanningHelpers::PredictiveIgnorIdsDP(
        closestWPs.at(j), maxPlanningDistance, all_cell_to_delete, pLaneCells,
        unique_lanes);
    for (unsigned int i = 0; i < pLaneCells.size(); i++) {
      path.clear();
      PlanningHelpers::TraversePathTreeBackwards(
          pLaneCells.at(i), closestWPs.at(j), globalPath, path,
      ...
      if (path.size() > 0) {
        ...
        path.at(0).beh_state = path.at(1).beh_state =
  ...
  if (bDirectionBased && bFindBranches) {
    ...
    PlanningHelpers::CreateManualBranchFromTwoPoints(
        p1, p2, branch_length, FORWARD_RIGHT_DIR, r_branch);
    PlanningHelpers::CreateManualBranchFromTwoPoints(
        p1, p2, branch_length, FORWARD_LEFT_DIR, l_branch);
...
double PlannerH::PredictPlanUsingDP(const WayPoint &startPose,
                                    WayPoint *closestWP,
                                    const double &maxPlanningDistance,
                                    std::vector<std::vector<WayPoint>> &paths,
                                    const bool &bFindBranches) {
  ...
  vector<WayPoint *> pLaneCells;
  int nPaths = PlanningHelpers::PredictiveDP(closestWP, maxPlanningDistance,
                                             all_cell_to_delete, pLaneCells);
  ...
  for (unsigned int i = 0; i < pLaneCells.size(); i++) {
    std::vector<WayPoint> path;
    PlanningHelpers::TraversePathTreeBackwards(pLaneCells.at(i), closestWP,
                                               globalPath, path,
                                               tempCurrentForwardPathss);
    if (path.size() > 0) {
      totalPlanDistance += path.at(path.size() - 1).cost;
