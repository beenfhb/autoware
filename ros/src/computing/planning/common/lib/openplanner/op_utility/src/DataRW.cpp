...
/// \file DataRW.cpp
/// \brief File operations for loading vector map files, loading kml map files
/// and writing log .csv files \author Hatem Darweesh \date Jun 23, 2016
...
void DataRW::WriteLogData(const std::string &logFolder,
...
void DataRW::WriteKMLFile(const string &fileName,
                          const vector<string> &gps_list) {
  TiXmlDocument kmldoc(UtilityH::GetHomeDirectory() +
                       DataRW::KmlMapsFolderName + "KmlTemplate.kml");
  ...
  // pV =
  // hKmlFile.FirstChild("Folder").FirstChild("Folder").FirstChild("Document").FirstChild("Placemark").FirstChild("LineString").FirstChild("coordinates").Element();
  pV = hKmlFile.FirstChild("Folder")
           .FirstChild("Document")
           .FirstChild("Placemark")
           .FirstChild("LineString")
           .FirstChild("coordinates")
           .Element();
  if (!pV)
    pV = hKmlFile.FirstChild("Placemark")
             .FirstChild("LineString")
             .FirstChild("coordinates")
             .Element();
...
void DataRW::WriteKMLFile(const string &fileName,
                          const vector<vector<string>> &gps_list) {
  TiXmlDocument kmldoc(UtilityH::GetHomeDirectory() +
                       DataRW::KmlMapsFolderName + "KmlTemplate.kml");
  ...
  pPlaceMarkNode = hKmlFile.FirstChild("Folder")
                       .FirstChild("Document")
                       .FirstChild("Placemark")
                       .Node();
  ...
  //	  	pV =
  //hKmlFile.FirstChild("Folder").FirstChild("Document").FirstChild("Placemark").FirstChild("LineString").FirstChild("coordinates").Element();
  //	  	if(!pV)
  //	  		pV = hKmlFile.FirstChild( "Placemark"
  //).FirstChild("LineString").FirstChild("coordinates").Element();
  ...
  if (pDocument) {
    for (unsigned int l = 0; l < gps_list.size(); l++) {
      ...
      TiXmlElement *pElement =
          pV->FirstChild("LineString")->FirstChild("coordinates")->ToElement();
...
SimpleReaderBase::SimpleReaderBase(const string &fileName, const int &nHeaders,
                                   ...
                                   const int &iDataTitles,
                                   const int &nVariablesForOneObject,
                                   const int &nLineHeaders,
                                   const string &headerRepeatKey) {
...
int LocalizationPathReader::ReadAllData(
    vector<LocalizationWayPoint> &data_list) {
  data_list.clear();
...
void AisanNodesFileReader::ParseNextLine(const vector_map_msgs::Node &_rec,
                                         AisanNode &data) {
  data.NID = _rec.nid;
  data.PID = _rec.pid;
}

AisanNodesFileReader::AisanNode *
AisanNodesFileReader::GetDataRowById(int _nid) {
...
void AisanPointsFileReader::ParseNextLine(const vector_map_msgs::Point &_rec,
                                          AisanPoints &data) {
  data.B = _rec.b;
  data.Bx = _rec.bx;
  data.H = _rec.h;
...
AisanPointsFileReader::AisanPoints *
AisanPointsFileReader::GetDataRowById(int _pid) {
  if (m_data_map.size() == 0)
    return nullptr;
...
void AisanLinesFileReader::ParseNextLine(const vector_map_msgs::Line &_rec,
                                         AisanLine &data) {
  data.BLID = _rec.blid;
  data.BPID = _rec.bpid;
  data.FLID = _rec.flid;
...
AisanLinesFileReader::AisanLine *
AisanLinesFileReader::GetDataRowById(int _lid) {
  if (m_data_map.size() == 0)
    return nullptr;
...
AisanCenterLinesFileReader::AisanCenterLine *
AisanCenterLinesFileReader::GetDataRowById(int _did) {
  if (m_data_map.size() == 0)
    return nullptr;
...
int AisanCenterLinesFileReader::ReadAllData(
    vector<AisanCenterLine> &data_list) {
  data_list.clear();
  AisanCenterLine data;
  // double logTime = 0;
  int count = 0;
...
void AisanLanesFileReader::ParseNextLine(const vector_map_msgs::Lane &_rec,
                                         AisanLane &data) {
  data.BLID = _rec.blid;
...
AisanLanesFileReader::AisanLane *
AisanLanesFileReader::GetDataRowById(int _lnid) {
  if (m_data_map.size() == 0)
    return nullptr;
...
bool AisanLanesFileReader::ReadNextLine(AisanLane &data) {
  ...
  if (ReadSingleLine(lineData)) {
    ...
    //		data.LeftLaneId  = 0;
    //		data.RightLaneId = 0;
    //		data.LeftLaneId 	= strtol(lineData.at(0).at(24).c_str(), NULL,
    //10); 		data.RightLaneId 	= strtol(lineData.at(0).at(25).c_str(), NULL,
    //10);
...
void AisanAreasFileReader::ParseNextLine(const vector_map_msgs::Area &_rec,
                                         AisanArea &data) {
  data.AID = _rec.aid;
  data.ELID = _rec.elid;
  data.SLID = _rec.slid;
...
AisanAreasFileReader::AisanArea *
AisanAreasFileReader::GetDataRowById(int _aid) {
  if (m_data_map.size() == 0)
    return nullptr;
...
AisanIntersectionFileReader::AisanIntersection *
AisanIntersectionFileReader::GetDataRowById(int _id) {
  if (m_data_map.size() == 0)
    return nullptr;
...
int AisanIntersectionFileReader::ReadAllData(
    vector<AisanIntersection> &data_list) {
  data_list.clear();
  AisanIntersection data;
  // double logTime = 0;
  int count = 0;
...
AisanStopLineFileReader::AisanStopLine *
AisanStopLineFileReader::GetDataRowById(int _id) {
  if (m_data_map.size() == 0)
    return nullptr;
...
AisanRoadSignFileReader::AisanRoadSign *
AisanRoadSignFileReader::GetDataRowById(int _id) {
  if (m_data_map.size() == 0)
    return nullptr;
...
void AisanSignalFileReader::ParseNextLine(const vector_map_msgs::Signal &_rec,
                                          AisanSignal &data) {
  data.ID = _rec.id;
  data.LinkID = _rec.linkid;
  data.PLID = _rec.plid;
...
AisanSignalFileReader::AisanSignal *
AisanSignalFileReader::GetDataRowById(int _id) {
  if (m_data_map.size() == 0)
    return nullptr;
...
void AisanVectorFileReader::ParseNextLine(const vector_map_msgs::Vector &_rec,
                                          AisanVector &data) {
  data.Hang = _rec.hang;
  data.PID = _rec.pid;
  data.VID = _rec.vid;
...
AisanVectorFileReader::AisanVector *
AisanVectorFileReader::GetDataRowById(int _id) {
  if (m_data_map.size() == 0)
    return nullptr;
...
void AisanCurbFileReader::ParseNextLine(const vector_map_msgs::Curb &_rec,
                                        AisanCurb &data) {
...
AisanRoadEdgeFileReader::AisanRoadEdge *
AisanRoadEdgeFileReader::GetDataRowById(int _id) {
  if (m_data_map.size() == 0)
    return nullptr;
...
AisanCrossWalkFileReader::AisanCrossWalk *
AisanCrossWalkFileReader::GetDataRowById(int _id) {
  if (m_data_map.size() == 0)
    return nullptr;
...
void AisanWayareaFileReader::ParseNextLine(const vector_map_msgs::WayArea &_rec,
                                           AisanWayarea &data) {
  data.AID = _rec.aid;
  data.ID = _rec.waid;
  // data.LinkID = _rec.;
...
AisanWayareaFileReader::AisanWayarea *
AisanWayareaFileReader::GetDataRowById(int _id) {
  if (m_data_map.size() == 0)
    return nullptr;
