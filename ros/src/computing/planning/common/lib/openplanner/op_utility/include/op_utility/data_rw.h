...
public:
  ...
  static void WriteLogData(const std::string& logFolder, const std::string& logTitle, const std::string& header,
...
public:
  ...
  SimpleReaderBase(const std::string& fileName, const int& nHeaders = 2, const char& separator = ',',
                   const int& iDataTitles = 1, const int& nVariablesForOneObject = 0, const int& nLineHeaders = 0,
...
public:
  ...
  MapRaw()
  {
    ...
    op_utility_ns::UtilityH::GetTickCount(_time_out);
  }
  ...
  int GetVersion()
  {
    bool bTimeOut = op_utility_ns::UtilityH::GetTimeDiffNow(_time_out) > 2.0;
