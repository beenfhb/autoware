...
double UtilityH::AngleBetweenTwoAnglesPositive(const double &a1,
                                               const double &a2) {
  double diff = a1 - a2;
  if (diff < 0)
    diff = a2 - a1;
...
double UtilityH::GetCircularAngle(const double &prevContAngle,
...
double UtilityH::GetTimeDiff(const struct timespec &old_t,
...
int UtilityH::tsCompare(struct timespec time1, struct timespec time2,
                        int micro_tolerance) {
...
double PIDController::getPID(const double &currValue,
...
double PIDController::getPID(const double &e) {
  // TODO Remember to add sampling time and multiply the time elapsed by the
  // error complex PID error calculation
...
void PIDController::ResetD() { bResetD = true; }

void PIDController::ResetI() { bResetI = true; }
...
LowpassFilter::LowpassFilter(const int &filterOrder, const double &sampleFreq,
                             const double &cutOffFreq) {
  Init(filterOrder, sampleFreq, cutOffFreq);
}

void LowpassFilter::Init(const int &n, const double &sampleFreq,
                         const double &cutOffFreq) {
  if (!(n == 2 || n == 4 || n == 6 || n == 8)) {
