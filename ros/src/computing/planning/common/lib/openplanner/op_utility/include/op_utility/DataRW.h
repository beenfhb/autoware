...
public:
  ...
  static void WriteKMLFile(const std::string &fileName,
                           const std::vector<std::string> &gps_list);
  static void
  WriteKMLFile(const std::string &fileName,
               const std::vector<std::vector<std::string>> &gps_list);
  static void WriteLogData(const std::string &logFolder,
...
public:
  ...
  SimpleReaderBase(const std::string &fileName, const int &nHeaders = 2,
                   ...
                   const int &nVariablesForOneObject = 0,
                   const int &nLineHeaders = 0,
...
public:
  ...
  void ParseNextLine(const vector_map_msgs::DTLane &_rec,
...
public:
  ...
  void ParseNextLine(const vector_map_msgs::CrossRoad &_rec,
...
public:
  ...
  void ParseNextLine(const vector_map_msgs::StopLine &_rec,
                     AisanStopLine &data);
...
public:
  ...
  void ParseNextLine(const vector_map_msgs::RoadSign &_rec,
                     AisanRoadSign &data);
...
public:
  ...
  void ParseNextLine(const vector_map_msgs::RoadEdge &_rec,
                     AisanRoadEdge &data);
...
public:
  ...
  void ParseNextLine(const vector_map_msgs::CrossWalk &_rec,
                     AisanCrossWalk &data);
...
public:
  ...
  int GetVersion() {
    ...
    //		else
    //		{
    //			bLoaded =  pLanes != nullptr && pPoints != nullptr && pCenterLines
    //!= nullptr; 			if(bLoaded && bTimeOut)
    //			{
    //				iVersion = 1;
    //				if(pNodes  == nullptr)
    //					pNodes = new
    //AisanNodesFileReader(vector_map_msgs::NodeArray());
    //			}
    //		}
    ...
    if (bLoaded && bTimeOut) {
      if (pIntersections == nullptr)
        pIntersections =
            new AisanIntersectionFileReader(vector_map_msgs::CrossRoadArray());
      ...
      if (pStopLines == nullptr)
        pStopLines =
            new AisanStopLineFileReader(vector_map_msgs::StopLineArray());
      ...
      if (pRoadedges == nullptr)
        pRoadedges =
            new AisanRoadEdgeFileReader(vector_map_msgs::RoadEdgeArray());
      ...
      if (pCrossWalks == nullptr)
        pCrossWalks =
            new AisanCrossWalkFileReader(vector_map_msgs::CrossWalkArray());
    }
