...
{
  std::string main_folder = UtilityH::GetHomeDirectory() + DataRW::LoggingMainfolderName + DataRW::ExperimentsFolderName
      + folderName;
  int dir_err = mkdir(main_folder.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
...
void DataRW::WriteLogData(const std::string& logFolder, const std::string& logTitle, const std::string& header,
...
void DataRW::WriteKMLFile(const std::string& fileName, const std::vector<std::string>& gps_list)
{
  ...
  pV = hKmlFile.FirstChild("Folder").FirstChild("Document").FirstChild("Placemark").FirstChild("LineString").FirstChild(
      "coordinates").Element();
  if (!pV)
...
void DataRW::WriteKMLFile(const std::string& fileName, const std::vector<std::vector<std::string> >& gps_list)
{
  TiXmlDocument kmldoc(UtilityH::GetHomeDirectory() + DataRW::KmlMapsFolderName + "KmlTemplate.kml");
...
SimpleReaderBase::SimpleReaderBase(const std::string& fileName, const int& nHeaders, const char& separator,
                                   const int& iDataTitles, const int& nVariablesForOneObject, const int& nLineHeaders,
...
bool SimpleReaderBase::ReadSingleLine(std::vector<std::vector<std::string> >& line)
{
...
int LocalizationPathReader::ReadAllData(std::vector<LocalizationWayPoint>& data_list)
{
  data_list.clear();
...
AisanNodesFileReader::AisanNodesFileReader(const vector_map_msgs::NodeArray& _nodes) :
...
AisanPointsFileReader::AisanPointsFileReader(const vector_map_msgs::PointArray& _points) :
    SimpleReaderBase("d", 1)
{
...
AisanLinesFileReader::AisanLinesFileReader(const vector_map_msgs::LineArray& _nodes) :
    SimpleReaderBase("d", 1)
{
  if (_nodes.data.size() == 0)
    return;
...
AisanCenterLinesFileReader::AisanCenterLinesFileReader(const vector_map_msgs::DTLaneArray& _Lines) :
    SimpleReaderBase("d", 1)
{
...
int AisanCenterLinesFileReader::ReadAllData(std::vector<AisanCenterLine>& data_list)
{
  data_list.clear();
  AisanCenterLine data;
  //double logTime = 0;
...
AisanLanesFileReader::AisanLanesFileReader(const vector_map_msgs::LaneArray& _lanes) :
    SimpleReaderBase("d", 1)
{
...
AisanAreasFileReader::AisanAreasFileReader(const vector_map_msgs::AreaArray& _areas) :
    SimpleReaderBase("d", 1)
{
...
AisanIntersectionFileReader::AisanIntersectionFileReader(const vector_map_msgs::CrossRoadArray& _inters) :
    SimpleReaderBase("d", 1)
{
...
int AisanIntersectionFileReader::ReadAllData(std::vector<AisanIntersection>& data_list)
{
  data_list.clear();
  AisanIntersection data;
  //double logTime = 0;
...
AisanStopLineFileReader::AisanStopLineFileReader(const vector_map_msgs::StopLineArray& _stopLines) :
    SimpleReaderBase("d", 1)
{
...
int AisanStopLineFileReader::ReadAllData(std::vector<AisanStopLine>& data_list)
{
  data_list.clear();
  AisanStopLine data;
  //double logTime = 0;
...
AisanRoadSignFileReader::AisanRoadSignFileReader(const vector_map_msgs::RoadSignArray& _signs) :
    SimpleReaderBase("d", 1)
{
...
int AisanRoadSignFileReader::ReadAllData(std::vector<AisanRoadSign>& data_list)
{
  data_list.clear();
  AisanRoadSign data;
  //double logTime = 0;
...
AisanSignalFileReader::AisanSignalFileReader(const vector_map_msgs::SignalArray& _signal) :
    SimpleReaderBase("d", 1)
{
...
AisanVectorFileReader::AisanVectorFileReader(const vector_map_msgs::VectorArray& _vectors) :
    SimpleReaderBase("d", 1)
{
...
AisanCurbFileReader::AisanCurbFileReader(const vector_map_msgs::CurbArray& _curbs) :
    SimpleReaderBase("d", 1)
{
...
AisanRoadEdgeFileReader::AisanRoadEdgeFileReader(const vector_map_msgs::RoadEdgeArray& _edges) :
    SimpleReaderBase("d", 1)
{
...
int AisanRoadEdgeFileReader::ReadAllData(std::vector<AisanRoadEdge>& data_list)
{
  data_list.clear();
  AisanRoadEdge data;
  //double logTime = 0;
...
AisanCrossWalkFileReader::AisanCrossWalkFileReader(const vector_map_msgs::CrossWalkArray& _crossWalks) :
    SimpleReaderBase("d", 1)
{
...
int AisanCrossWalkFileReader::ReadAllData(std::vector<AisanCrossWalk>& data_list)
{
  data_list.clear();
  AisanCrossWalk data;
  //double logTime = 0;
...
AisanWayareaFileReader::AisanWayareaFileReader(const vector_map_msgs::WayAreaArray& _wayAreas) :
    SimpleReaderBase("d", 1)
{
