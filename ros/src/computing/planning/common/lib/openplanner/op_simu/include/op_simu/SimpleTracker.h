...
public:
  ...
  void UpdateTracking(double _dt, const PlannerHNS::DetectedObject &oldObj,
    ...
    if (m_iLife > MinAppearanceCount) {
      ...
      predObj.acceleration =
          UtilityHNS::UtilityH::GetSign(predObj.center.v - prev_v);
    } else {
    ...
    if (predObj.centers_list.size() > 1) {
      ...
      if (hypot(diff_y, diff_x) > 0.1) {
        ...
        PlannerHNS::PlanningHelpers::SmoothPath(predObj.centers_list, 0.3, 0.4,
                                                0.1);
  ...
  void PredictTracking(double _dt, const PlannerHNS::DetectedObject &oldObj,
                       PlannerHNS::DetectedObject &predObj) {
    if (m_iLife < MinAppearanceCount) {
      forget_time -= _dt;
      return;
    ...
    if (m_iLife > MinAppearanceCount) {
      ...
      predObj.bVelocity = true;
      predObj.acceleration =
          UtilityHNS::UtilityH::GetSign(predObj.center.v - oldObj.center.v);
    } else {
      predObj.bDirection = false;
      predObj.bVelocity = false;
    }
