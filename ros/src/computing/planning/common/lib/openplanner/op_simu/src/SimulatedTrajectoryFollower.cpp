...
void SimulatedTrajectoryFollower::Init(const ControllerParams &params,
  ...
  m_pidSteer.Setlimit(vehicleInfo.max_steer_angle,
                      -vehicleInfo.max_steer_angle);
...
void SimulatedTrajectoryFollower::PrepareNextWaypoint(
    const PlannerHNS::WayPoint &CurPos, const double &currVelocity,
    const double &currSteering) {
  m_CurrPos = CurPos;
  FindNextWayPoint(m_Path, m_CurrPos, currVelocity, m_FollowMePoint,
                   m_PerpendicularPoint, m_LateralError, m_FollowingDistance);
}

void SimulatedTrajectoryFollower::UpdateCurrentPath(
    const std::vector<PlannerHNS::WayPoint> &path) {
  m_Path = path;
}

bool SimulatedTrajectoryFollower::FindNextWayPoint(
    const std::vector<PlannerHNS::WayPoint> &path,
  ...
  ////	if(m_iPrevWayPoint >=0  && m_iPrevWayPoint < path.size() && iWayPoint <
  ///m_iPrevWayPoint) /		iWayPoint = m_iPrevWayPoint;
  //
  //
...
int SimulatedTrajectoryFollower::SteerControllerUpdate(
  ...
  int ret = SteerControllerPart(m_CurrPos, m_FollowMePoint, m_LateralError,
                                desiredSteerAngle);
  if (ret < 0)
...
int SimulatedTrajectoryFollower::SteerControllerPart(
...
int SimulatedTrajectoryFollower::VeclocityControllerUpdate(
...
PlannerHNS::VehicleState SimulatedTrajectoryFollower::DoOneStep(
