...
std::vector<GPSPoint> PolygonGenerator::EstimateClusterPolygon(
    const pcl::PointCloud<pcl::PointXYZ> &cluster,
    const GPSPoint &original_centroid, GPSPoint &new_centroid,
  ...
  for (unsigned int i = 0; i < cluster.points.size(); i++) {
    ...
    GPSPoint v(p.pos.x - original_centroid.x, p.pos.y - original_centroid.y, 0,
               0);
    p.cost = pointNorm(v);
    p.pos.a =
