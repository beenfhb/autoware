...
ROSHelpers::CreateGenMarker(const double &x, const double &y, const double &z,
                            const double &a, const double &r, const double &g,
                            const double &b, const double &scale, const int &id,
                            const std::string &ns, const int &type) {
...
void ROSHelpers::InitMarkers(const int &nMarkers,
  ...
  for (int i = 0; i < nMarkers; i++) {
    visualization_msgs::Marker mkr =
        CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "CenterMarker",
  ...
  for (int i = nMarkers; i < nMarkers * 2; i++) {
    visualization_msgs::Marker mkr =
        CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "Directions",
  ...
  for (int i = nMarkers * 2; i < nMarkers * 3; i++) {
    visualization_msgs::Marker mkr =
        CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "InfoText",
  ...
  for (int i = nMarkers * 3; i < nMarkers * 4; i++) {
    visualization_msgs::Marker mkr =
        CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "detected_polygons",
  ...
  for (int i = nMarkers * 4; i < nMarkers * 5; i++) {
    visualization_msgs::Marker mkr =
        CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "tracked_trajectories",
                        visualization_msgs::Marker::LINE_STRIP);
    trajectories.markers.push_back(mkr);
  }
}

void ROSHelpers::InitMatchingMarkers(
    const int &nMarkers, visualization_msgs::MarkerArray &connections) {
  connections.markers.clear();
  for (int i = 0; i < nMarkers; i++) {
    visualization_msgs::Marker mkr =
        CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "matching_connections",
                        visualization_msgs::Marker::LINE_STRIP);
    connections.markers.push_back(mkr);
  }
}

void ROSHelpers::ConvertMatchingMarkers(
    const std::vector<std::pair<PlannerHNS::WayPoint, PlannerHNS::WayPoint>>
        &match_list,
    visualization_msgs::MarkerArray &tracked_traj_d,
    visualization_msgs::MarkerArray &tracked_traj, int start_id) {

  tracked_traj = tracked_traj_d;

  for (unsigned int i = 0; i < match_list.size(); i++) {
    visualization_msgs::Marker match_mkr = CreateGenMarker(
        0, 0, 0, 0, 1, 0, 0, 0.2, start_id + i, "matching_connections",
        visualization_msgs::Marker::LINE_STRIP);
...
int ROSHelpers::ConvertTrackedObjectsMarkers(
    ...
    const std::vector<PlannerHNS::DetectedObject> &trackedObstacles,
  ...
  for (unsigned int i = 0; i < trackedObstacles.size(); i++) {
    ...
    // Update Stage
    visualization_msgs::Marker center_mkr =
        CreateGenMarker(trackedObstacles.at(i).center.pos.x,
                        trackedObstacles.at(i).center.pos.y,
                        trackedObstacles.at(i).center.pos.z,
                        trackedObstacles.at(i).center.pos.a, 1, 0, 0, 0.5, i,
                        "CenterMarker", visualization_msgs::Marker::SPHERE);
    ...
    if (trackedObstacles.at(i).bDirection) {
      visualization_msgs::Marker dir_mkr =
          CreateGenMarker(trackedObstacles.at(i).center.pos.x,
                          trackedObstacles.at(i).center.pos.y,
                          trackedObstacles.at(i).center.pos.z + 0.5,
                          trackedObstacles.at(i).center.pos.a, 0, 1, 0, 0.3,
                          centers.markers.size() + i, "Directions",
                          visualization_msgs::Marker::ARROW);
      dir_mkr.scale.x = 0.4;
    ...
    //		if(speed > 3.0)
    //			text_mkr =
    //CreateGenMarker(trackedObstacles.at(i).center.pos.x+0.5,trackedObstacles.at(i).center.pos.y+0.5,trackedObstacles.at(i).center.pos.z+1,
    //					trackedObstacles.at(i).center.pos.a,1,0,0,0.75,centers.markers.size()*2+i,"InfoText",
    //visualization_msgs::Marker::TEXT_VIEW_FACING); 		else
    text_mkr = CreateGenMarker(trackedObstacles.at(i).center.pos.x + 0.5,
                               trackedObstacles.at(i).center.pos.y + 0.5,
                               trackedObstacles.at(i).center.pos.z + 1,
                               trackedObstacles.at(i).center.pos.a, 1, 1, 1,
                               1.2, centers.markers.size() * 2 + i, "InfoText",
                               visualization_msgs::Marker::TEXT_VIEW_FACING);

    std::ostringstream str_out;
    // str_out << trackedObstacles.at(i).id << " ( " << speed << " )" << " (" <<
    // trackedObstacles.at(i).distance_to_center << ")";
    str_out << trackedObstacles.at(i).id << " (" << speed << ")";
    ...
    // Polygons
    visualization_msgs::Marker poly_mkr = CreateGenMarker(
        0, 0, 0, 0, 1, 0.25, 0.25, 0.1, centers.markers.size() * 3 + i,
        "detected_polygons", visualization_msgs::Marker::LINE_STRIP);
    ...
    // Trajectories
    visualization_msgs::Marker traj_mkr = CreateGenMarker(
        0, 0, 0, 0, 1, 1, 0, 0.1, centers.markers.size() * 4 + i,
        "tracked_trajectories", visualization_msgs::Marker::LINE_STRIP);

    for (unsigned int p = 0; p < trackedObstacles.at(i).centers_list.size();
         p++) {
      geometry_msgs::Point point;
...
void ROSHelpers::CreateCircleMarker(const PlannerHNS::WayPoint &_center,
                                    const double &radius, const int &start_id,
                                    visualization_msgs::Marker &circle_points) {
  circle_points =
      CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 0.2, start_id, "Detection_Circles",
                      visualization_msgs::Marker::LINE_STRIP);
...
void ROSHelpers::InitPredMarkers(const int &nMarkers,
                                 visualization_msgs::MarkerArray &paths) {
  paths.markers.clear();
  for (int i = 0; i < nMarkers; i++) {
    visualization_msgs::Marker mkr =
        CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "Predicted_Trajectories",
                        visualization_msgs::Marker::LINE_STRIP);
    paths.markers.push_back(mkr);
  }
}

void ROSHelpers::InitCurbsMarkers(const int &nMarkers,
                                  visualization_msgs::MarkerArray &curbs) {
  curbs.markers.clear();
  for (int i = 0; i < nMarkers; i++) {
    visualization_msgs::Marker mkr =
        CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "map_detected_curbs",
                        visualization_msgs::Marker::SPHERE);
...
void ROSHelpers::ConvertPredictedTrqajectoryMarkers(
  ...
  for (unsigned int i = 0; i < paths.size(); i++) {
    ...
    //		}
    //
    //		visualization_msgs::Marker path_mkr =
    //CreateGenMarker(0,0,0,0,r,g,b,0.1,i,"Predicted_Trajectories",
    //visualization_msgs::Marker::LINE_STRIP);

    visualization_msgs::Marker path_mkr = CreateGenMarker(
        0, 0, 0, 0, 1.0 * prop, 0.1 * prop, 0.1 * prop, 0.1, i,
        "Predicted_Trajectories", visualization_msgs::Marker::LINE_STRIP);
...
void ROSHelpers::ConvertCurbsMarkers(
    const std::vector<PlannerHNS::DetectedObject> &curbs,
    visualization_msgs::MarkerArray &curbs_markers,
    visualization_msgs::MarkerArray &curbs_markers_d) {

  curbs_markers = curbs_markers_d;
  for (unsigned int i = 0; i < curbs.size(); i++) {
    if (curbs.at(i).contour.size() > 0) {
      visualization_msgs::Marker curb_mkr = CreateGenMarker(
          curbs.at(i).contour.at(0).x, curbs.at(i).contour.at(0).y,
          curbs.at(i).contour.at(0).z, 0, 1, 0.54, 0, 0.2, i,
          "map_detected_curbs", visualization_msgs::Marker::SPHERE);
...
void ROSHelpers::InitCollisionPointsMarkers(
    const int &nMarkers, visualization_msgs::MarkerArray &col_points) {
  col_points.markers.clear();
  for (int i = 0; i < nMarkers; i++) {
    visualization_msgs::Marker mkr =
        CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 1, i, "collision_points_rviz",
                        visualization_msgs::Marker::SPHERE);
    col_points.markers.push_back(mkr);
  }
}

void ROSHelpers::ConvertCollisionPointsMarkers(
    const std::vector<PlannerHNS::WayPoint> &col_points,
    visualization_msgs::MarkerArray &collision_markers,
    visualization_msgs::MarkerArray &collision_markers_d) {
  collision_markers = collision_markers_d;
  for (unsigned int i = 0; i < col_points.size(); i++) {
    visualization_msgs::Marker mkr = CreateGenMarker(
        col_points.at(i).pos.x, col_points.at(i).pos.y, col_points.at(i).pos.z,
        0, 1, 0, 0, 0.5, i, "collision_points_rviz",
        visualization_msgs::Marker::SPHERE);
...
void ROSHelpers::ConvertFromPlannerHToAutowarePathFormat(
    const std::vector<PlannerHNS::WayPoint> &path, const int &iStart,
...
void ROSHelpers::ConvertFromRoadNetworkToAutowareVisualizeMapFormat(
    const PlannerHNS::RoadNetwork &map,
  ...
  for (unsigned int i = 0; i < map.roadSegments.size(); i++) {
    for (unsigned int j = 0; j < map.roadSegments.at(i).Lanes.size(); j++) {
      ...
      for (unsigned int p = 0;
           p < map.roadSegments.at(i).Lanes.at(j).points.size(); p++) {
        geometry_msgs::Point point;
...
void ROSHelpers::InitPredParticlesMarkers(
    const int &nMarkers, visualization_msgs::MarkerArray &paths) {
  paths.markers.clear();
  for (int i = 0; i < nMarkers; i++) {
    visualization_msgs::Marker mkr =
        CreateGenMarker(0, 0, 0, 0, 1, 1, 1, 0.05, i, "Particles",
                        visualization_msgs::Marker::ARROW);
    mkr.scale.x = 0.3;
    paths.markers.push_back(mkr);
  }
...
void ROSHelpers::ConvertParticles(
    std::vector<PlannerHNS::WayPoint> &points,
    visualization_msgs::MarkerArray &part_mkrs,
    visualization_msgs::MarkerArray &part_markers_d) {
  part_mkrs = part_markers_d;
  for (unsigned int i = 0; i < points.size(); i++) {
    visualization_msgs::Marker mkr;
    if (points.at(i).bDir == PlannerHNS::STANDSTILL_DIR)
      mkr =
          CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y,
                          points.at(i).pos.z, points.at(i).pos.a, 1, 0, 0, 0.05,
                          i, "Particles", visualization_msgs::Marker::ARROW);
    else if (points.at(i).bDir == PlannerHNS::FORWARD_DIR)
      mkr =
          CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y,
                          points.at(i).pos.z, points.at(i).pos.a, 1, 1, 1, 0.05,
                          i, "Particles", visualization_msgs::Marker::ARROW);
    else if (points.at(i).bDir == PlannerHNS::FORWARD_RIGHT_DIR)
      mkr =
          CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y,
                          points.at(i).pos.z, points.at(i).pos.a, 0, 1, 0, 0.05,
                          i, "Particles", visualization_msgs::Marker::ARROW);
    else if (points.at(i).bDir == PlannerHNS::FORWARD_LEFT_DIR)
      mkr =
          CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y,
                          points.at(i).pos.z, points.at(i).pos.a, 0, 0, 1, 0.05,
                          i, "Particles", visualization_msgs::Marker::ARROW);
    else if (points.at(i).bDir == PlannerHNS::BACKWARD_DIR)
      mkr =
          CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y,
                          points.at(i).pos.z, points.at(i).pos.a, 1, 0, 1, 0.05,
                          i, "Particles", visualization_msgs::Marker::ARROW);
    else
      mkr =
          CreateGenMarker(points.at(i).pos.x, points.at(i).pos.y,
                          points.at(i).pos.z, points.at(i).pos.a, 1, 1, 0, 0.05,
                          i, "Particles", visualization_msgs::Marker::ARROW);
...
void ROSHelpers::ConvertFromPlannerHRectangleToAutowareRviz(
...
void ROSHelpers::TrajectoriesToMarkers(
    const std::vector<std::vector<std::vector<PlannerHNS::WayPoint>>> &paths,
    visualization_msgs::MarkerArray &markerArray) {
  visualization_msgs::Marker lane_waypoint_marker;
  lane_waypoint_marker.header.frame_id = "map";
  lane_waypoint_marker.header.stamp = ros::Time();
...
void ROSHelpers::TrajectoriesToColoredMarkers(
    const std::vector<std::vector<PlannerHNS::WayPoint>> &paths,
    const std::vector<PlannerHNS::TrajectoryCost> &traj_costs,
    const int &iClosest, visualization_msgs::MarkerArray &markerArray) {
  visualization_msgs::Marker lane_waypoint_marker;
  lane_waypoint_marker.header.frame_id = "map";
  lane_waypoint_marker.header.stamp = ros::Time();
...
void ROSHelpers::ConvertFromPlannerHToAutowareVisualizePathFormat(
    const std::vector<PlannerHNS::WayPoint> &curr_path,
    const std::vector<std::vector<std::vector<PlannerHNS::WayPoint>>> &paths,
    const PlannerHNS::LocalPlannerH &localPlanner,
    visualization_msgs::MarkerArray &markerArray) {
  visualization_msgs::Marker lane_waypoint_marker;
  lane_waypoint_marker.header.frame_id = "map";
  lane_waypoint_marker.header.stamp = ros::Time();
  ...
  for (unsigned int il = 0; il < paths.size(); il++) {
    for (unsigned int i = 0; i < paths.at(il).size(); i++) {
      ...
      lane_waypoint_marker.color.a = 0.9;
      if (localPlanner.m_TrajectoryCostsCalculatotor.m_TrajectoryCosts.size() ==
          paths.size()) {
        float norm_cost =
            localPlanner.m_TrajectoryCostsCalculatotor.m_TrajectoryCosts.at(i)
                .cost *
            paths.size();
        if (norm_cost <= 1.0) {
          lane_waypoint_marker.color.r = norm_cost;
      ...
      if ((int)i == localPlanner.m_iSafeTrajectory &&
          (int)il == localPlanner.m_iCurrentTotalPathId) {
        lane_waypoint_marker.color.r = 1.0;
        lane_waypoint_marker.color.g = 0.0;
        lane_waypoint_marker.color.b = 1.0;
      } else {
...
void ROSHelpers::ConvertFromPlannerObstaclesToAutoware(
    const PlannerHNS::WayPoint &currState,
    const std::vector<PlannerHNS::DetectedObject> &trackedObstacles,
  ...
  for (unsigned int i = 0; i < trackedObstacles.size(); i++) {
    // double distance =
    // hypot(currState.pos.y-trackedObstacles.at(i).center.pos.y,
    // currState.pos.x-trackedObstacles.at(i).center.pos.x);
    ...
    // std::cout << " Distance : " << distance << ", Of Object" <<
    // trackedObstacles.at(i).id << std::endl;
    ...
    direction_marker.pose.orientation =
        tf::createQuaternionMsgFromYaw(UtilityHNS::UtilityH::SplitPositiveAngle(
            trackedObstacles.at(i).center.pos.a));
    ...
    for (unsigned int iq = 0; iq < 8; iq++) {
      ...
      double a_q = UtilityHNS::UtilityH::SplitPositiveAngle(
          trackedObstacles.at(i).center.pos.a + (iq * M_PI_4));
    ...
    velocity_marker.text =
        str_out.str(); // vel.erase(vel.find_first_of(".") + 2);
    // if(speed > 0.5)
...
std::string
ROSHelpers::GetBehaviorNameFromCode(const PlannerHNS::STATE_TYPE &behState) {
...
void ROSHelpers::VisualizeBehaviorState(
    const PlannerHNS::WayPoint &currState, const PlannerHNS::BehaviorState &beh,
    const bool &bGreenLight, const int &avoidDirection,
    visualization_msgs::Marker &behaviorMarker, std::string ns,
    double size_factor) {
...
void ROSHelpers::ConvertFromAutowareBoundingBoxObstaclesToPlannerH(
  ...
  for (unsigned int i = 0; i < detectedObstacles.boxes.size(); i++) {
    ...
    obj.center = PlannerHNS::WayPoint(
        detectedObstacles.boxes.at(i).pose.position.x,
        detectedObstacles.boxes.at(i).pose.position.y, 0, 0);
    ...
    // double d = hypot(m_State.state.pos.y - obj.center.pos.y,
    // m_State.state.pos.x - obj.center.pos.x); std::cout << ", Distance of  : "
    // << d; if(d < 7)
    {
...
void ROSHelpers::ConvertFromAutowareCloudClusterObstaclesToPlannerH(
    const PlannerHNS::WayPoint &currState, const double &car_width,
    const double &car_length, const autoware_msgs::CloudClusterArray &clusters,
    vector<PlannerHNS::DetectedObject> &obstacles_list,
    const double max_obj_size, const double &min_obj_size,
    const double &detection_radius, const int &n_poly_quarters,
    const double &poly_resolution, int &nOriginalPoints, int &nContourPoints) {
  ...
  for (unsigned int i = 0; i < clusters.clusters.size(); i++) {
    ...
    obj.contour = polyGen.EstimateClusterPolygon(point_cloud, obj.center.pos,
                                                 avg_center, poly_resolution);

    obj.distance_to_center = hypot(obj.center.pos.y - currState.pos.y,
                                   obj.center.pos.x - currState.pos.x);

    object_size = hypot(obj.w, obj.l);

    if (obj.distance_to_center > detection_radius ||
        object_size < min_obj_size || object_size > max_obj_size)
      continue;
    ...
    if (distance_x <= car_length * 0.5 &&
        distance_y <= car_width * 0.5) // don't detect yourself
      continue;

    // obj.center.pos = avg_center;
    nOrPoints += point_cloud.points.size();
    nPoints += obj.contour.size();
    // std::cout << " Distance_X: " << distance_x << ", " << " Distance_Y: " <<
    // distance_y << ", " << " Size: " << object_size << std::endl;
    obstacles_list.push_back(obj);
  }
...
PlannerHNS::SHIFT_POS ROSHelpers::ConvertShiftFromAutowareToPlannerH(
    const PlannerHNS::AUTOWARE_SHIFT_POS &shift) {
...
PlannerHNS::AUTOWARE_SHIFT_POS ROSHelpers::ConvertShiftFromPlannerHToAutoware(
    const PlannerHNS::SHIFT_POS &shift) {
...
ROSHelpers::ConvertBehaviorStateFromPlannerHToAutoware(
    const PlannerHNS::BehaviorState &beh) {
...
void ROSHelpers::ConvertFromLocalLaneToAutowareLane(
    const std::vector<PlannerHNS::WayPoint> &path,
    autoware_msgs::Lane &trajectory, const unsigned int &iStart) {
  trajectory.waypoints.clear();
...
void ROSHelpers::ConvertFromLocalLaneToAutowareLane(
    const std::vector<PlannerHNS::GPSPoint> &path,
    autoware_msgs::Lane &trajectory) {
  trajectory.waypoints.clear();
  ...
  for (unsigned int i = 0; i < path.size(); i++) {
    ...
    wp.pose.pose.orientation = tf::createQuaternionMsgFromYaw(
        UtilityHNS::UtilityH::SplitPositiveAngle(path.at(i).a));

    trajectory.waypoints.push_back(wp);
  }
}

void ROSHelpers::ConvertFromAutowareLaneToLocalLane(
    const autoware_msgs::Lane &trajectory,
...
void ROSHelpers::createGlobalLaneArrayMarker(
  ...
  for (unsigned int i = 0; i < lane_waypoints_array.lanes.size(); i++) {
    ...
    for (unsigned int j = 0;
         j < lane_waypoints_array.lanes.at(i).waypoints.size(); j++) {
      geometry_msgs::Point point;
      point =
          lane_waypoints_array.lanes.at(i).waypoints.at(j).pose.pose.position;
      lane_waypoint_marker.points.push_back(point);
    }
    markerArray.markers.push_back(lane_waypoint_marker);
    count++;
...
void ROSHelpers::createGlobalLaneArrayVelocityMarker(
    const autoware_msgs::LaneArray &lane_waypoints_array,
    visualization_msgs::MarkerArray &markerArray) {
  ...
  for (unsigned int i = 0; i < lane_waypoints_array.lanes.size(); i++) {
    ...
    for (unsigned int j = 0;
         j < lane_waypoints_array.lanes.at(i).waypoints.size(); j++) {
      // std::cout << _waypoints[i].GetX() << " " << _waypoints[i].GetY() << " "
      // << _waypoints[i].GetZ() << " " << _waypoints[i].GetVelocity_kmh() <<
      // std::endl;
      velocity_marker.id = j;
      geometry_msgs::Point relative_p;
      relative_p.y = 0.5;
      velocity_marker.pose.position = calcAbsoluteCoordinate(
          relative_p,
      ...
      // double to string
      std::ostringstream str_out;
      str_out << lane_waypoints_array.lanes.at(i)
                     .waypoints.at(j)
  ...
  markerArray.markers.insert(markerArray.markers.end(),
                             tmp_marker_array.markers.begin(),
                             tmp_marker_array.markers.end());
}

void ROSHelpers::createGlobalLaneArrayOrientationMarker(
    const autoware_msgs::LaneArray &lane_waypoints_array,
    visualization_msgs::MarkerArray &markerArray) {
  visualization_msgs::MarkerArray tmp_marker_array;
  visualization_msgs::Marker lane_waypoint_marker;
  ...
  for (unsigned int i = 0; i < lane_waypoints_array.lanes.size(); i++) {
    ...
    for (unsigned int j = 0;
      ...
      } else if (lane_waypoints_array.lanes.at(i).waypoints.at(j).dtlane.dir ==
                 2) {
        lane_waypoint_marker.color.r = 0.0;
        lane_waypoint_marker.color.g = 0.0;
        lane_waypoint_marker.color.b = 1.0;
...
void ROSHelpers::GetTrafficLightForVisualization(
    std::vector<PlannerHNS::TrafficLight> &lights,
    visualization_msgs::MarkerArray &markerArray) {
  markerArray.markers.clear();
  for (unsigned int i = 0; i < lights.size(); i++) {
    if (lights.at(i).lightState == RED_LIGHT) {
      visualization_msgs::Marker mkr = CreateGenMarker(
          lights.at(i).pos.x, lights.at(i).pos.y, lights.at(i).pos.z, 0, 1, 0,
          0, 3, i, "traffic_light_visualize",
          visualization_msgs::Marker::SPHERE);
      markerArray.markers.push_back(mkr);
    } else if (lights.at(i).lightState == GREEN_LIGHT) {
      visualization_msgs::Marker mkr = CreateGenMarker(
          lights.at(i).pos.x, lights.at(i).pos.y, lights.at(i).pos.z, 0, 0, 1,
          0, 3, i, "traffic_light_visualize",
          visualization_msgs::Marker::SPHERE);
      markerArray.markers.push_back(mkr);
    }
...
void ROSHelpers::ConvertFromAutowareDetectedObjectToOpenPlannerDetectedObject(
    const autoware_msgs::DetectedObject &det_obj,
    PlannerHNS::DetectedObject &obj) {
  obj.id = det_obj.id;
  ...
  for (unsigned int j = 0; j < det_obj.candidate_trajectories.lanes.size();
       j++) {
    std::vector<PlannerHNS::WayPoint> _traj;
    PlannerHNS::ROSHelpers::ConvertFromAutowareLaneToLocalLane(
        det_obj.candidate_trajectories.lanes.at(j), _traj);
    for (unsigned int k = 0; k < _traj.size(); k++)
      _traj.at(k).collisionCost =
          det_obj.candidate_trajectories.lanes.at(j).cost;

    obj.predTrajectories.push_back(_traj);
  }
}

void ROSHelpers::ConvertFromOpenPlannerDetectedObjectToAutowareDetectedObject(
    const PlannerHNS::DetectedObject &det_obj, const bool &bSimulationMode,
  ...
  obj.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(
      0, 0, UtilityHNS::UtilityH::SplitPositiveAngle(det_obj.center.pos.a));
  ...
  for (unsigned int j = 0; j < det_obj.predTrajectories.size(); j++) {
    ...
    PlannerHNS::ROSHelpers::ConvertFromLocalLaneToAutowareLane(
        det_obj.predTrajectories.at(j), pred_traj);
...
void ROSHelpers::UpdateRoadMap(const AutowareRoadNetwork &src_map,
                               PlannerHNS::RoadNetwork &out_map) {
  ...
  for (unsigned int i = 0; i < src_map.points.data.size(); i++) {
    ...
    double sec =
        modf((src_map.points.data.at(i).l - deg) * 100.0, &integ_part) / 36.0;
  ...
  std::vector<UtilityHNS::AisanIntersectionFileReader::AisanIntersection>
      inters;
  std::vector<UtilityHNS::AisanLinesFileReader::AisanLine> line_data;
  std::vector<UtilityHNS::AisanStopLineFileReader::AisanStopLine>
      stop_line_data;
  ...
  PlannerHNS::GPSPoint
      origin; //(m_OriginPos.position.x, m_OriginPos.position.y,
              //m_OriginPos.position.z, 0);
  PlannerHNS::MappingHelpers::ConstructRoadNetworkFromROSMessage(
      lanes, points, dts, inters, areas, line_data, stop_line_data, signal_data,
      vector_data, curb_data, roadedge_data, way_area, crossing, nodes_data,
      conn_data, origin, out_map);
}

void ROSHelpers::GetIndicatorArrows(
    const PlannerHNS::WayPoint &center, const double &width,
    const double &length, const PlannerHNS::LIGHT_INDICATOR &indicator,
    const int &id, visualization_msgs::MarkerArray &markerArray) {
  double critical_lateral_distance = width / 2.0 + 0.2;
  // double critical_long_front_distance =  carInfo.length/2.0 ;
  PlannerHNS::GPSPoint top_right(critical_lateral_distance, length,
                                 center.pos.z, 0);
  PlannerHNS::GPSPoint top_left(-critical_lateral_distance, length,
                                center.pos.z, 0);
  ...
  visualization_msgs::Marker mkr_l = PlannerHNS::ROSHelpers::CreateGenMarker(
      top_left.x, top_left.y, top_left.z, top_left.a, color_l.r, color_l.g,
      color_l.b, 1.0, id, "simu_car_indicator_left",
      visualization_msgs::Marker::ARROW);
  mkr_l.scale.y = 0.4;
  mkr_l.scale.z = 0.4;
  visualization_msgs::Marker mkr_r = PlannerHNS::ROSHelpers::CreateGenMarker(
      top_right.x, top_right.y, top_right.z, top_right.a, color_r.r, color_r.g,
      color_r.b, 1.0, id, "simu_car_indicator_right",
      visualization_msgs::Marker::ARROW);
  mkr_r.scale.y = 0.4;
  mkr_r.scale.z = 0.4;
...
void ROSHelpers::TTC_PathRviz(const std::vector<PlannerHNS::WayPoint> &path,
                              visualization_msgs::MarkerArray &markerArray) {
  visualization_msgs::Marker lane_waypoint_marker;
  lane_waypoint_marker.header.frame_id = "map";
  lane_waypoint_marker.header.stamp = ros::Time();
