...
int main(int argc, char **argv) {
  ...
  if (argc < 2) {
    std::cout << "Usage: rosrun map_tools csv2pcd 'PointTyppe' '***.csv'"
              << std::endl;
    exit(-1);
  }
  ...
  for (i = 2; i < argc; i++) {
    ...
    if (point_type == "PointXYZ") {
      ...
      for (size_t i = 0; i < input_cloud->points.size(); ++i) {
        ofs << input_cloud->points[i].x << "," << input_cloud->points[i].y
            << "," << input_cloud->points[i].z << std::endl;
      }

      std::cout << "Input: " << input << " (" << input_cloud->points.size()
                << " points.)" << std::endl;
    ...
    if (point_type == "PointXYZI") {
      ...
      for (size_t i = 0; i < input_cloud->points.size(); ++i) {
        ofs << input_cloud->points[i].x << "," << input_cloud->points[i].y
            ...
            << input_cloud->points[i].intensity << std::endl;
      }
    ...
    if (point_type == "PointXYZRGB") {
      ...
      for (size_t i = 0; i < input_cloud->points.size(); ++i) {
        ofs << input_cloud->points[i].x << "," << input_cloud->points[i].y
            << "," << input_cloud->points[i].z << ","
            << input_cloud->points[i].rgb << std::endl;
      }
