...
template <class T> pcl::PointXYZ getReferencePoint(const T input_cloud) {
...
template <class T>
void translatePointCloud(const T input_cloud, T &output_cloud,
...
int main(int argc, char **argv) {
  ...
  if (argc < 3) {
    std::cout << "Usage: rosrun map_tools pcd_filter \"point_type "
                 "[PointXYZ|PointXYZI|PointXYZRGB]\" \"leaf_size\" \"***.pcd\" "
              << std::endl;
    return 1;
  }
  ...
  for (i = 3; i < argc; i++) {
    ...
    if (point_type == "PointXYZ") {
      ...
      std::cout << "Input: " << input << " (" << input_cloud->size()
                << " points) " << std::endl;

      // translate pointcloud to local frame to avoid losing precision
      pcl::PointCloud<pcl::PointXYZ>::Ptr translated_input_cloud(
          new pcl::PointCloud<pcl::PointXYZ>);
      ...
      pcl::PointCloud<pcl::PointXYZ>::Ptr translated_filtered_cloud(
          new pcl::PointCloud<pcl::PointXYZ>);
      pcl::PointXYZ inverse_origin;
      inverse_origin.x = -origin.x;
      inverse_origin.y = -origin.y;
      inverse_origin.z = -origin.z;
      translatePointCloud(filtered_cloud, translated_filtered_cloud,
                          inverse_origin);
      ...
      std::cout << "Output: " << output << " ("
                << translated_filtered_cloud->size() << " points) "
                << std::endl;
      ...
      // translate pointcloud to local frame to avoid losing precision
      pcl::PointCloud<pcl::PointXYZI>::Ptr translated_input_cloud(
          new pcl::PointCloud<pcl::PointXYZI>);
      pcl::PointXYZ origin = getReferencePoint(input_cloud);
      translatePointCloud(input_cloud, translated_input_cloud, origin);
      ...
      // translate pointcloud back to original frame
      pcl::PointCloud<pcl::PointXYZI>::Ptr translated_filtered_cloud(
          new pcl::PointCloud<pcl::PointXYZI>);
      pcl::PointXYZ inverse_origin;
      inverse_origin.x = -origin.x;
      inverse_origin.y = -origin.y;
      ...
      // translate pointcloud to local frame to avoid losing precision
      pcl::PointCloud<pcl::PointXYZRGB>::Ptr translated_input_cloud(
          new pcl::PointCloud<pcl::PointXYZRGB>);
      pcl::PointXYZ origin = getReferencePoint(input_cloud);
      translatePointCloud(input_cloud, translated_input_cloud, origin);
      ...
      // translate pointcloud back to original frame
      pcl::PointCloud<pcl::PointXYZRGB>::Ptr translated_filtered_cloud(
          new pcl::PointCloud<pcl::PointXYZRGB>);
      pcl::PointXYZ inverse_origin;
      inverse_origin.x = -origin.x;
      inverse_origin.y = -origin.y;
