 ...
 */
/*
int getLaneDetection(string infile, road_layout_estimation::msg_lines
*msg_lines)
{
    ...
    string  line="";
    char    index = 0;
    double  last_right_detection = std::numeric_limits<double>::min(); //uses
*ONLY* the valid lines double  last_left_detection  =
std::numeric_limits<double>::max();          //uses *ONLY* the valid lines
    double  naive_last_right_detection = std::numeric_limits<double>::min();
//uses all values, even if the line is not valid double
naive_last_left_detection  = std::numeric_limits<double>::max();    //uses all
values, even if the line is not valid
    ...
    // Number of lines in the file, 1 line 'in the picture' is one row in the
file, minus
    // one, the first, that is the number of "good" (current tracked in good
state) lines. msg_lines->number_of_lines = index -1 ;
    ...
    msg_lines->naive_width = abs(naive_last_left_detection) +
abs(naive_last_right_detection); msg_lines->way_id = 0; ///WARNING this value is
not used yet.
    ...
    ROS_DEBUG_STREAM("Number of LANEs: " << msg_lines->number_of_lines <<
"\tNumber of good LINEs "<<msg_lines->goodLines); ROS_DEBUG_STREAM("...
getLaneDetection ok"); return true;
}
*/
 ...
 * Directory -f [ --frequency  ] arg (=1)        set replay Frequency -a [ --all
 * ] [=arg(=1)] (=0) replay All data -v [ --velodyne   ] [=arg(=1)] (=0) replay
 * Velodyne data -g [ --gps        ] [=arg(=1)] (=0) replay Gps data -i [ --imu
 * ] [=arg(=1)] (=0) replay Imu data -G [ --grayscale  ] [=arg(=1)] (=0) replay
 * Stereo Grayscale images -C [ --color      ] [=arg(=1)] (=0) replay Stereo
 * Color images -V [ --viewer     ] [=arg(=1)] (=0) enable image viewer -T [
 * --timestamps ] [=arg(=1)] (=0) use KITTI timestamps -s [ --stereoDisp ]
 * [=arg(=1)] (=0) use pre-calculated disparities -D [ --viewDisp   ] [=arg(=1)]
 * (=0) view loaded disparity images -l [ --laneDetect ] [=arg(=1)] (=0) send
 * extra lanes message -F [ --frame      ] [=arg(=0)] (=0) start playing at
 * frame ...
 *
...
int main(int argc, char **argv) {
  ...
  desc.add_options()("help,h", "help message")(
      ...
      "viewDisp  ,D ",
      po::value<bool>(&options.viewDisparities)
          ->default_value(0)
          ->implicit_value(1),
      "view loaded disparity images")("laneDetect,l",
                                      po::value<bool>(&options.laneDetections)
                                          ->default_value(0)
                                          ->implicit_value(1),
                                      "send extra lanes message")(
      "frame     ,F",
      po::value<unsigned int>(&options.startFrame)
          ->default_value(0)
          ->implicit_value(0),
  ...
  string dir_laneDetections;
  string full_filename_laneDetections;
  string dir_laneProjected;
  string full_filename_laneProjected;
  string dir_oxts;
  ...
  // node.advertise<road_layout_estimation::msg_lines>("lanes",1,true);
  ...
  /// 01. Lane detections
  (*(options.path.end() - 1) != '/'
       ? dir_laneDetections = options.path + "/lane/"
       : dir_laneDetections = options.path + "lane/");
  (*(options.path.end() - 1) != '/'
       ? dir_laneProjected = options.path + "/all/"
       : dir_laneProjected = options.path + "all/");
  ...
  if ((options.all_data && ((opendir(dir_image00.c_str()) == NULL) ||
      ...
      //||
      //(options.laneDetections && (   (opendir(dir_laneDetections.c_str()) ==
      //NULL)))
      || (options.timestamps &&
  ...
  if (options.viewer) {
    ...
    if (options.viewDisparities || options.all_data) {
      ...
      full_filename_laneProjected =
          dir_laneProjected + boost::str(boost::format("%010d") % 0) + ".png";
      cv_laneProjected =
          cv::imread(full_filename_laneProjected, CV_LOAD_IMAGE_UNCHANGED);
      cv::waitKey(5);
    }
  ...
  do {
    ...
    /*
            if(options.laneDetections)
            {
                //msgLanes;
                //msgSingleLaneInfo;
                string file=dir_laneDetections+boost::str(boost::format("%010d")
       % entries_played )+".txt"; if(getLaneDetection(file,&msgLanes))
                {
                ...
                {
                    full_filename_laneProjected = dir_laneProjected +
       boost::str(boost::format("%010d") % entries_played ) + ".png";
                    cv_laneProjected = cv::imread(full_filename_laneProjected,
       CV_LOAD_IMAGE_UNCHANGED); cv::imshow("Reprojection of Detected
       Lines",cv_laneProjected); cv::waitKey(5);
                }
            }
    */
    if (options.color || options.all_data) {
