...
// catkin_create_pkg data_preprocessor pcd_tutorial std_msgs roscpp pcl_ros
// pcl_msgs pcl_conversions libpcl-all-dev cv_bridge
...
static void check_arguments(int argc, char *argv[]) {
  if (argc != 5) {
    cout << "Please set arguments like below\n'rosrun data_preprocessor "
            "get_PCD save_dir topic_name'\n";
    exit(EXIT_FAILURE);
  }
}
...
void SaveDepth::create_depth() {
  ...
  // cout << "Image height " << ImageSize_.width << " Height " <<
  // ImageSize_.height << endl; cout << time_stamp_ << endl; cout <<
  // points_.row_step << endl; cout << points_.point_step << endl;
  pcl::PointCloud<pcl::PointXYZI> data_;
  pcl::fromROSMsg(points_, data_);
  int count = 0;

  for (pcl::PointCloud<pcl::PointXYZI>::const_iterator item = data_.begin();
       item != data_.end(); item++) {
    // cout << "x " << item->x << endl;
    // cout << "y " << item->y << endl;
    // cout << "z " << item->z << endl;
    ...
    imagepoint.x = tmpx * tmpdist + 2 * DistCoeff_.at<double>(2) * tmpx * tmpy +
    ...
    imagepoint.x = CameraMat_.at<double>(0, 0) * imagepoint.x +
                   CameraMat_.at<double>(0, 2);
    imagepoint.y = CameraMat_.at<double>(1, 1) * imagepoint.y +
                   CameraMat_.at<double>(1, 2);
    ...
    if (0 <= px && px < width && 0 <= py && py < height) {
      count = count + 1;
      if ((depth.at<double>(py, px) == 0) ||
          (depth.at<double>(py, px) > point.at<double>(2))) {
...
void SaveDepth::depthConverter(int argc, char *argv[]) {
  ...
  ros::Subscriber image_sub =
      n.subscribe(image_topic_name_, 1, &SaveDepth::get_image, this);
  ros::Subscriber pc2_sub =
      n.subscribe(pc2_topic_name_, 1, &SaveDepth::get_pcd, this);
  ros::Rate r(100);
  while (ros::ok()) {
