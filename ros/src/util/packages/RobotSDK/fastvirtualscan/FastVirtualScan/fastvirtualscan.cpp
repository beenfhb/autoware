...
bool compareDistance(const SimpleVirtualScan &svs1,
                     const SimpleVirtualScan &svs2) {
...
void FastVirtualScan::calculateVirtualScans(int beamNum, double heightStep,
                                            double minFloor, double maxCeiling,
                                            double obstacleMinHeight,
    ...
    for (i = 0; i < n; i++) {
      ...
      if (rotid >= 0 && rotid < size) {
        ...
        if (length > minrange && svs[beamid][rotid].rotlength > rotlength) {
          ...
          svs[beamid][rotid].length = svs[beamid][rotid].rotlength * c +
                                      svs[beamid][rotid].rotheight * s;
          svs[beamid][rotid].height = -svs[beamid][rotid].rotlength * s +
                                      svs[beamid][rotid].rotheight * c;
        }
      }
    }
  }
    ...
    for (int i = 0; i < beamnum; i++) {
      ...
      for (j = 0; j < size; j++) {
        ...
        if (svs[i][j].rotlength < MAXVIRTUALSCAN && startid == j - 1) {
          ...
          if (svs[i][j].height - svs[i][startid].height < obstacleMinHeight &&
              svs[i][j].rotlength - svs[i][startid].rotlength >
                  -maxBackDistance) {
            double delta = (svs[i][j].rotlength - svs[i][startid].rotlength) /
                           (j - startid);
            int k;
            for (k = startid + 1; k < j; k++) {
              svs[i][k].rotlength = svs[i][j].rotlength - (j - k) * delta;
              svs[i][k].length =
                  svs[i][k].rotlength * c + svs[i][k].rotheight * s;
              svs[i][k].height =
                  -svs[i][k].rotlength * s + svs[i][k].rotheight * c;
            }
          }
          startid = j;
        }
...
void FastVirtualScan::getVirtualScan(double thetaminheight,
                                     double thetamaxheight, double maxFloor,
                                     double minCeiling, double passHeight,
      ...
      if (svs[i][j].rotlength == MAXVIRTUALSCAN) {
        ...
        if (denoiseflag) {
          if (startrotid + 1 == endrotid) {
            if (svs[i][j].rotlength - svs[i][candid].rotlength >=
                deltaminheight) {
              denoiseflag = 0;
              roadfilterflag = 1;
