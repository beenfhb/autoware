...
StateContext::getMultipleStates(uint64_t _state_num_set) {
  std::vector<BaseState *> ret;
  for (uint64_t mask = STATE_SUB_END; _state_num_set != 0 && mask != 0;
       mask >>= 1) {
...
bool StateContext::setCallbackUpdateFunc(const uint64_t &_state_num,
...
bool StateContext::setCallbackInFunc(const uint64_t &_state_num,
                                     const std::function<void(void)> &_f) {
  bool ret = false;
  if (getStateObject(_state_num)) {
...
bool StateContext::setCallbackOutFunc(const uint64_t &_state_num,
                                      const std::function<void(void)> &_f) {
  bool ret = false;
  if (getStateObject(_state_num)) {
...
bool StateContext::setCurrentState(BaseState *_state) {
  ...
  if (_state) {
    ...
    bool diff =
        isDifferentState(getStateNum(_state), HolderMap[getStateKind(_state)]);
...
std::string StateContext::getCurrentStateName(uint8_t _kind) {
  if (HolderMap[_kind]) {
    if (_kind == BEHAVIOR_STATE) {
      ...
      std::string ret = "";
      for (uint64_t mask = STATE_SUB_END; _current_state != 0 && mask != 0;
           mask >>= 1) {
