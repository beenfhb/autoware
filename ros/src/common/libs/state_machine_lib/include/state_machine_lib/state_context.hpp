...
private:
  ...
  void showStateMove(uint64_t _state_num) {
    std::cout << "State will be [" << StateStores[_state_num]->getStateName()
              << "]" << std::endl;
  }
...
public:
  StateContext(void) {
    ...
    StateStores[INITIAL_LOCATEVEHICLE_STATE] =
        LocateVehicleState::getInstance();
    StateStores[DRIVE_STATE] = DriveState::getInstance();
    StateStores[DRIVE_ACC_ACCELERATION_STATE] =
        DriveAccAccelerationState::getInstance();
    StateStores[DRIVE_ACC_DECELERATION_STATE] =
        DriveAccDecelerationState::getInstance();
    ...
    StateStores[DRIVE_ACC_STOPLINE_STATE] =
        DriveAccStopLineState::getInstance();
    StateStores[DRIVE_ACC_CRAWL_STATE] = DriveAccCrawlState::getInstance();
    StateStores[DRIVE_STR_STRAIGHT_STATE] =
        DriveStrStraightState::getInstance();
    ...
    StateStores[DRIVE_BEHAVIOR_LANECHANGE_LEFT_STATE] =
        DriveBehaviorLaneChangeLeftState::getInstance();
    StateStores[DRIVE_BEHAVIOR_LANECHANGE_RIGHT_STATE] =
        DriveBehaviorLaneChangeRightState::getInstance();
    StateStores[DRIVE_BEHAVIOR_TRAFFICLIGHT_RED_STATE] =
        DriveBehaviorTrafficLightRedState::getInstance();
    StateStores[DRIVE_BEHAVIOR_TRAFFICLIGHT_GREEN_STATE] =
        DriveBehaviorTrafficLightGreenState::getInstance();
    StateStores[DRIVE_BEHAVIOR_OBSTACLE_AVOIDANCE_STATE] =
        DriveBehaviorObstacleAvoidanceState::getInstance();
    StateStores[DRIVE_BEHAVIOR_STOPLINE_PLAN_STATE] =
        DriveBehaviorStoplinePlanState::getInstance();
    StateStores[DRIVE_BEHAVIOR_ACCEPT_LANECHANGE_STATE] =
        DriveBehaviorAcceptLanechangeState::getInstance();
  ...
  ~StateContext() { thread_loop = false; }
  ...
  bool setCallbackInFunc(const uint64_t &_state_num,
                         const std::function<void(void)> &_f);
  bool setCallbackOutFunc(const uint64_t &_state_num,
                          const std::function<void(void)> &_f);
  bool setCallbackUpdateFunc(const uint64_t &_state_num,
                             const std::function<void(void)> &_f);
  ...
  bool handleCurrentPose(double x, double y, double z, double roll,
