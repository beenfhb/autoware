...
watchdog::watchdog() {
  ...
  nh_.param<double>(ros::this_node::getName() + "/publish_rate", publish_rate_,
                    10);
  ...
  for (YAML::const_iterator it = config.begin(); it != config.end(); ++it) {
    if (it->first.as<std::string>() != ros::this_node::getName()) {
      ...
      diag_sub_[itr_node_name] = boost::make_shared<diag_subscriber>(
          itr_node_name, target_node["node_number"].as<int>());
      boost::shared_ptr<std::vector<diag_info>> target_node_watchdog_diag_info =
          boost::make_shared<std::vector<diag_info>>();
      for (const YAML::Node &error : target_node["errors"]) {
        if (error["category"].as<int>() == NODE_IS_DEAD) {
          diag_info info(error["num"].as<int>(),
...
void watchdog::write_error_code_csv_(YAML::Node config) {
  ...
  for (YAML::const_iterator it = config.begin(); it != config.end(); ++it) {
    ...
    for (const YAML::Node &error : target_node["errors"]) {
      ...
      if ((category == LOW_SUBSCRIBE_RATE) || (category == LOW_PUBLISH_RATE) ||
          (category == LOW_OPERATION_CYCLE)) {
        double threashold = error["threshold"].as<double>();
        std::string level = error["level"].as<std::string>();
        line = node_name + "," + std::to_string(node_number) + "," +
               std::to_string(num) + "," + name + "," +
               std::to_string(category) + "," + description + "," +
               std::to_string(threashold) + "," + level + "\n";
      } else {
        line = node_name + "," + std::to_string(node_number) + "," +
               std::to_string(num) + "," + name + "," +
               std::to_string(category) + "," + description + ",,\n";
  ...
  std::ofstream outputfile(
      std::string("/tmp/Autoware/Diag/error_list.csv").c_str());
  ...
  while (ros::ok()) {
    ...
    for (auto itr = watchdog_target_nodes_.begin();
      ...
      if (connection_status_[*itr] == false) {
        std::vector<diag_info> target_node_watchdog_diag_info =
...
void watchdog::update_connection_status_() {
  ...
  for (auto itr = watchdog_target_nodes_.begin();
       itr != watchdog_target_nodes_.end(); ++itr) {
    if (std::find(detected_nodes.begin(), detected_nodes.end(), *itr) !=
        detected_nodes.end()) {
