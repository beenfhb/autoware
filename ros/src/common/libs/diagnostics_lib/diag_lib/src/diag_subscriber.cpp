...
diag_subscriber::diag_subscriber(std::string target_node,
                                 int target_node_number)
    : target_node_(target_node), target_node_number_(target_node_number) {
  diag_sub_ = nh_.subscribe(target_node + "/diag", 1,
                            &diag_subscriber::callback_, this);
}
