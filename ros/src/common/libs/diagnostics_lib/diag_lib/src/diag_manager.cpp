...
diag_manager::diag_manager() {
  ...
  try {
    for (const YAML::Node &error : error_code_config_["errors"]) {
      diag_info info(error["num"].as<int>(), error["name"].as<std::string>(),
      ...
      if ((info.category == LOW_SUBSCRIBE_RATE) ||
        ...
        if (error["level"].as<std::string>() == "warn") {
          diag_info detail_info(error["num"].as<int>(),
                                error["name"].as<std::string>(),
                                error["category"].as<int>(),
                                error["description"].as<std::string>(),
                                error["threshold"].as<double>(), LEVEL_WARN);
          boost::shared_ptr<rate_checker> rate_checker_ptr =
              boost::make_shared<rate_checker>(RATE_CHECKER_BUFFER_LENGTH);
  ...
  boost::thread rate_check_thread(
      boost::bind(&diag_manager::check_rate_, this));
  ;
  enable_diag_ = true;
}
  ...
  for (auto diag_info_itr = diag_info_.begin();
    ...
    if (diag_info_itr->num == num) {
      if (diag_info_itr->threshold && diag_info_itr->level) {
        diag_info ret(diag_info_itr->num, diag_info_itr->name,
...
bool diag_manager::check_error_code(int requested_error_number,
