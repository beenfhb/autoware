...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr &scan) {
  ...
  for (std::vector<float>::const_iterator it = scan->ranges.begin();
    ...
    if (dist.data > *it && *it > scan->range_min && angle_tmp > angle_min &&
        angle_tmp < angle_max) {
      dist.data = *it;
...
int main(int argc, char **argv) {
  ...
  ROS_INFO_STREAM("Publishing estimated distance to target in topic "
                  << ros::this_node::getName() << "/" << dist_topic);
  ROS_INFO_STREAM("Publishing estimated angle to target in topic "
                  << ros::this_node::getName() << "/" << angle_topic);
  ...
  while (ros::ok()) {
    ...
    double eps =
        (double)1 / (double)100; // the epsilon time that is 'equal' in diff
    if (newMessage) {
