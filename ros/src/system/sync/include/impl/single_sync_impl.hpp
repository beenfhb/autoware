...
extern "C" double fabs_time_diff(std_msgs::Header *timespec1,
...
SingleSynchronizer<T1, T2, T3>::SingleSynchronizer(const std::string sub1_topic,
  ...
  type1_sub_ =
      nh.subscribe(sub1_topic, 2, &SingleSynchronizer::type1_callback, this);
  type2_sub_ =
      nh.subscribe(sub2_topic, 2, &SingleSynchronizer::type2_callback, this);
  req_sub_ =
      nh.subscribe(req_topic, 2, &SingleSynchronizer::req_callback, this);

  type1_pub_ = nh.advertise<T1>(ns + pub1_topic, 5);
  type2_pub_ = nh.advertise<T2>(ns + pub2_topic, 5);
  sync_time_diff_pub_ =
      nh.advertise<std_msgs::Float64>("/" + ns + "/time_diff", 5);
}
...
template <typename T1, typename T2, typename T3>
void SingleSynchronizer<T1, T2, T3>::publish_msg(T1 *p_type1_buf,
                                                 T2 *p_type2_buf) {
#ifdef PRINT
  ROS_ERROR("published");
#endif
...
bool SingleSynchronizer<T1, T2, T3>::publish() {
  ...
  if (buf_flag_) {
    ...
    if (get_time(&(type1_ringbuf_.front().header)) >=
        get_time(&(type2_ringbuf_.front().header))) {
      ...
      if (type1_ringbuf_.size() == 1) {
        ...
        if (is_req_ == true) {
          buf_flag_ = false;
          is_req_ = false;
          if (fabs_time_diff(&p_type1_buf_->header, &p_type2_buf_->header) ==
              0.0) {
        ...
        for (it++; it != type1_ringbuf_.end(); it++) {
          if (fabs_time_diff(&(type2_ringbuf_.front().header),
                             &((it - 1)->header)) <
    ...
    // image_obj_ranged < image_raw
      ...
      for (it++; it != type2_ringbuf_.end(); it++) {
        if (fabs_time_diff(&(type1_ringbuf_.front().header),
                           &((it - 1)->header)) <
...
template <typename T1, typename T2, typename T3>
void SingleSynchronizer<T1, T2, T3>::type1_callback(
    const typename T1::ConstPtr &type1_msg) {
#ifdef PRINT
...
template <typename T1, typename T2, typename T3>
void SingleSynchronizer<T1, T2, T3>::type2_callback(
    const typename T2::ConstPtr &type2_msg) {
#ifdef PRINT
...
template <typename T1, typename T2, typename T3>
void SingleSynchronizer<T1, T2, T3>::req_callback(
    const typename T3::ConstPtr &req_msg) {
  is_req_ = true;
#ifdef PRINT
