...
void publish_msg(autoware_msgs::ImageObj *p_image_obj_buf,
                 autoware_msgs::PointsImage *p_vscan_image_buf) {
  ROS_INFO("publish");
...
bool publish() {
  if (buf_flag) {
    ...
    if (get_time(&(image_obj_ringbuf.front().header)) >=
        get_time(&(vscan_image_ringbuf.front().header))) {
      p_vscan_image_buf = &(vscan_image_ringbuf.front());
      boost::circular_buffer<autoware_msgs::ImageObj>::iterator it =
          image_obj_ringbuf.begin();
      if (image_obj_ringbuf.size() == 1) {
        ...
        for (it++; it != image_obj_ringbuf.end(); it++) {
          if (fabs_time_diff(&(vscan_image_ringbuf.front().header),
                             &((it - 1)->header)) <
              fabs_time_diff(&(vscan_image_ringbuf.front().header),
                             &(it->header))) {
    ...
    // image_obj < vscan_image
      ...
      boost::circular_buffer<autoware_msgs::PointsImage>::iterator it =
          vscan_image_ringbuf.begin();
      ...
      for (it++; it != vscan_image_ringbuf.end(); it++) {
        if (fabs_time_diff(&(image_obj_ringbuf.front().header),
                           &((it - 1)->header)) <
            fabs_time_diff(&(image_obj_ringbuf.front().header),
                           &(it->header))) {
...
void *thread(void *args) {
  ...
  ros::Subscriber image_obj_ranged_sub = nh_rcv.subscribe(
      "obj_car/image_obj_ranged", 5, image_obj_ranged_callback);
  while (nh_rcv.ok()) {
#if 1
    ROS_INFO("debug1");
#endif
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber image_obj_sub =
      nh.subscribe("obj_car/image_obj", 1, image_obj_callback);
  ros::Subscriber vscan_image_sub =
      nh.subscribe("vscan_image", 1, vscan_image_callback);
  image_obj__pub =
      nh.advertise<autoware_msgs::ImageObj>("obj_car/image_obj_", 5);
  vscan_image__pub =
      nh.advertise<autoware_msgs::PointsImage>("vscan_image_", 5);
