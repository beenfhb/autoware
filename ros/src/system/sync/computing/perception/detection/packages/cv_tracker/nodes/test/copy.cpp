...
bool obj_person__image_obj_flag;
bool obj_car__image_obj_flag;
bool points_image_flag;
bool vscan_image_flag;

/* ----function---- */
double fabs_time_diff(std_msgs::Header *timespec1,
                      std_msgs::Header *timespec2) {
  double time1 = (double)timespec1->stamp.sec +
                 (double)timespec1->stamp.nsec / 1000000000L;
  double time2 = (double)timespec2->stamp.sec +
                 (double)timespec2->stamp.nsec / 1000000000L;

  return fabs(time1 - time2);
}

double get_time(const std_msgs::Header *timespec) {
  return (double)timespec->stamp.sec +
         (double)timespec->stamp.nsec / 1000000000L;
}
...
void publish_msg(sensor_msgs::Image *p_image_raw_buf,
                 sensor_msgs::PointCloud2 *p_points_raw_buf) {
  ROS_INFO("publish");
...
bool publish() {
  if (buf_flag) {
    ...
    if (get_time(&(image_raw_ringbuf.front().header)) >=
        get_time(&(points_raw_ringbuf.front().header))) {
      p_points_raw_buf = &(points_raw_ringbuf.front());
      boost::circular_buffer<sensor_msgs::Image>::iterator it =
          image_raw_ringbuf.begin();
      if (image_raw_ringbuf.size() == 1) {
        ...
        for (it++; it != image_raw_ringbuf.end(); it++) {
          if (fabs_time_diff(&(points_raw_ringbuf.front().header),
                             &((it - 1)->header)) <
              fabs_time_diff(&(points_raw_ringbuf.front().header),
                             &(it->header))) {
    ...
    // image_raw < points_raw
      ...
      boost::circular_buffer<sensor_msgs::PointCloud2>::iterator it =
          points_raw_ringbuf.begin();
      ...
      for (it++; it != points_raw_ringbuf.end(); it++) {
        if (fabs_time_diff(&(image_raw_ringbuf.front().header),
                           &((it - 1)->header)) <
            fabs_time_diff(&(image_raw_ringbuf.front().header),
                           &(it->header))) {
...
void obj_person__image_obj_callback(
...
void obj_car__image_obj_callback(
    const autoware_msgs::ImageObj::ConstPtr &image_obj_msg) {
...
void *thread(void *args) {
  ...
  ros::Subscriber obj_person__image_obj_sub = nh_rcv.subscribe(
      "/obj_person/image_obj", 5, obj_person__image_obj_callback);
  ros::Subscriber obj_car__image_obj_sub =
      nh_rcv.subscribe("/obj_car/image_obj", 5, obj_car__image_obj_callback);
  ros::Subscriber points_image_sub =
      nh_rcv.subscribe("/points_image", 5, points_image_callback);
  ros::Subscriber vscan_image_sub =
      nh_rcv.subscribe("/vscan_image", 5, vscan_image_callback);
  ...
  while (nh_rcv.ok()) {
    ...
    if ((obj_person__image_obj_flag == prev_obj_person__image_obj_flag) &&
        ...
        (points_image_flag == prev_points_image_flag ||
         prev_vscan_image_flag == vscan_image_flag)) {
      ROS_INFO("timeout");
      if (!publish()) {
        /* when to publish is failure, republish */
