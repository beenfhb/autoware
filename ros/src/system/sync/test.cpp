...
void publish_msg(cv_tracker::obj_label *p_obj_label_buf,
                 autoware_msgs::Centroids *p_cluster_centroids_buf) {
  ROS_INFO("publish");
...
bool publish() {
  if (buf_flag) {
    ...
    if (get_time(&(obj_label_ringbuf.front().header)) >=
        get_time(&(cluster_centroids_ringbuf.front().header))) {
      p_cluster_centroids_buf = &(cluster_centroids_ringbuf.front());
      boost::circular_buffer<cv_tracker::obj_label>::iterator it =
          obj_label_ringbuf.begin();
      if (obj_label_ringbuf.size() == 1) {
        ...
        for (it++; it != obj_label_ringbuf.end(); it++) {
          if (fabs_time_diff(&(cluster_centroids_ringbuf.front().header),
                             &((it - 1)->header)) <
              fabs_time_diff(&(cluster_centroids_ringbuf.front().header),
                             &(it->header))) {
    ...
    // obj_label < cluster_centroids
      ...
      boost::circular_buffer<autoware_msgs::Centroids>::iterator it =
          cluster_centroids_ringbuf.begin();
      ...
      for (it++; it != cluster_centroids_ringbuf.end(); it++) {
        if (fabs_time_diff(&(obj_label_ringbuf.front().header),
                           &((it - 1)->header)) <
            fabs_time_diff(&(obj_label_ringbuf.front().header),
                           &(it->header))) {
...
void obj_pose_callback(
    const visualization_msgs::MarkerArray::ConstPtr &obj_pose_msg) {
  pthread_mutex_lock(&mutex);
  obj_pose_flag = true;
...
void *thread(void *args) {
  ...
  ros::Subscriber obj_pose_sub =
      nh_rcv.subscribe("obj_pose", 5, obj_pose_callback);
  while (nh_rcv.ok()) {
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber obj_label_sub =
      nh.subscribe("/obj_car/obj_label", 1, obj_label_callback);
  ros::Subscriber cluster_centroids_sub =
      nh.subscribe("/cluster_centroids", 1, cluster_centroids_callback);
  obj_label__pub =
      nh.advertise<cv_tracker::obj_label>("/obj_car/obj_label_", 5);
  cluster_centroids__pub =
      nh.advertise<autoware_msgs::Centroids>("/cluster_centroids_", 5);
  while (!buf_flag) {
    ros::spinOnce();
    usleep(100000);
  }
