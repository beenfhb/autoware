...
public:
  ...
  /// Manipulation prevention. If active, shadowing of field parts may trigger
  /// an event.
...
private:
  ...
  double m_maxRadialCorridor; ///< [m]; max radial corridor (only active if
                              ///< distance dependency is active)
  ManipulationPrevention m_manipulationPrevention;
  double m_blankingSize; ///< [m]; minimum object size
  double m_minFieldExp;  ///< [m]; minimum radial distance between field start
                         ///< and end point
  UINT16
      m_fieldNumber; ///< index of field which is connected to evaluation case
