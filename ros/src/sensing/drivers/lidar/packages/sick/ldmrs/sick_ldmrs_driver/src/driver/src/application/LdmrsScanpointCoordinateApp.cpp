...
//
// Demo application. Receives scans, and searches each layer for the point at a
// certain horizontal scan angle. Adds up some scans and then prints the
// distance and coordinates of these points. This demo shows the scanpoint
// coordinates of scanner layers, e.g. the z coordinates. It can be used to
// verify the calculation of the coordinates. Also shows how to work with the
// mirror sides and 4- and 8-layer-scanners.
//
// In the constructor, set the angle and number of scans to the desired values.
// Note that scans must be enabled in the MRS device in order for this
// application to work.
//
...
LdmrsScanpointCoordinateApp::LdmrsScanpointCoordinateApp(Manager *manager) {
  ...
  // Number of scans to build the statistics with. Note that for 8-layer
  // scanners, the number of points in each layer will be half of this value as
  // the scanner needs two scans for a complete 8-layer-scan.
  ...
  printInfoMessage("LdmrsScanpointCoordinateApp constructor done.",
                   m_beVerbose);
}
...
void LdmrsScanpointCoordinateApp::setData(BasicData &data) {
  ...
  printInfoMessage("LdmrsScanpointCoordinateApp::setData(): Done.",
                   m_beVerbose);
}
...
void LdmrsScanpointCoordinateApp::calcAndShowStatistics() {
  ...
  printInfoMessage("Statistics for " + toString(m_numScansToCount) +
                       " scans at angle " +
                       toString(m_wantedHorzAngle * rad2deg, 1) + " degrees.",
                   true);
...
bool LdmrsScanpointCoordinateApp::isFourLayerScanner(Scan *scan) {
  printInfoMessage(
      "LdmrsScanpointCoordinateApp::isFourLayerScanner: Beam tilt=" +
          toString(fabs(scan->getScannerInfos().at(0).getBeamTilt() * rad2deg),
                   1) +
          " degrees.",
      m_beVerbose);
...
void LdmrsScanpointCoordinateApp::processScan(Scan *scan) {
  ...
  for (UINT8 layer = 0; layer < 4; layer++) {
    ...
    if (success == true) {
      //			printInfoMessage("LdmrsScanpointCoordinateApp::processScan:
      //Point found, hAngle is " + toString(point.getHAngle() * rad2deg, 1) + 			"
      //degrees in layer " + toString(layer) + ".", true);
    } else {
      printInfoMessage("LdmrsScanpointCoordinateApp::processScan: No point "
                       "found for hAngle " +
                           toString(m_wantedHorzAngle * rad2deg, 1) +
                           " degrees in layer " + toString(layer) + ".",
                       true);
    }
...
//
// Searches the scan for the best matching scanpoint in the given layer and
// close to the given horizontal scan angle. If no point is found within 2
// degrees around the wanted angle, no point is returned. Returns true if a
// point was found, false if not.
//
bool LdmrsScanpointCoordinateApp::getClosestScanpoint(const UINT8 layer,
  ...
  for (i = 0; i < scan->getNumPoints(); i++) {
    ...
    if (currentPoint.getLayer() == layer) {
      //			printInfoMessage("Checking angle " +
      //toString(currentPoint.getHAngle() * rad2deg, 1) + " against wanted " +
      //								toString(horzAngle
      //* rad2deg, 1) + ".", true);
      if (fabs(currentPoint.getHAngle() - horzAngle) <
          fabs(bestPoint.getHAngle() - horzAngle)) {
        bestPoint = currentPoint;
        //				printInfoMessage("Best point yet!",
        //true);
      }
    }
  }
