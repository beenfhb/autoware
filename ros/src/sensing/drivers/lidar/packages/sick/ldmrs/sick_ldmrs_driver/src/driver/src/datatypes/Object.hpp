 ...
 * classification.
 *
 * \image html objectcoords.png "Tracked object at position A, Course Angle Psi,
 * Velocity vector vh, and Size sx, xy"
 *
...
public:
  ...
  /** (Usually Unused.) Returns the velocity vector ("Geschwindigkeitsvektor")
   * of this object in [meter/seconds], relative to our vehicle's coordinate
   * system. Note: The currently implemented tracking will always
   * track only the absolute velocity; hence, this field
   * relativeVelocity will be unset and simply be zero (or some other irrelevant
   * values). */
  ...
  const Point2D &getRelativeVelocitySigma() const {
    return m_relativeVelocitySigma;
  }
  void setRelativeVelocitySigma(const Point2D &v) {
    m_relativeVelocitySigma = v;
  }
  ...
  /** Sets the velocity vector as absolute value. Note: This also updates
   * setMaxAbsoluteVelocity() accordingly. */
  ...
  const Point2D &getAbsoluteVelocitySigma() const {
    return m_absoluteVelocitySigma;
  }
  ...
  void getObjectBoxVarCovar(double &var_x, double &var_y,
                            double &covar_xy) const;
  ...
  double getNormalizedMeanPointDist() const {
    return m_normalizedMeanPointDist;
  }
  ...
  /** Returns a human-readable form of the content of this object (for debugging
   * output) The actual output is merely a conversion to string of the return
   * value of toConfigValues().
   */
  ...
  /// Returns the given classification value as a string with the integer number
  /// included.
  static std::string
  objectClassificationToStringWithNum(ObjectClassification v);
  ...
  static Object::ObjectClassification
  stringToObjectClassification(const std::string &s);
...
private:
  ...
  UINT32 m_objectAge;       ///< number of scans in which this object has been
                            ///< tracked, or instead time?
  UINT16 m_hiddenStatusAge; ///< Counts how long the object has not been
                            ///< observed but only predicted.
  Time m_timestamp; ///< Time of when the center point of this object was
                    ///< observed.

  ObjectClassification m_classification; ///< The object class that is most
                                         ///< likely for this object.
  UINT32 m_classificationAge; ///< Counts how long the object has been
                              ///< classified in the current classification.
  double
      m_classificationQuality; ///< The quality of the current classification.

  Point2D m_centerPoint; ///< Center point of object rectangle, given in Vehicle
                         ///< coordinate system.
  Point2D m_centerPointSigma;
  double m_courseAngle;       ///< named by ISO 8855; also called Orientation or
                              ///< Heading [rad]
  double m_courseAngleSigma;  // in [rad]
  Point2D m_relativeVelocity; ///< Velocity of this object [meter/seconds],
                              ///< relative to the vehicle coordinate system.
  Point2D m_relativeVelocitySigma;
  Point2D m_absoluteVelocity; ///< Velocity of this object [meter/seconds] as
                              ///< absolute velocity; the orientation is
                              ///< relative to the vehicle coordinate system.
  Point2D m_absoluteVelocitySigma;

  Point2D m_objectBox; ///< The object's length and width as a rectangle,
                       ///< relative to the object's coordinate system.
  Point2D m_objectBoxSigma;
  Point2D m_boundingBoxCenter; ///< Center of the bounding box.
  Point2D m_boundingBox; ///< A rectangle in parallel to the vehicle coordinate
                         ///< system (a paraxial rectangle) that contains
                         ///< (bounds) all of this object's points

  // These components are also proposed
  Point2D m_closestPoint; ///< The point of this object that is closest to the
                          ///< origin of the vehicle coordinate system.

  // This can also be calculated
  Polygon2D m_contourPoints; ///< A poly-line that describes the outline of the
                             ///< current object measurement.

  UINT64
      m_vehicleWLANid; ///< An identifier to be used by WLAN fusion algorithms.
  double m_objectHeight; ///< The height of this object in [m] (most probably
                         ///< received through WLAN data).
  double m_objectHeightSigma; ///< The standard deviation of the height of this
                              ///< object in [m] (most probably received through
                              ///< WLAN data).

  double m_objectMass; ///< The mass of this object in [kilogram] (as received
                       ///< e.g. through WLAN data)
...
//
//
class ObjectList : public std::vector<Object>, public BasicData {
public:
  ...
  const UINT32 getUsedMemory() const {
    return sizeof(*this) + size() * sizeof(Object);
  };

  // Get the timestamp of the object list. Typically it should be the
  // midTimestamp of the corresponding scan.
  const Time &getTimestamp() const { return m_timestamp; }

  // Set the timestamp of the object list. Typically it should be the
  // midTimestamp of the corresponding scan.
