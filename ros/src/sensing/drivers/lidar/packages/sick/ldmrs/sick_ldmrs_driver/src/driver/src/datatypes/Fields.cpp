...
void FieldSegmented::computePolygon() {
  ...
  for (FieldSegmentedPoints::const_iterator p = m_points.begin();
       p != m_points.end(); ++p) {
  ...
  if (startPoints.empty()) {
    ...
    for (Polygon2D::const_reverse_iterator r = startPoints.rbegin();
         r != startPoints.rend(); ++r) {
...
//
UINT32 FieldSegmented::getNumberOfPoints() { return m_points.size(); }

//
// Returns a copy of the points.
//
FieldSegmentedPoints FieldSegmented::getPoints() { return m_points; }
...
double FieldRectangle::getLength() const { return m_length; }

double FieldRectangle::getRefPointAngle() const { return m_refPointAngle; }

double FieldRectangle::getRefPointDist() const { return m_refPointDist; }

double FieldRectangle::getRotAngle() const { return m_rotAngle; }

double FieldRectangle::getWidth() const { return m_width; }
...
void FieldRectangle::setLength(double length) {
  if (length < 0.00001) {
    ...
    printError("FieldRectangle::setLength: Length must not be 0 or negative - "
               "aborting!");
    return;
  }
...
void FieldRectangle::setRotAngle(double rotAngle) {
  if ((rotAngle < -PI) || (rotAngle > PI)) {
    ...
    printWarning("FieldRectangle::setRotAngle: rotAngle is outside of the "
                 "limit [-PI, PI], did you forget deg-2-rad conversion?");
  }
...
void FieldRectangle::setWidth(double width) {
  if (width < 0.00001) {
    // Is this value mistakenly 0 or negative?
    printError("FieldRectangle::setWidth: Width must not be 0 or negative - "
               "aborting!");
    return;
  }
...
UINT16 FieldRadial::getFirstAngle() const { return m_firstAngle; }

UINT16 FieldRadial::getLastAngle() const { return m_lastAngle; }

UINT32 FieldRadial::getMaxDist() const { return m_maxDist; }

UINT32 FieldRadial::getMinDist() const { return m_minDist; }
...
void FieldRadial::setMaxDist(UINT32 maxDist) { this->m_maxDist = maxDist; }

void FieldRadial::setMinDist(UINT32 minDist) { this->m_minDist = minDist; }
...
double FieldDynamic::getMaxLength() const { return m_maxLength; }

double FieldDynamic::getSpeedMax() const { return m_speedMax; }

void FieldDynamic::setMaxLength(double maxLength) {
  this->m_maxLength = maxLength;
}

void FieldDynamic::setSpeedMax(double speedMax) { this->m_speedMax = speedMax; }
...
//
void Fields::add(FieldParameter *field) { m_fields.push_back(field); }
