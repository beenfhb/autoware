...
LDMRS::LDMRS(Manager *manager)
    : m_manager(manager), m_sopas(NULL), m_lux(NULL) {
  ...
  m_weWantScanData = true; // Enable or disable scan data reception via the MRS
                           // (=non-SOPAS) interface.
  m_weWantObjectData = false; // Enable or disable object data reception via the
                              // MRS (=non-SOPAS) interface.
  m_weWantFieldData = false; // Enable or disable protection field and eval case
                             // data reception via the SOPAS interface.
  m_weWantScanDataFromSopas =
      false; // Enable or disable scan data reception via the SOPAS interface.

  //	m_inputFileName = "/home/willhvo/Temp/raw_ldmrs_log.bin";		// File
  //name of input file. If this name is given, connects to file instead of TCP.
  m_inputFileName = "";
  m_ipAddress = "192.168.0.1"; // , "Network IP address where the device is
                               // located.", "0.0.0.0");
  m_luxPortNumber =
      12002; // Network port on which the legacy (non-sopas) device is contacted
             // (for scans and objects), typically 12002
  m_SopasPortNumber = 2111; // Network port on which the device is contacted,
                            // typically 2111 or 2112.
  ...
  //	m_scanAngleRes = 0.25;
  //	m_skipNumScans = 0;		// Use this value if the sensor scan
  //frequency is too high and if scans should be skipped.

  m_offsetX = 0.0;    // x-coordinate of sensor mounting position in the vehicle
                      // coordinate system.
  m_offsetY = 0.0;    // y-coordinate of sensor mounting position in the vehicle
                      // coordinate system
  m_offsetZ = 0.0;    // z-coordinate of sensor mounting position in the vehicle
                      // coordinate system
  m_yawAngle = 0.0;   // Yaw angle of sensor mounting position in the vehicle
                      // coordinate system ("left and right")
  m_pitchAngle = 0.0; // Pitch angle of sensor mounting position in the vehicle
                      // coordinate system ("up and down")
  m_rollAngle = 0.0;  // Roll angle of sensor mounting position in the vehicle
                      // coordinate system ("to the sides")
...
// Initialising.
//
// It would be nicer if this function would be called with all the device
// parameters, instead of setting them as module variables...
//
bool LDMRS::init() {
  ...
  m_lux =
      new LuxBase(m_manager, getSourceId(), longName, m_ipAddress,
                  m_luxPortNumber, m_scanFrequency, m_scanStartAngle,
                  m_scanEndAngle, m_offsetX, m_offsetY, m_offsetZ, m_yawAngle,
                  m_pitchAngle, m_rollAngle, m_beVerbose, m_inputFileName);

  printInfoMessage("LDMRS::init: Initializing the LuxBase object.",
                   m_beVerbose);
  if (m_inputFileName == "") {
    // TCP
  ...
  if (result == true) {
    // Success
    printInfoMessage("LDMRS::init: LuxBase was successfully initialized.",
                     m_beVerbose);
  } else {
    // Error
    printError("LDMRS::init: Failed to initialize LuxBase - Device init "
               "failed, aborting.");
    return false;
  }
  ...
  if (m_inputFileName == "") {
    // TCP
    result = m_lux->cmd_setNtpTimestamp(
        (UINT32)(t.seconds() + Time::secondsFrom1900to1970), 0);
    if (result == true) {
      // Success
      printInfoMessage("LDMRS::init: NTP timestamp was successfully set.",
                       m_beVerbose);
    } else {
      // Error
      printError("LDMRS::init: Failed to set NTP timestamp - Device init "
                 "failed, aborting.");
      return false;
    }
  }
...
void LDMRS::disconnectFunction() {
  printInfoMessage("LDMRS::disconnectFunction: LDMRS " + getDeviceName() +
                       " was disconnected.",
                   m_beVerbose);
...
//
void LDMRS::shutdown() {
  printInfoMessage("LDMRS::shutdown: called. Shutting down the LDMRS" +
                       getDeviceName() + ".",
                   m_beVerbose);
...
bool LDMRS::writeField(UINT16 fieldNum, const FieldParameter &para) {
  ...
  if (result == true) {
    printInfoMessage(
        "LDMRS::writeField: Login was successful, writing field now.",
        beVerboseHere);
    result = m_sopas->action_writeField(fieldNum, para);
  }

  if (result == true) {
    printInfoMessage("LDMRS::writeField: Field was written, logging out now.",
                     beVerboseHere);
...
bool LDMRS::writeEvalCases(const EvalCases &evalCases) {
  ...
  printInfoMessage("LDMRS::writeEvalCases: Called. Logging in now.",
                   beVerboseHere);
  result = m_sopas->action_login();

  if (result == true) {
    printInfoMessage(
        "LDMRS::writeEvalCases: Login was successful, writing eval cases now.",
        beVerboseHere);
  ...
  if (result == true) {
    printInfoMessage(
        "LDMRS::writeEvalCases: Eval cases were written, logging out now.",
        beVerboseHere);
    result = m_sopas->action_logout();
  }
...
bool LDMRS::flashSopasConfig() {
  ...
  if (result == true) {
    printInfoMessage("LDMRS::flashSopasConfig: Configuration was saved.",
                     beVerboseHere);
  } else {
    // Failure
...
bool LDMRS::setScanAngles(double startAngle, double endAngle) {
  ...
  if (result == true) {
    printInfoMessage("LDMRS::setScanAngles: scan angles were set.",
                     beVerboseHere);
  } else {
    // Failure
...
bool LDMRS::setSyncAngleOffset(double syncAngle) {
  ...
  if (result == true) {
    printInfoMessage("LDMRS::setSyncAngleOffset: sync angle offset was set.",
                     beVerboseHere);
  } else {
    // Failure
  ...
  printInfoMessage("LDMRS::setSyncAngleOffset: All done, leaving.",
                   beVerboseHere);
  return result;
}
...
bool LDMRS::setScanFrequency(double scanFreq) {
  ...
  if (result == true) {
    printInfoMessage("LDMRS::setScanFrequency: scan frequency was set.",
                     beVerboseHere);
  } else {
    // Failure
  ...
  printInfoMessage("LDMRS::setScanFrequency: All done, leaving.",
                   beVerboseHere);
  return result;
}

std::string LDMRS::getIpAddress() { return m_ipAddress; }

void LDMRS::setIpAddress(std::string ipAdress) { m_ipAddress = ipAdress; }
...
bool LDMRS::run() {
  if (m_lux == NULL) {
    printError("LDMRS::run: called, but pointer to internal data handler is "
               "NULL. Was init() called? Aborting!");
    return false;
  }
  ...
  // LUX-Part
  //
  printInfoMessage("LDMRS::run: Called. Run the LDMRS" + getDeviceName() + ".",
                   m_beVerbose);
  ...
  if (luxSuccess == true) {
    // Success
    printInfoMessage("LDMRS::run: Device " + getDeviceName() + " is running.",
                     m_beVerbose);
    m_lux->setOnScanReceiveCallbackFunction(LDMRS::onScanReceivedS,
                                            (void *)this);
  } else {
    // Fail
    printError("LDMRS::run: Failed to run device " + getDeviceName() +
               ", aborting!");
    return false;
  }
  ...
  //
  if ((m_inputFileName == "") &&
      ((m_weWantFieldData == true) || (m_weWantScanDataFromSopas == true))) {
    // TCP only
    bool sopasSuccess = true;
    printInfoMessage(
        "LDMRS::run: Now starting SOPAS part (eval cases) of device " +
            getDeviceName() + ".",
        m_beVerbose);

    // Initialize Sopas part
    m_sopas = new LdmrsSopasLayer(m_manager, getSourceId(), m_ipAddress,
                                  m_SopasPortNumber, m_weWantFieldData,
                                  m_weWantScanDataFromSopas, m_readOnlyMode);
    sopasSuccess = m_sopas->init(LDMRS::disconnectFunctionS, (void *)this);

    if (sopasSuccess == false) {
      printError("LDMRS::run: Failed to initialize SOPAS part of device " +
                 getDeviceName() + ", aborting!");
      stop();
      return false;
    }

    // Success, now run the device
    printInfoMessage("LDMRS::run: SOPAS part is initialized, now running it.",
                     m_beVerbose);
    sopasSuccess = m_sopas->run();
    if (sopasSuccess == false) {
      printError("LDMRS::run: Failed to run SOPAS part of device " +
                 getDeviceName() + ", aborting!");
      stop();
      return false;
    }

    printInfoMessage("LDMRS::run: SOPAS part is running, device " +
                         getDeviceName() + " is running, all done.",
                     m_beVerbose);
  }
...
//
bool LDMRS::stop() {
  printInfoMessage("LDMRS::stop: Called. Stopping the LDMRS" + getDeviceName() +
                       ".",
                   m_beVerbose);
...
// after / before every scan we have to post the current sensor state as
