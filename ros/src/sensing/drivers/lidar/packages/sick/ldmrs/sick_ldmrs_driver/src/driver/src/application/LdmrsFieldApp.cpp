...
void LdmrsFieldApp::setData(BasicData &data) {
  ...
  case Datatype_Scan:
        ...
        "Scan (" + ::toString(((Scan &)data).getNumPoints()) + " points)";
    {
      ...
      if (info != NULL) {
        ...
        printInfoMessage("LdmrsApp::setData(): Scan start time: " +
                             time.toString(),
                         m_beVerbose);
      }
    }
  ...
  printInfoMessage("LdmrsApp::setData(): Called with data of type " +
                       datatypeStr + " from ID " + sourceIdStr + ".",
                   m_beVerbose);
}

//
// Demo function that removes all fields by overwriting them with segmented
// fields, and setting them to invalid.
//
void LdmrsFieldApp::thread_removeAllEvalCases(devices::LDMRS *ldmrs) {
  bool beVerboseHere = m_beVerbose;
  beVerboseHere = true;

  printInfoMessage("LdmrsFieldApp::thread_removeAllEvalCases: Called.",
                   beVerboseHere);
  ...
  if (result == false) {
    // Failure
    printError("LdmrsFieldApp::thread_removeAllEvalCases: Failed to write "
               "empty EvalCase structure!");
  }

  printInfoMessage(
      "LdmrsFieldApp::thread_removeAllEvalCases: All done, leaving.",
      beVerboseHere);
}
...
void LdmrsFieldApp::thread_removeAllFields(devices::LDMRS *ldmrs) {
  ...
  printInfoMessage("LdmrsFieldApp::thread_removeAllFields: Called.",
                   beVerboseHere);
  ...
  field.setAngleScaleOffset(
      -1920); // -1920 = -60 deg (=60 deg due to other coordinate system)
  field.setComment("");
  field.setFieldName("");
  field.setEnableLayerFilter(false);
  field.setVersionNumber(1); // Version number is 1
  field.setFieldTypeIntern(
      FieldParameter::FieldTypeIntern_SEGMENTED); // Segmented = 2
  field.setFieldNumber(0);                        // 0 is an invalid field
  field.setField(NULL);                           // (FieldDescription*)

  for (UINT16 i = 0; i < 16; i++) {
    printInfoMessage("LdmrsFieldApp::thread_removeAllFields: Removing field " +
                         toString(i) + ".",
                     beVerboseHere);
  ...
  printInfoMessage("LdmrsFieldApp::thread_removeAllFields: All done, leaving.",
                   beVerboseHere);
}
...
void LdmrsFieldApp::thread_createEvalCase(devices::LDMRS *ldmrs) {
  ...
  printInfoMessage("LdmrsFieldApp::thread_createEvalCase(): Called.",
                   beVerboseHere);
  ...
  if (result == false) {
    // Failure
    printError("LdmrsFieldApp::thread_createEvalCase: Failed to write the "
               "EvalCase structure!");
  }

  printInfoMessage("LdmrsFieldApp::thread_createEvalCase: All done, leaving.",
                   beVerboseHere);
}
...
void LdmrsFieldApp::thread_createRectangularField(devices::LDMRS *ldmrs) {
  ...
  printInfoMessage("LdmrsFieldApp::thread_createRectangularField(): Called.",
                   beVerboseHere);
  ...
  field.setVersionNumber(1); // Version number is 1
  field.setFieldTypeIntern(
      FieldParameter::FieldTypeIntern_RECTANGLE); // Rectangle = 1
  ...
  if (result == false) {
    // Failure
    printError("LdmrsFieldApp::thread_createRectangularField(): Failed to "
               "write the field!");
  }

  printInfoMessage(
      "LdmrsFieldApp::thread_createRectangularField(): All done, leaving.",
      beVerboseHere);
}
...
void LdmrsFieldApp::changeThreadFunction(bool &endThread, UINT16 &waitTimeMs) {
  ...
  printInfoMessage("LdmrsFieldApp::changeThreadFunction(): started",
                   beVerboseHere);
  ...
  if (ldmrs == NULL) {
    ...
    printError("LdmrsFieldApp::changeThreadFunction(): Failed to read device "
               "pointer, aborting!");
    endThread = true;
    waitTimeMs = 0;
    return;
  ...
  //
  printInfoMessage(
      "LdmrsFieldApp::changeThreadFunction(): Now removing all eval cases.",
      beVerboseHere);
  ...
  //
  printInfoMessage(
      "LdmrsFieldApp::changeThreadFunction(): Now removing all fields.",
      beVerboseHere);
  ...
  //
  printInfoMessage(
      "LdmrsFieldApp::changeThreadFunction(): Now writing a rectangular field.",
      beVerboseHere);
  ...
  //
  printInfoMessage("LdmrsFieldApp::changeThreadFunction(): Now writing an eval "
                   "case for the rectangular field.",
                   beVerboseHere);
  ...
  //
  //	printInfoMessage("LdmrsFieldApp::changeThreadFunction(): Now flashing
  //the current field configuration.", beVerboseHere);
  ...
  endThread = true;
  waitTimeMs = 0;
  printInfoMessage("LdmrsFieldApp::changeThreadFunction(): All done, leaving.",
                   m_beVerbose);
}
