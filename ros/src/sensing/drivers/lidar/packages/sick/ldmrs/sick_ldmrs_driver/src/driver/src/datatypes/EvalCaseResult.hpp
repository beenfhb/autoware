...
//
// The EvalCaseResult class is a container for the current status of the
// configured EvalCases. Some scanners (such as the LDMRS) send such a structure
// with every scan, others (like the LMSxxx) send them only upon a change in any
// parameter value, e.g. if a field infringement happens.
//
