...
Polygon2D Line2D::toPolygon2D() const { return Polygon2D(first, second); }

Polygon2D Line2D::toPolygon2D(unsigned samplePoints) const {
  assert(samplePoints > 1);
  if (samplePoints <= 1)
    return Polygon2D(); // sanity check for when NDEBUG disables the assert()
                        // check
...
Line2D Line2D::getUnitVector() const {
  ...
  return Line2D(first, Point2D(first.getX() + diff.getX() / thislength,
...
Point2D Line2D::projectOntoLine(const Point2D &point) const {
  ...
  if (!fuzzyCompare(line_length, 0.0)) {
    ...
    double length_of_projection = dot_product(line_unitvec, vec_to_point);
...
double Line2D::distanceFromLineSegment(const Point2D &point) const {
  ...
  //    parameter r, which indicates P's position along AB, is computed by the
  //    dot product of AC and AB divided by the square of the length of AB:
  //
  //    (1)     AC dot AB
  //        r = ---------
  //            ||AB||^2
  ...
  const double s =
      ((ay - cy) * (bx - ax) - (ax - cx) * (by - ay)) / r_denomenator;
...
Line2D::IntersectingType Line2D::isIntersecting(const Line2D &other,
                                                Point2D *point) const {
  if (isZero() || other.isZero())
    return NotIntersecting;

  bool has_inside_intersection =
      line2d_intersect(first.getX(), first.getY(), second.getX(), second.getY(),
  ...
  if (is_vertical && other_is_vertical)
    ...
    intersectPoint.setXY(first.getX(), oa * first.getX() + other.first.getY() -
                                           oa * other.first.getX());
  } else if (other_is_vertical) {
    Point2D diff = getDiff();
    double a = diff.getY() / diff.getX();
    intersectPoint.setXY(other.first.getX(), a * other.first.getX() +
                                                 first.getY() -
                                                 a * first.getX());
  } else {
    Point2D otherdiff = other.getDiff();
    ...
    if (oa == ta) // parallel lines
      return NotIntersecting;
    double x = (-other.first.getY() + oa * other.first.getX() + first.getY() -
                ta * first.getX()) /
               (oa - ta);
...
Line2D Line2D::fromLinearRegression(const Polygon2D &points) {
  if (points.size() >= 2) {
    ...
    for (Polygon2D::const_iterator point = points.begin();
...
Point2D Line2D::getCenterPoint() const { return getP1() + (0.5 * getDiff()); }
