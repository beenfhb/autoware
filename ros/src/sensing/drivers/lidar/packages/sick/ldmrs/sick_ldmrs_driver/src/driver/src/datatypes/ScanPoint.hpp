...
public:
  ...
  enum Flags {
    ...
    FlagThresholdSwitching =
        0x0010, ///< Scan point was measured using the higher threshold (since
                ///< FGPA version 8.0.08)
    ///< Old: Scan point was measured in a shot with threshold switching
    ///< (earlier FPGA versions)
    FlagReflector =
        0x0020, ///< EPW of scan point is high enough to be a reflector
    FlagLeftCovered = 0x0100,  ///< Left neighbour point may be covered
    FlagRightCovered = 0x0200, ///< Right neighbour point may be covered
    FlagBackground = 0x0400,   ///< Point has been recognized as background and
                               ///< should not be used in the tracking anymore
    FlagMarker = 0x0800,       ///< Point is "marked" (see above)
    FlagTransparent = 0x1000,  ///< There is at least one more echo behind this
                               ///< scan point (B or C echo)
    MaskInvalid = FlagGround | FlagDirt | FlagRain |
                  FlagBackground, ///< All flags of invalid scan points
...
public:
  ...
  double getX() const {
    return m_x;
  } // Returns the Cartesian x-coordinate, in [m]
  double getY() const { return m_y; }
  double getZ() const { return m_z; }

  double getDist() const {
    return m_dist;
  } // Returns the polar distance coordinate, in [m]
  double getHAngle() const {
    return m_hAngle;
  } // Returns the polar azimuth (horizontal) angle, in [rad]
  double getVAngle() const {
    return m_vAngle;
  } // Returns the polar elevation (vertical) angle, in [rad]

  double getEchoWidth() const {
    return m_echoWidth;
  }                                           // Returns the echo pulse width
  ...
  UINT8 getLayer() const {
    return m_channel;
  } // Returns the channel ("Layer", "Scan-Ebene")
  UINT8 getEchoNum() const {
    return m_subchannel;
  } // Returns the recording subchannel ("Echo-Nummer")
  ...
  /// Checks if there is at least one more echo behind this scan point (B or C
  /// echo)
  bool isTransparent() const { return m_flags & FlagTransparent; }

  /** \brief Returns the distance between this and the given other scanpoint, in
   * [m].
   *
  ...
  static double getDistanceBetweenScanpoints(const ScanPoint &pt1,
                                             const ScanPoint &pt2);

  void setPoint3D(const Point3D &pt); // Set the Cartesian point coordinates
  void setCartesian(double x, double y, double z);
  void setPolar(double dist, double hAngle,
                double vAngle); // Set the polar point coordinates
  ...
  void setSourceId(UINT8 id) {
    m_sourceId = id;
  } // Set the device ID of the source scanner

  void setLayer(UINT8 ch) {
    m_channel = ch;
  } // Set the recording layer / channel
