...
class Polygon3D : public std::vector<Point3D>, public BasicData {
public:
  ...
  inline virtual const UINT32 getUsedMemory() const {
    return ((sizeof(*this)) + sizeof(Point3D) * size());
  };
   ...
   * This function is intended to create a path with a more or less constant
   * width. The polygon should not have succeeding points with reversing
   * direction (zick-zack). This function operates in the x-y-plane only.
   */
  ...
  std::istream &
  read(std::istream &is,
       UINT32 version); ///< Reads this object from an input stream
  void read(const BYTE *&buf,
            UINT32 version); ///< Reads this object from a memory buffer
  std::ostream &
  write(std::ostream &os,
        UINT32 version) const; ///< Writes this object to an output stream
  void write(BYTE *&buf,
             UINT32 version) const; ///< Writes this object to a memory buffer
                                    ///< and increments the buffer pointer
  std::streamsize
  getSerializedSize(UINT32 version) const; ///< Returns the number of bytes this
                                           ///< object needs in serialized form
  std::string
  toString() const; ///< Text output for debugging; opposite of fromString().
...
std::ostream &operator<<(std::ostream &os,
                         const Polygon3D &point); ///< Text output for debugging
