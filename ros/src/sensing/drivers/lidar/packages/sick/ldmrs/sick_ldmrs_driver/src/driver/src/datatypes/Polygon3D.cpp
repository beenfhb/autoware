...
boost::char_separator<char>(Polygon2D::getSeparatorCharacters())); for
...
listOfTokens.end(); ++iter)
        {
                ...
                if (iter == listOfTokens.end())
                        throw std::runtime_error("When parsing a Polygon3D
string, the last point only has a x component."); float y =
boost::lexical_cast<float>(*iter);
                ++iter;
                if (iter == listOfTokens.end())
                        throw std::runtime_error("When parsing a Polygon3D
string, the last point only has a x and y component."); float z =
boost::lexical_cast<float>(*iter); this->push_back(Point3D(x, y, z));
        }
}
*/
...
Polygon3D Polygon3D::getMovedPath(double dist) const {
  ...
  // The first segment is also handles in the loop. The needed but not existing
  // preceeding segment is assumed to have the same direction.
  ...
  for (i = 1; i < size(); i++) {
    ...
    // The direction to the new path point (new edge)  has the direction of the
    // half angle between both segments.
