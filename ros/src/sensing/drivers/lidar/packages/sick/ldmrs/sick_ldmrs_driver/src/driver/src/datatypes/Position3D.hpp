...
public:
  ...
  inline const double operator()(const UINT16 rowNr,
                                 const UINT16 columnNr) const;
...
// Fuer x = m(r,c);
// Keine Pruefung des Index wg. Laufzeit
inline const double Matrix::operator()(const UINT16 rowNr,
                                       const UINT16 columnNr) const {
...
public:
   ...
   */
  Position3D(value_type yaw, value_type pitch, value_type roll, value_type x,
  ...
  bool transformToVehicle(Point3D *pt); // double *dXPos, double *dYPos, double
                                        // *dZPos, const CRect& r)
  ...
  /// Returns the x/y/z components of this class, converted into a Point3D
  /// object.
