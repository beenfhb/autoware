...
public:
  ...
  /**
   * \note The size components must be non-negative, otherwise an assertion will
   * fail.
   *
   * \param center The center point
   * \param size The size of the box in the box' coordinate system. Must be
   * non-negative.
   *
   ...
   *
   * \param x_size The size of the box in X-direction in the box' coordinate
   * system. Must be non-negative. \param y_size The size of the box in
   * Y-direction in the box' coordinate system. Must be non-negative.
   *
   * \param rotation The rotation of the box' coordinate system
   * around its center point in [radians]. Must be in the interval
   * [-pi,pi], which can be obtained by using
   * ::normalizeRadians().
   */
  Box2D(value_type x_center, value_type y_center, value_type x_size,
        value_type y_size, value_type rotation = 0.0);
  ...
  /** \brief Returns a Box that is copied from this one but with its center
   * point moved
   */
  ...
  /// Returns the mean distance from the outline of this Box2D to all points of
  /// the point vector.
  /**
   ...
   */
  Point2D::value_type
  distanceFromOutline(const std::vector<Point2D> &points) const;

  /// Returns the mean distance from the outline of this Box2D to all points of
  /// the given point iterator range.
  /**
   * For each given point, this function calculates the minimum
   * distance over the distances from the given point to each of the
   * four outside lines of this Box2D and returns that minimum
   ...
   */
  Point2D::value_type
  distanceFromOutline(const std::vector<Point2D>::const_iterator &begin,
...
inline bool operator==(const Box2D &b1, const Box2D &b2) {
  return (b1.m_center == b2.m_center) && (b1.m_size == b2.m_size) &&
