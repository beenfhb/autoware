...
public:
  ...
  // Returns the start timestamp of the scan received by this scanner (in terms
  // of the host computer clock)
  const Time &getStartTimestamp() const { return m_scanStartTime; }

  /// Returns the end timestamp of the scan (in terms of the host computer
  /// clock)
  const Time &getEndTimestamp() const { return m_scanEndTime; }

  /// Set the start and end timestamp of the scan received by this scanner (in
  /// terms of the host computer clock)
  void setTimestamps(const Time &start, const Time &end);

  /// Returns the start timestamp of this scan as given by the measuring device
  /// (which most probably has a clock offset)
  //	const boost::posix_time::ptime& getStartDeviceTimestamp() const { return
  //m_scanStartDeviceTime; }

  /// Returns the end timestamp of this scan as given by the measuring device
  /// (which most probably has a clock offset)
  //	const boost::posix_time::ptime& getEndDeviceTimestamp() const { return
  //m_scanEndDeviceTime; }

  /// Set the start and end timestamp of the scan as given by the measuring
  /// device (which most probably has a clock offset)
  //	void setDeviceTimestamps (const boost::posix_time::ptime& start, const
  ...
  // Returns true if the scan was scanned with the rear mirror side. In 8-layer
  // scanners, this side is usually tilted upward. This information is part of
  // the processing flags.
  bool isRearMirrorSide();

  // Returns true if the scan was scanned with the front mirror side. In 8-layer
  // scanners, this side is usually tilted downward. This information is part of
  // the processing flags.
  ...
  //   currentBeamTilt = getBeamTilt() * std::sqrt(2) * std::sin(0.25*::PI -
  //   0.5*hAngle)
