...
public:
  ...
  enum IntersectingType {
    NotIntersecting ///< The lines are not intersecting, i.e. they are parallel
                    ///< or zero
    ,
    LineIntersecting ///< The lines are intersecting within their line segments
    ,
    OutsideIntersecting ///< The lines are intersecting, but outside of their
                        ///< line segments
  };

  /// Empty constructor
  Line2D() : first(), second() { m_datatype = Datatype_Line2D; }

  /// Constructor with two points
  Line2D(const Point2D &a, const Point2D &b) : first(a), second(b) {
