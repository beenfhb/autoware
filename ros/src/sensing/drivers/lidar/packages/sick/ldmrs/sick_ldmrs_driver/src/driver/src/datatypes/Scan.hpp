...
public:
  ...
  enum ScanFlags {
    FlagVehicleCoordinates =
        0x00000800 ///< Bit 11: Scanpoint coordinate system; 0 = scanner
                   ///< coordinates, 1 = vehicle / reference coordinates
  };
...
public:
  // Constructor
  Scan(size_type maxPoints =
           5280); // 5280 = Maxpoints with 1/8deg resolution, for 50- -60deg
  ...
  // Returns the number of points in this scan. The same number can be retrieved
  // by size().
  ...
  // Returns the total size of the current scan object, in [bytes]. This value
  // is the actual memory usage, including all sub-structures. Intended usage:
  // Find out how much memory will be used when storing this object in a buffer.
  //
  // Note that the returned value is an estimation that may not match up to the
  // last byte...
  ...
  //	bool isGroundLabeled()	 const { return ((m_flags & FlagGroundLabeled)
  //!= 0); }

  // Returns true if the dirt detection ran on this scan.
  //	bool isDirtLabeled() 	 const { return ((m_flags & FlagDirtLabeled) !=
  //0); }

  // Returns true if the background detection ran on this scan.
  //	bool isBackgroundLabeled() const { return ((m_flags &
  //FlagBackgroundLabeled) != 0); }

  // Returns true if the scan points are given in vehicle coordinates, or false
  // if the scan points are given in the Laserscanner coordinate system.
  //	bool isVehicleCoordinates() const { return ((m_flags &
  //FlagVehicleCoordinates) != 0); }
  ...
  //	void setGroundLabeled(bool isGroundLabeled = true); ///< Set the Scan
  //has ground labeled. 	void setDirtLabeled(bool isDirtLabeled = true); ///< Set
  //the Scan has dirt labeled. 	void setRainLabeled(bool isRainLabeled = true);
  /////< Set the Scan has rain labeled. 	void setCoverageLabeled(bool
  //isCoverageLabeled = true); ///< Set the Scan has coverage labeled. 	void
  //setBackgroundLabeled(bool isBackgroundLabeled = true); ///< Set the Scan has
  //background labeled. 	void setReflectorLabeled(bool isReflectorLabeled =
  //true); ///< Set the Scan has reflector labeled.
  ...
  // Returns the n-th scan point (read/write). No range checking to avoid
  // runtime costs.
  reference operator[](size_type n) { return m_points[n]; }

  // Returns the n-th scan point (read only). No range checking to avoid runtime
  // costs.
  ...
  void addPolarOffset(double distOffset, double hAngleOffset,
                      double vAngleOffset);
