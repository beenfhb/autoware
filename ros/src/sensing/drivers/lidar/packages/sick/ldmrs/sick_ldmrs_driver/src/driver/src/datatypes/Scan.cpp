...
//				 constructor. This can be increased later by calling
//reserve().
//
Scan::Scan(size_type maxPoints) : m_flags(0), m_scanNumber(0x0000), m_points() {
...
Scan::Scan(const Scan &other) : m_points() { copy(other); }

// Estimate the memory usage of this object
const UINT32 Scan::getUsedMemory() const {
  return (sizeof(*this) + (getNumPoints() * sizeof(ScanPoint)) +
...
Scan &Scan::operator=(const Scan &other) { return copy(other); }
...
bool Scan::operator==(const Scan& other) const
{
        return
                ...
                && (!m_scannerInfos.empty()
                        || ((m_startAngle        == other.m_startAngle
                                 || (isNaN(m_startAngle) &&
isNaN(other.m_startAngle)))
                                && (m_endAngle       == other.m_endAngle
                                        || (isNaN(m_endAngle) &&
isNaN(other.m_endAngle)))
                                && m_scannerStatus  == other.m_scannerStatus
                                && m_deviceID       == other.m_deviceID
                                && m_scannerType    == other.m_scannerType
                           ));
...
void Scan::resize(size_type new_size, const ScanPoint &default_point) {
  if (size_type((UINT16)new_size) != new_size) {
    throw std::out_of_range("Scan::resize was called with new size larger than "
                            "what fits into UINT16: " +
                            toString(new_size) + " but only 65536 is allowed!");
  }
...
void Scan::reserve(size_type new_capacity) {
  if (size_type((UINT16)new_capacity) != new_capacity) {
    throw std::out_of_range("Scan::reserve was called with new capacity larger "
                            "than what fits into UINT16: " +
...
void Scan::addCartesianOffset(double offsetX, double offsetY, double offsetZ) {
  for (PointList::iterator point = getPointListBegin();
       point != getPointListEnd(); point++)
    point->addCartesianOffset(offsetX, offsetY, offsetZ);
}

void Scan::addPolarOffset(double distOffset, double hAngleOffset,
                          double vAngleOffset) {
  for (PointList::iterator point = getPointListBegin();
       point != getPointListEnd(); point++)
...
void Scan::setScannerInfos(const ScannerInfoVector &v) {
  ...
  for (ScannerInfoVector::const_iterator it = v.begin(); it != v.end(); it++) {
    ...
    if (availableDevicesOnce.count(deviceID) == 0) {
      ...
      throw std::logic_error(
          ...
          ::toString(int(deviceID)) + ", type " +
          ScannerInfo::scannerTypeToString(sinfo.getScannerType()) +
          ". This is not allowed - for each scanner at most one ScannerInfo "
          "may be stored in the ScannerInfoVector.");
    }
  }
...
//
// Transform the scan to vehicle coordinates, but do not sort the resulting
// scanpoints by angles.
//
bool Scan::transformToVehicleCoordinatesUnsorted() {
  ...
  if ((getFlags() & FlagVehicleCoordinates) != 0) {
    ...
    // traceDebug(SCAN_VERSION) << "convertToVehicleCoordinates: Scan (#" <<
    // getScanNumber() << ") already in vehicle coordinates. Nothing to do." <<
    // std::endl;
  ...
  if (getScannerInfos().empty()) {
    printWarning(
        "Scan::convertToVehicleCoordinates: Scan (#" +
        ::toString(getScanNumber()) +
  ...
  //	printInfoMessage("Scan::transformToVehicleCoordinatesUnsorted: Mounting
  //position is " + mountPos.toString() + ".", true);
  ...
  for (ScannerInfoVector::iterator siIt = getScannerInfos().begin();
       siIt != getScannerInfos().end(); siIt++) {
...
Scan::ScannerInfoVector &Scan::getScannerInfos() { return m_scannerInfos; }
