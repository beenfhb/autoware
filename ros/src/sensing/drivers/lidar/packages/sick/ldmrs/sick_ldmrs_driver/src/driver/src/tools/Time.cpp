...
// Duration
//
TimeDuration::TimeDuration() { m_duration = 0.0; }

const double
    Time::m_secondFractionNTPtoNanoseconds(0.2328306436538696); // = 2^-32 * 1e9
const double
    Time::m_nanosecondsToSecondFractionNTP(4.29496729600000); // = 2^32 * 1e-9
...
//
//
void Time::set(timeval time) { m_time = time; }
...
void Time::set(UINT64 ntpSeconds, UINT32 ntpFractionalSeconds) {
  //	const UINT32 startUnixEpoche = 2208988800; // seconds from 1.1.1900
  //to 1.1.1900
  m_time.tv_sec = ntpSeconds - secondsFrom1900to1970;
  m_time.tv_usec =
      ntpFractionalSeconds * m_secondFractionNTPtoNanoseconds / 1000;
}
...
Time Time::operator-(const double &seconds) const {
  ...
  /*
          if (m_time.tv_sec > other.m_time.tv_sec)
          {
                  ...
                  t.m_time.tv_usec = (m_time.tv_usec + offset) -
     other.m_time.tv_usec;
          }
          else if (m_time.tv_sec == other.m_time.tv_sec)
          {
                  t.m_time.tv_sec = 0;
                  ...
                  {
                          t.m_time.tv_usec = m_time.tv_usec -
     other.m_time.tv_usec;
                  }
          }
          else
