...
//
// The App starts a thread that, after some time, sets the new time in the
// scanner.
//
...
// Constructor
//
LdmrsNtpTimeApp::LdmrsNtpTimeApp(Manager *manager) : m_manager(manager) {
  m_beVerbose = true;
...
//
// Convert UNIX system time (seconds since 1.1.1970) to NTP time (secons
// since 1.1.1900).
//
ntp_time_t LdmrsNtpTimeApp::convertUnixTimeToNtpTime(struct timeval &unixTime) {
  ...
  ntpTime.fraction =
      (uint32_t)((double)(unixTime.tv_usec + 1) * (double)(1LL << 32) * 1.0e-6);
  return ntpTime;
}
...
//
void LdmrsNtpTimeApp::changeThreadFunction(bool &endThread,
                                           UINT16 &waitTimeMs) {
  printInfoMessage("LdmrsNtpTimeApp::changeThreadFunction(): Started.",
                   m_beVerbose);
  bool result;
  ...
  if (ldmrs != NULL) {
    ...
    if (result == true) {
      // Success
      printInfoMessage("LdmrsNtpTimeApp::changeThreadFunction(): Successfully "
                       "set the NTP time.",
                       true);
    } else {
      // Failure
    ...
    printError("LdmrsNtpTimeApp::changeThreadFunction():  Failed to get a "
               "valid pointer to the MRS, aborting!");
  }

  endThread = true;
  waitTimeMs = 0;
  printInfoMessage(
      "LdmrsNtpTimeApp::changeThreadFunction():  All done, leaving.",
      m_beVerbose);
}
