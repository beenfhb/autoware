...
EvalCases::EvalCases() { m_datatype = Datatype_EvalCases; }

void EvalCases::clear() { m_evalCases.clear(); }
...
void EvalCases::add(EvalCase_ptr evalCase) {
  if (m_evalCases.size() >= 16) {
    // Error
    printError("EvalCases::add: Cannot add this eval case. The MRS can only "
               "handle up to 16 eval cases!");
  } else {
    // Add the case
...
const EvalCaseVector &EvalCases::getEvalCases() const { return m_evalCases; }
