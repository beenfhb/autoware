...
//
// All source code provided here is intended for demonstration and reference
// purposes only. SICK provides no warranty whatsoever for the function of this
// code.
//
// Note that not all features are available with all firmware versions of the
// MRS. Please refer to the manual if the intended functionality is available
// with your device.
//
// Change log:
// 2013-09-18, willhvo:
...
//
// This example demonstrates setting and receiving the NTP timestamp of the MRS.
// The timestamp can be set with two commands, and is automatically included in
// every data packet received from the sensor. This feature is available in all
// firmware versions.
//
...
int mrsNtpTimeApp() {
  ...
  printInfoMessage("mrsNtpTimeApp: Adding the application MrsNtpTimeApp.",
                   true);
  ...
  if (result == false) {
    printError("mrsNtpTimeApp: Failed to add application " + name +
               ", aborting!");
    return 0;
  }
...
//
// This example demonstrates setting and checking the FlexibleResolution feature
// of the LDMRS. This feature is not available in all firmware versions.
//
// The MRS device should be configured like this:
// m_weWantScanData:          true
// m_weWantObjectData:        false
...
int sectorChangeApp() {
  ...
  // started. But - this is not needed here.
  printInfoMessage(
      "sectorChangeApp: Adding the application MrsSectorChangeApp.", true);
  type = Sourcetype_MrsChangeApp;
  name = "MRS Example Sector Change App";
  id = 51;
  result = manager.addApplication(type, name, id);
  if (result == false) {
    printError("sectorChangeApp: Failed to add application " + name +
               ", aborting!");
    return 0;
  }
  printInfoMessage("sectorChangeApp: Application SectorChange is running.",
                   true);
...
// Note 1: If only scan data is required, it is recommended to use the MRS
// interface instead. Note 2: Not all MRS devices have a SOPAS interface.
//
// The MRS device should be configured like this:
...
int mrsSopasScandataApp() {
  ...
  // started. Here, we are using the general data display app because we only
  // want to show the received data.
  Sourcetype type;
  std::string name;
  UINT16 id;
  bool result = false;
  ...
  result = manager.addApplication(type, name, id);
  if (result == false) {
    printError("mrsSopasScandataApp: Failed to add application " + name +
               ", aborting!");
    return 0;
  }
  ...
  result = manager.addAndRunDevice(type, name, id);
  if (result == false) {
    printError("mrsSopasScandataApp: Failed to add device " + name +
               ", aborting!");
    return 0;
  }
...
int mrsFieldApp() {
  ...
  result = manager.addApplication(type, name, id);
  if (result == false) {
    printError("mrsFieldApp: Failed to add application " + name +
               ", aborting!");
    return 0;
  }
...
//
// Demo application for scanpoint coordinate calculation. Receives the
// scanpoints, and prints coordinates of the points at the configured scan
// angle.
//
...
int mrsScanpointCoordinateApp() {
  ...
  printInfoMessage("mrsScanpointCoordinateApp: Adding the application "
                   "MrsScanpointCoordinateApp.",
                   true);
  ...
  result = manager.addApplication(type, name, id);
  if (result == false) {
    printError("mrsScanpointCoordinateApp: Failed to add application " + name +
               ", aborting!");
    return 0;
  }
  ...
  result = manager.addAndRunDevice(type, name, id);
  if (result == false) {
    printError("mrsScanpointCoordinateApp: Failed to add device " + name +
               ", aborting!");
    return 0;
  }
...
int main(int argc, char **argv) {
  ...
  // The MRS-App connects to an MRS, reads its configuration and displays
  // scanpoint coordinates.
  ...
  // The FieldApp demonstrates the usage of the SOPAS interface. It removes and
  // creates fields and eval cases.
  //	result = mrsFieldApp();

  // The SopasScandataApp demonstrates reading scan data via the SOPAS
  // interface.
