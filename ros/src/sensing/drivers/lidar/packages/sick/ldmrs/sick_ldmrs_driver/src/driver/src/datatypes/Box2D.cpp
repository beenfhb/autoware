...
Box2D::Box2D() : m_center(0, 0), m_size(0, 0), m_rotation(0) {
...
Box2D::Box2D(value_type x_center, value_type y_center, value_type x_size,
             value_type y_size, value_type rotation)
    : m_center(x_center, y_center), m_size(x_size, y_size),
      m_rotation(rotation) {
  assert((m_size.getX() >= 0.0) || isNaN(m_size.getX()));
  assert((m_size.getY() >= 0.0) || isNaN(m_size.getY()));
#ifdef ASSERT_NUMERIC_RANGES
...
void Box2D::moveBy(const Point2D &pointvalues) { m_center += pointvalues; }
...
static inline floatT distanceFromStraightBox(floatT pointX, floatT pointY,
                                             floatT boxSizeX, floatT boxSizeY) {
  // Rotation is zero, hence we can directly check this in
  // cartesian coordinates.
...
static inline Box2D::value_type distanceFromOutline(const Box2D &box,
                                                    const PointT &point) {
  ...
  if (fuzzyCompare(box.getRotation(), 0.0)) {
    ...
    return distanceFromStraightBox(dCos * deltaX + dSin * deltaY,
                                   -dSin * deltaX + dCos * deltaY, boxSizeX,
                                   boxSizeY);
  }
}
...
static Box2D calcOrientatedBox(double orientation, const IterT &begin,
                               const IterT &end) {
  Box2D result;
...
// Box2D Box2D::orientatedBox(value_type orientation,
// 						   const
// std::vector<ScanPoint>::const_iterator& begin, 						   const
// std::vector<ScanPoint>::const_iterator& end)
// {
// 	return calcOrientatedBox(orientation, begin, end);
// }
//
...
void Box2D::verifyNumericRanges() {
  ...
  if (m_size.getX() < 0 || m_size.getY() < 0) {
    printWarning("Size of Box2D was given as negative value (" +
                 ::toString(m_size.getX(), 2) + "," +
                 ::toString(m_size.getY(), 2) +
    ...
    // throw std::out_of_range("Size of Box2D negative - this is an invalid
    // Box2D.");
  }

  if (isNaN(m_size.getX())) {
    m_size.setX(0);
    printWarning("Size.getX() of Box2D was given as NaN value - silently using "
                 "Zero instead.");
    correctRange = false;
  }
  if (isNaN(m_size.getY())) {
    m_size.setY(0);
    printWarning("Size.getY() of Box2D was given as NaN value - silently using "
                 "Zero instead.");
    correctRange = false;
  }
  if (isNaN(m_rotation)) {
    m_rotation = 0;
    printWarning("Rotation of Box2D was given as NaN value - silently using "
                 "Zero instead.");
    correctRange = false;
  }
  value_type normd_rot = normalizeRadians(m_rotation);
  if (!fuzzyCompare(normd_rot, m_rotation)) {
    printWarning("Rotation of Box2D (" + ::toString(m_rotation, 2) +
                 ") was outside of its [-pi,pi] definition range - silently "
                 "normalizing it back into that range (" +
