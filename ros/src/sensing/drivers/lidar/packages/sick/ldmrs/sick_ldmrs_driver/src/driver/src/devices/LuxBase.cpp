...
// HISTORY
//
// This device is indended as a customer reference for the LD-MRS interface and
// coding / decoding of messages. Its interaction with other parts of the system
// and its usage of functions is intentionally kept simple, although this
// results in a somewhat lower performance than the original LUX/MRS device
// classes.
//
// Usage notes
//
// This device opens a TCP connection to an LD-MRS scanner. It then sets the
// desired parameters and receives incoming data. Currently, this is:
// - scans
// - objects
// - warning and error messages
//
// Internally, it has 2 buffers. The first level, called inputBuffer, is filled
// with whatever data is received on the interface. Scans and Objects are
// decoded right out of this buffer. Other messages, such as command replies and
// error messages, are forwarded to the cmdReplyBuffer where they are decoded.
// Note the different thread contexts: Scans and Objects are decoded in the
// receive-thread context, while data in the cmdReplyBuffer is decoded in the
// main device context asynchronously.
//
...
LuxBase::LuxBase(Manager *manager, const UINT8 deviceId,
                 const std::string longName, std::string ipAddress,
                 UINT16 tcpPortNumber, double scanFrequency,
                 double scanStartAngle, double scanEndAngle, double offsetX,
                 double offsetY, double offsetZ, double yawAngle,
                 double pitchAngle, double rollAngle, bool beVerbose,
                 std::string inputFileName)
    : m_manager(manager), m_longName(longName), m_deviceId(deviceId),
      m_ipAddress(ipAddress), m_tcpPortNumber(tcpPortNumber),
      m_inputFileName(inputFileName), m_scanFrequency(scanFrequency),
      m_scanStartAngle(scanStartAngle), m_scanEndAngle(scanEndAngle),
      m_offsetX(offsetX), m_offsetY(offsetY), m_offsetZ(offsetZ),
      m_yawAngle(yawAngle), m_pitchAngle(pitchAngle), m_rollAngle(rollAngle),
      m_inBufferLevel(0), m_beVerbose(beVerbose) {
...
void LuxBase::readCallbackFunctionS(void *obj, BYTE *buffer,
                                    UINT32 &numOfBytes) {
  ((LuxBase *)(obj))->readCallbackFunction(buffer, numOfBytes);
}

void LuxBase::setOnScanReceiveCallbackFunction(
    OnScanReceiveCallbackFunction function, void *obj) {
...
//
bool LuxBase::initTcp(Tcp::DisconnectFunction function,
                      void *obj) // , bool beVerbose)
{
  ...
  printInfoMessage("LuxBase::initTcp: Opening the tcp interface (Addr=" +
                       m_ipAddress + ":" + toString(m_tcpPortNumber) + ").",
                   m_beVerbose);
  ...
  if (result == true) {
    ...
    printInfoMessage("LuxBase::initTcp(): TCP connection established.",
                     m_beVerbose);
  } else {
    printError("LuxBase::initTcp(): ERROR: Failed to establish TCP connection, "
               "aborting.");
    return false;
  }

  // Set the data input callback for our TCP connection
  m_tcp.setReadCallbackFunction(LuxBase::readCallbackFunctionS,
                                (void *)this); // , this, _1, _2));
  ...
  //
  // Read the beam tilt.
  // We need this parameter later for the calculation of cartesian scan point
  // coordinates.
  //
  ...
  if (result == true) {
    ...
    infoMessage(m_longName + " Beam tilt angle is " +
                    toString(m_beamTiltAngle * rad2deg, 1) + " degrees.",
                m_beVerbose);
  } else {
    printError("LuxBase::initTcp(): ERROR: Failed to read scanner beam tilt "
               "angle, aborting.");
    return false;
  }
  ...
  // coordinates.
  //
  printInfoMessage("LuxBase::initTcp(): Calling readUpsideDown().",
                   m_beVerbose);
  ...
  if (result == true) {
    ...
    infoMessage(m_longName + " UpsideDown not supported by firmware.",
                m_beVerbose);
  }
...
//
bool LuxBase::initFile(File::DisconnectFunction function,
                       void *obj) // , bool beVerbose)
{
  // Initialise base variables
  //	m_beVerbose = beVerbose; // true = Show extended status info (DEBUG)
  ...
  // File
  printInfoMessage(
      "LuxBase::init: Opening the input file (Name=" + m_inputFileName + ").",
      m_beVerbose);
  result = m_file.open(m_inputFileName, false); // m_beVerbose);
  if (result == true) {
    printInfoMessage("LuxBase::initFile(): File opened successfully.",
                     m_beVerbose);
  } else {
  ...
  m_file.setReadCallbackFunction(LuxBase::readCallbackFunctionS,
                                 (void *)this); // , this, _1, _2));
...
void LuxBase::updateThreadFunction(bool &endThread, UINT16 &sleepTimeMs) {
  ...
  if (result == true) {
    printInfoMessage("updateThreadFunction(): Got new status. Temp= " +
                         toString(getTemperature(), 1) + " deg.C.",
                     m_beVerbose);
  } else {
...
//
//
bool LuxBase::isRunning() { return m_isRunning; }
...
bool LuxBase::cmd_setMountingPos(Position3D mp) {
  ...
  uValue = (UINT32)
      value; // Note the cast, including the sign transformation to unsigned!
...
bool LuxBase::cmd_setScanFrequency(double scanFreq) {
  ...
  if ((uValue > 3100) && (uValue < 3300)) {
    ...
    uValue = 3200; // DEFAULT
    printWarning("Para 'ScanFrequency' out of range, setting a default scan "
...
//
// Note that the two angles are set one after the other. Still, in the sensor,
// there is a condition that the StartAngle must always be greater than the
// EndAngle. Obviously, there are cases in which the mechanism implemented here
// will fail as the correct order is violated. However, for most cases, this
// will work.
//
bool LuxBase::cmd_setScanAngles(double startAngle, double endAngle) {
  ...
  //
  // ** NOTE **
  // Some combinations of start and end angles just do not work. The result is
  // that no scans are being sent by the scanner; this is a known issue of the
  // scanner firmware. Our workaround is to use even TIC values; this will work
  // fine.
  //
  // ** NOTE 2 **
  // The scan start angle must always be greater than the scan end angle.
  // Therefore, the method used below will not work for all configurations. To
  // set the angles correctly, you should read the current angles, select the
  // order in which to set the parameters, and then write them.
  //
  ...
  bool result;
  UINT32 uValue;
  INT16 value = (INT16)(startAngle * rad2deg *
                        32.0); // Note that startAngle is in radians
  ...
  //
  value =
      (INT16)(endAngle * rad2deg * 32.0); // Note that endAngle is in radians
  // Value should be even
  makeIntValueEven(value);
  if (value > 1598) {
    value = 1598; //
...
bool LuxBase::cmd_setSyncAngleOffset(double syncAngle) {
  ...
  UINT32 uValue;
  INT16 value =
      (INT16)(syncAngle * rad2deg * 32.0); // Note that syncAngle is in radians
  if (value > 5759) {
    value = 5759;
  ...
  uValue = (UINT32)
      value; // Note the cast, including the sign transformation to unsigned!
  uValue =
      uValue & (1 << 14) - 1; // = 0x00003FFF  (SyncAngleOffset is an INT14)
...
bool LuxBase::cmd_getStatus() {
  ...
  if (result == true) {
    printInfoMessage(
        "MRS command 'GetStatus' was sent successfully, waiting for reply.",
        m_beVerbose);
    ...
    if (result == true) {
      printInfoMessage(
          "LuxBase::cmd_getStatus: " + m_longName +
              " MRS Status was read successfully. Firmware version is 0x" +
              toHexString(m_firmwareVersion) + ".",
          m_beVerbose);
    } else {
      printError("LuxBase::cmd_getStatus: " + m_longName +
...
bool LuxBase::readBeamTilt() {
  ...
  if (success == true) {
    ...
    infoMessage("LuxBase::readBeamTilt: " + m_longName +
                " Beam tilt read. Angle is " +
                toString(rad2deg * m_beamTiltAngle, 1) + " degrees.");
  } else {
    // Failed to read parameters
    printError("LuxBase::readBeamTilt: " + m_longName +
...
bool LuxBase::readUpsideDown() {
  ...
  if (success == true) {
    if (value != 0) {
      ...
      infoMessage("LuxBase::readUpsideDown: " + m_longName +
                  " UpsideDown is active.");
    } else {
      m_upsideDownActive = false;
      infoMessage("LuxBase::readUpsideDown: " + m_longName +
                  " UpsideDown is not active.");
    }
  } else {
    // Failed to read parameter
    infoMessage("LuxBase::readUpsideDown: " + m_longName +
                " UpsideDown not supported by firmware.");
...
bool LuxBase::cmd_stopMeasure() {
  if (m_tcp.isOpen() == false) {
    // There is no connection
    printError("LuxBase::cmd_stopMeasure: " + m_longName +
  ...
  if (result == true) {
    printInfoMessage(
        "MRS command 'StopMeasure' was sent successfully, waiting for reply.",
        m_beVerbose);

    // The command was sent
    result = receiveMrsReply(cmdId, 2000);
    if (result == true) {
      printInfoMessage("LuxBase::cmd_stopMeasure: " + m_longName +
                           " StopMeasure was acknowledged by the scanner.",
                       m_beVerbose);
    } else {
      printError("LuxBase::cmd_stopMeasure: " + m_longName +
...
bool LuxBase::cmd_setNtpTimestamp(UINT32 seconds, UINT32 fractionalSec) {
  if (m_tcp.isOpen() == false) {
    // There is no connection
    printError("LuxBase::cmd_setNtpTimestamp: " + m_longName +
               " ERROR: stopMeasure() called, but there is no connection!");
    return false;
  }
  ...
  if (result == true) {
    printInfoMessage(
        "LuxBase::cmd_setNtpTimestamp: MRS command 'SetNTPTimestampSec' was "
    ...
    if (result == true) {
      printInfoMessage(
          "LuxBase::cmd_setNtpTimestamp: " + m_longName +
              " Timestamp setting was acknowledged by the scanner.",
          m_beVerbose);
    } else {
      printError(
          "LuxBase::cmd_setNtpTimestamp: " + m_longName +
    ...
    // Failed to send command
    printError("LuxBase::cmd_setNtpTimestampSec: " + m_longName +
               " ERROR: Failed to send command, aborting.");
    return false;
  }
  ...
  if (result == true) {
    printInfoMessage(
        "LuxBase::cmd_setNtpTimestamp: MRS command 'SetNTPTimestampFrac' was "
        "sent successfully, waiting for reply.",
        m_beVerbose);
    ...
    if (result == true) {
      printInfoMessage(
          "LuxBase::cmd_setNtpTimestampFrac: " + m_longName +
              " Timestamp setting was acknowledged by the scanner.",
          m_beVerbose);
    } else {
      printError(
    ...
    // Failed to send command
    printError("LuxBase::cmd_setNtpTimestampFrac: " + m_longName +
               " ERROR: Failed to send command, aborting.");
    return false;
  }
...
bool LuxBase::cmd_setParameter(MrsParameterId parameter, UINT32 value) {
  if (m_tcp.isOpen() == false) {
    // There is no connection
    printError(
        "LuxBase::cmd_setParameter: " + m_longName +
  ...
  printInfoMessage("LuxBase::cmd_setParameter: Sending MRS command "
                   "'SetParameter' with para=0x" +
                       toHexString((UINT16)parameter) + " and value=0x" +
                       toHexString(value) + ".",
                   m_beVerbose);
  ...
  if (result == true) {
    printInfoMessage("LuxBase::cmd_setParameter: MRS command 'SetParameter' "
                     "was sent successfully, waiting for reply.",
                     m_beVerbose);

    // The command was sent
    result = receiveMrsReply(cmdId, 2000);
    if (result == true) {
      printInfoMessage("LuxBase::cmd_setParameter: " + m_longName +
                           " SetParameter was acknowledged by the scanner.",
                       m_beVerbose);
    } else {
      printError("LuxBase::cmd_setParameter: " + m_longName +
...
bool LuxBase::cmd_getParameter(MrsParameterId parameter, UINT32 *value) {
  if (m_tcp.isOpen() == false) {
    // There is no connection
    printError(
        "LuxBase::cmd_getParameter: " + m_longName +
        " ERROR: setParameter() called, but there is no connection, aborting!");
    return false;
  }
  ...
  printInfoMessage("LuxBase::cmd_getParameter: Sending MRS command "
                   "'SetParameter' with para=0x" +
                       toHexString((UINT16)parameter) + ".",
                   m_beVerbose);

  bool result = sendMrsCommand(cmdId, parameter);

  if (result == true) {
    printInfoMessage("LuxBase::cmd_getParameter: MRS command 'SetParameter' "
                     "was sent successfully, waiting for reply.",
                     m_beVerbose);
    ...
    if (result == true) {
      printInfoMessage("LuxBase::cmd_getParameter: " + m_longName +
                           " SetParameter was acknowledged by the scanner.",
                       m_beVerbose);
    } else {
      printError("LuxBase::cmd_getParameter: " + m_longName +
...
bool LuxBase::cmd_setDataOutputFlags() {
  ...
  if (result == true) {
    printInfoMessage("LuxBase::cmd_setDataOutputFlags: Output flags were set "
                     "successfully to " +
                         toHexString(flags) + ".",
                     m_beVerbose);
  } else {
    // Failed
    printError("LuxBase::cmd_setDataOutputFlags:" + m_longName +
...
// Note 2: These flags may not prevent the data to be transferred, but it will
// not be decoded and
...
bool LuxBase::cmd_startMeasure(bool weWantScanData, bool weWantObjectData) {
  ...
  if (m_tcp.isOpen() == false) {
    // There is no connection
    printError(
        "LuxBase::cmd_startMeasure:" + m_longName +
  ...
  if ((m_weWantScanData == false) && (m_weWantObjectData == false)) {
    // We want no data?
    printWarning("LuxBase::cmd_startMeasure:" + m_longName +
                 " Warning: StartMeasure called, but neither scans nor objects "
                 "are requested!");
  }

  // Enable scan and/or object output
  printInfoMessage("LuxBase::cmd_startMeasure: Enabling data output.",
                   m_beVerbose);
  bool result = cmd_setDataOutputFlags();
  if (result == true) {
    printInfoMessage(
        "LuxBase::cmd_startMeasure: Data output was successfully enabled.",
        m_beVerbose);
  } else {
    // Failed
    printError("LuxBase::cmd_startMeasure:" + m_longName +
  ...
  printInfoMessage(
      "LuxBase::cmd_startMeasure: Sending MRS command 'StartMeasure'.",
      m_beVerbose);
  result = sendMrsCommand(cmdId);

  if (result == true) {
    printInfoMessage("LuxBase::cmd_startMeasure: MRS command 'StartMeasure' "
                     "was sent successfully, waiting for reply.",
                     m_beVerbose);
    ...
    result = receiveMrsReply(cmdId, 2000);
    if (result == true) {
      printInfoMessage("LuxBase::cmd_startMeasure: " + m_longName +
                           " StartMeasure was acknowledged by the scanner.",
                       m_beVerbose);
    } else {
      printError("LuxBase::cmd_startMeasure:" + m_longName +
...
|| sizeof(T) == 8); value = detail::memreadcLE<T>(buf); buf += sizeof(T);
}
*/
...
bool LuxBase::decodeGetStatus() {
  ...
  // Debug
  printInfoMessage("LuxBase::decodeGetStatus: " + m_longName +
                       " Received a GetStatus with CMD " + toHexString(cmd) +
                       " and FirmwareVersion " +
                       toHexString(m_firmwareVersion) + ".",
                   m_beVerbose);
...
bool LuxBase::decodeGetParameter(UINT32 *value) {
  ...
  // Debug
  printInfoMessage("LuxBase::decodeGetParameter: " + m_longName +
                       " Received a GetParameter with CMD " + toHexString(cmd) +
                       " and parameter index 0x" + toHexString(index) +
 ...
 *				 here a double argument is passed to support the
 *exact conversion of \e averaged temperature raw data. \return The temperature
 *converted to degree Celsius. \sa celsius2Int()
 */
...
double LuxBase::getTemperature() {
  ...
  static UINT16 maxRawTemperature =
      celsius2Int(-273.16f); // = 1568 = theoretical raw value of 0 Kelvin
...
std::string LuxBase::getSerialNumber() {
  ...
  if (m_serialNumber[0] == 0xFFFF) {
    ...
    if (isNewStyle) {
      ...
      oss << std::setfill('0') << std::hex << std::setw(4) << m_serialNumber[0]
      ...
      oss << std::setfill('0') << std::hex << std::setw(4) << m_serialNumber[0]
          << '-' << std::setw(4) << m_serialNumber[1] << std::setw(2)
          << (m_serialNumber[2] >> 8);
    }
    return oss.str();
  }
...
std::string LuxBase::int2Version(UINT16 val) {
  if (val == 0xFFFF) {
    ...
    std::ostringstream oss;
    oss << std::hex << std::uppercase << ((val & 0xF000) >> 12) << '.'
...
std::string LuxBase::version2string(UINT16 version, const UINT16 timestamp[3]) {
  if (isValidVersion(version)) {
    ...
    oss << int2Version(version) << ' ' << std::setfill('0') << std::hex
        << std::setw(4) << year << '-' << std::setw(2) << month << '-'
 ...
 *
 * Reads a value of 1, 2 or 4 bytes from the buffer. The value encoding in the
 * buffer is little endian, that means the lowest-value byte is first in the
 * buffer. Example: Buffer = 0x12 0x34 ==> Value = 0x3412
 */
UINT32 LuxBase::readUValueLE(UINT8 *buffer, UINT8 bytes) {
  ...
  default:
    printError(
        "LuxBase::readUValueLE: " + m_longName +
...
INT32 LuxBase::readValueLE(UINT8 *buffer, UINT8 bytes) {
  ...
  default:
    printError(
        "LuxBase::readValueLE: " + m_longName +
        " ERROR: Invalid number of bytes to read, can only handle 1,2 or 4.");
    value = 0xFFFFFFFF;
  }
 ...
 *
 * Reads a value of 1, 2 or 4 bytes from the buffer. The value encoding in the
 * buffer is big endian, that means the highest-value byte is first in the
 * buffer. Example: Buffer = 0x12 0x34 ==> Value = 0x1234
 */
UINT32 LuxBase::readUValueBE(UINT8 *buffer, UINT8 bytes) {
  ...
  default:
    printError(
        "LuxBase::readUValueBE: " + m_longName +
        " ERROR: Invalid number of bytes to read, can only handle 1,2 or 4.");
    value = 0xFFFFFFFF;
  }
...
UINT16 LuxBase::decodeAnswerInInputBuffer() {
  ...
  printInfoMessage("LuxBase::decodeAnswerInInputBuffer: Called.",
                   beVerboseHere);
  ...
  if (m_inBufferLevel <= headerLen) {
    // Not enough data
    printInfoMessage("LuxBase::decodeAnswerInInputBuffer(): Not enough data in "
                     "input buffer, just " +
                         toString(m_inBufferLevel) + " bytes available, done.",
                     beVerboseHere);
    return 0;
  }

  // Sync to magic word. This should not happen too often, as data should come
  // in structured - unless we start to miss complete IP packages...
  UINT32 magicWord = 0;
  ...
  if ((i > 0) && (magicWord != 0xAFFEC0C2)) {
    printInfoMessage("LuxBase::decodeAnswerInInputBuffer(): Out of sync, and "
                     "no magic word found - clearing buffer.",
                     beVerboseHere);
    m_inBufferLevel = 0;
    ...
    printInfoMessage("LuxBase::decodeAnswerInInputBuffer(): Out of sync, "
                     "adjusting the buffer by " +
                         toString(i) + " bytes.",
                     beVerboseHere);
  ...
  if (magicWord != 0xAFFEC0C2) {
    // Not found.
    printInfoMessage(
        "LuxBase::decodeAnswerInInputBuffer(): Magic word not found, aborting!",
        beVerboseHere);
    return 0;
  } else {
    printInfoMessage("LuxBase::decodeAnswerInInputBuffer(): Magic word found, "
                     "now decoding header.",
                     beVerboseHere);
  }
  ...
  if (m_inBufferLevel >= headerLen) {
    ...
    if (m_inBufferLevel >= (payloadLen + headerLen)) {
      ...
      default:
        // Unknown!
        printWarning(
            "LuxBase::decodeAnswerInInputBuffer(): Unknown datatype 0x" +
            toHexString(datatype) +
            "(Length=" + ::toString(headerLen + payloadLen) +
  ...
  if (beVerboseHere == true) {
    if (datatype > 0) {
      infoMessage("LuxBase::decodeAnswerInInputBuffer(): Header decoded "
                  "successfully, datatype 0x" +
                  toHexString(datatype) + ".");
    } else {
      infoMessage("LuxBase::decodeAnswerInInputBuffer(): Header decoded "
                  "successfully, but message is incomplete.");
      dumpHeader();
    }
  }
  return datatype;
...
//
// Decodes an error message that is completely present as the first message in
// the input buffer.
//
void LuxBase::decodeErrorMessage() {
  //	printInfoMessage("decodeErrorMessage(): There is an error/warning
  //message.", m_beVerbose);
  ...
  if ((m_errorRegister1 != 0) || (m_errorRegister2 != 0)) {
    ...
    std::string text =
        "LuxBase::decodeErrorMessage: LD-MRS reports an error: errReg1=0x" +
        toHexString(m_errorRegister1) + ", errReg2=0x" +
        toHexString(m_errorRegister2) + ". Meaning: ";
    ...
    std::string text =
        "LuxBase::decodeErrorMessage: LD-MRS reports a warning: warnReg1=0x" +
        toHexString(m_warnRegister1) + ", warnReg2=0x" +
        toHexString(m_warnRegister2) + ". Meaning: ";
...
//
// Decodes a scan that is completely present as the first message in the input
// buffer.
///
void LuxBase::decodeScan() {
  //	printInfoMessage("LuxBase::decodeScan(): We have received a scan that is
  //now being decoded.", m_beVerbose);
  ...
  //	NTPTime startTime = NTPTime (timestamp);	// read NTPTime using Little
  //Endian
  ...
  double startAngle =
      convertTicktsToAngle(startAngleTicks); // startAngle is in [rad]
  ...
  // Bit 10: mirror side: 0=front (for 8-Layer, tilted downward), 1=rear (for
  // 8-layer, tilted upward) All other flags are reserved-internal and should
  // not be evaluated.
  ...
  // Time per angle (rad). Used later to calculate the time of the beam inside
  // the scan.
  // 	double angleTime;	// Time in the scan for 1 degree
  // 	if (m_scanFrequency > 0)
  // 	{
  // 		angleTime = (1.0 / (double)m_scanFrequency) / (360.0 * deg2rad); //
  // in [s/rad]
  // 	}
  // 	else
  // 	{
  // 		angleTime = 0.001; // Default to avoid errors: 1 ms in [s]; for
  // emergencies only.
  // 	}
  ...
  for (UINT16 s = 0; s < scanPoints; s++) {
    ...
    UINT16 flags = (UINT16)readUValueLE(
        &(scanPtBuffer[1]),
        1); // 0x01:transparent; 0x02:clutter; 0x04:ground; 0x08: dirt
    INT16 horzAngleTicks =
        (INT16)readValueLE(&(scanPtBuffer[2]), 2); // H-Angle of this shot
    UINT16 radialDistanceCm = (UINT16)readUValueLE(
        &(scanPtBuffer[4]), 2); // Radial distance in scanner coords
    UINT16 echoPulseWidthCm = (UINT16)readUValueLE(
        &(scanPtBuffer[6]),
        2); // Echo pulse width; Indicator for energy of incoming beam
    ...
    // Set the coordinates of the new point. Also automatically generates x-y-z
    // coordinates.
    ...
    //((UINT32)((startAngle - hAngle) * angleTime * 1000000.0))); // Time offset
    //of scan point 		newPoint.setSegmentId(0); // Not available
  }
  ...
  si.setScannerType(Sourcetype_LDMRS); // for compatibility, if no value is set
                                       // in the scanner's config.
  Time start, end;
  ...
  //	printInfoMessage("decodeScan(): Decoded scan with " +
  //toString(scanPoints) + " points.", m_beVerbose);
}
...
void LuxBase::decodeSensorInfo() {
  ...
  // decode sensor info
  //	UINT16 sensorInfoVersion = (UINT16)readUValueLE(&(scanBuffer[0]), 2); //
  //here: 1 	UINT16 relatedScanNumber = (UINT16)readUValueLE(&(scanBuffer[2]),
  //2); 	UINT16 fpgaErrorRegister1 = (UINT16)readUValueLE(&(scanBuffer[4]), 2);
...
/*
 * Calculate the elevation angle of the scanpoint depending on scan layer,
 * mirror side, horziontal angle and UpsideDown flag.
 */
double LuxBase::getVAngleOfLayer(bool isRearMirrorSide, UINT8 layerNumber,
                                 double hAngle) {
  // Calculate the effective mirror tilt as a function of the beam tilt at 0
  // degree horizontally and the current horizontal angle
  const double effMirrorTilt =
      m_beamTiltAngle / 2.0 * M_SQRT2 * std::sin(0.5 * hAngle - M_PI_4);

  // Distinguish between the front and back side of the mirror.
  // Front mirror side: beam tilted/pitched down (positive pitch), rear mirror
  // side: beam tilted/pitched up (negative pitch)
  ...
  // Calculate the layer offset of each layer. This calculation defines 0 degree
  // as a symetrical layer.
  const double numLayers = 4.0;
  const double verticalBeamDivergence = 0.8 * deg2rad;
  const double layerOffset =
      ((numLayers - 1) / 2 - layerNumber) * verticalBeamDivergence;
...
double LuxBase::convertTicktsToAngle(INT16 angleTicks) {
  ...
  if ((angle > -1000.0) && (angle < 1000.0)) {
    ...
    printError("convertTicktsToAngle(): The angle value " + toString(angle, 2) +
...
//
// Decodes object data that is completely present as the first message in the
// input buffer.
//
// Note that not all LD-MRS versions can deliver object data.
//
void LuxBase::decodeObjects() {
  printInfoMessage("decodeObjects(): We have received a dataset and are now "
                   "decoding objects.",
                   m_beVerbose);
  //	printWarning("decodeObjects(): We have received a dataset, BUT THIS
  //FUNCTION IS NOT IMPLEMENTED, ignoring the data!"); 	return;

  // Unlike the scan data decoder, we are using a bufferOffset pointer here. As
  // the number of contour points varies for each object, this is a simple way
  // to keep track of our position in the buffer.
  ...
  UINT16 numObjects =
      (UINT16)readUValueLE(&(m_inputBuffer[bufferOffset + 8]), 2);
  bufferOffset = 24 + 10;
  ...
  for (UINT16 i = 0; i < numObjects; i++) {
    ...
    UINT16 objectPredictionAge =
        (UINT16)readUValueLE(&(m_inputBuffer[bufferOffset]), 2);
    bufferOffset += 2;
    newObject.setHiddenStatusAge(objectPredictionAge);

    // Offset 6: Relative timestamp
    UINT16 relativeTimestamp =
        (UINT16)readUValueLE(&(m_inputBuffer[bufferOffset]), 2);
    bufferOffset += 2;
    Time t;
    ...
    INT16 objectBoxOrientationTicks =
        (INT16)readValueLE(&(m_inputBuffer[bufferOffset]), 2);
    bufferOffset += 2;
    ...
    if (absoluteVelocity.getX() < -320.0) {
      ...
      // Absolute velocity is valid. This will only be the case if vehicle
      // movement data is sent to the sensor via CAN.
    ...
    UINT16 classificationAge =
        (UINT16)readUValueLE(&(m_inputBuffer[bufferOffset]), 2);
    bufferOffset += 2;
    newObject.setClassificationAge(classificationAge);

    UINT16 classificationCertainty =
        (UINT16)readUValueLE(&(m_inputBuffer[bufferOffset]), 2);
    bufferOffset += 2;
  ...
  printInfoMessage("LuxBase::decodeObjects(): We have received " +
                       toString(numObjects) + " objects. Now leaving.",
                   m_beVerbose);
}
...
//
// Moves data from the input to the command reply buffer, and removes this data
// from the input buffer. Note that any data that may have been present in the
// reply buffer is overwritten, so be sure to mutex this section against
// cross-thread access.
//
void LuxBase::moveDataFromInputToCmdBuffer(UINT32 bytesToBeMoved) {
  if (m_beVerbose == true) {
    infoMessage("moveDataFromInputToCmdBuffer(): Moving " +
                toString(bytesToBeMoved) +
                " bytes from input to command buffer.");
    if (m_cmdBufferLevel > 0) {
      infoMessage(
          "moveDataFromInputToCmdBuffer(): ...and " +
          toString(m_cmdBufferLevel) +
          " bytes in the command buffer were destroyed in the process!");
    }
  }
 ...
 *
 * Note that if the host system is fast enough, this does not happen, but the
 * buffer is just invalidated.
 */
void LuxBase::removeDataFromInputBuffer(UINT32 bytesToBeRemoved) {
  if (bytesToBeRemoved == m_inBufferLevel) {
    ...
    printError("removeDataFromInputBuffer(): The buffer holds " +
               toString(m_inBufferLevel) + " bytes, but " +
               toString(bytesToBeRemoved) +
  ...
  memmove(&(m_inputBuffer[0]), &(m_inputBuffer[bytesToBeRemoved]),
          bytesRemaining);
  m_inBufferLevel = bytesRemaining;
}

/**
 * Returns the ID ("command") of the message in the reply buffer. If a complete
 * dataset is in the buffer, the return value is non-zero (=valid). Note that
 * the buffer may hold only one complete (!) dataset.
 *
 * Note: Access to command and input buffer must be mutex'ed, with same mutex to
 * avoid cross-overwrite while decoding!!
 */
UINT16 LuxBase::decodeAnswerInCmdReplyBuffer() {
  ...
  if (m_cmdBufferLevel == 0) {
    ...
    if (m_cmdBufferLevel == 0) {
      // Still no data
      printInfoMessage(
          "decodeAnswerInCmdReplyBuffer(): No data in input buffer.",
          beVerboseHere);
      return 0;
    }
  }
  ...
  if (m_cmdBufferLevel <= headerLen) {
    printError("decodeAnswerInCmdReplyBuffer(): ERROR: Not enough data in "
               "reply buffer for the data header, just " +
               toString(m_cmdBufferLevel) +
               " bytes available. Clearing buffer!");
    m_cmdBufferLevel = 0;
    return 0;
  }
  ...
  if (magicWord != 0xAFFEC0C2) {
    printError("decodeAnswerInCmdReplyBuffer(): Magic word not found, clearing "
               "cmd buffer.");
    m_cmdBufferLevel = 0;
    return 0;
  } else {
    // Magic word found
    printInfoMessage("decodeAnswerInCmdReplyBuffer(): Magic word found, now "
                     "decoding header.",
                     beVerboseHere);
  }
  ...
  if (m_cmdBufferLevel == (payloadLen + headerLen)) {
    ...
    printInfoMessage("decodeAnswerInCmdReplyBuffer(): Header decoded "
                     "successfully, command = " +
                         toHexString(commandId) + ".",
                     beVerboseHere);
  } else {
    printError("decodeAnswerInCmdReplyBuffer(): Inconsistent length of data in "
               "reply buffer, expected length was " +
               toString(payloadLen + headerLen) + ", but buffer length is " +
               toString(m_cmdBufferLevel) + " bytes - clearing buffer.");
    m_cmdBufferLevel = 0;
    return 0;
  }
...
void LuxBase::removeAnswerFromInputBuffer() {
  ...
  // Check if a command can be complete. Any command has at least the header + 2
  // bytes command ID.
  if (m_inBufferLevel <= (headerLen + 1)) {
    printWarning("LuxBase::removeAnswerFromInputBuffer: Not enough data in "
                 "input buffer to remove a dataset, aborting!");
    return;
  }
  ...
  if (magicWord != 0xAFFEC0C2) {
    printError("LuxBase::removeAnswerFromInputBuffer: Magic word does not "
               "match, aborting!");
    return;
  }
  ...
  if (m_inBufferLevel < msgLen) {
    printError("LuxBase::removeAnswerFromInputBuffer: The buffer does not hold "
               "enough data for the message!");
    return;
  }
  ...
  if (m_inBufferLevel == msgLen) {
    ...
    m_inBufferLevel = 0;
    printInfoMessage(
        "removeAnswerFromInputBuffer(): Clearing the whole buffer.",
        m_beVerbose);
  } else {
    ...
    printInfoMessage("LuxBase::removeAnswerFromInputBuffer(): Removed " +
                         toString(msgLen) + " bytes from buffer, new size is " +
                         toString(m_inBufferLevel) + " bytes.",
                     m_beVerbose);
  }
}
...
//
//
bool LuxBase::receiveMrsReply(MrsCommandId cmd, UINT32 timeoutMs,
                              UINT32 *value) {
  ...
  for (UINT32 i = 0; i < maxLoops; i++) {
    ...
    {
      ScopedLock lock(
          &m_inputBufferMutex); // Mutex for access to the input buffer
      UINT16 cmdInBuffer =
          decodeAnswerInCmdReplyBuffer(); // Get the command ID, if any
      if (cmdInBuffer == cmd) {
        // Ok, it is the wanted reply
        printInfoMessage("LuxBase::receiveMrsReply: " + m_longName +
                             " There is a valid message (cmd=0x" +
                             toHexString(cmdInBuffer) + ").",
                         beVerboseHere);
        if (cmd == CmdMrsGetStatus) {
          printInfoMessage("LuxBase::receiveMrsReply: " + m_longName +
                               " Decoding a status message.",
                           beVerboseHere);
          decodeGetStatus();
        }
        if (cmd == CmdMrsStopMeasure) {
          printInfoMessage("LuxBase::receiveMrsReply: " + m_longName +
                               " StopMeasure acknowledge received.",
                           beVerboseHere);
        }
        if (cmd == CmdMrsStartMeasure) {
          printInfoMessage("LuxBase::receiveMrsReply: " + m_longName +
                               " StartMeasure acknowledge received.",
                           beVerboseHere);
        }
        if (cmd == CmdMrsGetParameter) {
          printInfoMessage("LuxBase::receiveMrsReply: " + m_longName +
                               " GetParameter acknowledge received.",
                           beVerboseHere);
          if (value != NULL) {
      ...
      if (cmdInBuffer != 0) {
        printInfoMessage("LuxBase::receiveMrsReply: " + m_longName +
                             " Removing the command from the reply buffer.",
                         beVerboseHere);
        m_cmdBufferLevel = 0;
      }

      if (cmdInBuffer == 0x8000 + cmd) {
        printWarning("LuxBase::receiveMrsReply: " + m_longName +
                     " Received that an error occurred.");
        result = false;
        break;
      }
    }
  ...
  printInfoMessage("LuxBase::receiveMrsReply: " + m_longName + " Leaving.",
                   beVerboseHere);
  return result;
}
 ...
 */
bool LuxBase::sendMrsCommand(
    MrsCommandId cmd, UINT16 para,
    UINT32 value) // , boost::posix_time::time_duration timeOut)
{
  UINT8 cmdBuffer[256];
  ...
  case CmdMrsSetNTPTimestampFracSec:
    ...
    break;
  default:
    printError("LuxBase::sendMrsCommand: " + m_longName +
  ...
  UINT32 bufferPos;
  bufferPos = buildDataHeader(cmdBuffer, sizeOfThisMsg, m_deviceId,
                              0x2010); // DataTypeCommand);

  // Add the payload
  bufferPos += writeValueToBufferLE(&(cmdBuffer[bufferPos]), cmd,
                                    2); // Note: Little Endian always!
  UINT16 version = 3;
  bufferPos += writeValueToBufferLE(&(cmdBuffer[bufferPos]), version,
                                    2); // Note: Little Endian!

  // For a setParameter command, add the parameter index and the value to be set
  if (cmd == CmdMrsSetParameter || cmd == CmdMrsSetNTPTimestampSec ||
...
// Big-Endian-Write
//
UINT32 LuxBase::writeValueToBufferBE(UINT8 *buffer, UINT32 value,
                                     UINT8 numBytes) {
  switch (numBytes) {
  case 1:
    buffer[0] = value & 0xff;
  ...
  default:
    printError(
        "LuxBase::writeValueToBufferBE: " + m_longName +
...
// Little-Endian-Write
//
UINT32 LuxBase::writeValueToBufferLE(UINT8 *buffer, UINT32 value,
                                     UINT8 numBytes) {
  switch (numBytes) {
  case 1:
    buffer[0] = value & 0xff;
  ...
  default:
    printError(
        "LuxBase::writeValueToBufferLE: " + m_longName +
        " ERROR: Invalid number of bytes to write, can only handle 1,2 or 4.");
  }
...
/**
 * Write the data header for the command into the buffer.The buffer must be at
 * least 24 bytes long.
 *
 * Return value is the number of bytes that have been written.
 */
UINT32 LuxBase::buildDataHeader(UINT8 *buffer, UINT32 sizeOfThisMsg,
                                UINT8 deviceId, UINT16 dataType) {
...
bool LuxBase::run(bool weWantScanData, bool weWantObjectData) {
  ...
  if ((m_inputFileName == "") && (m_tcp.isOpen() == true)) {
    ...
    printInfoMessage("LuxBase::run(): We are called, but we are not connected. "
                     "Calling init() now.",
                     m_beVerbose);
    bool result = false;
    if (m_inputFileName == "") {
    ...
    if (result == false) {
      ...
      // We are connected
      printInfoMessage(
          "LuxBase::run(): Call to init() was successful, we are connected.",
          m_beVerbose);
    }
  }
  ...
  if (isRunning() == true) {
    ...
    printInfoMessage("LuxBase::run(): Duplicate RUN command, " + m_longName +
                         " is running already. Ignoring command.",
                     true);
    return true;
  }

  printInfoMessage("LuxBase::run(): Beginning scanner start sequence for " +
                       m_longName + ".",
                   m_beVerbose);
  ...
  if ((m_readOnlyMode == false) && (m_inputFileName == "")) {
    ...
    Position3D mp(m_yawAngle, m_pitchAngle, m_rollAngle, m_offsetX, m_offsetY,
                  m_offsetZ);
    ...
    infoMessage("LuxBase::run(): Running in read-only mode, so we're just "
                "using current sensor parameters.");
  }
...
void LuxBase::readCallbackFunction(UINT8 *buffer, UINT32 &numOfBytes) {
  bool beVerboseHere = false; // = m_beVerbose;
  printInfoMessage("LuxBase::readCallbackFunction(): Called with " +
                       toString(numOfBytes) + " available bytes.",
                   beVerboseHere);
  ...
  printInfoMessage("LuxBase::readCallbackFunction(): Transferring " +
                       toString(bytesToBeTransferred) +
                       " bytes from TCP to input buffer.",
                   beVerboseHere);
  ...
  if (bytesToBeTransferred > 0) {
    ...
    // There was input data from the TCP interface, but our input buffer was
    // unable to hold a single byte. Either we have not read data from our
    // buffer for a long time, or something has gone wrong. To re-sync, we clear
    // the input buffer here.
    m_inBufferLevel = 0;
  }

  printInfoMessage("LuxBase::readCallbackFunction(): Added " +
                       toString(bytesToBeTransferred) +
                       " bytes to input buffer, new fill level is now " +
                       toString(m_inBufferLevel) + " bytes.",
                   beVerboseHere);
}
...
bool LuxBase::stop() {
  ...
  if (result == true) {
    // Erfolg
    printInfoMessage("LuxBase::stop: LD-MRS device stopped scanning.",
                     m_beVerbose);
    m_isRunning = false;
  } else {
    // Fehler
