...
Object::Object()
    : m_objectId(0), m_flags(0), m_objectAge(0), m_hiddenStatusAge(0),
      m_timestamp(), m_classification(Object::Unclassified),
      m_classificationAge(0), m_classificationQuality(0.0f),
      m_centerPoint(Point2D(0, 0)), m_centerPointSigma(Point2D(0, 0)),
      m_courseAngle(0.0f), m_courseAngleSigma(0.0f),
      m_relativeVelocity(Point2D(0, 0)), m_relativeVelocitySigma(Point2D(0, 0)),
      m_absoluteVelocity(Point2D(0, 0)), m_absoluteVelocitySigma(Point2D(0, 0)),
      m_objectBox(Point2D(0, 0)), m_objectBoxSigma(Point2D(0, 0)),
      m_boundingBoxCenter(Point2D(0, 0)), m_boundingBox(Point2D(0, 0)),
      m_closestPoint(Point2D(0, 0)), m_contourPoints(), m_vehicleWLANid(0),
      m_objectHeight(0), m_objectHeightSigma(0), m_objectMass(0),
      m_maxAbsoluteVelocity(NaN_double), m_normalizedMeanPointDist(0),
      m_totalTrackingDuration(0), m_totalTrackedPathLength(0), m_isValid(true) {
}

Object::~Object() {}

bool Object::operator==(const Object &other) const {
  bool b = m_objectId == other.m_objectId && m_flags == other.m_flags &&
...
void Object::setCourseAngle(double new_angle) {
  ...
  if (!fuzzyCompare(normd_angle, new_angle)) {
    printWarning("Object::setCourseAngle was called with angle " +
                 ::toString(new_angle, 2) +
                 " which is outside of its [-pi,pi] definition range (but the "
                 "calling function should have ensured to normalize into that "
                 "interval) - normalizing it back into that range now (" +
...
void Object::setAbsoluteVelocity(const Point2D &v) {
  ...
  if (isNaN(getMaxAbsoluteVelocity()) || absolute > getMaxAbsoluteVelocity()) {
...
void Object::setBoundingBoxCenter(const Point2D &v) { m_boundingBoxCenter = v; }
...
std::string Object::toString() const {
  ...
  text += std::string(", classification=") +
          objectClassificationToString(m_classification);
  ...
  text +=
      ", normalizedMeanPointDist=" + ::toString(m_normalizedMeanPointDist, 2);
...
const char *Object::objectClassificationToString(ObjectClassification v) {
  switch (v) {
  case TYPE_TO_STR(Unclassified); case TYPE_TO_STR(UnknownSmall);
      case TYPE_TO_STR(UnknownBig); case TYPE_TO_STR(Pedestrian);
      case TYPE_TO_STR(Bike); case TYPE_TO_STR(Car); case TYPE_TO_STR(Truck);
      case TYPE_TO_STR(Structure_Pylon); case TYPE_TO_STR(Structure_Beacon);
      case TYPE_TO_STR(Structure_GuardRail);
      case TYPE_TO_STR(Structure_ConcreteBarrier); case TYPE_TO_STR(NumClasses);
      case TYPE_TO_STR(Unknown); default:
    throw std::invalid_argument("Unknown object classification " +
                                ::toString(int(v)));
    return "<unknown>";
  }
}
...
Object::stringToObjectClassification(const std::string &s) {
...
std::string
Object::objectClassificationToStringWithNum(ObjectClassification v) {
...
void Object::getObjectBoxVarCovar(double &var_x, double &var_y,
...
void Object::compensateEgoMovement(const Point2D& deltaPos, double deltaAngle)
{
        ...
        // transformation of the contour points - otherwise the contour points
are invalid!
...
void Object::incrementObjectAge() { m_objectAge++; }

void Object::setMaxAbsoluteVelocity(double v) {
  //	if (v < 0)
  //		throw InvalidArgumentException("setMaxAbsoluteVelocity called with
  //negative argument " + ::toString(v, 2) + ", but must be non-negative.");
  m_maxAbsoluteVelocity = v;
}
void Object::setNormalizedMeanPointDist(double v) {
  //	if (v < 0)
  //		throw InvalidArgumentException("setNormalizedMeanPointDist called
  //with negative argument " + ::toString(v, 2) + ", but must be
  //non-negative.");
  m_normalizedMeanPointDist = v;
}
void Object::setTotalTrackingDuration(double v) {
  //	if (v < 0)
  //		throw InvalidArgumentException ("setTotalTrackingDuration called
  //with negative argument " + ::toString(v, 2) + ", but must be
  //non-negative.");
...
void Object::setTotalTrackedPathLength(double v) {
  //	if (v < 0)
  //		throw InvalidArgumentException("setTotalTrackedPathLength called
  //with negative argument " + ::toString(v,2) + ", but must be non-negative.");
...
ObjectList::ObjectList() : base_class() {
...
/*
void ObjectList::compensateEgoMovement(const Point2D& deltaPos, double
deltaAngle)
{
        iterator obj;
