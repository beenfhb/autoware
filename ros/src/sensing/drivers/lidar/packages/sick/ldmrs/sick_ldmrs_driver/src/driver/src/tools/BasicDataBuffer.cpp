...
BasicDataBuffer::BasicDataBuffer()
    : m_beVerbose(false), m_bytesMax(0), m_bytesUsed(0) {
...
BasicDataBuffer::~BasicDataBuffer() {
  ...
  infoMessage("~BasicDataBuffer(): Destructor is done - object is dead.",
              m_beVerbose);
}
 ...
 *
 * Falls bereits zu viel Speicher benutzt wird, wird dieser nicht sofort
 * freigegeben, aber keine weiteren Elemente akzeptiert.
 */
 ...
 *
 */
UINT32 BasicDataBuffer::getUsedBytes() { return m_bytesUsed; }

/**
 * Liefert die Anzahl der aktuell gespeicherten Datensaetze.
 *
 */
UINT32 BasicDataBuffer::getBufferSize() { return m_buffer.size(); }

//
// Speichert den Zeiger (!) auf das Datum im Puffer, sofern der Platz im Puffer
// (mitgerechnet wird der Speicherplatz des Daten-Objekts) noch reicht.
//
...
//
// Liefert den Zeiger auf das aelteste Datum im Puffer. War kein Datum
// vorhanden, ist der Zeiger NULL.
//
// Die Bytes-Used-Verwaltung funktioniert nicht, falls die Daten im Puffer von
// der Applikation veraendert werden. Dann werden naemlich eine andere Anzahl
// Bytes entfernt, als hinzugefuegt wurden...
//
