...
Position3D::Position3D()
    : m_yawAngle(0), m_pitchAngle(0), m_rollAngle(0), m_point(0.0, 0.0, 0.0) {}
...
Position3D::Position3D(value_type yaw, value_type pitch, value_type roll,
                       const Point3D &pt)
    : m_yawAngle(yaw), m_pitchAngle(pitch), m_rollAngle(roll), m_point(pt) {}
...
bool Position3D::transformToVehicle(
    Point3D *pt) // double *dXPos, double *dYPos, double *dZPos, const CRect& r)
{
...
bool Position3D::operator==(const Position3D &other) const {
  return m_yawAngle == other.m_yawAngle && m_pitchAngle == other.m_pitchAngle &&
         m_rollAngle == other.m_rollAngle && m_point == other.m_point;
}
...
Point3D Position3D::toPoint3D() const { return m_point; }
...
std::string Position3D::toString() const {
  ...
  ostr << "(x " << getX() << ", y " << getY() << ", z " << getZ() << "m"
       << "; yaw " << getYawAngle() * rad2deg << ", pitch "
       << getPitchAngle() * rad2deg << ", roll " << getRollAngle() * rad2deg
       << "deg)";
  return ostr.str();
}
