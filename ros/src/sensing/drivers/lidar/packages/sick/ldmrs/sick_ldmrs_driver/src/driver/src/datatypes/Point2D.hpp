...
public:
  ...
  Point2D() : m_x(0), m_y(0) { m_datatype = Datatype_Point2D; }

  /** Constructs a point with the given coordinates (x, y). */
  Point2D(value_type x, value_type y) : m_x(x), m_y(y) {
  ...
  /** Constructs a point from the given Point3D. Its z-coordinate will be
   * ignored. */
  ...
  /// Returns the x/y components of this class, converted into a Point3D object
   ...
   * \param version 1 == compressed meter values, 4 bytes total; 2 == float
   * values, 8 bytes total
   */
  ...
  friend inline Point2D::value_type operator*(const Point2D &p1,
                                              const Point2D &p2);
...
inline bool Point2D::isZero() const {
  return (fuzzyCompare(m_x, value_type(0.0)) &&
          fuzzyCompare(m_y, value_type(0.0)));
}
...
inline Point2D::value_type Point2D::dist() const { return hypot(m_x, m_y); }

inline Point2D::value_type Point2D::angle() const { return atan2(m_y, m_x); }
...
inline void Point2D::toPolar(Point2D::value_type &r,
                             Point2D::value_type &psi) const {
  r = dist();
  psi = angle();
}

inline std::pair<Point2D::value_type, Point2D::value_type>
