 ...
 * exakt die Laenge digits_before_decimal_point + digits_after_decimal_point +
 * 1, ausser, die Vorkommazahl passte nicht in die Laengenvorgabe
...
std::string doubleToString(double val,
  ...
  if (dotPosition != std::string::npos) {
    ...
    if (dotPosition < digits_before_decimal_point) {
      ...
      std::string::size_type numExtraSpaces =
          digits_before_decimal_point - dotPosition;
  ...
  if (text.length() <
      (digits_before_decimal_point + digits_after_decimal_point + 1)) {
...
UINT32 memread_UINT32(BYTE *&buffer) {
  UINT32 value = (((UINT32)buffer[0]) << 24) + (((UINT32)buffer[1]) << 16) +
...
UINT16 memread_UINT16(BYTE *&buffer) {
  UINT16 value = (((UINT16)buffer[0]) << 8) + ((UINT16)buffer[1]);
  buffer += 2;
  return value;
}
...
//
// Read a float value from a buffer. The buffer has the value in Big Endian
// format, so it cannot be read directly on x86 machines.
//
