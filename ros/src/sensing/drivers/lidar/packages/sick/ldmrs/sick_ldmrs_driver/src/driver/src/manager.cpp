...
// ****************************************************************************
Manager::Manager() : m_beVerbose(false), m_nextSourceId(0) {
...
//
bool Manager::addAndRunDevice(Sourcetype deviceType, std::string deviceName,
                              UINT16 wantedId) {
  BasicDevice *device;
  ...
  case Sourcetype_LDMRS:
    ...
    break;
  default:
    printError("Manager::addAndRunDevice: Unknown device (Type=" +
               toString((UINT16)deviceType) + " , Name=" + deviceName +
               "). Ignoring this call!");
    return false;
  }
...
//
// Note that device needs to be on the heap and will automatically be deleted
// when the manager is destructed.
//
bool Manager::addAndRunDevice(devices::BasicDevice *device,
                              std::string deviceName, UINT16 wantedId) {
  ...
  std::string text = "Manager::addAndRunDevice: Created Device of type " +
                     toString(device->getDevicetype()) + " with Name " +
                     deviceName + " under the ID " +
                     toString(device->getSourceId()) +
  ...
  if (result == true) {
    printInfoMessage("Manager::addAndRunDevice: Device is initialized.",
                     beVerboseHere);
  } else {
    printError(
  ...
  result = device->run();
  if (result == true) {
    printInfoMessage("Manager::addAndRunDevice: Device is running.",
                     beVerboseHere);
  } else {
  ...
  if (m_sourceThread.isRunning() == false) {
    printInfoMessage(
        "Manager::addAndRunDevice: Added device, now running source thread.",
        beVerboseHere);
    m_sourceThread.run(this);
  }

  printInfoMessage("Manager::addAndRunDevice: All done, leaving.",
                   beVerboseHere);
  return result;
}
...
// It would be nice to add an ID-handling, to make the IDs safely unique!
//
bool Manager::addApplication(Sourcetype appType, std::string appName,
                             UINT16 wantedId) {
  ...
  case Sourcetype_MrsScanpointCoordinateApp:
    ...
    break;
  default:
    printError("Manager::addApplication: Unknown application (Type=" +
               toString((UINT16)appType) + " , Name=" + appName +
               "). Ignoring this call!");
    return false;
  }
...
bool Manager::addApplication(BasicApplication *app, UINT16 wantedId) {
  ...
  std::string text = "Manager::addApplication: Created application " +
                     toString(app->getApplicationType()) + " with Name " +
                     app->getApplicationName() + " under the ID " +
...
bool Manager::runAllDevices() {
  ...
  for (UINT32 i = 0; i < m_deviceList.size(); i++) {
    ...
    text = "Manager: Running device with ID " +
           toString(device->getSourceId()) + ".";
...
void Manager::stopAllDevices() {
  ...
  for (UINT32 i = 0; i < m_deviceList.size(); i++) {
    ...
    text = "Manager::stopAllDevices: Stopping device with ID " +
           toString(device->getSourceId()) + ".";
    infoMessage(text, m_beVerbose);
...
//
// Receiver for data from the devices. The data is forwarded to the
// applications.
//
...
void Manager::setDeviceData(BasicData *data) {
  ...
  ScopedLock lock(&m_sourceBufferMutex); // Wg. der folgenden Textmeldung. Der
                                         // Puffer selber ist sicher...
  bool result = m_sourceBuffer.pushData(data);
  if (result == true) {
    // Datum wurde hinzugefuegt
    if (m_beVerbose == true) {
      std::string text;
      text = "Manager::setDeviceData(): Added data. Device data buffer size is "
             "now " +
             toString(m_sourceBuffer.getBufferSize()) + " Elements (" +
             toString(m_sourceBuffer.getUsedBytes()) + " bytes).";
      infoMessage(text, m_beVerbose);
    }
  } else {
    // Datum konnte nicht hinzugefuegt werden
    std::string text;
    text = "Manager::setDeviceData(): Failed to add data. Device data buffer "
           "size is currently " +
           toString(m_sourceBuffer.getBufferSize()) + " Elements (" +
           toString(m_sourceBuffer.getUsedBytes()) +
           " bytes), and the buffer refused to add " +
           toString(data->getUsedMemory()) + " bytes.";
    printWarning(text);
 ...
 *
 * Pro Aufruf wird nur 1 Datum verteilt. Grund ist es, auch mit einem Prozessor
 * den anderen Threads ggf. genug Rechenleistung zu ueberlassen. Hier koennten
 * pro Aufruf aber auch mehr Daten verteilt werden, etwa wenn CAN-Nachrichten
 * mit hoher Frequenz eintreffen.
 *
...
void Manager::sourceThreadFunction(bool &endThread, UINT16 &waitTimeMs) {
  ...
  // Zugriffsschutz hier nicht erforderlich, da der Buffer selber Thread-Safe
  // ist.
  BasicData *data = NULL;
  ...
  if (data != NULL) {
    ...
    if (m_beVerbose == true) {
      std::string text;
      text = "Manager::sourceThreadFunction(): Popped a dataset of type " +
             toString(data->getDatatype()) + ", created from Source-ID " +
             toString(data->getSourceId()) + ".";
      infoMessage(text, m_beVerbose);
    }
  ...
  if (m_sourceBuffer.getBufferSize() > 0) {
    ...
    waitTimeMs = 0;
    //		infoMessage("Source fill level: " +
    //toString(m_sourceBuffer.getBufferSize()) + ".", true);
  } else {
    // Es sind aktuell keine weiteren Daten da, also ruhig etwas laenger
    // schlafen
    waitTimeMs = 2;
  }
}
...
devices::BasicDevice *Manager::getDeviceById(UINT32 id) {
  ...
  for (DeviceList::iterator iter = m_deviceList.begin();
       iter != m_deviceList.end(); ++iter) {
