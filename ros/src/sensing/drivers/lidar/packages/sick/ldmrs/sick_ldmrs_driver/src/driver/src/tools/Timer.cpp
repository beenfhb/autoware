...
TimeDuration Timer::elapsed() const {
  ...
  duration =
      diff.seconds(); //  (double)seconds + ((double)useconds / 1000000.0);
  return duration;
}
 ...
 * This function may be called without restarts as it measures the time between
 *consecutive calls to this function. Usage:
 *
 *		t;
 *		// ...
...
/**
 * Documentation see "elapsedMillisecondsSinceLastCall()", but it measures
