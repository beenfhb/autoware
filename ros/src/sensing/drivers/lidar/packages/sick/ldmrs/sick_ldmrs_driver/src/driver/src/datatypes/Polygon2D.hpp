 ...
 */
class Polygon2D : public std::vector<Point2D>, public BasicData {
public:
  ...
  inline virtual const UINT32 getUsedMemory() const {
    return ((sizeof(*this)) + sizeof(Point2D) * size());
  };
  ...
  static Polygon2D fromCircle(const Point2D &center,
  ...
  static Polygon2D fromEllipse(const Point2D &center,
  ...
  /// Static function to create a Polygon2D from ellipse parameters
  static Polygon2D fromEllipse(const Ellipse2D &ellipse,
                               const UINT32 samplingPoints = 32);
  ...
  static Polygon2D rhombus(const Point2D &center,
                           const floatingpoint_type radius);
   ...
   */
  static Polygon2D createRectangle(const Point2D &lowerLeft,
  ...
  /// Returns a Box in parallel to the coordinate system that bounds this
  /// polygon
  /**
