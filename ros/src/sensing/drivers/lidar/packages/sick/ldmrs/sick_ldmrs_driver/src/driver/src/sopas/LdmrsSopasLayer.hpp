...
public:
  LdmrsSopasLayer(Manager *manager, const UINT8 deviceID, std::string ipAddress,
                  UINT16 portNumber, bool weWantFieldData, bool weWantScanData,
                  bool readOnlyMode);

  virtual ~LdmrsSopasLayer();

  bool
  init(Tcp::DisconnectFunction function,
       void *obj); // UINT32 ipAddress, UINT16 portNumber, bool weWantFieldData,
                   // bool readOnlyMode, Tcp::DisconnectFunction function);
  bool run();
  ...
  bool isFieldDataSubscribed() const { return m_fieldEventIsRegistered; }
