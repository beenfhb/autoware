...
EvalCaseResult::EvalCaseResult()
    : uiVersionNo(0), CaseHdr(), m_eCaseResult(), m_sCaseName("no name"),
      m_sComment("no comment"), aTimeBlock() {
...
const UINT32 EvalCaseResult::getUsedMemory() const {
  return sizeof(*this) + m_sCaseName.length() + m_sComment.length();
}
...
std::string EvalCaseResult::toString() const {
  ...
  os << "EvalCaseResult members" << std::endl
     ...
     << "sCaseName:             "
     << ">" << m_sCaseName << "<" << std::endl
     << "sComment:              "
     << ">" << m_sComment << "<" << std::endl
     << "aTimeBlock:            " << (UINT16)(aTimeBlock.uiYear) << ":"
     << (UINT16)(aTimeBlock.usiMonth) << ":" << (UINT16)(aTimeBlock.usiDay)
     << ":" << (UINT16)(aTimeBlock.usiHour) << ":"
     << (UINT16)(aTimeBlock.usiMinute) << ":" << (UINT16)(aTimeBlock.usiSec)
