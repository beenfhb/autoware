...
/**
 *  Stores the given stringValue to the given buffer. The string is inserted at
 * position pos, and pos is increased by the number of bytes that were written.
 *  If the length of stringValue is longer than the remaining space of the
 * buffer, than filling stopps at the end of buffer.
 *
 *
 */
void addStringToBuffer(UINT8 *buffer, UINT16 &pos,
 ...
 *  bufferLength is the length of the full buffer (incl. magic word and
 *checksum).
 *
 ...
 */
std::string getIdentifierFromBuffer(UINT8 *buffer, UINT16 &nextData,
                                    UINT16 bufferLength);

/**
 *   Stores the given cmdBuffer (sopas command) to the given sendBuffer (with
 * header and checksum).
 */
void addFrameToBuffer(UINT8 *sendBuffer, UINT8 *cmdBuffer, UINT16 *len);

//  Returns the requested value. pos points then to the first byte of the next
//  data field.
 ...
 *  If width of intValue is greater than given type T, the higher bytes are
 * truncated.
 *
...
void addIntegerToBuffer(UINT8 *buffer, UINT16 &pos, T intValue) {
  ...
  for (int i = 0; i < width; i++) {
    buffer[pos + width - 1 - i] =
        (intValue >> (8 * i)) & 0xff; // BIG ENDIAN: width-1-i
  }
 ...
 *  Accepts all integer types (any bit width and sign).
 *
 *  Returns the requested intValue. pos points then to the first byte of the
 * next data field.
 */
template <typename T> T getIntegerFromBuffer(UINT8 *buffer, UINT16 &pos) {
  UINT16 width = sizeof(T);
 ...
 *
 *  The floatValue will be stored in BigEndian byte order (as required for
 * CoLaB). pos points then to the first byte of the next data field.
 */
template <typename T>
