 ...
 */
inline bool fuzzyCompare(double a, double b) { return std::abs(a - b) < 1E-11; }
 ...
 */
inline bool fuzzyCompare(float a, float b) { return std::abs(a - b) < 1E-6f; }
 ...
 */
template <typename floatT> inline bool isNaN(floatT x) { return (x != x); }
 ...
 */
template <typename IntT> inline IntT round_to_int(float floatValue) {

  //	assert (std::numeric_limits<float>::round_style ==
  //std::round_toward_zero); 	static_cast<IntT>() rounds toward zero
 ...
 * \return \a floatValue rounded to the closest integer
 */
template <typename IntT> inline IntT round_to_int(double floatValue) {

  //	assert (std::numeric_limits<double>::round_style ==
  //std::round_toward_zero); 	static_cast<IntT>() rounds toward zero
