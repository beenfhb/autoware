 ...
 */
double ScanPoint::getDistanceBetweenScanpoints(const ScanPoint &pt1,
                                               const ScanPoint &pt2) {
...
//
void ScanPoint::addCartesianOffset(double xOffset, double yOffset,
                                   double zOffset) {
  m_x += xOffset;
  m_y += yOffset;
  m_z += zOffset;
 ...
 * resulting angle will be normalized into the interval [-pi,+pi).
 */
void ScanPoint::addPolarOffset(double distOffset, double hAngleOffset,
                               double vAngleOffset) {
  m_dist += distOffset;
 ...
 * to keep polar and Cartesian coordinates consistent.
 *
 * \note Note that up to 6. June 2009, a negative vertical angle resulted in a
 * neg. \a z component which was incorrect. Now, the resulting \a z component is
 * positive to comply with DIN 70000.
 */
 ...
 */
void ScanPoint::setEchoWidth(double echoWidth) { m_echoWidth = echoWidth; }
...
std::ostream &operator<<(std::ostream &os, const ScanPoint &point) {
  os << "[(" << point.getX() << ", " << point.getY() << ", " << point.getZ()
     << ")"
     << " channel " << int(point.getLayer()) << " subch "
     << int(point.getEchoNum()) << " devID " << int(point.getSourceId()) << "]";
  return os;
}
...
bool operator==(const ScanPoint &p1, const ScanPoint &p2) {
  return (p1.getSourceId() == p2.getSourceId()) &&
         ...
         (p1.getX() == p2.getX() ||
          (::isNaN(p1.getX()) && ::isNaN(p2.getX()))) &&
         (p1.getY() == p2.getY() ||
          (::isNaN(p1.getY()) && ::isNaN(p2.getY()))) &&
