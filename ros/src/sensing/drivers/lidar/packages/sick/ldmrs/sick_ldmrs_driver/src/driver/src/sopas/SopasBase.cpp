...
const std::string
    SopasBase::COMMAND_Send_Event_ByIndex("SI"); // receive data event
...
const std::string
    SopasBase::COMMAND_Send_Event_ByName("SN"); // receive data event

const UINT16 SopasBase::INDEX_DEVICE_IDENT = 0;

SopasBase::SopasBase() : m_state(CONSTRUCTED) {
  m_beVerbose = false;
...
SopasBase::~SopasBase() {
  ...
  // Derived device must be stopped in it's constructor. No virtual functions
  // may be called here!
  //	assert(isRunning() == false);

  printInfoMessage("Sopas device destructor: Stopped, now disconnecting.",
                   m_beVerbose);
  ...
  //	printInfoMessage("Sopas device destructor: Disconnected, now deleting
  //socket.", m_beVerbose);

  printInfoMessage("Sopas device destructor: Done, device is deleted.",
                   m_beVerbose);
}
...
//
bool SopasBase::init(SopasProtocol protocol, std::string ipAddress,
                     UINT16 portNumber, bool weWantScanData,
                     bool weWantFieldData, bool readOnlyMode,
                     Tcp::DisconnectFunction disconnectFunction, void *obj) {
  m_protocol = protocol;
...
bool SopasBase::connect() {
  ...
  m_tcp.setReadCallbackFunction(&SopasBase::readCallbackFunctionS,
                                this); // , this, _1, _2));
  ...
  if (success == true) {
    ...
    if (success == true) {
      printInfoMessage("SopasBase::connect: Initialisation was successful.",
                       m_beVerbose);
    } else {
      printError(
...
//
bool SopasBase::isConnected() { return (m_state == CONNECTED); }
...
//
// (Un)set read only mode. Setting this mode, the sensor will ignore commands
// which will change any parameters on the device. The sensor itself will
// execute such commands.
//
void SopasBase::setReadOnlyMode(bool mode) { m_readOnlyMode = mode; }

bool SopasBase::isReadOnly() { return m_readOnlyMode; }
...
bool SopasBase::openTcpConnection() {
  printInfoMessage(
      "SopasBase::openTcpConnection: Connecting TCP/IP connection to " +
          m_ipAddress + ":" + toString(m_portNumber) + " ...",
      m_beVerbose);

  bool success = m_tcp.open(m_ipAddress, m_portNumber, m_beVerbose);
  if (success == false) {
    printError("SopasBase::openTcpConnection: ERROR: Failed to establish TCP "
               "connection, aborting!");
    return false;
  }
...
// Static entry point.
//
void SopasBase::readCallbackFunctionS(void *obj, UINT8 *buffer,
                                      UINT32 &numOfBytes) {
  ((SopasBase *)obj)->readCallbackFunction(buffer, numOfBytes);
}

/**
 * Read callback. Diese Funktion wird aufgerufen, sobald Daten auf der
 * Schnittstelle hereingekommen sind.
 */
void SopasBase::readCallbackFunction(UINT8 *buffer, UINT32 &numOfBytes) {
  bool beVerboseHere = false;
  printInfoMessage("SopasBase::readCallbackFunction(): Called with " +
                       toString(numOfBytes) + " available bytes.",
                   beVerboseHere);

  ScopedLock lock(&m_receiveDataMutex); // Mutex for access to the input buffer
  UINT32 remainingSpace =
      sizeof(m_receiveBuffer) - m_numberOfBytesInReceiveBuffer;
  UINT32 bytesToBeTransferred = numOfBytes;
  if (remainingSpace < numOfBytes) {
    bytesToBeTransferred = remainingSpace;
    printWarning("SopasBase::readCallbackFunction(): Input buffer space is to "
                 "small, transferring only " +
                 ::toString(bytesToBeTransferred) + " of " +
                 ::toString(numOfBytes) + " bytes.");
  } else {
    printInfoMessage("SopasBase::readCallbackFunction(): Transferring " +
  ...
  if (bytesToBeTransferred > 0) {
    ...
    memcpy(&(m_receiveBuffer[m_numberOfBytesInReceiveBuffer]), buffer,
    ...
    while (1) {
      ...
      size = frame.size();
      if (size == 0) {
        // Framesize = 0: There is no valid frame in the buffer. The buffer is
        // either empty or the frame is incomplete, so leave the loop
        printInfoMessage("SopasBase::readCallbackFunction(): No complete frame "
                         "in input buffer, we are done.",
                         beVerboseHere);
  ...
  printInfoMessage("SopasBase::readCallbackFunction(): Leaving. Current input "
                   "buffer fill level is " +
                       ::toString(m_numberOfBytesInReceiveBuffer) + " bytes.",
                   beVerboseHere);
}
  ...
  if (m_protocol == CoLa_A) {
    ...
    if (magicWord != 0x02020202) {
      ...
      for (i = 1; i <= m_numberOfBytesInReceiveBuffer - 4; i++) {
        pos = i; // this is needed, as the position value is updated by
      ...
      if (i > m_numberOfBytesInReceiveBuffer - 4) {
        ...
        memmove(&(m_receiveBuffer[0]), &(m_receiveBuffer[i]),
                bytesToMove); // payload+magic+length+s+checksum
    ...
    if (m_numberOfBytesInReceiveBuffer < 9) {
      ...
      printInfoMessage("SopasBase::findFrameInReceiveBuffer: Frame cannot be "
                       "decoded yet, only " +
                           ::toString(m_numberOfBytesInReceiveBuffer) +
                           " bytes in the buffer.",
                       m_beVerbose);
      return SopasEventMessage();
    }
    ...
    printInfoMessage(
        "SopasBase::findFrameInReceiveBuffer: Decoded payload length is " +
            ::toString(payloadlength) + " bytes.",
        m_beVerbose);
    ...
    if (payloadlength > (sizeof(m_receiveBuffer) - 9)) {
      ...
      printWarning("SopasBase::findFrameInReceiveBuffer: Frame too big for "
                   "receive buffer. Frame discarded with length:" +
    ...
    if ((payloadlength + 9) > m_numberOfBytesInReceiveBuffer) {
      ...
      printInfoMessage(
          "SopasBase::findFrameInReceiveBuffer: Frame not complete yet. "
          "Waiting for the rest of it (" +
              ::toString(payloadlength + 9 - m_numberOfBytesInReceiveBuffer) +
              " bytes missing).",
          m_beVerbose);
      return SopasEventMessage(); // frame not complete
    }

    // Calculate the total frame length in bytes: Len = Frame (9 bytes) +
    // Payload
    ...
    if (temp_xor != checkSum) {
      printWarning("SopasBase::findFrameInReceiveBuffer: Wrong checksum, Frame "
                   "discarded.");
      m_numberOfBytesInReceiveBuffer = 0;
      return SopasEventMessage();
    }
 ...
 * Waits for a certain answer by name. Event data (scans) are filtered and
 * processed by read thread.
 *
 * By Name: name = "<Name>"
 * timeout: Number of cycles to check for an answer (approx. 1ms per cycle)
 */
bool SopasBase::receiveAnswer(SopasCommand cmd, std::string name,
                              UINT32 timeout, SopasAnswer *&answer) {
  switch (m_protocol) {
  case CoLa_A:
    printInfoMessage("SopasBase::receiveAnswer: calling receiveAnswer_CoLa_A.",
                     m_beVerbose);
    return receiveAnswer_CoLa_A(cmd, name, timeout, answer);
  case CoLa_B:
    printInfoMessage("SopasBase::receiveAnswer: calling receiveAnswer_CoLa_B.",
                     m_beVerbose);
    return receiveAnswer_CoLa_B(cmd, name, timeout, answer);
  default:
    printWarning("SopasBase::receiveAnswer: Wrong protocol (is either not "
                 "CoLa-A or CoLa-B).");
  }
...
/**
 * Take answer from read thread and decode it.
 * Waits for a certain answer by index. Event data (scans) are filtered and
 * processed by read thread.
 *
 * By Name: index = "<Index>"
 * timeout: Number of cycles to check for an answer (approx. 1ms per cycle)
 */
bool SopasBase::receiveAnswer(SopasCommand cmd, UINT16 index, UINT32 timeout,
                              SopasAnswer *&answer) {
  switch (m_protocol) {
  case CoLa_A:
    printInfoMessage("SopasBase::receiveAnswer: Calling receiveAnswer_CoLa_A.",
...
//
//
bool SopasBase::receiveAnswer_CoLa_A(SopasCommand cmd, UINT16 index,
                                     UINT32 timeout, SopasAnswer *&answer) {
  printInfoMessage("SopasBase::receiveAnswer_CoLa_A: entering function.",
                   m_beVerbose);
      ...
      if (m_numberOfBytesInResponseBuffer > 0) {
        printInfoMessage(
            "SopasBase::receiveAnswer_CoLa_A: Response received (len= " +
                ::toString(m_numberOfBytesInResponseBuffer) + " bytes).",
            m_beVerbose);
        rxData = colaa::convertRxBufferToString(
            m_responseBuffer, m_numberOfBytesInResponseBuffer);
    ...
    if (rxData.length() > 0) {
      ...
      if (cmd == receivedCommand) {
        ...
        if (receivedIndex == index) {
          ...
          if (cmd == RA || cmd == AN) {
            printInfoMessage("SopasBase::receiveAnswer_CoLa_A: *** receive "
                             "answer with data in return *** ",
                             m_beVerbose);
            ...
            printInfoMessage("SopasBase::receiveAnswer_CoLa_A: Event (by "
                             "index) successfully (un)registered: " +
          ...
          printInfoMessage("SopasBase::receiveAnswer_CoLa_A: This is not the "
      ...
      // Error answer
        ...
        printInfoMessage("SopasBase::Error answer received: FA " + rxData + ".",
                         m_beVerbose);
        answer = new SopasAnswer((BYTE *)rxData.c_str(), rxData.length());
        return false;
      } else {
...
bool SopasBase::receiveAnswer_CoLa_A(SopasCommand cmd, std::string name,
                                     UINT32 timeout, SopasAnswer *&answer) {
  printInfoMessage("SopasBase::receiveAnswer_CoLa_A: entering function.",
                   m_beVerbose);
  ...
  for (UINT32 i = 0; i < timeout; i++) {
    ...
    if (rxData.length() > 0) {
      ...
      if (cmd == receivedCommand) {
        ...
        if (receivedName == name) {
          ...
          if (cmd == RA || cmd == AN) {
            ...
            printInfoMessage(
                "SopasBase::  Event successfully (un)registered: " +
                    receivedName + ".",
                m_beVerbose);
            m_numberOfBytesInResponseBuffer = 0;
            return true;
      ...
      // Error answer
        ...
        printInfoMessage(
            "SopasBase::  Error answer received: FA " + rxData + ".", true);
        answer = new SopasAnswer((BYTE *)rxData.c_str(), rxData.length());
        return false;
      } else {
        printInfoMessage("SopasBase::receiveAnswer_CoLa_A: This is not the "
...
bool SopasBase::receiveAnswer_CoLa_B(SopasCommand cmd, std::string name,
                                     UINT32 timeout, SopasAnswer *&answer) {
  printInfoMessage("SopasBase::receiveAnswer_CoLa_B: Entering function.",
                   m_beVerbose);
  ...
  for (UINT32 i = 0; i < timeout; i++) {
    if (m_numberOfBytesInResponseBuffer > 0) {
      //			if (m_beVerbose)
      //			{
      //				printInfoMessage("SopasBase::receiveAnswer_CoLa_B:
      //There is something in receive buffer." << std::endl;
      //			}
      ...
      SopasEventMessage frame(m_responseBuffer, CoLa_B,
                              m_numberOfBytesInResponseBuffer);

      // Parse Sopas Kommando
      //			receivedCommand =
      //stringToSopasCommand(colab::getCommandStringFromBuffer(m_responseBuffer));
      receivedCommand = stringToSopasCommand(frame.getCommandString());

      printInfoMessage("SopasBase::receiveAnswer_CoLa_B: receivedCommand= " +
                           frame.getCommandString() + ".",
                       m_beVerbose);

      if (receivedCommand == FA) {
        // Fehlermeldung vom Sensor
        nextData = 0;
        UINT16 errorCode = colab::getIntegerFromBuffer<UINT16>(
            &(m_responseBuffer[11]), nextData);
        printWarning(
      ...
      if (cmd != receivedCommand) {
        ...
        // da der sensor über sopas immer nur eine anfrage zur zeit bedienen
        // kann, koennen wir die Nachricht wegwerfen
        printInfoMessage(
            "SopasBase::receiveAnswer_CoLa_B: This is not the answer (" +
                sopasCommandToString(receivedCommand) + ") are waiting for (" +
                sopasCommandToString(cmd) + ").",
            m_beVerbose);
        m_numberOfBytesInResponseBuffer = 0;
        return false;
      }
      ...
      //			UINT16 nameLength =
      //colab::getIntegerFromBuffer<UINT16>(m_responseBuffer, nextData);
      //			receivedName =
      //colab::getStringFromBuffer(m_responseBuffer, nextData);

      receivedName = colab::getIdentifierFromBuffer(m_responseBuffer, nextData,
                                                    sizeof(m_responseBuffer));

      printInfoMessage("SopasBase::receiveAnswer_CoLa_B: receicedName= " +
                           receivedName + ".",
                       m_beVerbose);

      if (name != receivedName) {
        // Nicht die gesuchte Antwort
        printInfoMessage("SopasBase::receiveAnswer_CoLa_B: This is not the "
                         "answer we are waiting for. (" +
                             frame.getCommandString() + ", " + name + ").",
                         m_beVerbose);
      ...
      if (receivedCommand == WA) {
        ...
        printInfoMessage(
            "SopasBase::receiveAnswer_CoLa_B: Variable successfully set.",
            m_beVerbose);
        m_numberOfBytesInResponseBuffer = 0;
        return true;
      }
        ...
        printInfoMessage(
            "SopasBase::receiveAnswer_CoLa_B: Answer to (un)subscribe event " +
                receivedName + ".",
            m_beVerbose);
        m_numberOfBytesInResponseBuffer = 0;
        return true;
      ...
      // Antwort auf Methode ByName
        ...
        printInfoMessage(
            "SopasBase::receiveAnswer_CoLa_B: Answer to method call " +
                receivedName + ".",
            m_beVerbose);
        answer = new SopasAnswer(&m_responseBuffer[nextData],
                                 m_numberOfBytesInResponseBuffer - nextData);
        m_numberOfBytesInResponseBuffer = 0;
        return true;
        ...
        printInfoMessage(
            "SopasBase::receiveAnswer_CoLa_B: Unkown Sopas command.", true);
        m_numberOfBytesInResponseBuffer = 0;
        return false;
      } else {
        // unbekannter oder unbehandelter Befehl
        printInfoMessage(
            "SopasBase::receiveAnswer_CoLa_B: Untreated Sopas command.",
            m_beVerbose);
        m_numberOfBytesInResponseBuffer = 0;
        return false;
      }
  ...
  // Not successful, timeout.
  printInfoMessage("SopasBase::receiveAnswer_CoLa_B: Leaving with timeout.",
                   m_beVerbose);
...
//
bool SopasBase::receiveAnswer_CoLa_B(SopasCommand cmd, UINT16 index,
                                     UINT32 timeout, SopasAnswer *&answer) {
  bool beVerboseHere = m_beVerbose;
  //	beVerboseHere = true;

  printInfoMessage("SopasBase::receiveAnswer_CoLa_B_idx: Entering function.",
                   beVerboseHere);
  ...
  for (UINT32 i = 0; i < timeout; i++) {
    if (m_numberOfBytesInResponseBuffer > 0) {
      printInfoMessage("SopasBase::receiveAnswer_CoLa_B_idx: There is "
                       "something in receive buffer.",
                       beVerboseHere);

      // Protect response buffer
      ScopedLock lock(&m_receiveDataMutex);

      // There are only complete frames in response buffer, so we can just use
      // it here. Debug: Print buffer
      //			traceBuffer("Response in response buffer:",
      //m_responseBuffer, m_numberOfBytesInResponseBuffer);
      SopasEventMessage frame(m_responseBuffer, CoLa_B,
                              m_numberOfBytesInResponseBuffer);
      ...
      printInfoMessage(
          "SopasBase::receiveAnswer_CoLa_B_idx: ReceivedCommand= " +
              frame.getCommandString() + ".",
          beVerboseHere);
      ...
      UINT16 receivedIndex =
          colab::getIntegerFromBuffer<UINT16>(m_responseBuffer, nextData);
      //			printInfoMessage("SopasBase::receiveAnswer_CoLa_B_idx:
      //read command " + frame.getCommandString() + ".", beVerboseHere);
      if (receivedCommand == FA) {
        // Error message from the sensor. The next 2 bytes (in the receiveIndex)
        // are not the index but the error code.
        printWarning(
        ...
        // da der sensor über sopas immer nur eine anfrage zur zeit bedienen
        // kann, koennen wir die Nachricht wegwerfen
        printInfoMessage("SopasBase::receiveAnswer_CoLa_B_idx: This is not the "
                         "index we are waiting for (expected=" +
                             toString(index) +
                             ", received=" + toString(receivedIndex) + ").",
                         beVerboseHere);
        m_numberOfBytesInResponseBuffer = 0;
        return false;
      }
        ...
        // As the sensor can only handle one command at a time on the SOPAS
        // interface, we can discard the message.
        printInfoMessage(
            "SopasBase::receiveAnswer_CoLa_B_idx: This is not the answer (" +
                sopasCommandToString(receivedCommand) +
                ") we are waiting for (" + sopasCommandToString(cmd) + "). ",
            beVerboseHere);
        m_numberOfBytesInResponseBuffer = 0;
        return false;
      }
        ...
        // Variable schreiben erfolgreich
        printInfoMessage(
            "SopasBase::receiveAnswer_CoLa_B_idx: Variable successfully set.",
            beVerboseHere);
        m_numberOfBytesInResponseBuffer = 0;
        return true;
      }
        ...
        // Antwort auf Event-Abonnement setzen/loeschen.
        printInfoMessage("SopasBase::receiveAnswer_CoLa_B_idx: Event "
        ...
        printInfoMessage("SopasBase::receiveAnswer_CoLa_B_idx: Answer to "
                         "method call with index " +
                             ::toString(receivedIndex) + ".",
                         beVerboseHere);
        answer = new SopasAnswer(&m_responseBuffer[nextData],
                                 m_numberOfBytesInResponseBuffer - nextData);
        ...
        // Antwort auf das Lesen einer Variablen
        printInfoMessage("SopasBase::receiveAnswer_CoLa_B_idx: Answer to read "
                         "variable with index " +
                             ::toString(receivedIndex) + ".",
                         beVerboseHere);
        ...
        // unbekannter oder unbehandelter Befehl
        printInfoMessage(
            "SopasBase::receiveAnswer_CoLa_B_idx: Untreated Sopas command.",
            beVerboseHere);
        m_numberOfBytesInResponseBuffer = 0;
        return false;
      }
  ...
  // Not successful, timeout.
  printInfoMessage("SopasBase::receiveAnswer_CoLa_B_idx: Leaving with timeout.",
                   beVerboseHere);
...
void SopasBase::processFrame(SopasEventMessage &frame) {
  ...
  if (m_protocol == CoLa_A) {
    printInfoMessage(
        "SopasBase::processFrame: Calling processFrame_CoLa_A() with " +
            ::toString(frame.size()) + " bytes.",
        m_beVerbose);
    processFrame_CoLa_A(frame);
  } else if (m_protocol == CoLa_B) {
    printInfoMessage(
        "SopasBase::processFrame: Calling processFrame_CoLa_B() with " +
            ::toString(frame.size()) + " bytes.",
        m_beVerbose);
...
void SopasBase::processFrame_CoLa_A(SopasEventMessage &frame) {
  ...
  if (frame.getMessageType() == MSG_SEND_EVENT) {
    if (frame.size() > 8) {
      ...
      UINT32 i = 5; // Pointer to the fist Letter of the event Name (without the
                    // first 5 bytes: "STX's''S''N'' '")
      while (m_receiveBuffer[i] != ' ' && i < frame.size()) {
        // go to the ' ' after the event Name
        i++;
      }
      eventName =
          std::string((char *)m_receiveBuffer, 5, i - 5); // get the event Name

      if (m_decoderFunctionMapByName[eventName] != NULL) {
        // Not empty
        m_decoderFunctionMapByName[eventName](
            frame); // call the callback of the Event
      } else {
        // Das Event ist unbekannt
        printWarning("SopasBase::Received an unknown event! Event name is: " +
                     eventName + ".");
      }
      //
      //			if (eventName == scanString) // Scan data
      //(hopefully, event name not parsed yet...)
      //			{
      //				if
      //(!m_decoderFunctionMapByName[eventName].empty())
      //				{
      //					m_scanDecoderFunction(frame);
      //				}
      //				else
      //				{
      //					printInfoMessage("SopasBase::no
      //scan decoder registered - discarding data." << std::endl;
      //				}
      //			}
      //			else if (eventName == fieldString) // eval case
      //result data
      //			{
      //
      ...
      //				{
      //					printInfoMessage("SopasBase::no
      //eval case decoder registered - discarding data." << std::endl;
      //				}
      //			}
      //			else
      //			{ // Das Event ist unbekannt
      //				printWarning("SopasBase::Received an
      //unknown event! Short name is: " << eventName << "." << std::endl;
      //			}
    } else {
      // Das Kommando ist insgesamt zu kurz zum dekodieren
      printWarning("SopasBase::Received a very short (and unknown) event! "
                   "Frame length is " +
                   ::toString(frame.size()) + " bytes.");
    }
  } else {
...
//
// Note that this function typically runs in TCP callback context, not the
// device thread.
//
void SopasBase::processFrame_CoLa_B(SopasEventMessage &frame) {
  ...
  printInfoMessage("SopasBase::processFrame_CoLa_B: Command is " + command +
                       ".",
                   m_beVerbose);
  ...
  if (frame.getMessageType() == MSG_SEND_EVENT) // LDMRS: SI, LMS100: SN
  {
    if (frame.size() > 12) {
      ...
      //			printInfoMessage("SopasBase::SN empfangen,
      //Laenge = " << frameLength << "Bytes ." << std::endl;
      std::string eventName;
      UINT32 eventIndex = 0;
      UINT16 pos = 0;
      ...
      default: // by name
        printWarning("SopasBase::processFrame_CoLa_B: Decoding of events by "
      ...
      if (frameWasProcessed == false) {
        ...
        printWarning("SopasBase::processFrame_CoLa_B: Don't know how to "
                     "process the incoming event with the short name <" +
                     eventName + "> and the index " + ::toString(eventIndex) +
                     ", ignoring it!");
      }
    } else {
      // The frame was too short to be decoded
      printWarning("SopasBase::processFrame_CoLa_B: Received a very short (and "
                   "unknown) event! Frame length is " +
                   ::toString(frame.size()) + " bytes.");
    }
  } else {
    // Copy frame to response buffer. Old contents are destroyed.
...
void SopasBase::copyFrameToResposeBuffer(UINT32 frameLength) {
  printInfoMessage("SopasBase::copyFrameToResposeBuffer: Copying a frame of " +
                       ::toString(frameLength) + " bytes to response buffer.",
                   m_beVerbose);
  ...
  if (frameLength <= sizeof(m_responseBuffer)) {
    ...
    printError(
        ...
        " bytes) to response buffer because the response buffer is too small "
        "(buffer size=" +
...
void SopasBase::removeFrameFromReceiveBuffer(UINT32 frameLength) {
  ...
  if (frameLength < m_numberOfBytesInReceiveBuffer) {
    ...
    printInfoMessage("SopasBase::removeFrameFromReceiveBuffer: Removing " +
                         ::toString(frameLength) +
                         " bytes from the input buffer. New length is " +
                         ::toString(newLen) + " bytes.",
                     m_beVerbose);
    memmove(m_receiveBuffer, &(m_receiveBuffer[frameLength]), newLen);
    m_numberOfBytesInReceiveBuffer = newLen;
  } else {
    // No other data in buffer, just mark as empty
    printInfoMessage("SopasBase::removeFrameFromReceiveBuffer: Done, no more "
 ...
 * and convert string to enum value accordingly. The command string is removed
 * from input.
 */
...
SopasBase::SopasCommand
SopasBase::stringToSopasCommand(const std::string &cmdString) {
  // Request by Name
  if (cmdString == "RN") {
    return RN;
  } // Read Variable
  ...
  // Response
    ...
    printError("SopasBase::stringToSopasCommand: Trying to resolve an unknown "
               "command: " +
               cmdString + ".");
  }
  return CMD_UNKNOWN;
...
std::string SopasBase::sopasCommandToString(SopasCommand cmd) {
  ...
  // Response
    ...
    printError("SopasBase::sopasCommandToString: Trying to resolve an unknown "
               "command!");
  }
...
void SopasBase::colaB_decodeScannerTypeAndVersion(UINT8 *buffer, UINT16 pos) {
  printInfoMessage(
      "SopasBase::colaB_decodeScannerTypeAndVersion: Entering function.",
      m_beVerbose);
  ...
  printInfoMessage("SopasBase::colaB_decodeScannerTypeAndVersion: scanner '" +
                       m_scannerName + "', version '" + m_scannerVersion + "'.",
                   m_beVerbose);
}
...
//
//
bool SopasBase::invokeMethod(const std::string &methodeName, BYTE *parameters,
                             UINT16 parametersLength, SopasAnswer *&answer) {
  // Build command
  BYTE cmdBuffer[128];
  ...
  case CoLa_A:
    cmdBufferLen +=
        colaa::addStringToBuffer(cmdBuffer, COMMAND_Invoke_Method_ByName);
    cmdBuffer[cmdBufferLen++] = ' ';
    // Name
    cmdBufferLen +=
        colaa::addStringToBuffer(&(cmdBuffer[cmdBufferLen]), methodeName);
    ...
    break;
  case CoLa_B:
    colab::addStringToBuffer(cmdBuffer, cmdBufferLen,
                             COMMAND_Invoke_Method_ByName);
    // add length of string as UINT16
    colab::addIntegerToBuffer<UINT16>(cmdBuffer, cmdBufferLen,
                                      methodeName.size());
    colab::addStringToBuffer(cmdBuffer, cmdBufferLen,
                             methodeName); // remote method name
    break;
  }

  if (parametersLength > 0) {
    // add parameters (which must be already in the right encoding (colaa or
    // colab))
  ...
  // Evaluate answer
  if (result == true) {
    printInfoMessage("SopasBase::invokeMethod: Calling of " + methodeName +
                         " was successful.",
                     m_beVerbose);
  } else {
    printWarning("SopasBase::invokeMethod: Calling of " + methodeName +
                 " was NOT successful.");
  }
...
//
//
bool SopasBase::invokeMethod(UINT16 index, BYTE *parameters,
                             UINT16 parametersLength, SopasAnswer *&answer) {
  // Build command
  BYTE cmdBuffer[128];
  UINT16 cmdBufferLen = 0;
  ...
  switch (m_protocol) {
  case CoLa_A:
    printError("SopasBase::invokeMethod: Invoke method cola-a by index not "
               "supported.");
    return false;
    break;
  case CoLa_B:
    colab::addStringToBuffer(cmdBuffer, cmdBufferLen,
                             COMMAND_Invoke_Method_ByIndex);
    // add length of string as UINT16
  ...
  // Evaluate answer
  if (result == true) {
    printInfoMessage("SopasBase::invokeMethod: Calling of method with index=" +
                         ::toString(index) + " was successful.",
                     m_beVerbose);
  } else {
    printWarning("SopasBase::invokeMethod: Calling of method with index=" +
                 ::toString(index) + " was NOT successful.");
  }
...
//
//
bool SopasBase::readVariable(const std::string &variableName,
                             SopasAnswer *&answer) {
  // Build command
  BYTE cmdBuffer[128];
  UINT16 cmdBufferLen = 0;
  ...
  if (m_protocol == CoLa_A) {
    cmdBufferLen +=
        colaa::addStringToBuffer(&(cmdBuffer[0]), COMMAND_Read_Variable_ByName);
    cmdBuffer[cmdBufferLen++] = ' ';
    cmdBufferLen +=
        colaa::addStringToBuffer(&(cmdBuffer[cmdBufferLen]), variableName);
  } else {
    // up to now only tested with LD-MRS
    colab::addStringToBuffer(cmdBuffer, cmdBufferLen,
                             COMMAND_Read_Variable_ByName);
    cmdBuffer[cmdBufferLen++] = ' ';
  ...
  if (result) {
    printInfoMessage("SopasBase::readVariable: Answer to " + variableName +
                         " received.",
                     m_beVerbose);
  } else {
    printWarning("SopasBase::readVariable: Answer to " + variableName +
                 " not successful.");
  }
...
bool SopasBase::readVariable(UINT16 index, SopasAnswer *&answer) {
  ...
  if (m_protocol == CoLa_A) {
    cmdBufferLen += colaa::addStringToBuffer(&(cmdBuffer[0]),
                                             COMMAND_Read_Variable_ByIndex);
    cmdBuffer[cmdBufferLen++] = ' ';
    cmdBufferLen +=
        colaa::addUINT32ToBuffer(&(cmdBuffer[cmdBufferLen]), (UINT32)index);
  } else {
    colab::addStringToBuffer(cmdBuffer, cmdBufferLen,
                             COMMAND_Read_Variable_ByIndex);
    colab::addIntegerToBuffer<UINT16>(cmdBuffer, cmdBufferLen, index);
  ...
  if (result) {
    printInfoMessage("SopasBase::readVariable: Answer to " + ::toString(index) +
                         " received.",
                     m_beVerbose);
  } else {
    printWarning("SopasBase::readVariable: Answer to " + ::toString(index) +
                 " not successful.");
  }
...
bool SopasBase::writeVariable(const std::string &variableName, BYTE *parameters,
                              UINT16 parametersLength) {
  if (m_readOnlyMode) {
    printInfoMessage("SopasBase::writeVariable: ReadOnly Modus - ignore "
                     "writing to variable '" +
                         variableName + "'",
                     m_beVerbose);
    return true;
  ...
  if (m_protocol == CoLa_A) {
    ...
    printError("SopasBase::writeVariable: Write variable cola-b by Name: NOT "
               "IMPLEMENTED");
    return false;
    // 		colab::addStringToBuffer(cmdBuffer, cmdBufferLen,
    // COMMAND_Write_Variable_ByName);
    // 		colab::addIntegerToBuffer<UINT16>(cmdBuffer, cmdBufferLen,
    // variableName.size()); // add length of string as UINT16
    // 		colab::addStringToBuffer(cmdBuffer, cmdBufferLen, variableName);
    // 		or ?
    // 		colab::addStringToBuffer(cmdBuffer, cmdBufferLen,
    // COMMAND_Write_Variable_ByName); 		colab::addStringToBuffer(cmdBuffer,
    // cmdBufferLen, variableName);
  }
  ...
  if (result) {
    printInfoMessage("SopasBase::writeVariable: Answer to " + variableName +
                         " received.",
                     m_beVerbose);
  } else {
    printInfoMessage("SopasBase::writeVariable: Answer to " + variableName +
...
//
bool SopasBase::writeVariable(UINT16 variableIndex, BYTE *parameters,
                              UINT16 parametersLength) {
  bool beVerboseHere = m_beVerbose;
  //	beVerboseHere = true;
  ...
  printInfoMessage(
      "SopasBase::writeVariable: Sending command buffer now (payload len=" +
          toString(parametersLength + 4) + " bytes).",
      beVerboseHere);
  sendCommandBuffer(cmdBuffer, cmdBufferLen);

  printInfoMessage(
      "SopasBase::writeVariable: Command sent, waiting for reply...",
      beVerboseHere);
  SopasAnswer *answer = NULL;
  // Wait for answer
  ...
  if (result) {
    printInfoMessage("SopasBase::writeVariable: Answer to " +
                         toString(variableIndex) + " received.",
                     beVerboseHere);
  } else {
    printWarning("SopasBase::writeVariable: Answer to " +
                 toString(variableIndex) + " not successful!");
  }

  printInfoMessage("SopasBase::writeVariable: All done, leaving.",
                   beVerboseHere);
  return result;
}
...
bool SopasBase::registerEvent(const std::string &eventName) {
  ...
  if (m_protocol == CoLa_A) {
    cmdBufferLen += colaa::addStringToBuffer(&(cmdBuffer[0]),
                                             COMMAND_Register_Event_ByName);
    cmdBuffer[cmdBufferLen++] = ' ';
    cmdBufferLen +=
        colaa::addStringToBuffer(&(cmdBuffer[cmdBufferLen]), eventName);
    cmdBuffer[cmdBufferLen++] = ' ';
    cmdBuffer[cmdBufferLen++] = '1';
  } else {
    // traceError(SOPASBASE_VERSION) << "register event cola-B by name not
    // supported." << std::endl;
    colab::addStringToBuffer(cmdBuffer, cmdBufferLen,
                             COMMAND_Register_Event_ByName);
    cmdBuffer[cmdBufferLen++] = ' ';
...
bool SopasBase::registerEvent(UINT16 index) {
  ...
  case CoLa_B:
    colab::addStringToBuffer(cmdBuffer, cmdBufferLen,
                             COMMAND_Register_Event_ByIndex);
    colab::addIntegerToBuffer<UINT16>(cmdBuffer, cmdBufferLen, index);
    colab::addIntegerToBuffer<UINT8>(cmdBuffer, cmdBufferLen,
                                     1); // 1 to subscribe
    break;
  }
  ...
  if (result == true) {
    printInfoMessage(
        "SopasBase::registerEvent: Calling of register with index=" +
            ::toString(index) + " was successful.",
        m_beVerbose);
  } else {
    printError("SopasBase::registerEvent: Calling of method with index=" +
               ::toString(index) + " was NOT successful.");
  }
  return result;
}
...
bool SopasBase::unregisterEvent(UINT16 index) {
  ...
  switch (m_protocol) {
  case CoLa_A:
    printError("SopasBase::unregisterEvent: Unregister event cola-a by index "
               "not supported.");
  ...
  // Evaluate answer
  if (result == true) {
    printInfoMessage("SopasBase::calling of register with index=" +
                         ::toString(index) + " was successful.",
                     m_beVerbose);
  } else {
    printInfoMessage("SopasBase::calling of register with index=" +
...
bool SopasBase::unregisterEvent(const std::string &eventName) {
  ...
  if (m_protocol == CoLa_A) {
    ...
    //		traceError(SOPASBASE_VERSION) << "unregister event cola-B by name not
    //supported." << std::endl;
    colab::addStringToBuffer(cmdBuffer, cmdBufferLen,
                             COMMAND_Register_Event_ByName);
    cmdBuffer[cmdBufferLen++] = ' ';
...
SopasEventMessage::SopasEventMessage()
    : m_buffer(NULL), m_protocol(SopasBase::CoLa_A), m_frameLength(0),
      m_encoding(SopasBase::ByName) {}
...
UINT32 SopasEventMessage::getPayLoadLength() const {
  ...
  case SopasBase::CoLa_B:
    payLoadLength =
        m_frameLength - 9; // everything except start 0x02020202(4byte),
                           // payloadLength(4byte) and checksum(1 byte)
  }
...
INT32 SopasEventMessage::getVariableIndex() {
  ...
  if (m_encoding != SopasBase::ByIndex) {
    ...
    printWarning("SopasEventMessage::getVariableIndex: Encoding is not "
                 "ByIndex, aborting!");
    return index;
  }
...
//
// Read the variable name from a sensor message. This works only if the encoding
// "ByName" is used!
//
std::string SopasEventMessage::getVariableName() {
  ...
  if (m_encoding == SopasBase::ByName) {
    switch (m_protocol) {
    case SopasBase::CoLa_A:
      printError("SopasEventMessage::getVariableName: Protocol CoLa-A is not "
                 "supported, aborting!");
      return "";
      break;
    case SopasBase::CoLa_B:
...
void SopasEventMessage::detectMessageType() {
  ...
  } else if (command == SopasBase::COMMAND_Send_Event_ByIndex ||
             command == SopasBase::COMMAND_Send_Event_ByName) {
...
SopasAnswer::SopasAnswer(const BYTE *answer, UINT32 answerLength)
