...
public:
  ...
  void transformToTangentialPlane(const PositionWGS84 &origin, double *easting,
                                  double *northing, double *height) const;
  void transformToTangentialPlane(const PositionWGS84 &origin, double *easting,
                                  double *northing) const;
  void transformFromTangentialPlane(double dX, double dY,
                                    const PositionWGS84 &origin);

  /**
   * Returns the distance, in [m], to the given position. Note that this
   * distance is reliable only in the vicinity of our position, for some 100 m,
   * before the curvature of the earth introduces a relatively high error.
   */
   ...
   * \brief Sets the latitude value of the WGS-84 position.
   *
   * \param[in] Dm Latitude value in DDmm.mmmmm, where D stands for value in
   * degree and m for value in decimal minutes (signed).
   *
   * \sa setLatitudeInDeg()
   * \sa setLatitudeInRad()
   * \sa setLongitudeInRad()
   ...
   *
   * \param[in] Dm Latitude value in DDmm.mmmmm, where D stands for value in
   * degree and m for value in decimal minutes. \param[in] H Hemisphere. For
   * longitude it can be [W]est or [E]ast.
   *
   * \sa setLongitudeInRad()
   * \sa setLongitudeInDeg()
   * \sa setLatitudeInRad()
  ...
  void setCourseAngleSigmaInDeg(double val) {
    setCourseAngleSigmaInRad(val * deg2rad);
  }
  ...
  void setYawAngleSigmaInDeg(double val) {
    setYawAngleSigmaInRad(val * deg2rad);
  }
  ...
  void setPitchAngleSigmaInDeg(double val) {
    setPitchAngleSigmaInRad(val * deg2rad);
  }
  ...
  void setRollAngleSigmaInDeg(double val) {
    setRollAngleSigmaInRad(val * deg2rad);
  }
  ...
  double getPitchAngleSigmaInDeg() const {
    return (m_pitchAngleSigma * rad2deg);
  }
...
private:
  ...
  // NOTE 2008/01/18 (dw) : Change VERSION in cpp file when the interface is
  // changed or bugs are fixed.
  //						  Document changes in the HISTORY
  //above.
  ...
  //								   Unit
  //Serialized size:
  //	UINT8 m_deviceID;				// ID of device
  //+ 1 Bytes
  double m_latitude;         // [rad] + 8 Bytes
  double m_latitudeSigma;    // [m]								+ 8
                             // Bytes
  double m_longitude;        // [rad]							+
                             // 8 Bytes
  double m_longitudeSigma;   // [m]								+ 8
                             // Bytes
  double m_altitudeMSL;      // [m]								+ 8
                             // Bytes
  double m_altitudeMSLSigma; // [m]
                             // + 8 Bytes
  double
      m_courseAngle;                // [rad] + 8 Bytes
  double m_courseAngleSigma;        // [rad]
                                    // + 8 Bytes
  double m_yawAngle;                // [rad] + 8 Bytes
  double m_yawAngleSigma;           // [rad]							+ 8
                                    // Bytes
  double m_pitchAngle;              // [rad]							+ 8
                                    // Bytes
  double m_pitchAngleSigma;         // [rad]							+ 8
                                    // Bytes
  double m_rollAngle;               // [rad]							+
                                    // 8 Bytes
  double m_rollAngleSigma;          // [rad]							+ 8
                                    // Bytes
  PositionWGS84SourceType m_source; // Source of position information	+ 2 Bytes
                                    // (encoded as UINT16)
                                    //																	= 115
                                    //Bytes
};
