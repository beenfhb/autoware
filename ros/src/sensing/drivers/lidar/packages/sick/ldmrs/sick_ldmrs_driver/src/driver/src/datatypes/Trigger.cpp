...
Trigger::Trigger() : m_number(0) { m_datatype = Datatype_Trigger; }

/**
 * \param number Number of this message (0..UINT32)
 */
Trigger::Trigger(UINT32 number, UINT8 sourceId) : m_number(number) {
...
void Trigger::setNumber(UINT32 number) { m_number = number; }
...
std::string Trigger::toString() {
  ...
  stream << "DeviceID=" << m_sourceId << ", Number=" << m_number;
