...
//
// Konvertiert ein ASCII-Zeichen (= Hex-Wert-Nibble oder Ziffer) in seinen
// Zahlenwert.
//
UINT16 getValueOfChar(UINT8 c) {
  ...
  if ((c >= '0') && (c <= '9')) {
    ...
    throw std::out_of_range(
        "Unknown character where 0..9 or A..F was expected: '" +
        std::string(c, 1) + "'");
  }
...
UINT8 nibbleToAscii(UINT8 value) {
  ...
  if (value > 0x0f) {
    throw std::out_of_range("Tried to convert value > 0x0f into hex-nibble: " +
...
UINT16 detail::writeToBuffer(BYTE *buffer, double value) {
  ...
  printWarning("detail::writeToBuffer: Warning - Writing of floating-point "
               "values has not been cross-checked in Cola-A format!");
...
std::string getNextStringToken(std::string *rxData) {
  ...
  // Token finden
  size_type tokenStart =
      rxData->find_first_not_of(' '); // Fuehrende Spaces loeschen
...
/**
 * Lese einen String aus dem Empfangspuffer. Der Empfangspuffer wird
 * anschliessend um die Stringlaenge + 1 Zeichen (Trenn-Leerzeichen) gekuerzt.
 *
 * Ist der Parameter len = 0 (oder fehlt ganz), dann wird der naechste
 * verfuegbare String ausgelesen. Ist len > 0, dann werden genau len Zeichen
 * gelesen.
 */
...
/**
 * Lese einen INT-Wert aus dem Empfangspuffer.
 * Es wird vorausgesetzt, dass der übergebene String nur den Token für die Zahl
 * enthält. Ergebnis ist ein INT32-Wert.
 */
 ...
 * Ergebniswerts, usw.
 *
 * HINWEIS: der Iterator wird weitergeschoben len-1 mal. Um das nächste Element
 * zu lesen muss vorher ++tok aufgerufen werden.
 *
...
tokenizer::const_iterator& end, UINT16 len)
{
        ...
        assert (len < 5);	// Wir koennen nur bis zu 4 Bytes in einen
UINT32 packen
