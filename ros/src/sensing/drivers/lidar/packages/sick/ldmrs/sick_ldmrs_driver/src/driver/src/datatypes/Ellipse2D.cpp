...
Ellipse2D::Ellipse2D() : m_center(0, 0), m_radius(0, 0), m_rotation(0) {
  m_datatype = Datatype_Circle2D;
}

Ellipse2D::Ellipse2D(const Point2D &center, const Point2D &radius,
                     value_type rotation)
...
Ellipse2D::Ellipse2D(value_type x_center, value_type y_center,
                     value_type x_radius, value_type y_radius,
                     value_type rotation)
    : m_center(x_center, y_center), m_radius(x_radius, y_radius),
      m_rotation(rotation) {
  assert((m_radius.getX() >= 0.0) || isNaN(m_radius.getX()));
  assert((m_radius.getY() >= 0.0) || isNaN(m_radius.getY()));
  verifyNumericRanges();
...
bool Ellipse2D::containsPoint(const Point2D &point) const {
  if (fuzzyCompare(m_rotation, 0.0)) {
    ...
    // rotated point).
    if (std::max(std::abs(deltaX), std::abs(deltaY)) >
        m_radius.getX() + m_radius.getY())
      return false;
...
void Ellipse2D::verifyNumericRanges() {
  ...
  if (m_radius.getX() < 0 || m_radius.getY() < 0) {
    ...
    printWarning("Radius of Ellipse2D was given as negative value - silently "
                 "using the absolute value instead.");
    // This is probably better than throwing an exception.
    // throw std::out_of_range("Radius of Ellipse2D negative - this is an
    // invalid Ellipse2D.");
  }

  if (isNaN(m_radius.getX())) {
    m_radius.setX(0);
    printWarning("Radius.getX() of Ellipse2D was given as NaN value - silently "
                 "using Zero instead.");
  }
  if (isNaN(m_radius.getY())) {
    m_radius.setY(0);
    printWarning("Radius.getY() of Ellipse2D was given as NaN value - silently "
                 "using Zero instead.");
  }
  if (isNaN(m_rotation)) {
    m_rotation = 0;
    printWarning("Rotation of Ellipse2D was given as NaN value - silently "
                 "using Zero instead.");
  }
