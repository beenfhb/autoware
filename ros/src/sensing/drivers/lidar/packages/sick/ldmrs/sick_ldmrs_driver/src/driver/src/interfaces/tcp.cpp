...
bool Tcp::write(UINT8 *buffer, UINT32 numberOfBytes) {
  ...
  if (bytesSent != (INT32)numberOfBytes) {
    ...
    // Erfolg
    printInfoMessage("Tcp::write: Sent " + toString(numberOfBytes) +
                         " bytes to client.",
                     m_beVerbose);
    result = true;
  }
...
bool Tcp::isOpen() {
  if (m_connectionSocket >= 0) {
    //		printInfoMessage("Tcp::isOpen: Reporting open connection.",
    //m_beVerbose);
    return true;
  }
...
//
bool Tcp::open(std::string ipAddress, UINT16 port,
               bool enableVerboseDebugOutput) {
  INT32 result;
  m_beVerbose = enableVerboseDebugOutput;

  //	printInfoMessage("Tcp::open: Setting up input buffer with size=" +
  //convertValueToString(requiredInputBufferSize) + " bytes.", m_beVerbose);
  ...
  printInfoMessage("Tcp::open: Connecting. Target address is " + ipAddress +
                       ":" + toString(port) + ".",
                   m_beVerbose);
  ...
  if (result < 0) {
    ...
    std::string text = "Tcp::open: Failed to open TCP connection to " +
                       ipAddress + ", aborting.";
    printError(text);
    return false;
  }
  ...
  printInfoMessage(
      "Tcp::open: Connection established. Now starting read thread.",
      m_beVerbose);
...
void Tcp::readThreadFunction(bool &endThread, UINT16 &waitTimeMs) {
  ...
  if (result < 0) {
    ...
    if (m_readThread.m_threadShouldRun == true) {
      ...
      printInfoMessage("Tcp::readThreadMain: Connection is lost! Read thread "
                       "terminates now.",
                       m_beVerbose);
    }
    waitTimeMs = 0;
...
INT32 Tcp::readInputData() {
  ...
  if (recvMsgSize < 0) {
    ...
    // Erfolg
    printInfoMessage("Tcp::readInputData: Read " + toString(recvMsgSize) +
    ...
    // Verbindungsabbruch
    printInfoMessage("Tcp::readInputData: Read 0 bytes - connection is lost!",
                     true);
...
void Tcp::close() {
  ...
  if (isOpen() == true) {
    ...
    printInfoMessage(
        "Tcp::close: Waiting for the server thread to terminate...",
        m_beVerbose);
    ...
    printInfoMessage(
        "Tcp::close: Nothing to do - no open connection? Aborting.",
        m_beVerbose);
  }
...
void Tcp::stopReadThread() {
  ...
  printInfoMessage("Tcp::stopReadThread: Done - Read thread is now closed.",
                   m_beVerbose);
}
 ...
 */
UINT32 Tcp::getNumReadableBytes() { return m_rxBuffer.size(); }
...
std::string Tcp::readString(UINT8 delimiter) {
  ...
  if (m_rxString.length() > maxStringLength) {
    if (m_longStringWarningPrinted == false) {
      // Die lange Version
      printWarning("Receive-String has excessive length (" +
                   toString(m_rxString.length()) +
                   " bytes). Clearing string. On serial devices, incorrect "
                   "bitrate settings may cause this behaviour.");
      m_longStringWarningPrinted = true;
    } else {
      // Die Kurzfassung
      printWarning("Receive-String has excessive length (" +
                   toString(m_rxString.length()) + " bytes). Clearing string.");
    }
    m_rxString.clear();
  }
