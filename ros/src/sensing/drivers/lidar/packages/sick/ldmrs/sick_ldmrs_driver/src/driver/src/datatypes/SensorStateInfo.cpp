...
SensorStateInfo::SensorStateInfo() { m_datatype = Datatype_SensorStateInfo; }
...
const EvalCases &SensorStateInfo::getEvalCases() const { return m_evalCases; }

const Fields &SensorStateInfo::getFields() const { return m_fields; }
...
void SensorStateInfo::setInputStates(
    const SensorStateInfo::StateVector &m_inputStates) {
  this->m_inputStates = m_inputStates;
}

void SensorStateInfo::setMeasurementList(
    const MeasurementList &m_measurementList) {
  this->m_measurementList = m_measurementList;
}

void SensorStateInfo::setOutputStates(
    const SensorStateInfo::StateVector &m_outputStates) {
...
std::string SensorStateInfo::toString() const {
  ...
  for (meas = m_measurementList.m_list.begin();
       meas != m_measurementList.m_list.end(); ++meas) {
    output << meas->getName() << "=";
  ...
  for (mapIter = m_stateMap.begin(); mapIter != m_stateMap.end(); ++mapIter) {
    ...
    output << mapIter->first << "="
           << std::string(mapIter->second ? "true" : "false")
           << " "; // << std::endl;
  }
  output << std::endl;
  ...
  for (field = m_fields.getFields().begin();
       field != m_fields.getFields().end(); ++field) {
    const FieldParameter &fp = *(*field);
    output << "  " << fp.getFieldNumber() << ": " << fp.getFieldName() << ": "
           << FieldDescription::fieldTypeToString(fp.getFieldType())
           << std::endl;
  ...
  number = 0;
  for (ecp = m_evalCases.getEvalCases().begin();
       ecp != m_evalCases.getEvalCases().end(); ++ecp) {
    const EvalCase &evalCase = *(*ecp);
    output << evalCase.getCaseNumber() << "="
           << evalCase.getCaseName(); //  << std::endl;
    output << ": "
           << "Strat.: "
           << EvalCase::strategyToString(evalCase.getStrategy()); // std::endl;
    output << "; "
           << "Blnk. size: " << evalCase.getBlankingSize()
           << "m"; //  << std::endl;
    output << "; "
           << "Dist. dep.: "
           << std::string(evalCase.isDistDependent()
                              ? "true"
                              : "false"); // << std::endl;
    output << "; "
           << "FieldNumber: " << evalCase.getFieldNumber();

    UINT32 responseTime = evalCase.getResponseTime();
    if (responseTime < 100000) {
      // Normal: Microsekunden
      output << "; "
             << "Resp. time: " << responseTime << " us"; //  << std::endl;
    } else {
      // Skaliert: Millisekunden
      output << "; "
             << "Resp. time: " << (UINT32)(responseTime / 1000)
             << " ms"; //  << std::endl;
    }
    output << "; "
           << "Output nr: " << evalCase.getOutputNumber(); //  << std::endl;
    output << "; "
           << "Res. neg.: "
           << std::string(evalCase.isResultNegation() ? "true" : "false")
           << std::endl;
    ++number;
  }
...
FieldParameter::CaseResult
isFieldInfringed(const SensorStateInfo &sensorStateInfo,
                 const FieldParameter &field, const EvalCaseResults &results) {
  UINT8 isInfringed = 0;

  // find eval case that belongs to the field
  EvalCaseVector::const_iterator evalIter;
  for (evalIter = sensorStateInfo.getEvalCases().getEvalCases().begin();
       evalIter != sensorStateInfo.getEvalCases().getEvalCases().end();
       ++evalIter) {
...
void SensorStateInfo::setLastKnownEvalCaseResults(
    const EvalCaseResults &evalCaseResults) {
  Fields::FieldVector::const_iterator fieldIter;
  for (fieldIter = m_fields.getFields().begin();
       fieldIter != m_fields.getFields().end(); ++fieldIter) {
    FieldParameter &f = const_cast<FieldParameter &>(*(*fieldIter));
    FieldParameter::CaseResult fieldInfringed =
        isFieldInfringed(*this, f, evalCaseResults);
