...
public:
  ...
  bool addApplication(Sourcetype appType, std::string appName,
                      UINT16 wantedId = 0xFFFF);
  bool addApplication(application::BasicApplication *app,
                      UINT16 wantedId = 0xFFFF);
  bool addAndRunDevice(Sourcetype deviceType, std::string deviceName,
                       UINT16 wantedId = 0xFFFF);
  bool addAndRunDevice(devices::BasicDevice *device, std::string deviceName,
                       UINT16 wantedId = 0xFFFF);
  bool importApplications();
  bool runAllDevices();
  void stopAllDevices();
  void setDeviceData(
      BasicData *data); // Datenquelle: Hier laden die Devices ihre Daten ab
...
private:
  ...
  void sourceThreadFunction(bool &endThread,
                            UINT16 &waitTimeMs); // Die Verteiler-Funktion
  SickThread<Manager, &Manager::sourceThreadFunction>
      m_sourceThread;        // Der Verteiler-Thread
