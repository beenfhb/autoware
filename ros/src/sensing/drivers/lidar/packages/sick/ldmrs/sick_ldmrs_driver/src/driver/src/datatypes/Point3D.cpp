//
// Point3D.cpp
// A point in the 3-D cartesian plane. All data is stored in
...
Point3D::Point3D(const Point2D &p)
    : m_x(double(p.getX())), m_y(double(p.getY())), m_z(0.0) {
...
bool Point3D::isZero() const {
  return (fuzzyCompare(m_x, 0.0) && fuzzyCompare(m_y, 0.0) &&
 ...
 * Note that the returned vector product is not normalized (Hint: use
 * normalize() to do this).
 */
 ...
 *
 * If the vector has zero length (isZero() returns true), it will be left
 * unchanged.
 */
...
/**
 * Rotate the point around the Z axis (with a yaw angle). The input is an angle
 * in [rad]. Positive angles rotate counterclockwise.
 */
...
/**
 * Rotate the point around the X axis (with a roll angle). The input is an angle
 * in [rad]. Positive angles rotate clockwise (seen from the origin).
 */
...
/**
 * Rotate the point around the Y axis (with a pitch angle). The input is an
 * angle in [rad]. Positive angles rotate clockwise (seen from the origin).
 */
...
/**
 * Calculates the dist from the origin (0,0,0) to the point. Assuming the point
 * is a vector, this is the length of the vector (see also length()).
 */
double Point3D::distFromOrigin() const { return hypot(m_x, m_y, m_z); }

/**
 * Calculates the dist from the origin (0,0,0) to the point. Assuming the point
 * is a vector, this is the length of the vector.
 */
double Point3D::length() const { return distFromOrigin(); }

/**
 * Returns the angle that the projection of the point onto the x-y-plane has.
 * This angle is measured against the 0-degree-direction (x axis). Note that the
 * z-coordinate has no effect here.
 *
 *           ^ x
 *           |
 *           |
...
/**
 * Returns the angle that the projection of the point onto the x-z-plane has.
 This angle
...
/**
 * Returns the angle that the projection of the point onto the z-y-plane has.
 * This angle is measured against the 0-degree-direction (y axis). Note that the
 * z-coordinate has no effect here.
 *
 *           ^ y
 *           |
 ...
 */
double Point3D::getDistanceBetweenPoints(const Point3D &pt1,
                                         const Point3D &pt2) {
  return pt1.dist(pt2);
}
 ...
 * The vector is given with a start point (Deutsch: Aufpunkt) and a direction
 * vector (Deutsch: Richtungsvektor). The plane is given as a start point
 * (Deutsch: Aufpunkt) and a normal vector (Deutsch: Normalenvektor).
 *
 * Note that the plane has infinite size, so the intersection point may not be
 * where you expect it to be, e.g. "behind" the vector start point, that is, in
 * negative vector direction. Note also that there may not be an intersection
 * point. In this case, the resulting point will be NaN in all components.
 *
 * Is this function better located in geom3D?
 */
Point3D Point3D::calcIntersectionPointOfVectorWithPlane(
    const Point3D &PlaneStartpoint, const Point3D &PlaneNormal,
    const Point3D &VectorStartpoint, const Point3D &VectorDirection) {
  ...
  if (fuzzyCompare(nenner, 0.0) == false) {
    ...
    intersectionPoint =
        VectorStartpoint +
        ((PlaneNormal * (PlaneStartpoint - VectorStartpoint)) / nenner) *
            VectorDirection;
  } else {
