...
/**
 * Class SopasBase encapsuls the communication to a sensor via SopasProtocol. It
 * offers the functions: </br>
 * 			- invokeMethode<br/>
 * 			- readVariable<br/>
 * 			- writeVariable<br/>
 * 			- (un)registerEvent<br/>
 *
 * 		Callback functions are used to inform you about incoming events
 * (scans or eval cases).
 */
class SopasBase {
public:
  ...
  enum SopasEncoding {
    ...
    ByIndex ///< read/write variable, invoke methods by index (indexes will be
            ///< generated !!!)
  };
  ...
  typedef void (*DecoderFunction)(
      SopasEventMessage &frame); //  Decoder for events
   ...
   * @return
   */
  virtual bool init(SopasProtocol protocol, std::string ipAddress,
  ...
  /**
   * @brief Reads the scanner type and version variable from the sensor and
   * stores it in the member variables. This is done always by name.
   ...
   * @param parameters byte buffer with parameter (NOTE: you have to fill this
   * buffer with the correct protocol - cola-a or cola-b)
   * @param parametersLength length of the byte buffer
   * @param answer pointer to an answer message (NOTE: memory for this object
   * will be allocated - free this after usage !!!)
   * @return true if no errors occurred.
   */
  bool invokeMethod(const std::string &methodeName, BYTE *parameters,
   ...
   * @return true if no errors occurred.
   */
  bool invokeMethod(UINT16 index, BYTE *parameters, UINT16 parametersLength,
                    SopasAnswer *&answer);
   ...
   * @return true if no errors occurred.
   */
  bool writeVariable(const std::string &variableName, BYTE *parameters,
   ...
   * @param eventName
   */
  void setEventCallbackFunction(DecoderFunction decoderFunction,
...
protected:
  ...
  enum State {
    ...
    ,
    CONNECTED
    /// Object is connected and emitting data. Use stop() to go back into
    /// CONNECTED, or disconnect() to go back into CONSTRUCTED state.
    //		, RUNNING
  };

  /**
   * @brief Take answer from read thread and decode it.
   *        Waits for a certain answer by name. Event data (scans) are filtered
   * and processed by read thread.
   * @param cmd Waits for the answer to this command.
   * @param name name of the method/variable.
   * @param timeout in [ms]
   * @param answer Pointer to answer. Will be filled if answer contains
   * parameter.
   * @return true if no error occurred.
   */
  bool receiveAnswer(SopasCommand cmd, std::string name, UINT32 timeout,
                     SopasAnswer *&answer);
  bool receiveAnswer_CoLa_A(SopasCommand cmd, std::string name, UINT32 timeout,
                            SopasAnswer *&answer);
  bool receiveAnswer_CoLa_B(SopasCommand cmd, std::string name, UINT32 timeout,
                            SopasAnswer *&answer);
  bool receiveAnswer(SopasCommand cmd, UINT16 index, UINT32 timeout,
                     SopasAnswer *&answer);
  bool receiveAnswer_CoLa_A(SopasCommand cmd, UINT16 index, UINT32 timeout,
                            SopasAnswer *&answer);
  bool receiveAnswer_CoLa_B(SopasCommand cmd, UINT16 index, UINT32 timeout,
                            SopasAnswer *&answer);
...
protected:
  ...
  bool m_weWantFieldData; ///< Flag to enable/disable protection field data
                          ///< reception
...
private:
  ...
  static void readCallbackFunctionS(void *obj, UINT8 *buffer,
                                    UINT32 &numOfBytes);
...
private:
  ...
  UINT8 m_responseBuffer[1024]; ///< Receive buffer for everything except scan
                                ///< data and eval case data.
  Mutex m_receiveDataMutex;     ///< Access mutex for buffer

  // Receive buffer
  UINT32 m_numberOfBytesInReceiveBuffer; ///< Number of bytes in buffer
  UINT8 m_receiveBuffer[25000]; ///< Low-Level receive buffer for all data
                                ///< (25000 should be enough for NAV300 Events)
...
public:
  ...
  /**
   * @brief Constructor. This class will only store a pointer to the byte
   * buffer. It will not deallocate the memory. Please make sure that the buffer
   * is not deallocated while you are working with this class.
   ...
   */
  SopasEventMessage(BYTE *buffer, SopasBase::SopasProtocol protocol,
                    UINT32 frameLength);

  SopasBase::SopasProtocol getProtocolType() const { return m_protocol; }

  SopasBase::SopasEncoding getEncodingType() const { return m_encoding; }

  SopasBase::SopasMessageType getMessageType() const { return m_messageType; }

  UINT32 size() const { return m_frameLength; }
  ...
  /// Returns the index of a variable (answer to read variable by index). In
  /// case of error a negative value will be returned
  INT32 getVariableIndex();

  /// Returns the name of a variable (answer to read variable by name). In case
  /// of error an empty value will be returned
...
/// Class that encapsulates a buffer that was sent as return to a sync call.
/// (variable / method)
class SopasAnswer {
public:
  /// Constructor. Copies the content of the answer into the buffer of this
  /// object.
