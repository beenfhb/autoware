...
public:
  ...
  bool open(std::string ipAddress, UINT16 port,
            bool enableVerboseDebugOutput = false);
  bool open(UINT32 ipAddress, UINT16 port,
            bool enableVerboseDebugOutput = false);
  void close();  // Closes the connection, if it was open.
  bool isOpen(); // "True" if a connection is currently open.
  bool write(UINT8 *buffer, UINT32 numberOfBytes); // Writes numberOfBytes bytes
                                                   // to the open connection.
  std::string
  readString(UINT8 delimiter); // Reads a string, if available. Strings are
                               // separated with the delimiter char.

  // Buffer read function (for polling)
  UINT32 getNumReadableBytes(); // Returns the number of bytes currently
                                // available for reading.
  UINT32 read(UINT8 *buffer,
              UINT32 bufferLen); // Reads up to bufferLen bytes from the buffer.

  // Read callbacks (for being called when data is available)
  typedef void (*ReadFunction)(void *obj, UINT8 *inputBuffer,
                               UINT32 &numBytes); //  ReadFunction
  ...
  void setDisconnectCallbackFunction(DisconnectFunction discFunction,
                                     void *obj);
...
private:
  ...
  INT32 m_connectionSocket; // Socket, wenn wir der Client sind (z.B. Verbindung
                            // zum Scanner)
  ...
  /*
          ...
          boost::condition m_readThreadCondition;	///< Condition to wait
     for the spawned thread to be started boost::mutex m_readThreadMutex;
     ///< Mutex to wait for the spawned thread to be started void readThread();
     ///< Receive-thread for incoming scans. INT32 readInputData();
     ...
     Eingangspuffer fuer Strings bool m_longStringWarningPrinted;
  */
};
