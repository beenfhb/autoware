...
public:
  ...
  Point3D() : m_x(0.0), m_y(0.0), m_z(0.0) { m_datatype = Datatype_Point3D; }
  ...
  double
  length() const; ///< Length of the vector (identical to distFromOrigin())
  double distFromOrigin() const; ///< Dist from the point (0,0,0) to this point

  double getAngleAroundX()
      const; ///< Returns the rotation angle around x (in the z-y-plane)
  double getAngleAroundY()
      const; ///< Returns the rotation angle around y (in the z-x-plane)
  double getAngleAroundZ()
      const; ///< Returns the rotation angle around z (in the x-y-plane)
  ...
  void rotateAroundX(
      double rollAngle); ///< Rotate the point around the X-axis ("Roll angle")
  void rotateAroundY(double pitchAngle); ///< Rotate the point around the Y-axis
                                         ///< ("Pitch angle")
  void rotateAroundZ(
      double dYawAngle); ///< Rotate the point around the Z-axis ("Yaw angle")

  void normalize(); ///< Normalizes this vector (point) to length 1.0

  Point3D &
  operator+=(const Point3D &point); ///< Adds the given point to this point and
                                    ///< returns a reference to this point.
  Point3D &
  operator-=(const Point3D &point); ///< Subtracts the given point to this point
  ...
  double dist(const Point3D &point)
      const; ///< Calculates the distance to the given point
  //\}

  /** \name Geometrical relations to other objects */
  //\{
  static Point3D
  vectorProduct(const Point3D &v1,
                const Point3D &v2); ///< Returns the vector product
                                    ///< ("Kreuzprodukt") of the two vectors
  static Point3D calcIntersectionPointOfVectorWithPlane(
  ...
  static double getDistanceBetweenPoints(
      const Point3D &pt1,
      const Point3D
          &pt2); ///< Returns the distance between the two point coordinates
  //\}
   ...
   */
  std::istream &read(std::istream &is); // , UINT32 version);	///< Reads a
                                        // Point3D from an input stream
  void read(const BYTE *&buf);          // , UINT32 version);			///< Reads
                               // a Point3D from a memory buffer and increments
                               // the buffer pointer
  std::ostream &
  write(std::ostream &os) const; // , UINT32 version) const;	///< Writes a
                                 // Point3D to an output stream
  void write(BYTE *&buf)
      const; //, UINT32 version) const;			///< Writes a Point2D to a memory
  ...
  getSerializedSize(); // UINT32 version);	///< Returns the number of bytes
                       // this object needs in serialized form
...
std::ostream &operator<<(std::ostream &os,
                         const Point3D &point); ///< Text output for debugging
...
inline bool operator==(const Point3D &p1, const Point3D &p2) {
  return (fuzzyCompare(p1.m_x, p2.m_x) || (isNaN(p1.m_x) && isNaN(p2.m_x))) &&
         (fuzzyCompare(p1.m_y, p2.m_y) || (isNaN(p1.m_y) && isNaN(p2.m_y))) &&
         (fuzzyCompare(p1.m_z, p2.m_z) || (isNaN(p1.m_z) && isNaN(p2.m_z)));
}
inline bool operator!=(const Point3D &p1, const Point3D &p2) {
