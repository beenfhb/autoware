...
const UINT32 EvalCase::getUsedMemory() const {
  return sizeof(*this) + m_caseName.length() + m_comment.length();
}

double EvalCase::getBlankingSize() const { return m_blankingSize; }

const std::string &EvalCase::getCaseName() const { return m_caseName; }

UINT8 EvalCase::getCaseNumber() const { return m_caseNumber; }

const std::string &EvalCase::getComment() const { return m_comment; }

UINT16 EvalCase::getFieldNumber() const { return m_fieldNumber; }

EvalCase::FilterType EvalCase::getFilterType() const { return m_filterType; }
...
const UINT8 EvalCase::getLogicalInputState_as_UINT8() const {
  UINT8 state = (((UINT8)(m_logicalInputState[1])) << 2) +
                ((UINT8)(m_logicalInputState[0]));
  return state;
}

double EvalCase::getMaxRadialCorridor() const { return m_maxRadialCorridor; }

double EvalCase::getMinFieldExp() const { return m_minFieldExp; }

UINT8 EvalCase::getOutputNumber() const { return m_outputNumber; }

UINT32 EvalCase::getResponseTime() const { return m_responseTime; }
...
bool EvalCase::isDistDependent() const { return m_distDependent; }

bool EvalCase::isResultNegation() const { return m_resultNegation; }
...
void EvalCase::setBlankingSize(double blankingSize) {
  ...
  if (m_blankingSize > 10.0) {
    printWarning("EvalCase::setBlankingSize: Blanking size is very large. Was "
                 "it really written in [m]?");
  }
}
...
void EvalCase::setCaseNumber(UINT8 caseNumber) {
  ...
  if (m_caseNumber == 0) {
    printWarning("EvalCase::setCaseNumber: Case number must be >0 for a valid "
                 "EvalCase!");
  }
}
...
bool EvalCase::getDistDependent() { return m_distDependent; }
...
// void EvalCase::setHardwareInputs(const EvalCase::InputStateVector&
// hardwareInputs)
// {
// 	this->m_hardwareInputs = hardwareInputs;
// }

// void EvalCase::setLogicalInputs(const EvalCase::InputStateVector&
// logicalInputs)
// {
...
bool EvalCase::getResultNegation() { return this->m_resultNegation; }
...
//
UINT16 EvalCase::getVersionNumber() { return (this->m_versionNumberIntern); }

EvalCase::ManipulationPrevention EvalCase::getManipulationPrevention() const {
  return m_manipulationPrevention;
}

void EvalCase::setManipulationPrevention(
    EvalCase::ManipulationPrevention manPrev) {
...
void EvalCase::setLogicalInputState_from_UINT8(UINT8 value) {
  ...
  case 2:
    ...
    break;
  default:
    printError("EvalCase::setLogicalInputState_from_UINT8: Configuration of "
               "input 1 has an invalid value, aborting!");
    return;
  }
  ...
  default:
    printError("EvalCase::setLogicalInputState_from_UINT8: Configuration of "
               "input 2 has an invalid value, aborting!");
    return;
  }
}
