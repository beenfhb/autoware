...
bool File::open(std::string inputFileName, bool beVerbose) {
  ...
  if (m_inputFileStream.fail() == true) {
    printError("File::open(): Failed to open the file " + m_inputFileName +
               ", aborting!");
...
void File::startReadThread() {
  if ((m_readThread.isRunning() == false) && (m_readFunctionObjPtr != NULL) &&
      (m_inputFileName != "")) {
...
void File::readThreadFunction(bool &endThread, UINT16 &waitTimeMs) {
  ...
  if (result < 0) {
    ...
    if (m_readThread.m_threadShouldRun == true) {
      ...
      printInfoMessage("File::readThreadMain: End of file reached! Read thread "
                       "terminates now.",
                       m_beVerbose);
    }
    waitTimeMs = 0;
  } else if (result == 0) {
    // No data. We may have reached the end of the file. In any case, there is
...
INT32 File::readInputData() {
  ...
  if (recvMsgSize > 0) {
    // Success
    printInfoMessage("File::readInputData: Read " + toString(recvMsgSize) +
    ...
    if (m_readFunction != NULL) {
      ...
      printWarning("File::readInputData: Discarding data because there is no "
                   "callback function!");
    }
  } else if (recvMsgSize == 0) {
    // Verbindungsabbruch
    printInfoMessage("File::readInputData: Read 0 bytes - end of file or file "
                     "connection is lost!",
                     true);
