...
ScannerInfo::ScannerInfo()
    : m_deviceID(0), m_scannerType(0), m_scanNumber(0),
      m_startAngle(NaN_double), m_endAngle(NaN_double), m_processingFlags(0),
      m_scanStartTime(Time()), m_scanEndTime(Time()),
      m_scanFrequency(NaN_double), m_beamTilt(0), m_scanFlags(0),
      m_mountingPosition() {
...
std::string ScannerInfo::scannerTypeToString(UINT8 st) {
  ...
  case Scannertype_LMS1xx:
    ...
    break;
  default:
    printError("scannerTypeToString: Unknown scanner type, code=" +
               ::toString((UINT16)st) + ".");
...
bool ScannerInfo::operator==(const ScannerInfo &other) const {
  return m_deviceID == other.m_deviceID &&
         ...
         (m_scanFrequency == other.m_scanFrequency ||
          (isNaN(m_scanFrequency) && isNaN(other.m_scanFrequency))) &&
         m_beamTilt == other.m_beamTilt && m_scanFlags == other.m_scanFlags &&
...
bool ScannerInfo::isFrontMirrorSide() { return !isRearMirrorSide(); }
...
void ScannerInfo::setScannerType(UINT8 v) { m_scannerType = v; }
 ...
 */
void ScannerInfo::setTimestamps(const Time &start, const Time &end) {
  assert(start < end);
...
// bool ScannerInfo::isGroundLabeled()		const { return (m_scanFlags &
// Scan::FlagGroundLabeled)		!= 0; } bool
// ScannerInfo::isDirtLabeled()		const { return (m_scanFlags &
// Scan::FlagDirtLabeled )		!= 0; } bool
// ScannerInfo::isRainLabeled()		const { return (m_scanFlags &
// Scan::FlagRainLabeled)		!= 0; } bool
// ScannerInfo::isCoverageLabeled()	const { return (m_scanFlags &
// Scan::FlagCoverageLabeled)	!= 0; } bool ScannerInfo::isBackgroundLabeled()
// const { return (m_scanFlags & Scan::FlagBackgroundLabeled)	!= 0; } bool
// ScannerInfo::isReflectorLabeled()	const { return (m_scanFlags &
// Scan::FlagReflectorLabeled)	!= 0; } bool ScannerInfo::isUpsideDown()
// const { return (m_scanFlags & Scan::FlagUpsideDown)			!= 0; }
// bool ScannerInfo::isRearMirrorSide()	const { return (m_scanFlags &
// Scan::FlagRearMirrorSide)		!= 0; }
