...
public:
  ...
  /**
   * \note The radius components must be non-negative, otherwise an assertion
   * will fail.
   *
   * \param center The center point
   * \param radius The size of the ellipse in the ellipse' coordinate system.
   ...
   * ::normalizeRadians().
   */
  Ellipse2D(const Point2D &center, const Point2D &radius,
   ...
   *
   * \param x_radius The radius of the ellipse in X-direction in the ellipse'
   * coordinate system. Must be non-negative. \param y_radius The radius of the
   * ellipse in Y-direction in the ellipse' coordinate system. Must be
   * non-negative.
   *
   * \param rotation The rotation of the ellipse' coordinate system
   * around its center point in [radians]. Must be in the interval
   ...
   * ::normalizeRadians().
   */
  Ellipse2D(value_type x_center, value_type y_center, value_type x_radius,
            value_type y_radius, value_type rotation = 0.0);
  ...
  /** Returns the rotation angle of this Ellipse2D in [radians], counter clock
   * wise.
   *
  ...
  /// Returns true if the given Point2D is inside this ellipse or on its
  /// outline.
  /**
...
inline bool operator==(const Ellipse2D &b1, const Ellipse2D &b2) {
  return (b1.m_center == b2.m_center) && (b1.m_radius == b2.m_radius) &&
