...
#define MRS_CMDREPLYBUFFERSIZE                                                 \
  8192 // Size of the command reply buffer (some bytes should be enough...)
...
enum MrsCommandId {
  ...
  CmdMrsResetDefaultParameters =
      0x001A ///< 0x001A = resets all parameters to the factory defaults
  ,
  CmdMrsStartMeasure = 0x0020 ///< 0x0020 = starts the measurement with the
                              ///< currenly configured settings
  ,
  ...
  CmdMrsSetNTPTimestampFracSec =
      0x0031 ///< 0x0031 = sets NTP fractional seconds
};

// Parameters of the MRS
enum MrsParameterId {
  ParaDataOutputFlag =
      0x1012 // Sets the output flags to enable or disable scans, objects, ...
  ,
  ParaContourPointDensity =
      0x1014 // 0: closest point only, 1: low density, 2: high density
  ,
  ParaMinimumObjectAge = 0x1017 // Minimum tracking age (number of scans) of an
                                // object to be transmitted. valid: 0..65535
  ,
  ParaMaximumPredictionAge =
      0x1018 // Maximum prediction age (number of scans) of an object to be
             // transmitted after last observation. valid: 0..65535
  ,
  ParaScanFrequency = 0x1102 // Sets the scan frequency, in 1/256 Hz (valid =
                             // 3200 (12.5 Hz),6400 (25.0 Hz) and 12800 (50 Hz)
  ,
  ParaStartAngle =
      0x1100 // 1/32 deg, Valid is 1600..-1919; Start angle > end angle!
  ,
  ParaEndAngle =
      0x1101 // 1/32 deg, Valid is 1599..-1920; Start angle > end angle!
  ,
  ParaSyncAngleOffset =
      0x1103 // 1/32 deg, Valid is -5760..5759; angle under which the LD-MRS
             // measures at the time of the external sync pulse
  ,
  ParaAngularResolutionType =
      0x1104 // angular resolution type: 0=focused, 1=constant(0.25°), 2=FlexRes
  ,
  ParaRangeReduction =
      0x1108 // Available for LDMRS800001.S01 only; 0: full sensitivity
             // (default), 1: lower 4 layers reduced, 2: upper 4 layers reduced,
             // 3: both reduced
  ,
  ParaUpsideDownMode = 0x1109 // UpsideDown mode on/off (0=off, 1=on)
  ,
  ParaIgnoreNearRange = 0x110A // Available for LDMRS800001.S01 only; 0: do not
                               // ignore points in near range (up to 15m), 1:
                               // ignore points in near range if 0x1108 is 1
  ,
  ParaSensitivityControl =
      0x110B // 0: not active (default), 1: Sensitivity will be reduced
             // dynamically down to 60% in case of direct sun light.
  ,
...
private:
  ...
  //
  // Attention: If reply is received from a LUX3, this member contains the
  // SVN-Revision-Number of current LUX3-Firmware. The Error value is not needed
  // anymore for LUX3.
  //
  UINT16 m_FPGAError; // 0000
  //
  // Attention: If reply is received from a LUX3, this member contains the
  // scanner type. The Error value is not needed anymore for LUX3.
  //
  ...
  // The CMD REPLY buffer is a separate buffer for everything except scans and
  // object data
  ...
  bool receiveMrsReply(MrsCommandId cmd, UINT32 timeoutMs,
                       UINT32 *value = NULL);
  UINT32 buildDataHeader(UINT8 *buffer, UINT32 sizeofThisMsg, UINT8 deviceId,
                         UINT16 dataType);
  ...
  UINT16 decodeAnswerInCmdReplyBuffer(); // Decodes the reply buffer (everything
                                         // except scnas & objects)
  ...
  bool readBeamTilt();
  bool readUpsideDown();
  double getVAngleOfLayer(bool isRearMirrorSide, UINT8 layerNumber,
                          double hAngle);

  static void readCallbackFunctionS(void *obj, BYTE *buffer,
                                    UINT32 &numOfBytes);
  ...
  static bool isValidVersion(UINT16 version) {
    return (version != 0x0000) && (version != 0xFFFF);
  }
...
public:
  LuxBase(Manager *manager, const UINT8 deviceID, const std::string longName,
          std::string ipAddress, UINT16 tcpPortNumber, double scanFrequency,
          double scanStartAngle, double scanEndAngle, double offsetX,
          double offsetY, double offsetZ, double yawAngle, double pitchAngle,
          double rollAngle, bool beVerbose, std::string inputFileName);
  virtual ~LuxBase();

  virtual bool initFile(File::DisconnectFunction function,
                        void *obj); // , bool beVerbose = false);
  virtual bool initTcp(Tcp::DisconnectFunction function,
                       void *obj); // , bool beVerbose = false);
  virtual bool run(bool weWantScanData = true, bool weWantObjectData = true);
  virtual bool isRunning();
  virtual bool stop();

  void setOnScanReceiveCallbackFunction(OnScanReceiveCallbackFunction function,
                                        void *obj);

  bool cmd_getStatus();
  bool cmd_stopMeasure();
  bool cmd_startMeasure(bool weWantScanData = true,
  ...
  std::string getFPGAVersion() {
    ...
    return version2string(m_FPGAVersion, m_FPGATimestamp);
  }
  std::string getFirmwareVersion() {
    ScopedLock lock(&m_updateMutex);
    return version2string(m_firmwareVersion, m_firmwareTimestamp);
  }
  UINT16 getFPGAError() {
    ScopedLock lock(&m_updateMutex);
    return m_FPGAError;
  ...
  static const UINT32 ANGULAR_TICKS_PER_ROTATION =
      11520; // [ticks], 360*32= 11520 ticks;
};
