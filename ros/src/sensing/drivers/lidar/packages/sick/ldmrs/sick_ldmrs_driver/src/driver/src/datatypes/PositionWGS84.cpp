 ...
 */
PositionWGS84::PositionWGS84() { resetToDefault(); }
...
bool PositionWGS84::operator==(const PositionWGS84 &other) const {
  return m_timeOfMeasurement == other.m_timeOfMeasurement &&
         m_sourceId == other.m_sourceId && m_latitude == other.m_latitude &&
...
void PositionWGS84::setLatitudeInRad(double new_latitude) {
  ...
  if ((new_latitude < -M_PI_2) ||
      (new_latitude > M_PI_2)) // note: M_PI_2 = M_PI / 2
  {
    printWarning("PositionWGS84::setLatitude: The given latitude (" +
                 ::toString(new_latitude * rad2deg, 6) +
                 " deg.) is outside the definition range for this value which "
                 "is [-90,90]. " +
                 "Ignoring this error condition here - hopefully this value "
                 "isn't used anywhere.");
  }
}
...
/**
 * \brief Calculates the WGS84 coordinate in rad from the mixed degree-minute
 * value usually found on NMEA compatible GPS receivers.
 *
 ...
 *
 * \param[in] Dm Latitude value [0...9000] where format is DDmm.mmmmm. D stands
 * for value in degree and m for value in decimal minutes. \param[in] H
 * Hemisphere. For Latitude it can be [N]orth or [S]outh.
 *
 * \sa setLatitudeInDeg()
 * \sa setLatitudeInRad()
 ...
 *
 * \param[in] Dm Longitude value [0...18000] where format is DDDmm.mmmmm. D
 * stands for value in degree and m for value in decimal minutes. \param[in] H
 * Hemisphere. For Longitude it can be [W]est or [E]ast.
 *
 * \sa setLongitudeInDeg()
 * \sa setLongitudeInNMEA()
 * \sa setLatitudeInRad()
...
std::string PositionWGS84::toString() const {
  ...
  out << "[PositionWGS84(lat:" << std::setprecision(12) << rad2deg * m_latitude
      << " deg; lon:" << std::setprecision(12) << rad2deg * m_longitude
      << " deg; hdg:" << rad2deg * m_courseAngle
      << " deg; alt:" << m_altitudeMSL << " m; yaw:" << rad2deg * m_yawAngle
      << " deg; pitch:" << rad2deg * m_pitchAngle
      << " deg; roll:" << rad2deg * m_rollAngle
      << " deg; source:" << ::toString(m_source) << ")]";
  return out.str();
}
...
double PositionWGS84::dist(const PositionWGS84 &pos) const {
  return 6378388 * acos(sin(m_latitude) * sin(pos.getLatitudeInRad()) +
                        cos(m_latitude) * cos(pos.getLatitudeInRad()) *
                            cos(pos.getLongitudeInRad() - m_longitude));
}

/** transformToTangentialPlane()
 *
 * Transforms this point (given in WGS-coordinates) onto a tangential (x-y-)
 * plane. The plane is defined by a reference point (origin), relative to which
 * the x-y-coordinates are calculated. Note that this point should be somewhat
 * close to the origin (several km is ok) as the tangential plane is flat.
 *
 * Die Ausrichtung ist in beiden Koordinatensystemen gleich.
 */
// void PositionWGS84::transformToTangentialPlane(double dLat, double dLong,
// double dOriginLat, double dOriginLong, double* pdX, double* pdY)
void PositionWGS84::transformToTangentialPlane(const PositionWGS84 &origin,
                                               double *easting,
...
void PositionWGS84::transformToTangentialPlane(const PositionWGS84 &origin,
                                               ...
                                               double *northing,
                                               double *height) const {
  if (isNaN(getLatitudeInRad()) || isNaN(getLongitudeInRad()) ||
      isNaN(getAltitudeInMeterMSL())) {
    printWarning("transformToTangentPlan(): At least one of the values "
                 "latitude, longitude or altitude is NaN. Can't perform "
                 "transformation! Returning NaN values!");
    *easting = *northing = *height = NaN_double;
    return;
  }

  if (isNaN(origin.getLatitudeInRad()) || isNaN(origin.getLongitudeInRad()) ||
      isNaN(origin.getAltitudeInMeterMSL())) {
    printWarning("transformToTangentPlan(): At least one of the origin's "
                 "values latitude, longitude or altitude is NaN. Can't perform "
                 "transformation! Returning NaN values!");
    *easting = *northing = *height = NaN_double;
    return;
  }
  ...
  const double dWGS84_Flatness =
      (dWGS84_SemiMajorAxis - dWGS84_SemiMinorAxis) / dWGS84_SemiMajorAxis;
  const double dWGS84_Eccentricity2 =
      dWGS84_Flatness * (2.0 - dWGS84_Flatness); // Eccentricity squared

  // WGS84 --> ECEF
  const double dLat = getLatitudeInRad();
  const double dLong = getLongitudeInRad();
  const double dHeight = getAltitudeInMeterMSL(); // TODO: Wrong! This should be
                                                  // height above ellipsoid!

  const double dN =
      dWGS84_SemiMajorAxis /
      (sqrt(1.0 - (dWGS84_Eccentricity2 * sin(dLat) * sin(dLat))));
  const double dX_ECEF = (dN + dHeight) * cos(dLat) * cos(dLong);
  const double dY_ECEF = (dN + dHeight) * cos(dLat) * sin(dLong);
  const double dZ_ECEF =
      ((dN * (1.0 - dWGS84_Eccentricity2)) + dHeight) * sin(dLat);
  ...
  const double dOriginHeight =
      origin.getAltitudeInMeterMSL(); // TODO: Wrong! This should be height
                                      // above ellipsoid!

  const double dN_0 =
      dWGS84_SemiMajorAxis /
      (sqrt(1.0 - (dWGS84_Eccentricity2 * sin(dOriginLat) * sin(dOriginLat))));
  const double dX_0_ECEF =
      (dN_0 + dOriginHeight) * cos(dOriginLat) * cos(dOriginLong);
  const double dY_0_ECEF =
      (dN_0 + dOriginHeight) * cos(dOriginLat) * sin(dOriginLong);
  const double dZ_0_ECEF =
      ((dN_0 * (1.0 - (dWGS84_Eccentricity2))) + dOriginHeight) *
      sin(dOriginLat);
 ...
 *
 * Inverse function of transformToTangentialPlane: Generates this point from
 * x-y-coordinates on a tangential plane defined by the origin. Note that the
 * x-y-coordinates should be somewhat close to the origin to get accurate
 * results.
 */
// void PositionWGS84::GPS_TransformToWGS(double dX, double dY, double
// dOriginLat, double dOriginLong, double* pdLat, double* pdLong)
void PositionWGS84::transformFromTangentialPlane(double dX, double dY,
  ...
  const double dGps_Flatness =
      (dGps_SemiMajorAxis - dGps_SemiMinorAxis) / dGps_SemiMajorAxis;
  ...
  dX_ECEF = -cos(dPhi) * sin(dLambda) * dX + sin(dPhi) * dY +
            cos(dPhi) * cos(dLambda) * 0.0 + dX_0_ECEF;
  dY_ECEF = -sin(dPhi) * sin(dLambda) * dX - cos(dPhi) * dY +
            sin(dPhi) * cos(dLambda) * 0.0 + dY_0_ECEF;
  ...
  for (nCount = 0; nCount < nNoIterationSteps; nCount++) {
    ...
    dN = dGps_SemiMajorAxis /
         (sqrt(1.0 - (dGps_Eccentricity2 * sin(dLambda) * sin(dLambda))));
  }
...
/**
 * Get a cartesian coordinate from the latitude/longitude position relative to
 * the gigen reference point (origin). Note: this function is only valid, if the
 * reference point is not to far (some km) from the given reference point.
 */
