...
Polygon2D::Polygon2D(const Point2D &p1) { push_back(p1); }
...
double Polygon2D::getArea() const {
  ...
  result += at(size() - 1).getX() * at(0).getY() -
            at(0).getX() * at(size() - 1).getY();

  // Calculate absolute value in order to work with counter-clockwise polygon
  // description.
...
Point2D Polygon2D::getCenterOfGravity() const {
  ...
  for (Polygon2D::const_iterator edgeIter = begin(); edgeIter != end();
       ++edgeIter) {
...
bool Polygon2D::isClosed() const { return !empty() && (front() == back()); }
...
Polygon2D Polygon2D::fromCircle(const Point2D &center,
...
Polygon2D Polygon2D::fromEllipse(const Point2D &center,
...
Polygon2D Polygon2D::fromEllipse(const Ellipse2D &ellipse,
                                 const UINT32 samplingPoints) {
...
Polygon2D Polygon2D::fromArc(const Ellipse2D &ellipse,
  ...
  if (fabs(endAngle - startAngle) >
      double(2.0 * M_PI + 1e-3)) // allow 10^-3 epsilon
  {
  ...
  const floatingpoint_type step =
      (endAngle - startAngle) / floatingpoint_type(samplingPoints - 1);
  ...
  for (UINT32 p = 0; p < samplingPoints; p++) {
    ...
    Point2D point;
    point.setX(ellipse.getRadius().getX() * std::cos(t) * cosAngle -
               ellipse.getRadius().getY() * std::sin(t) * sinAngle);
    point.setY(ellipse.getRadius().getX() * std::cos(t) * sinAngle +
               ellipse.getRadius().getY() * std::sin(t) * cosAngle);
...
const char *Polygon2D::getSeparatorCharacters() { return "()[],; \t\"'"; }

Polygon2D Polygon2D::rhombus(const Point2D &center,
                             const floatingpoint_type radius) {
  Polygon2D contour;
...
Polygon2D Polygon2D::createRectangle(const Point2D &lowerLeft,
...
void Polygon2D::fromString(const std::string& polyString)
{
        ...
        tokenizer listOfTokens(polyString,
boost::char_separator<char>(getSeparatorCharacters())); for
(tokenizer::const_iterator iter = listOfTokens.begin(); iter !=
listOfTokens.end(); ++iter)
        {
                floatingpoint_type x =
boost::lexical_cast<floatingpoint_type>(*iter);
                ++iter;
                if (iter == listOfTokens.end())
                        throw std::runtime_error("When parsing a Polygon2D
string, the last point only has a x component."); floatingpoint_type y =
boost::lexical_cast<floatingpoint_type>(*iter); this->push_back(Point2D(x, y));
        }
}
*/
...
Box2D Polygon2D::getBoundingBox() const {
  ...
  return Box2D(0.5 * (maxX + minX), 0.5 * (maxY + minY), maxX - minX,
               maxY - minY);
}

std::pair<Polygon2D::floatingpoint_type, Polygon2D::floatingpoint_type>
Polygon2D::getBoundingAngles() const {
  if (empty())
  ...
  for (; iter != end(); ++iter) {
    if (fuzzyCompare(iter->getY(), 0.0) && (iter->getX() < 0.0)) {
 ...
 *   -# Then include the first component' points, repeating its first point
 * after the last point.
 *   -# Include another (0,0) point.
 *   -# Include another component or hole, repeating its first point after the
 * last point.
...
bool Polygon2D::containsPoint(const Point2D &point) const {
  ...
  for (i = 0; i < size(); j = i++) {
    if (((p[i].getY() > point.getY()) != (p[j].getY() > point.getY())) &&
        (point.getX() <
         ((p[j].getX() - p[i].getX()) * (point.getY() - p[i].getY()) /
              (p[j].getY() - p[i].getY()) +
          p[i].getX()))) {
      isInside = !isInside;
    }
...
void projectOntoAxis(const Point2D &axis, const Polygon2D &polygon,
  ...
  for (Polygon2D::const_iterator edge = polygon.begin(); edge != polygon.end();
       ++edge) {
...
Polygon2D::floatingpoint_type
intervalDistance(const Polygon2D::floatingpoint_type minA,
...
bool Polygon2D::isColliding(const Polygon2D &p2) const {
  ...
  for (Polygon2D::const_iterator edge = both.begin(); edge != both.end();
       ++edge) {
...
Polygon2D::base_class Polygon2D::isIntersecting(const Line2D &other) const {
  ...
  for (size_t k = 1; k < size(); ++k) {
    ...
    if (currentline.isIntersecting(other, &isect) == Line2D::LineIntersecting)
