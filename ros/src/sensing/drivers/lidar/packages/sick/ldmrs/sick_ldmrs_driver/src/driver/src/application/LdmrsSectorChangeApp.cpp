...
//
// The App starts a thread that, after some time, sets the ResolutionMap of the
// scanner, activates it and later deactivates it.
//
...
//
// Convert the current Sector or ResolutionMap (which is a vector of sectors) to
// a readable string.
//
std::string sectorToString(const ScannerInfo::ResolutionMap &sector) {
  ...
  oss << std::endl;
  for (ScannerInfo::ResolutionMap::const_iterator s = sector.begin();
       s != sector.end(); ++s) {

    oss << "Sector (" << i
        << "): start=" << doubleToString(s->first * rad2deg, 3)
        << " res=" << doubleToString(s->second * rad2deg, 3) << std::endl;
    i++;
  }
...
Scan::const_iterator getNextPointInSameLayer(Scan::const_iterator iter,
                                             const Scan &scan) {
...
void LdmrsSectorChangeApp::checkResolution(Scan &scan) {
  ...
  if ((scan.getFlags() & Scan::FlagVehicleCoordinates) != 0) {
    ...
    printWarning("LdmrsSectorChangeApp::checkResolution: Scan is in vehicle "
                 "coordinates and thus cannot be processed!");
    return;
  }
...
INT16 radian2Int(float radAngle) {
  float rad2ticks = float(devices::LuxBase::ANGULAR_TICKS_PER_ROTATION) /
                    (2.f * PI); // conversion factor from [rad] to [ticks]
...
bool LdmrsSectorChangeApp::readDetailedErrorCode(UINT32 *errCode) {
  ...
  devices::LDMRS *scanner = dynamic_cast<devices::LDMRS *>(
      m_manager->getFirstDeviceByType(Sourcetype_LDMRS));
  if (scanner) {
    ...
    case devices::ErrFlexResNumSectorsInvalid:
      printInfoMessage("Detailed Error code: FlexResNumSectorsInvalid",
                       m_beVerbose);
      break;
    case devices::ErrFlexResNumShotsInvalid:
      printInfoMessage("Detailed Error code: FlexResNumShotsInvalid",
                       m_beVerbose);
      break;
    case devices::ErrFlexResResolutionInvalid:
      printInfoMessage("Detailed Error code: FlexResResolutionInvalid",
                       m_beVerbose);
      break;
    case devices::ErrFlexResScannerNotIdle:
      printInfoMessage("Detailed Error code: FlexResScannerNotIdle",
                       m_beVerbose);
      break;
    case devices::ErrFlexResSectorsOverlapping:
      printInfoMessage("Detailed Error code: FlexResSectorsOverlapping",
                       m_beVerbose);
      break;
    case devices::ErrFlexResSizeOneEighthSectorInvalid:
      printInfoMessage("Detailed Error code: FlexResSizeOneEighthSectorInvalid",
                       m_beVerbose);
      break;
    default:
      break;
...
bool LdmrsSectorChangeApp::changeFlexResConfiguration(
    const ScannerInfo::ResolutionMap &configuredRM) {
  ...
  devices::LDMRS *scanner = dynamic_cast<devices::LDMRS *>(
      m_manager->getFirstDeviceByType(Sourcetype_LDMRS));
  if (scanner) {
    if (!scanner->setParameter(devices::ParaNumSectors,
                               UINT32(configuredRM.size()))) {
      printInfoMessage("LdmrsSectorChangeApp::changeFlexResConfiguration(): "
                       "set NumSectors not successful",
                       m_beVerbose);
    ...
    for (UINT32 i = 0; i < configuredRM.size(); ++i) {
      // set start angle
      if (!scanner->setParameter((devices::MrsParameterId)(0x4001 + i),
                                 radian2Int(configuredRM[i].first))) {
        printInfoMessage("LdmrsSectorChangeApp::changeFlexResConfiguration(): "
                         "set start angle not successful: " +
                             doubleToString(configuredRM[i].first, 3),
                         m_beVerbose);
        readDetailedErrorCode();
        return false;
      }
      // set resolution
      if (!scanner->setParameter((devices::MrsParameterId)(0x4009 + i),
                                 radian2Int(configuredRM[i].second))) {
        printInfoMessage("LdmrsSectorChangeApp::changeFlexResConfiguration(): "
                         "set resolution not successful: " +
                             doubleToString(configuredRM[i].first, 3),
                         m_beVerbose);
        readDetailedErrorCode();
        return false;
...
//
bool LdmrsSectorChangeApp::changeAngularResolutionType(
    devices::AngularResolutionType type) {
  devices::LDMRS *scanner = dynamic_cast<devices::LDMRS *>(
      m_manager->getFirstDeviceByType(Sourcetype_LDMRS));
  if (scanner) {
    if (scanner->setParameter(devices::ParaAngularResolutionType,
                              UINT32(type))) {
      //			printInfoMessage("LdmrsSectorChangeApp::changeThreadFunction():
      //set NumSectors to 9 not successful", m_beVerbose);
    } else {
      //			traceNote(VERSION) << "NOT ABLE to change
      //ResType to: type=" << type  << std::endl;
      readDetailedErrorCode();
      return false;
    }
  }
...
//
void LdmrsSectorChangeApp::changeThreadFunction(bool &endThread,
                                                UINT16 &waitTimeMs) {
  printInfoMessage("LdmrsSectorChangeApp::changeThreadFunction(): started",
                   m_beVerbose);

  devices::LDMRS *ldmrs = dynamic_cast<devices::LDMRS *>(
      m_manager->getFirstDeviceByType(Sourcetype_LDMRS));
  ...
  if (ldmrs) {
    ...
    if (!ldmrs->getParameter(devices::ParaNumSectors, &value)) {
      ...
      printInfoMessage("LdmrsSectorChangeApp::changeThreadFunction(): read "
                       "parameter NumSectors=" +
                           toString(value),
                       m_beVerbose);
    }
    ...
    if (!ldmrs->setParameter(devices::ParaNumSectors, 9)) {
      ...
      printInfoMessage("LdmrsSectorChangeApp::changeThreadFunction(): set "
                       "NumSectors to 9 not successful",
                       m_beVerbose);
  ...
  waitTimeMs = 0;
  printInfoMessage(
      "LdmrsSectorChangeApp::changeThreadFunction():  All done, leaving.",
      m_beVerbose);
}
