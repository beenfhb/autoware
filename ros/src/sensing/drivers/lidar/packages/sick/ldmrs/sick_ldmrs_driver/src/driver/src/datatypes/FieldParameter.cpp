...
const UINT32 FieldParameter::getUsedMemory() const {
  UINT32 length = sizeof(*this) + m_fieldName.length() + m_comment.length();
  if (m_field != NULL) {
...
INT32 FieldParameter::getAngleScaleOffset() const { return m_angleScaleOffset; }

double FieldParameter::getDistScaleFactor() const { return m_distScaleFactor; }

double FieldParameter::getDistScaleOffset() const { return m_distScaleOffset; }

FieldDescription *FieldParameter::getField() const { return m_field; }
...
const std::string &FieldParameter::getComment() const { return m_comment; }

const std::string &FieldParameter::getFieldName() const { return m_fieldName; }
...
UINT8 FieldParameter::getVersionNumber() const { return m_versionNumber; }
...
FieldParameter::getLastKnownInfringementState() const {
  return m_lastKnownInfringementState;
}

void FieldParameter::setLastKnownInfringementState(
    FieldParameter::CaseResult m_lastKnownInfringementState) {
...
const FieldDescription::FieldType FieldParameter::getFieldType() const {
  if (m_field == NULL) {
    // There is no associated field data structure, so we do not have a field
    // type.
    ...
    // No match!
    printError(
        "FieldParameter::getFieldType(): The internal field type and the "
        "associated field structure do no match, returning UNDEFINED!");
  }

  return FieldDescription::Undefined;
}

const UINT16 FieldParameter::getFieldNumber() const { return m_fieldNumber; }
