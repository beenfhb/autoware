...
LdmrsSopasLayer::LdmrsSopasLayer(Manager *manager, const UINT8 deviceId,
                                 std::string ipAddress, UINT16 portNumber,
                                 bool weWantFieldData, bool weWantScanData,
                                 bool readOnlyMode) {
  m_beVerbose = false;
...
bool LdmrsSopasLayer::init(Tcp::DisconnectFunction disconnectFunction,
                           void *obj) {
  bool success = SopasBase::init(CoLa_B, m_ipAddress, m_portNumber,
                                 m_weWantScanData, m_weWantFieldData,
                                 m_readOnlyMode, disconnectFunction, obj);

  if (success == true) {
    // Success
    printInfoMessage(
        "LdmrsSopasLayer::init: SopasBase was initialized successfully.",
        m_beVerbose);
  } else {
  ...
  if (success == true) {
    // Success
    printInfoMessage(
        "LdmrsSopasLayer::init: Connected to scanner successfully.",
        m_beVerbose);
  } else {
    printError(
  ...
  if (m_weWantFieldData == true) {
    // Field configuration
    printInfoMessage(
        "LdmrsSopasLayer::init: Reading current field configuration.",
        m_beVerbose);
    success = action_readFields();
    if (success == true) {
      printInfoMessage(
          "LdmrsSopasLayer::init: Successfully read the field configuration (" +
              ::toString(m_fields.getFields().size()) + " fields).",
          m_beVerbose);
      ...
      *f = m_fields;
      ...
      // Fail
      printError("LdmrsSopasLayer::init: Failed to read the field "
    ...
    // Eval cases
    printInfoMessage(
        "LdmrsSopasLayer::init: Reading current eval case configuration.",
        m_beVerbose);
    success = action_readEvalCases();
    if (success == true) {
      printInfoMessage(
          "LdmrsSopasLayer::init: Successfully read the eval cases (" +
              ::toString(m_evalCases.getEvalCases().size()) + " cases).",
          m_beVerbose);
    ...
    printInfoMessage("LdmrsSopasLayer::init: Skipping field configuration as "
                     "we want no field data.",
                     m_beVerbose);
  }
...
bool LdmrsSopasLayer::run() {
  ...
  if (m_weWantFieldData == true) {
    printInfoMessage(
        "LdmrsSopasLayer::run: Now subscribing to EvalCaseResults.",
        beVerboseHere);
    result = action_subscribeEvalCaseResults();
    if (result == false) {
      // Fail
      printError("LdmrsSopasLayer::run: Failed to subscribe to "
                 "EvalCaseResults, aborting.");
      return false;
    }
    printInfoMessage(
        "LdmrsSopasLayer::run: Subscription to EvalCaseResults was successful.",
        beVerboseHere);
  }

  // Here, we could also subscribe to scan data. However, scans are handeled
  // more efficientely over the LUX interface, so we ignore this part here.
  if (m_weWantScanData == true) {
    //		printWarning("LdmrsSopasLayer::run: Scan data reading via SOPAS / CoLa
    //is not implemented, ignoring this setting.");
    printInfoMessage("LdmrsSopasLayer::run: Now subscribing to scan data.",
                     beVerboseHere);
    ...
    printInfoMessage(
        "LdmrsSopasLayer::run: Subscription to scan data was successful.",
        beVerboseHere);
  }
...
bool LdmrsSopasLayer::action_flashFieldParameters() {
  ...
  printInfoMessage("LdmrsSopasLayer::action_flashFieldParameters: Called.",
                   beVerboseHere);

  if (isConnected() == false) {
    printWarning("LdmrsSopasLayer::action_flashFieldParameters: LD-MRS not "
                 "connected - aborting.");
    return false;
  }
  ...
  result = invokeMethod(index_meth_FlashFieldParameters, parameterBuffer,
                        parameterBufferLen, answer);

  if ((result == true) && (answer != NULL) && (answer->isValid() == true) &&
      (answer->size() > 0)) {
    ...
    if (success == 0) {
      ...
      m_isLoggedIn = false;
      result = false;
      printError("LdmrsSopasLayer::action_flashFieldParameters: FlashFieldPara "
                 "command was not successful!");
    } else {
      // Success
      m_isLoggedIn = true;
      result = true;
      printInfoMessage("LdmrsSopasLayer::action_flashFieldParameters: Field "
                       "parameters flashed successfully.",
                       beVerboseHere);
    }
  }
  ...
  printInfoMessage(
      "LdmrsSopasLayer::action_flashFieldParameters: All done, leaving.",
      beVerboseHere);
  return result;
}
...
bool LdmrsSopasLayer::action_flashMrsParameters() {
  ...
  printInfoMessage("LdmrsSopasLayer::action_flashMrsParameters: Called.",
                   beVerboseHere);

  if (isConnected() == false) {
    printWarning("LdmrsSopasLayer::action_flashMrsParameters: LD-MRS not "
                 "connected - aborting.");
    return false;
  }
  ...
  result = invokeMethod(index_meth_MthdFlashLUXParameters, parameterBuffer,
                        parameterBufferLen, answer);
  ...
  if ((result == true) && (answer != NULL) && (answer->isValid() == true) &&
    ...
    if (success == 0) {
      ...
      m_isLoggedIn = false;
      result = false;
      printError("LdmrsSopasLayer::action_flashMrsParameters: FlashLUXPara "
                 "command was not successful!");
    } else {
      // Success
      m_isLoggedIn = true;
      result = true;
      printInfoMessage("LdmrsSopasLayer::action_flashMrsParameters: MRS "
                       "parameters flashed successfully.",
                       beVerboseHere);
    }
  }
  ...
  printInfoMessage(
      "LdmrsSopasLayer::action_flashMrsParameters: All done, leaving.",
      beVerboseHere);
...
bool LdmrsSopasLayer::action_login() {
  ...
  // cola-B ByIndex
  printInfoMessage(
      "LdmrsSopasLayer::action_login: Trying to log in as authorized client.",
      beVerboseHere);
  ...
  if (isConnected() == true) {
    ...
    colab::addIntegerToBuffer<INT8>(parameterBuffer, parameterBufferLen,
                                    level); // user level
    colab::addIntegerToBuffer<UINT32>(parameterBuffer, parameterBufferLen,
                                      passwordHash_LDMRS); // password hash

    result = invokeMethod(index_meth_SetAccessMode, parameterBuffer,
                          parameterBufferLen, answer);
    ...
    if ((result == true) && (answer != NULL) && (answer->isValid() == true) &&
      ...
      if (success == 0) {
        ...
        m_isLoggedIn = true;
        result = true;
        printInfoMessage("LdmrsSopasLayer::action_login: Login was successful.",
                         beVerboseHere);
      }
    }
...
bool LdmrsSopasLayer::action_logout() {
  ...
  // cola-B ByIndex
  printInfoMessage("LdmrsSopasLayer::action_logout: Logging out now.",
                   beVerboseHere);

  bool result = false;

  if (isConnected() == true) {
    const static UINT32 passwordHash_LDMRS =
        0x00000000; // Not needed for LOGOUT
    SopasAnswer *answer = NULL;
    ...
    if ((result == true) && (answer != NULL) && (answer->isValid() == true) &&
      ...
      if (success == 0) {
        ...
        result = true;
        printInfoMessage(
            "LdmrsSopasLayer::action_logout: Logout was successful.",
            beVerboseHere);
      }
    }
    ...
    printWarning("LdmrsSopasLayer::action_logout: LD-MRS not connected - "
                 "cannot log out, aborting.");
  }
...
//
// Subscribe to EvalCase results. After the subscription, the scanner will send
// EvalCaseResults whenever there is a change in the EvalCase status.
//
bool LdmrsSopasLayer::action_subscribeEvalCaseResults() {
  ...
  if (isConnected() == false) {
    printError("LdmrsSopasLayer::action_subscribeEvalCaseResults: LD-MRS not "
               "connected, aborting!");
    return false;
  }
  ...
  if (result == true) {
    ...
    printError("LdmrsSopasLayer::action_subscribeEvalCaseResults: Failed to "
               "subscribe, aborting!");
  }
...
bool LdmrsSopasLayer::action_unSubscribeEvalCaseResults() {
  ...
  if (result == true) {
    ...
    printError("LdmrsSopasLayer::action_unSubscribeEvalCases: Failed to "
               "un-subscribe, aborting!");
  }
...
//
// Subscribe to scan data. After the subscription, the scanner will send new
// scan data whenever a scan is ready.
//
bool LdmrsSopasLayer::action_subscribeScanData() {
  bool beVerboseHere = m_beVerbose;
  //	beVerboseHere = true;
  printInfoMessage("LdmrsSopasLayer::action_subscribeScanData: Called.",
                   beVerboseHere);
  ...
  if (isConnected() == false) {
    printError("LdmrsSopasLayer::action_subscribeScanData: LD-MRS not "
               "connected, aborting!");
    return false;
  }
  ...
  if (result == true) {
    ...
    printError("LdmrsSopasLayer::action_subscribeScanData: Failed to "
               "subscribe, aborting!");
  }

  printInfoMessage("LdmrsSopasLayer::action_subscribeScanData: Done, leaving.",
                   beVerboseHere);
  return result;
}
...
bool LdmrsSopasLayer::action_unSubscribeScanData() {
  ...
  if (result == true) {
    ...
    printError("LdmrsSopasLayer::action_unSubscribeScanData: Failed to "
               "un-subscribe, aborting!");
  }
...
// Note that this is 'just' the configuration of the eval cases, not their
// current evaluation status, which is the EvalCaseResult.
//
...
EvalCases LdmrsSopasLayer::colaB_evalCaseDecoder(SopasAnswer *answer) {
  ...
  printInfoMessage("LdmrsSopasLayer::colaB_evalCaseDecoder: Called.",
                   beVerboseHere);
  ...
  if (answer != NULL && answer->size() > 0) {
    ...
    for (UINT16 i = 0; i < arrayLength; ++i) {
      ...
      evalCase.setStrategy(
          EvalCase::EvaluationStrategy(memread_UINT8(bufferPos)));
      ...
      //			evalCase.setHardwareInputs(EvalCaseParameter::inputState_from_UINT8(memread_UINT8(bufferPos),
      //2)); 			memread_UINT8(bufferPos); // reserved
      ...
      evalCase.setMaxRadialCorridor(double(memread_UINT16(bufferPos)) /
                                    1000.); // conversion from [mm] to [m]
      evalCase.setManipulationPrevention(
          EvalCase::ManipulationPrevention(memread_UINT8(bufferPos)));
      evalCase.setBlankingSize(double(memread_UINT16(bufferPos)) /
                               1000.); // conversion from [mm] to [m]
      evalCase.setMinFieldExp(double(memread_UINT16(bufferPos)) /
                              1000.); // conversion from [mm] to [m]
      ...
      printInfoMessage("LdmrsSopasLayer::colaB_evalCaseDecoder: Decoding "
                       "EvalCase with the name " +
                           name + ". Inputs: " + ::toHexString(inputs) + ".",
                       beVerboseHere);

      UINT16 commentLength = memread_UINT16(bufferPos);
      std::string comment = std::string((char *)bufferPos, commentLength);
      bufferPos += commentLength, evalCase.setComment(comment);
  ...
  printInfoMessage("LdmrsSopasLayer::colaB_evalCaseDecoder: Done. Decoded " +
                       ::toString(evalCases.getEvalCases().size()) + " cases.",
                   beVerboseHere);
...
//
UINT32 LdmrsSopasLayer::colaB_evalCaseEncoder(BYTE *buffer,
                                              const EvalCases &evalCases) {
  bool beVerboseHere = m_beVerbose;
  //	beVerboseHere = true;

  printInfoMessage("LdmrsSopasLayer::colaB_evalCaseEncoder: Called.",
                   m_beVerbose);
  ...
  for (UINT16 i = 0; i < arrayLength; i++) {
    ...
    printInfoMessage("LdmrsSopasLayer::colaB_evalCaseEncoder: Encoding "
                     "EvalCase with the name " +
    ...
    // RadialCorridor
    UINT16 maxRadialCorridor =
        (UINT16)((evalCase->getMaxRadialCorridor() * 1000.0) + 0.5);
    memwrite_UINT16(bufferPos, maxRadialCorridor);

    // Sensitivity
    UINT8 manipulationPrevention =
        (UINT8)(evalCase->getManipulationPrevention()); // Effectively boolean
    memwrite_UINT8(bufferPos, manipulationPrevention);

    // Blanking size
    UINT16 blankingSize =
        (UINT16)((evalCase->getBlankingSize() * 1000.0) + 0.5);
  ...
  printInfoMessage("LdmrsSopasLayer::colaB_evalCaseEncoder: Done. Encoded " +
                       ::toString(evalCases.getEvalCases().size()) +
                       " cases. Used " + ::toString(len) + " bytes.",
                   beVerboseHere);
...
//
bool LdmrsSopasLayer::action_writeField(UINT16 fieldNum,
                                        const FieldParameter &para) {
  bool beVerboseHere = m_beVerbose;
  //	beVerboseHere = true;

  printInfoMessage("LdmrsSopasLayer::action_writeField: Called for field " +
                       toString(fieldNum) + ".",
                   beVerboseHere);
  bool result = false;
  ...
  if (result == false) {
    // Failure
    printError("LdmrsSopasLayer::action_writeField: Failed to write field " +
               toString(fieldNum) + ", aborting!");
    return false;
  }

  // All done
  printInfoMessage("LdmrsSopasLayer::action_writeField: All done, leaving.",
                   beVerboseHere);
  return result;
}
...
bool LdmrsSopasLayer::action_writeEvalCases(const EvalCases &evalCases) {
  ...
  printInfoMessage("LdmrsSopasLayer::action_writeEvalCases: Called, with " +
                       toString(evalCases.getEvalCases().size()) +
                       " eval cases.",
                   beVerboseHere);
  bool result = false;
  ...
  if (evalCases.getEvalCases().size() > 16) {
    printError("LdmrsSopasLayer::action_writeEvalCases: The MRS can only "
               "handle up to 16 eval cases, aborting!");
    return false;
  }
  ...
  if (result == false) {
    // Failure
    printError("LdmrsSopasLayer::action_writeEvalCases: Failed to write eval "
               "cases, aborting!");
    return false;
  }

  // All done
  printInfoMessage("LdmrsSopasLayer::action_writeEvalCases: All done, leaving.",
                   beVerboseHere);
  return result;
}
...
bool LdmrsSopasLayer::action_readFields() {
  ...
  /*	success = readVariable(index_var_numOfParamFields, answer);
                  ...
                  (answer->size() > 0))
          {
                  ...
                  printInfoMessage("LdmrsSopasLayer::action_readFields:
     NumOfParamFields=" + toString(numOfParamFields) + ".", m_beVerbose);
          }
  ...
  */
  ...
  // As the parameter "index_var_numOfParamFields" seems unreliable, read all
  // (!) possible fields.
  for (UINT16 i = 0; i < MAX_NUM_OF_FIELDS; ++i) {
    printInfoMessage("LdmrsSopasLayer::action_readFields: Reading field " +
                         toString(i) + ".",
                     true);
    success = readVariable(index_var_field000 + i, answer);

    if ((success == true) && (answer != NULL) && (answer->size() > 0)) {
...
/// computes an angle in [rad] from INT32 as 1/10000 deg in scanner coordinate
/// system
...
//
// Decode an incoming Field strucure. This is the parameter set for one of the
// 16 possible fields.
//
...
FieldParameter *LdmrsSopasLayer::colaB_fieldDecoder(SopasAnswer *answer) {
  ...
  if ((answer != NULL) && (answer->size() > 0)) {
    ...
    field.setFieldTypeIntern(
        memread_UINT8(bufferPos)); // Field type (Rectangular or Segmented)
    field.setFieldNumber(memread_UINT8(bufferPos));

    printInfoMessage(
        "LdmrsSopasLayer::colaB_fieldDecoder: setDistScaleFactor: " +
            toString(field.getDistScaleFactor(), 2) + ".",
        beVerboseHere);
    printInfoMessage(
        "LdmrsSopasLayer::colaB_fieldDecoder: setDistScaleOffset: " +
            toString(field.getDistScaleOffset(), 2) + ".",
        beVerboseHere);
    printInfoMessage(
        "LdmrsSopasLayer::colaB_fieldDecoder: setAngleScaleFactor: " +
            toString(field.getAngleScaleFactor()) + ".",
        beVerboseHere);
    printInfoMessage(
        "LdmrsSopasLayer::colaB_fieldDecoder: setAngleScaleOffset: " +
            toString(field.getAngleScaleOffset()) + ".",
        beVerboseHere);
    printInfoMessage(
        "LdmrsSopasLayer::colaB_fieldDecoder: setFieldTypeIntern: " +
            toString((INT32)(field.getFieldTypeIntern())) + ".",
        beVerboseHere);
    printInfoMessage("LdmrsSopasLayer::colaB_fieldDecoder: Field number: " +
                         toString(field.getFieldNumber()) + ".",
                     beVerboseHere);
    ...
    if (fieldSeg == 1) {
      printInfoMessage(
          "LdmrsSopasLayer::colaB_fieldDecoder: Found segmented field.",
          beVerboseHere);

      UINT8 arrayLength = memread_UINT16(bufferPos);
      FieldSegmented *seg(new FieldSegmented);

      printInfoMessage("LdmrsSopasLayer::colaB_fieldDecoder: ArrayLength=" +
      ...
      for (UINT8 i = 0; i < arrayLength; ++i) {
        ...
        printInfoMessage("LdmrsSopasLayer::colaB_fieldDecoder: Point " +
                             toString(i) + ": AngIdx=" + toString(angleIdx) +
                             ", StartDist=" + toString(startDist) +
                             ", EndDist=" + toString(endDist) + ".",
                         beVerboseHere);
        ...
        double s =
            (startDist == 65535)
                ? ::NaN_double
                : ((double)startDist / 1000.) * field.getDistScaleFactor() +
                      (field.getDistScaleOffset() / 1000.); // from [mm] to [m]
        printInfoMessage("LdmrsSopasLayer::colaB_fieldDecoder: StartDist is " +
                             toString(s, 2) + ".",
                         beVerboseHere);

        // transform to 1/32° tics
        double angle = field.getAngleScaleOffset() +
                       (double)angleIdx * field.getAngleScaleFactor();
        // Quick hack: The LD-MRS treats the angle direction differently as
        // expected and defined in DIN70000. Therefore, the sign has to be
        // reversed here.
        angle = -angle;
        printInfoMessage("LdmrsSopasLayer::colaB_fieldDecoder: Angle is " +
                             toString(angle, 2) + ".",
                         beVerboseHere);
        ...
        double e =
            (endDist == 65535)
                ? ::NaN_double
                : ((double)endDist / 1000.) * field.getDistScaleFactor() +
                      (field.getDistScaleOffset() / 1000.); // from [mm] to [m]
        printInfoMessage("LdmrsSopasLayer::colaB_fieldDecoder: EndDist is " +
                             toString(e, 2) + ".",
                         beVerboseHere);
    ...
    if (fieldRect == 1) {
      printInfoMessage(
          "LdmrsSopasLayer::colaB_fieldDecoder: Found rectangular field.",
          beVerboseHere);

      FieldRectangle *rect(new FieldRectangle);
      INT32 refPointAngle = memread_INT32(bufferPos);
      // Quick hack: The LD-MRS treats the angle direction differently as
      // expected. Therefore, the sign has to be reversed here.
      refPointAngle = -refPointAngle;
      UINT32 refPointDist =
          (memread_UINT16(bufferPos) * field.getDistScaleFactor()) +
          field.getDistScaleOffset();
      INT32 rotAngle = memread_INT32(bufferPos);
      // Quick hack: The LD-MRS treats the angle direction differently as
      // expected. Therefore, the sign has to be reversed here.
      rotAngle = -rotAngle;
      UINT32 length =
          memread_UINT32(bufferPos); // width and length changed due to other
                                     // coordiante system in LD-MRS!!!
      UINT32 width = memread_UINT32(bufferPos);

      printInfoMessage("LdmrsSopasLayer::colaB_fieldDecoder: refPointDist is " +
      ...
      printInfoMessage(
          ...
          beVerboseHere);
      printInfoMessage("LdmrsSopasLayer::colaB_fieldDecoder: rotAngle is " +
                           toString(rotAngle, 2) + ".",
                       beVerboseHere);
      printInfoMessage("LdmrsSopasLayer::colaB_fieldDecoder: length is " +
                           toString(length, 2) + ".",
                       beVerboseHere);
      printInfoMessage("LdmrsSopasLayer::colaB_fieldDecoder: width is " +
                           toString(width, 2) + ".",
                       beVerboseHere);
    ...
    if (fieldRad == 1) {
      printError("LdmrsSopasLayer::colaB_fieldDecoder: Found radial field, but "
                 "the LD-MRS does not support radial fields!");
      return NULL;
    }
    ...
    if (fieldDyn == 1) {
      printError("LdmrsSopasLayer::colaB_fieldDecoder: Found dynamic field, "
                 "but the LD-MRS does not support dynamic fields!");
      return NULL;
    }
    ...
    if (fieldNameLength > 0) {
      std::string fieldName =
          colab::getStringFromBuffer(bufferPos, fieldNameLength);
    ...
    if (commentLength > 0) {
      std::string comment =
          colab::getStringFromBuffer(bufferPos, commentLength);
...
//
// Note that the created scans are not sorted by (increasing) hporizontal
// scanpoint angle. Instead, the scanpoints are decoded into the scan structure
// in the order in which they are transmitted in the message. If a sorted scan
// is required, this has to be done.
//
void LdmrsSopasLayer::scanDataDecoder(SopasEventMessage &frame) {
  ...
  // cola-B by index
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Called. Decoding SOPAS "
                   "frame with length=" +
                       ::toString(frame.size()) + " bytes.",
                   beVerboseHere);
  ...
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Reading variable index.",
                   beVerboseHere);
  receiveIndex = frame.getVariableIndex(); // byte number 12 13

  if (receiveIndex != index_event_ScanDataMonitor) {
    printError("LdmrsSopasLayer::scanDataDecoder: This is not a scan data "
               "message, aborting!");
    return;
  } else {
    printInfoMessage(
        "LdmrsSopasLayer::scanDataDecoder: This is a scan data message.",
        beVerboseHere);
  }

  // Obviously scan data events are registered. This is just for global
  // information.
  m_scanEventIsRegistered = true;

  bufferPos =
      &(frame.getPayLoad()[5]); // pay load without "sSI + index (2 byte)"
  UINT16 versionNumber = memread_UINT16(bufferPos);
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Version number=" +
                       ::toString(versionNumber) + ".",
                   beVerboseHere);
  ...
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Status bits=" +
                       ::toString(statusBits) + ".",
                   beVerboseHere);

  // Status block
  UINT16 telegramCount = memread_UINT16(bufferPos);
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Telegram count=" +
                       ::toString(telegramCount) + ".",
                   beVerboseHere);
  UINT16 scanCount = memread_UINT16(bufferPos);
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Scan count=" +
                       ::toString(scanCount) + ".",
                   beVerboseHere);
  UINT32 systemTimeScan = memread_UINT32(bufferPos);
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: System time scan=" +
                       ::toString(systemTimeScan) + ".",
                   beVerboseHere);
  UINT32 systemTimeTransmit = memread_UINT32(bufferPos);
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: System time transmit=" +
                       ::toString(systemTimeTransmit) + ".",
                   beVerboseHere);
  UINT16 inputState = memread_UINT16(bufferPos);
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Input state=" +
                       ::toString(inputState) + ".",
                   beVerboseHere);
  UINT16 outputState = memread_UINT16(bufferPos);
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Output state=" +
                       ::toString(outputState) + ".",
                   beVerboseHere);
  UINT16 reserved_1 = memread_UINT16(bufferPos);
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Reserved_1=" +
                       ::toString(reserved_1) + ".",
                   beVerboseHere);
  UINT32 scanFrequency = memread_UINT32(bufferPos); // [1/256 Hz]
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Scan frequency=" +
                       ::doubleToString(double(scanFrequency) / 256.0, 2) +
                       " Hz.",
                   beVerboseHere);
  UINT32 measurementFrequency = memread_UINT32(bufferPos); // [100 Hz]
  printInfoMessage(
      "LdmrsSopasLayer::scanDataDecoder: Measurement frequency=" +
          ::doubleToString(double(measurementFrequency) / 10.0, 1) + " kHz.",
      beVerboseHere);
  ...
  for (UINT16 i = 0; i < numOfEncoders; i++) {
    ...
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Encoder position=" +
                         ::toString(encoderPos) + " increments.",
                     beVerboseHere);
    INT16 encoderSpeed = memread_INT16(bufferPos); // [inc/mm]
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Encoder speed=" +
                         ::toString(encoderSpeed) + " inc/mm.",
                     beVerboseHere);
  }
  ...
  // Setup of the vertical layer angles. Only valid for standard "4-layer"
  // devices, so-called "8-layer" devices have different angles!
  double vAngles[4];
  ...
  for (UINT16 i = 0; i < numOf16BitDataChannels; i++) {
    ...
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Scale factor=" +
                         ::doubleToString(double(scaleFactor), 10) + ".",
                     beVerboseHere);
    float scaleOffset = memread_float(bufferPos);
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Scale offset=" +
                         ::doubleToString(double(scaleOffset), 10) + ".",
                     beVerboseHere);
    INT32 startAngle = memread_INT32(bufferPos); // [1/10000 deg]
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Start angle=" +
                         ::doubleToString(double(startAngle) / 10000.0, 2) +
                         ".",
                     beVerboseHere);
    UINT32 angularResolution =
        memread_UINT16(bufferPos); // [1/10000 deg]. Decoded value is UINT16,
                                   // but needed as UINT32 later.
    printInfoMessage(
        "LdmrsSopasLayer::scanDataDecoder: Angular resolution=" +
            ::doubleToString(double(angularResolution) / 10000.0, 2) + ".",
        beVerboseHere);
    ...
    if (layer > 3) {
      printError("LdmrsSopasLayer::scanDataDecoder: We have decoded an invalid "
                 "layer value of " +
                 toString(layer) + ", allowed is 0..3.");
      return;
    }
    if (echo > 2) {
      printError("LdmrsSopasLayer::scanDataDecoder: We have decoded an invalid "
                 "echo value of " +
                 toString(echo) + ", allowed is 0..2.");
      return;
    }
    ...
    for (UINT16 d = 0; d < numOf16BitData; d++) {
      ...
      //				printInfoMessage("LdmrsSopasLayer::scanDataDecoder:
      //Data=" + ::toString(data) + ".", beVerboseHere);

      // Add the point only if it is valid.
      if ((data > 0) && (data != 0xFFFF)) {
        // Create the new point structure in the scan, and fill it with the
        // values
        ScanPoint &newPoint = scan->addNewPoint();

        // Horizontal angle
        double hAngle = ((startAngle + d * angularResolution) / 10000.0) *
                        deg2rad; // hAngle is in [rad]
  ...
  if (numOfPositionBlocks == 1) {
    ...
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Position X=" +
                         ::doubleToString(double(posX), 2) + ".",
                     beVerboseHere);
    float posY = memread_float(bufferPos);
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Position Y=" +
                         ::doubleToString(double(posY), 2) + ".",
                     beVerboseHere);
    float posZ = memread_float(bufferPos);
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Position Z=" +
                         ::doubleToString(double(posZ), 2) + ".",
                     beVerboseHere);
    float rotX = memread_float(bufferPos); // roll angle
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Rot angle X=" +
                         ::doubleToString(double(rotX), 2) + ".",
                     beVerboseHere);
    float rotY = memread_float(bufferPos);
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Rot angle Y=" +
                         ::doubleToString(double(rotY), 2) + ".",
                     beVerboseHere);
    float rotZ = memread_float(bufferPos);
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Rot angle Z=" +
                         ::doubleToString(double(rotZ), 2) + ".",
                     beVerboseHere);
    UINT8 rotMode = memread_UINT8(bufferPos);
    printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Rot mode=" +
                         ::toString(rotMode) + ".",
                     beVerboseHere);
  }
  ...
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Number of device names=" +
  ...
  //
  // Ignore the rest of the data as the MRS does not fill it with any
  // information.
  //
  ...
  //	printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Scan start angle=" +
  //::doubleToString(scanStartAngle * rad2deg, 1) + ".", beVerboseHere);
  //	printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Scan end angle=" +
  //::doubleToString(scanEndAngle * rad2deg, 1) + ".", beVerboseHere);

  si.setScanFrequency(double(scanFrequency) / 256.0);
  si.setBeamTilt(0.0); // For 8-layer devices, the beam tilt is different
                       // (either 1.2 or 1.6 deg)!
  si.setScanFlags(0);
  ...
  Time start, end;
  start.set(double(systemTimeScan) /
            1000000.0); // No real information available
  end.set(
      (double(systemTimeScan) / 1000000.0) +
      (1.0 / (double(scanFrequency) / 256.0))); // No real information available
  ...
  // All done
  printInfoMessage("LdmrsSopasLayer::scanDataDecoder: Done, leaving.",
                   beVerboseHere);
}
...
void LdmrsSopasLayer::evalCaseResultDecoder(SopasEventMessage &frame) {
  // cola-B by index
  printInfoMessage("LdmrsSopasLayer::evalCaseResultDecoder: Called. Decoding "
                   "SOPAS frame with length=" +
                       ::toString(frame.size()) + " bytes.",
                   m_beVerbose);
  ...
  if (m_weWantFieldData == true) {
    ...
    printInfoMessage(
        "LdmrsSopasLayer::evalCaseResultDecoder: Reading variable index.",
        m_beVerbose);
    receiveIndex = frame.getVariableIndex(); // byte number 12 13

    if (receiveIndex != index_event_aEvalCaseResult) {
      printError("LdmrsSopasLayer::evalCaseResultDecoder: This is not an eval "
                 "case message, aborting!");
      return;
    } else {
      printInfoMessage("LdmrsSopasLayer::evalCaseResultDecoder: This is an "
                       "eval case message.",
                       m_beVerbose);
    }

    // Obviously field events are registered. This is just for global
    // information.
    m_fieldEventIsRegistered = true;
    ...
    bufferPos =
        &(frame.getPayLoad()[5]); // pay load without "sSI + index (2 byte)"
    UINT16 cases = memread_UINT16(bufferPos); // 14 15, array length in sopas is
                                              // always decoded in uint16 !!!
    printInfoMessage(
        "LdmrsSopasLayer::evalCaseResultDecoder: This frame contains " +
            ::toString(cases) + " cases to decode.",
        m_beVerbose);
    ...
    for (UINT16 i = 0; i < cases; i++) {
      ...
      result.CaseHdr.uiAngleScaleFactor =
          memread_UINT32(bufferPos); // 31 32 33 34
      result.CaseHdr.iAngleScaleOffset =
          memread_INT32(bufferPos); // 35 36 37 38
      ...
      // Dummy read for aFieldInfringement (always 0). Currently not in use, use
      // the result from above instead.
      length = memread_UINT16(bufferPos); // 40 41
                                          // 			if (length == 0)
                                          // 			{
      // 				printInfoMessage("LdmrsSopasLayer::evalCaseResultDecoder:
      // FieldInfringement found.", m_beVerbose); 				std::string fieldInfringement
      // = colab::getStringFromBuffer(bufferPos, length);
      // 				printInfoMessage("LdmrsSopasLayer::evalCaseResultDecoder:
      // FieldInfringement: " + fieldInfringement + ".", m_beVerbose);
      // 			}
      ...
      if (length == 0) {
        ...
        printInfoMessage("LdmrsSopasLayer::evalCaseResultDecoder: Case name: " +
                             result.m_sCaseName + ".",
                         m_beVerbose);
      }
      ...
      if (length == 0) {
        ...
        printInfoMessage("LdmrsSopasLayer::evalCaseResultDecoder: Comment: " +
                             result.m_sComment + ".",
                         m_beVerbose);
      }
    ...
    if ((evalCases->size() > 0) && ((*evalCases) != m_lastEvalCaseResults)) {
      m_lastEvalCaseResults = *evalCases;
      m_manager->setDeviceData((BasicData *)evalCases);
    }
  } else {
    printWarning("LdmrsSopasLayer::evalCaseResultDecoder: Received eval case, "
                 "but we do not want to listen to eval cases!");
...
SensorStateInfo LdmrsSopasLayer::getSensorStateInfo() {
  ...
  // traceDebug("") << "state: " << stateBits << " - inputs: " << inputBits << "
  // - outputs: " << outputBits << std::endl;
  ...
  //	{
  //		inputStates[j] = inputBits[j] ? SensorStateInfo::ON :
  //SensorStateInfo::OFF; 		outputStates[j] = outputBits[j] ? SensorStateInfo::ON
  //: SensorStateInfo::OFF;
  //	}
...
//
// Encode a FieldParameter structure into CoLa-B-data. This structure can then
// be sent to the MRS to parametrize one of the 16 possible fields. Returns the
// number of bytes that have been written into the buffer.
//
UINT32 LdmrsSopasLayer::colaB_fieldEncoder(BYTE *buffer,
  ...
  printInfoMessage("LdmrsSopasLayer::colaB_fieldEncoder: DistScaleFactor: " +
                       toString(field.getDistScaleFactor(), 2) + ".",
                   beVerboseHere);
  printInfoMessage("LdmrsSopasLayer::colaB_fieldEncoder: DistScaleOffset: " +
                       toString(field.getDistScaleOffset(), 2) + ".",
                   beVerboseHere);
  printInfoMessage("LdmrsSopasLayer::colaB_fieldEncoder: AngleScaleFactor: " +
                       toString(field.getAngleScaleFactor()) + ".",
                   beVerboseHere);
  printInfoMessage("LdmrsSopasLayer::colaB_fieldEncoder: AngleScaleOffset: " +
                       toString(field.getAngleScaleOffset()) + ".",
                   beVerboseHere);
  printInfoMessage("LdmrsSopasLayer::colaB_fieldEncoder: FieldTypeIntern: " +
                       toString((INT32)(field.getFieldTypeIntern())) + ".",
                   beVerboseHere);
  printInfoMessage("LdmrsSopasLayer::colaB_fieldEncoder: Field number: " +
                       toString(field.getFieldNumber()) + ".",
                   beVerboseHere);
  ...
  if (field.getFieldType() == FieldDescription::Segmented) {
    // Yes
    printInfoMessage(
        "LdmrsSopasLayer::colaB_fieldEncoder: Writing a segmented field.",
        beVerboseHere);
    ...
    for (UINT16 p = 0; p < seg->getNumberOfPoints(); p++) {
      ...
      //			double angle =
      //angleToRad(field.getAngleScaleOffset() + (double)angleIdx *
      //field.getAngleScaleFactor());
      ...
      printInfoMessage("LdmrsSopasLayer::colaB_fieldEncoder: AngleIdx = " +
                           toString(angleIdx) + ".",
                       beVerboseHere);
      ...
      if (startDist == NaN_double) {
        ...
        printInfoMessage(
            "LdmrsSopasLayer::colaB_fieldEncoder: StartDist = invalid.",
            beVerboseHere);
      } else {
        startDist *= 1000;
        ...
        printInfoMessage("LdmrsSopasLayer::colaB_fieldEncoder: StartDist = " +
      ...
      if (endDist == NaN_double) {
        ...
        memwrite_UINT16(buffer, 0xFFFF);
        printInfoMessage(
            "LdmrsSopasLayer::colaB_fieldEncoder: EndDist = invalid.",
            beVerboseHere);
      } else {
        endDist *= 1000;
        ...
        printInfoMessage("LdmrsSopasLayer::colaB_fieldEncoder: EndDist = " +
                             toString(endDistInt) + ".",
                         beVerboseHere);
      }
    }
  ...
  if (field.getFieldType() == FieldDescription::Rectangle) {
    printInfoMessage(
        "LdmrsSopasLayer::colaB_fieldEncoder: Writing rectangular field.",
        beVerboseHere);
    memwrite_UINT16(buffer, 1);
    ...
    // RefPointAngle
    INT32 refPointAngle = rect->getRefPointAngle() * rad2deg *
                          32.0; // ) .getField(). = memread_INT32(bufferPos);
    refPointAngle = -refPointAngle;
    memwrite_INT32(buffer, refPointAngle);

    // RefPointDist
    UINT16 refPointDist = (UINT16)(
        ((rect->getRefPointDist() * 1000.0) / field.getDistScaleFactor()) +
        field.getDistScaleOffset());
  ...
  printInfoMessage("LdmrsSopasLayer::colaB_fieldEncoder: Encoded " +
                       toString(len) + " bytes. All done, leaving.",
                   beVerboseHere);
