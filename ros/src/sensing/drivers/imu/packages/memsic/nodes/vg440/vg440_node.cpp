 ...
 *******************************************************************************/
int Size(QUEUE_TYPE *queue_ptr) { return queue_ptr->count; }
/*******************************************************************************
 * FUNCTION: Empty
 * ARGUMENTS: queue_ptr is pointer to the queue
 * RETURNS: return 1 if empty, 0 if not
 *******************************************************************************/
int Empty(QUEUE_TYPE *queue_ptr) { return queue_ptr->count <= 0; }
/*******************************************************************************
 * FUNCTION: Full
 * ARGUMENTS: queue_ptr is pointer to the queue
 * RETURNS: return 1 if full, 0 if not full
 *******************************************************************************/
int Full(QUEUE_TYPE *queue_ptr) { return queue_ptr->count >= MAXQUEUE; }
...
inline int MKInt(const unsigned char *data) {
  return (int)data[3] + (int)data[2] * 256 + (int)data[1] * 256 * 256 +
         (int)data[0] * 256 * 256 * 256;
}
...
void Nav1ToROSImu(const SNAV1Msg &rNav1, sensor_msgs::Imu &ImuData) {
  ...
  ImuData.orientation = tf::createQuaternionMsgFromRollPitchYaw(
      rNav1.dRollAngle, rNav1.dPitchAngle, rNav1.dYawAngle);
}

void RunVG440(const std::string &rsPort, int nBaudRate,
              const std::string &rsTopic, const std::string &rsFrame) {
  ...
  try {
    ...
    while (ros::ok()) {
      try {
        size_t nRet = boost::asio::read(port, boost::asio::buffer(DataBuf),
