...
//! @version 1.1
//
//! @description Implements an interface to a MIP device.  Performs stream
//! parsing and
...
//!                        mip_interface *device_interface, u32
//!                        packet_timeout_val)
//
//! @section DESCRIPTION
...
//! @param [in] u32 packet_timeout_val          - Timeout for the incoming
...
u16 mip_interface_init(const char *portstr, u32 baudrate,
  ...
  if (mip_sdk_port_open(&device_interface->port_handle, portstr, baudrate) !=
      MIP_USER_FUNCTION_OK)
    return MIP_INTERFACE_ERROR;

  // Initialize the command parser ring buffer
  if (ring_buffer_init_static(
          &device_interface->input_buffer, device_interface->input_buffer_bytes,
  ...
  // Callback 0 is always the command response handler & needs a pointer to the
  // interface
  device_interface->callback_function_list[0] =
      &__mip_interface_command_response_handler;
...
//! @section NOTES
//!
//! This function should be called regularly (e.g. every step of a minor
...
u16 mip_interface_update(mip_interface *device_interface) {
  ...
  if (ring_buffer_remaining_entries(&device_interface->input_buffer) <
      num_bytes)
  ...
  if (num_bytes > 0)
    mip_sdk_port_read(device_interface->port_handle, local_buffer, num_bytes,
                      &bytes_read, MIP_INTERFACE_PORT_READ_TIMEOUT_MS);

  // Write the local buffer to the ring buffer
  if (bytes_read > 0) {
    // printf("Got %d bytes\n", bytes_read);
    ring_buffer_write_multi(&device_interface->input_buffer, local_buffer,
...
//
//! @fn
//! u16 mip_interface_add_descriptor_set_callback(mip_interface
//! *device_interface, u8 data_set, void *user_ptr,
//!                      void (*packet_callback)(void *user_ptr, u8 *packet, u16
//!                      packet_size, u8 callback_type))
//
//! @section DESCRIPTION
...
//! @section DETAILS
//!
//! @param [in] mip_interface *device_interface - pointer to the mip interface
//! structure.
//! @param [in] u8 data_set                     - data set used to trigger the
//! callback.
//! @param [in] void *user_ptr                  - pointer to the user data,
//! which is passed to the callback.
//! @param [in] void (*packet_callback)         - function pointer for packet
//! callback function.
//
...
u16 mip_interface_add_descriptor_set_callback(
    mip_interface *device_interface, u8 data_set, void *user_ptr,
...
//
//! @fn
//! u16 mip_interface_delete_descriptor_set_callback(mip_interface
//! *device_interface, u8 data_set)
//
//! @section DESCRIPTION
...
u16 mip_interface_delete_descriptor_set_callback(
    mip_interface *device_interface, u8 data_set) {
  u16 i, found = 0;

  // Loop through all of the entries and remove any that reference the
  // descriptor set (note: callback 0 is always reserved for commands/responses)
  for (i = 1; i < MIP_INTERFACE_MAX_CALLBACKS + 1; i++) {
...
//
//! @fn
//! u16 mip_interface_write(mip_interface *device_interface, u8 *data, u32
//! num_bytes, u32 *bytes_written)
//
//! @section DESCRIPTION
...
//! @param [in] mip_interface *device_interface - pointer to the mip interface
//! structure.
//! @param [in] u8 *data                        - pointer to data buffer to be
//! written.
//! @param [in] u32 num_bytes                   - the number of bytes to be
//! written.
//
//! @retval MIP_INTERFACE_ERROR  Data not written or less than num_bytes
//! written.\n
...
u16 mip_interface_write(mip_interface *device_interface, u8 *data,
  ...
  *bytes_written = 0;
  ...
  for (i = 0; i < num_bytes; i++) {
    if (ring_buffer_write(&device_interface->input_buffer, &data[i], 1) ==
        RING_BUFFER_OK)
      (*bytes_written)++;
    else
      break;
...
//
//! @fn
//! u16 mip_interface_write_blocking(mip_interface *device_interface, u8 *data,
//! u32 num_bytes, u32 *bytes_written, u32 timeout_ms)
//
//! @section DESCRIPTION
//! Writes \c num_bytes of \c data to the device interface, blocks until the
//! space is available.
//
//! @section DETAILS
//!
...
//! @param [in] u32 num_bytes                   - the number of bytes to be
//! written.
//! @param [in] u32 timeout_ms                  - the timeout for the write in
//! milliseconds.
//
//! @retval MIP_INTERFACE_ERROR  Data not written or less than num_bytes
//! written.\n
...
u16 mip_interface_write_blocking(mip_interface *device_interface, u8 *data,
  ...
  while (num_bytes >
         ring_buffer_remaining_entries(&device_interface->input_buffer)) {
    if (__mip_interface_time_timeout(initial_time, timeout_ms) ==
...
u16 __mip_interface_parse_input_buffer(mip_interface *device_interface) {
  ...
  ///
  // Are we searching for a start condition? Requires: SYNC byte 1 & 2, and a
  // valid header
  ///
  ...
  if (device_interface->mip_packet_byte_count < sizeof(mip_header)) {
    ...
    while ((device_interface->mip_packet_byte_count == 0) &&
           ring_buffer_count(&device_interface->input_buffer)) {
      ret = ring_buffer_read(&device_interface->input_buffer,
                             &device_interface->mip_packet[0], 1);

      // Found a potential start byte
      if ((ret == RING_BUFFER_OK) &&
          (device_interface->mip_packet[0] == MIP_SYNC_BYTE1)) {
    ...
    if (device_interface->mip_packet_byte_count > 0) {
      // Need at least the size of (header - start_byte) in the buffer to start
      // processing
      if (ring_buffer_count(&device_interface->input_buffer) >=
          sizeof(mip_header) - 1) {
        // Try to get a header
        for (i = 0; i < sizeof(mip_header) - 1; i++) {
          ring_buffer_lookahead_read(&device_interface->input_buffer, i,
                                     &device_interface->mip_packet[i + 1], 1);
        }

        // If the header is valid, then continue to the next step
        if ((header_ptr->sync2 == MIP_SYNC_BYTE2) &&
            (header_ptr->payload_size + MIP_HEADER_SIZE + MIP_CHECKSUM_SIZE <=
             MIP_MAX_PACKET_SIZE)) {
      ...
      // Check for timeout on incoming packet
        ...
        // If a timeout happens, we probably had a badly-formed packet, start
        // over
        if (__mip_interface_time_timeout(device_interface->parser_start_time,
                                         device_interface->packet_timeout) ==
            MIP_INTERFACE_TIMEOUT) {
  ...
  if (device_interface->mip_packet_byte_count >= sizeof(mip_header)) {
    ...
    if (ring_buffer_count(&device_interface->input_buffer) >=
        (sizeof(mip_header) - 1 + header_ptr->payload_size +
         MIP_CHECKSUM_SIZE)) {
      // Get the remaining packet bytes
      for (i = 0; i < header_ptr->payload_size + MIP_CHECKSUM_SIZE; i++) {
        ring_buffer_lookahead_read(
            &device_interface->input_buffer, sizeof(mip_header) - 1 + i,
      ...
      if (mip_is_checksum_valid(device_interface->mip_packet) == MIP_OK) {
        device_interface->mip_packet_byte_count +=
            header_ptr->payload_size + MIP_CHECKSUM_SIZE;

        // Trigger the callback with the valid packet
        if (__mip_interface_find_callback(
                device_interface, header_ptr->descriptor_set,
                &callback_user_ptr, &callback_function) == MIP_INTERFACE_OK) {
          (*callback_function)(callback_user_ptr, device_interface->mip_packet,
                               device_interface->mip_packet_byte_count,
                               MIP_INTERFACE_CALLBACK_VALID_PACKET);
        }

        ring_buffer_consume_entries(&device_interface->input_buffer,
                                    device_interface->mip_packet_byte_count -
                                        1);
...
//
//! @fn
//! u16 __mip_interface_find_callback(mip_interface *device_interface, u8
//! data_set,
...
//! @section DETAILS
//!
//! @param [in]  mip_interface *device_interface       - pointer to the mip
//! interface structure.
//! @param [in]  u8 data_set                           - dataset to search for.
//! @param [out] void *callback_user_ptr               - pointer to the user
//! data for the callback.
//! @param [out] parser_callback_ptr callback_function - pointer to callback
//! function.
//
//! @retval MIP_INTERFACE_ERROR  The callback could not be located or there was
//! an error.\n
...
//! @section DETAILS
//!
//! @param [in]  u32 initial_time  - the start time to check against in
//! milliseconds.
...
//! @param [in]  u8 *packet       - The MIP packet that matches the waiting
//! command set.
...
void __mip_interface_command_response_handler(void *user_ptr, u8 *packet,
  ...
  if (callback_type == MIP_INTERFACE_CALLBACK_VALID_PACKET) {
    ...
    if (mip_get_first_field(packet, &field_header_ptr, &field_data_ptr,
                            &field_offset) == MIP_OK) {
      ack_nack_field_ptr = (global_ack_nack_field *)field_data_ptr;
      device_interface->command_id = ack_nack_field_ptr->command_echo_byte;
      device_interface->command_acknack_response =
          ack_nack_field_ptr->error_code;
    }

    // Get the remaining data & subtract the size of the first field
    if (mip_get_next_field(packet, &field_header_ptr, &field_data_ptr,
                           &field_offset) == MIP_OK) {
...
//
//! @fn
//! u16 mip_interface_send_command(mip_interface *device_interface, u8
//! command_set, u8 command_descriptor,
//!                                u8 *command_data, u16 command_data_size, u8
//!                                wait_for_response, u32 timeout_ms)
//
//! @section DESCRIPTION
...
//! @section DETAILS
//!
//! @param [in]  mip_interface *device_interface - a pointer to the
//! mip_interface structure.
...
//! @param [in]  u8 wait_for_response            - 1: The function will wait for
//! the response, 0: it will not wait.
//! @param [in]  u32 timeout_ms                  - the timeout in milliseconds.
//
//! @retval MIP_INTERFACE_OK    If a response was received and ACK'd or no
//! resonse requested.\n
//! @retval MIP_INTERFACE_ERROR If the response was not received in the timeout
//! period or NACK'd.\n
//
//! @section NOTES
//!
...
u16 mip_interface_send_command(mip_interface *device_interface, u8 command_set,
                               u8 command_descriptor, u8 *command_data,
                               u16 command_data_size, u8 wait_for_response,
                               u32 timeout_ms) {
  ...
  mip_add_field(mip_packet, MIP_MAX_PACKET_SIZE, command_data,
                command_data_size, command_descriptor);
  packet_size = mip_finalize(mip_packet);

  // Send the packet to the device
  return mip_interface_send_preformatted_command(
      device_interface, mip_packet, packet_size, wait_for_response, timeout_ms);
}
...
u16 mip_interface_send_preformatted_command(mip_interface *device_interface,
  ...
  // Send the packet
  if (mip_sdk_port_write(device_interface->port_handle, command, command_size,
                         &bytes_written, timeout_ms) != MIP_INTERFACE_OK)
    return MIP_INTERFACE_ERROR;
  ...
  if (mip_get_first_field(command, &field_header_ptr, &field_data_ptr,
                          &field_offset) != MIP_OK) {
    return MIP_INTERFACE_ERROR;
  }
  ...
  return_code = __mip_interface_wait_for_response(
      device_interface, command_set, command_descriptor, &acknack_response,
      &response_data, &response_data_size, timeout_ms);

  if ((return_code != MIP_INTERFACE_OK) ||
      (acknack_response != MIP_ACK_NACK_ERROR_NONE))
    return MIP_INTERFACE_ERROR;
...
//
//! @fn
//! u16 mip_interface_send_command_with_response(mip_interface
//! *device_interface, u8 command_set, u8 command_descriptor, u8 *command_data,
//!                                              u16 command_data_size, u8
//!                                              **response_data, u16
...
//! @param [in]  u8 *command_data                - preformatted command data.
//! @param [in]  u16 command_data_size           - size of command data.
//! @param [out] u8 **response_data              - pointer to a pointer that
//! will point to the beginning of the response.
...
//! @section NOTES
//!
//! \c response_data will point to an internal buffer within the MIP
//! interface.\n The user should copy the information to their own buffer before
//! manipulation.
//!
/////////////////////////////////////////////////////////////////////////////

u16 mip_interface_send_command_with_response(
    mip_interface *device_interface, u8 command_set, u8 command_descriptor,
  ...
  // Send the packet to the device
  return mip_interface_send_preformatted_command_with_response(
      device_interface, mip_packet, packet_size, response_data,
...
//
//! @fn
//! u16 mip_interface_send_preformatted_command_with_response(mip_interface
//! *device_interface, u8 *command, u16 command_size,
//!                                                           u8 *response_data,
//!                                                           u16
//!                                                           **response_data_size,
//!                                                           u32 timeout_ms)
...
u16 mip_interface_send_preformatted_command_with_response(
    mip_interface *device_interface, u8 *command, u16 command_size,
...
//
//! @fn
//! u16  __mip_interface_wait_for_response(mip_interface *device_interface, u8
//! command_set, u8 command_descriptor,
//!                                        u8 *acknack_response, u8
//!                                        **response_data, u16
//!                                        *response_data_size, u32 timeout_ms)
//
//! @section DESCRIPTION
...
//! @param [in]  mip_interface *device_interface - a pointer to the
//! mip_interface structure.
//! @param [in]  u8 command_set                  - the command set we are
//! waiting for.
//! @param [in]  u8 command_descriptor           - the command descriptor we are
//! waiting for.
//! @param [out] u8  *acknack_response           - the resulting ack/nack
//! response from the device.
//! @param [out] u8 **response_data              - a pointer to a pointer to the
//! response data, NULL if no data field exists.
//! @param [out] u16 *response_data_size         - size of the response data, 0
//! if no data field exists.
//! @param [in]  u32 timeout_ms                  - the timeout in milliseconds.
//
...
u16 __mip_interface_wait_for_response(mip_interface *device_interface,
                                      ...
                                      u8 *acknack_response, u8 **response_data,
                                      u16 *response_data_size, u32 timeout_ms) {
  ...
  while (__mip_interface_time_timeout(start_time, timeout_ms) ==
         MIP_INTERFACE_NO_TIMEOUT) {
    // Allow the parser to run
    mip_interface_update(device_interface);

    // Got a response
    if ((device_interface->command_response_received == 1) &&
        (device_interface->command_id == command_descriptor)) {
      // Set the feedback
