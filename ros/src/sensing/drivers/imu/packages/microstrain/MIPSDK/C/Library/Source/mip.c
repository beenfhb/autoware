...
//! @section DETAILS
//!
//! @param [out] u8 *mip_buffer    - pointer to memory used to construct the mip
//! packet.
...
//! @section DETAILS
//!
//! @param [in] u8 *mip_buffer    - pointer to memory used to construct the mip
//! packet.
//! @param [in] u8 descriptor_set - expected descriptor set field value.
//
//! @retval MIP_OK           - MIP packet initialized.\n
//! @retval MIP_ERROR        - The pointer is NULL or the MIP packet is not
//! intialized.\n
//
//! @section NOTES
//!
...
//
//! @fn
//! u16 mip_add_field(u8 *mip_buffer, u16 buffer_size, void *field_data, u32
//! data_size, u16 data_descriptor)
//
//! @section DESCRIPTION
...
//! @section DETAILS
//!
//! @param [in,out] u8 *mip_buffer     - pointer to memory used to construct the
//! mip packet.
//! @param [in] u16 buffer_size        - size of the mip mempory buffer.
//! @param [in] void *field_data       - buffer containing the field data.
//! @param [in] u8 u16 data_size       - size of the field data (exlcuding the
//! field header).
...
//! @retval MIP_MEMORY_ERROR - Not enough room in the mip buffer to add the
//! field or\n
...
u16 mip_add_field(u8 *mip_buffer, u16 buffer_size, void *field_data,
  ...
  new_packet_size = sizeof(mip_header) + header_ptr->payload_size + data_size +
                    sizeof(mip_field_header) + MIP_CHECKSUM_SIZE;

  // Check if there is enough room left to store the field
  if ((new_packet_size > MIP_MAX_PACKET_SIZE) ||
      (new_packet_size > buffer_size))
    return MIP_MEMORY_ERROR;

  // Jump to the end of the packet
  field_header_ptr = (mip_field_header *)(mip_buffer + sizeof(mip_header) +
...
//
//! @section DESCRIPTION
//! Adds a pre-formatted field (header and data already assembled) to an
//! initialized MIP packet buffer.
//
//! @section DETAILS
//!
//! @param [in,out] u8 *mip_buffer  - pointer to memory used to construct the
//! mip packet.
//! @param [in] u16 buffer_size     - size of the mip mempory buffer.
//! @param [in] void *field         - buffer containing the field (must have
//! header and data already assembled).
//
//! @retval MIP_OK           - field added to MIP packet.\n
//! @retval MIP_ERROR        - A pointer is NULL.\n
//! @retval MIP_MEMORY_ERROR - Not enough room in the mip buffer to add the
...
u16 mip_add_formatted_field(u8 *mip_buffer, u16 buffer_size, void *field) {
  ...
  new_packet_size = sizeof(mip_header) + header_ptr->payload_size +
                    field_header_ptr->size + MIP_CHECKSUM_SIZE;
...
//! @section DETAILS
//!
//! @param [in,out] u8 *mip_buffer  - pointer to memory containing the mip
//! packet.
//
//! @retval MIP_OK           - Header is valid.\n
//! @retval MIP_ERROR        - The pointer is NULL.\n
//! @retval MIP_MEMORY_ERROR - The buffer does not contain a valid MIP packet
//! header.
//
//! @section NOTES
//!
...
//
//! @section DESCRIPTION
//! Gets the first MIP field (a wrapper for mip_get_next_field that makes it
//! easier to use).
//
//! @section DETAILS
//!
//! @param [in]  u8 *mip_buffer  - pointer to memory containing the mip packet.
//! @param [out] mip_field_header **field_header - pointer to the first field
//! header.
//! @param [out] u8               **field_data   - pointer to the first field
//! data.
//! @param [out] u16              *field_offset  - integer offset from start of
//! packet payload of first field.
//
...
//! @section DETAILS
//!
//! @param [in]     u8 *mip_buffer                  - pointer to memory
//! containing the mip packet.
//! @param [out]    mip_field_header **field_header - pointer to the current
//! field header.
//! @param [out]    u8  **field_data                - pointer to the current
//! field data.
//! @param [in,out] u16 *field_offset               - integer offset from start
//! of packet payload of next field.
//
//! @retval MIP_OK                  - c\ field_header and \c field_data contain
//! the parsed information.\n
//! @retval MIP_ERROR               - A pointer is NULL.\n
//! @retval MIP_INVALID_PACKET      - The buffer does not contain a valid MIP
//! packet.
...
//! @param [in] u8 *mip_buffer  - pointer to memory containing the mip packet.
//
//! @returns  The MIP descriptor set byte if the buffer points to a valid
//! packet.\n
//!           0x00 otherwise.
//
//! @section NOTES
...
u16 mip_is_checksum_valid(u8 *mip_buffer) {
  ...
  packet_checksum = ((u16)mip_buffer[checksum_offset] << 8) +
                    (u16)mip_buffer[checksum_offset + 1];
...
//! @param [in] u8 *mip_buffer  - pointer to memory containing the mip packet.
//
//! @returns  The MIP packet checksum, if mip_buffer contains a valid MIP
//! packet.\n
//!           0x00 otherwise.
//
//! @section NOTES
