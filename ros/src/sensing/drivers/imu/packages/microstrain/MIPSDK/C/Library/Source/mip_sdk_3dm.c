...
//
//! @fn
//! u16 mip_3dm_cmd_poll_ahrs(mip_interface *device_interface, u8
//! option_selector,
...
//! @param [in] u8 option_selector              - ACK/NACK packet generation
//! option.
//! @param [in] u8 num_descriptors              - The number of descriptors
//! provided.
//! @param [in] u8 *descriptor_list             - A descriptor list containing
//! \c num_descriptors descriptor values.
//
...
//! @section NOTES
//!
//! The user receives the AHRS packet using the callback function for the AHRS
//! data class.
//!
...
u16 mip_3dm_cmd_poll_ahrs(mip_interface *device_interface, u8 option_selector,
  ...
  // Check the number of descriptors does not exceed the maximum that will fit
  // into the command data
  ...
  return mip_interface_send_command(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_POLL_AHRS_MESSAGE,
      command_data, sizeof(u8) * 2 + num_descriptors * 3, wait_for_response,
...
//
//! @fn
//! u16 mip_3dm_cmd_poll_gps(mip_interface *device_interface, u8
//! option_selector,
//!                          u8 num_descriptors, u8 *descriptor_list)
//
//! @section DESCRIPTION
...
//! @section NOTES
//!
//! The user receives the GPS packet using the callback function for the GPS
//! data class.
//!
//! Possible \c option_selector values:\n\n
//!    \li 0x00 - normal ACK/NACK reply\n
...
u16 mip_3dm_cmd_poll_gps(mip_interface *device_interface, u8 option_selector,
  ...
  // Send the command
  return mip_interface_send_command(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_POLL_GPS_MESSAGE,
      command_data, sizeof(u8) * 2 + num_descriptors * 3, wait_for_response,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
//
//! @fn
//! u16 mip_3dm_cmd_poll_filter(mip_interface *device_interface, u8
//! option_selector,
//!                          u8 num_descriptors, u8 *descriptor_list)
//
//! @section DESCRIPTION
...
//! @section NOTES
//!
//! The user receives the NAV packet using the callback function for the NAV
//! data class.
//!
//! Possible \c option_selector values:\n\n
//!    \li 0x00 - normal ACK/NACK reply\n
...
u16 mip_3dm_cmd_poll_filter(mip_interface *device_interface, u8 option_selector,
  ...
  // Send the command
  return mip_interface_send_command(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_POLL_NAV_MESSAGE,
      command_data, sizeof(u8) * 2 + num_descriptors * 3, wait_for_response,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
//
//! @fn
//! u16 mip_3dm_cmd_get_ahrs_base_rate(mip_interface *device_interface, u16
//! *base_rate)
//
//! @section DESCRIPTION
...
//!
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [out] u16 *base_rate                 - buffer for the returned base
//! rate.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
...
u16 mip_3dm_cmd_get_ahrs_base_rate(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_GET_AHRS_BASE_RATE,
      NULL, 0, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_3dm_cmd_get_gps_base_rate(mip_interface *device_interface, u16
//! *base_rate)
//
//! @section DESCRIPTION
...
u16 mip_3dm_cmd_get_gps_base_rate(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_GET_GPS_BASE_RATE,
      NULL, 0, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_3dm_cmd_get_filter_base_rate(mip_interface *device_interface, u16
//! *base_rate)
//
//! @section DESCRIPTION
...
u16 mip_3dm_cmd_get_filter_base_rate(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_GET_NAV_BASE_RATE,
      NULL, 0, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_3dm_cmd_ahrs_message_format(mip_interface *device_interface, u8
//! function_selector,
//!                                     u8 *num_entries, u8 *descriptors, u16
//!                                     *descimation)
//
//! @section DESCRIPTION
...
//! perform.
//! @param [in,out] u8 *num_entries             - The number of descriptors
//! provided or the number read back from the device. (used to set or get
//! depending on \c function_selector)
//! @param [in,out] u8 *descriptors             - A descriptor list containing
//! \c num_descriptors descriptor values. (used to set or get depending on \c
//! function_selector)
//! @param [in,out] u8 *decimation              - A decimation list containing
//! \c num_descriptors decimation values. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
...
//!
//! \c num_entries may be 0, descriptors = NULL, and decimation = NULL for the
//! following \c function_selector values:
//!
...
u16 mip_3dm_cmd_ahrs_message_format(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_AHRS_MESSAGE_FORMAT,
      command_data, command_data_size, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    // Must have at least the number of entries in the message format to
    // continue
...
//
//! @fn
//! u16 mip_3dm_cmd_gps_message_format(mip_interface *device_interface, u8
//! function_selector,
//!                                    u8 *num_entries, u8 *descriptors, u16
//!                                    *descimation)
//
...
//! perform.
//! @param [in,out] u8 *num_entries             - The number of descriptors
//! provided/available or the number read back from the device. (used to set or
...
u16 mip_3dm_cmd_gps_message_format(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_GPS_MESSAGE_FORMAT,
      command_data, command_data_size, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_3dm_cmd_filter_message_format(mip_interface *device_interface, u8
//! function_selector,
//!                                    u8 *num_entries, u8 *descriptors, u16
//!                                    *descimation)
//
...
u16 mip_3dm_cmd_filter_message_format(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_NAV_MESSAGE_FORMAT,
      command_data, command_data_size, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//!                                        u8 function_selector, u8
//!                                        device_selector, u8 *enable)
//
//! @section DESCRIPTION
...
//! @param [in] u8 function_selector            - Selects which function to
//! perform.
//! @param [in] u8 device_selector              - Selects which device is
//! affected.
//! @param [in,out] u8 *enable                  - the enable/disable flag. (used
...
u16 mip_3dm_cmd_continuous_data_stream(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_CONTROL_DATA_STREAM,
      command_data, 3, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//! @param [in] u8 function_selector            - Selects which function to
//! perform.
//! @param [in,out] u8 *dynamics_mode           - The dynamics mode. (used to
...
u16 mip_3dm_cmd_gps_dynamics_mode(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_SET_GPS_DYNAMICS_MODE,
...
//
//! @fn
//! u16 mip_3dm_cmd_gps_advanced_settings(mip_interface *device_interface, u8
//! function_selector)
//
//! @section DESCRIPTION
...
u16 mip_3dm_cmd_gps_advanced_settings(mip_interface *device_interface,
  ...
  return mip_interface_send_command(
      device_interface, MIP_3DM_COMMAND_SET,
      MIP_3DM_CMD_SAVE_RESTORE_GPS_SETTINGS, &function_selector, 1, 1,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
//! to perform.
//! @param [in,out] mip_ahrs_signal_settings *settings - The AHRS signal
//! conditioning settings structure. (used to set or get depending on \c
//! function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
...
//! datastream. If the NAV datastream is available, the values remain
...
u16 mip_3dm_cmd_ahrs_signal_conditioning(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_SET_AHRS_SIGNAL_COND,
      command_data, sizeof(u8) + sizeof(mip_ahrs_signal_settings),
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_3DM_REPLY_AHRS_SIGNAL_SETTINGS) &&
        (field_header_ptr->size >=
         sizeof(mip_field_header) + sizeof(mip_ahrs_signal_settings))) {
      memcpy(settings, response_data + sizeof(mip_field_header),
...
//
//! @fn
//! u16 mip_3dm_cmd_ahrs_timestamp(mip_interface *device_interface, u8
//! function_selector,
...
//! @param [in] u8 function_selector            - Selects which function to
//! perform.
//! @param [in,out] u8 *time_selector           - The selection for the format
//! of the time value.
//! @param [in,out] u32 *time                   - The current time value. (used
//! to set or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
...
u16 mip_3dm_cmd_ahrs_timestamp(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_SET_AHRS_TIMESTAMP,
      command_data, 6, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_3DM_REPLY_AHRS_TIMESTAMP_VALUE) &&
        (field_header_ptr->size >=
         sizeof(mip_field_header) + sizeof(u8) + sizeof(u32))) {
      memcpy(time_selector, response_data + sizeof(mip_field_header),
             sizeof(u8));
...
//
//! @fn
//! u16 mip_3dm_cmd_startup_settings(mip_interface *device_interface, u8
//! function_selector)
//
//! @section DESCRIPTION
...
u16 mip_3dm_cmd_startup_settings(mip_interface *device_interface,
                                 u8 function_selector) {
  return mip_interface_send_command(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_SAVE_DEVICE_SETTINGS,
      &function_selector, 1, 1,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
//
//! @fn
//! u16 mip_3dm_cmd_uart_baudrate(mip_interface *device_interface, u8
//! function_selector, u32 *baudrate)
//
//! @section DESCRIPTION
...
//! @param [in] u8 function_selector            - Selects which function to
//! perform.
//! @param [in,out] u32 *baudrate               - The baudrate value. (used to
//! set or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
...
u16 mip_3dm_cmd_uart_baudrate(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_UART_BAUDRATE,
      command_data, 5, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_3dm_cmd_datastream_format(mip_interface *device_interface, u8
//! function_selector, u8 device_selector, u8 *format)
//
//! @section DESCRIPTION
//! Set or read the datastream format on a particular datastream.  Valid on the
//! GX3-35 only.
//
//! @section DETAILS
//!
...
//! @param [in] u8 device_selector              - Selects which device is
//! affected.
//! @param [in,out] u8 *format                  - The selected format of the
//! data. (used to set or get depending on \c function_selector)
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_datastream_format(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_DATASTREAM_FORMAT,
      command_data, 3, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_3dm_cmd_power_state(mip_interface *device_interface, u8
//! function_selector, u8 device_selector, u8 *power_state)
//
//! @section DESCRIPTION
//! Set or read the power state on a particular datastream.  Valid on the GX3-25
//! and GX3-35 only.
//
//! @section DETAILS
//!
...
//! @param [in] u8 device_selector              - Selects which device is
//! affected.
//! @param [in,out] u8 *power_state             - The selected power state.
...
u16 mip_3dm_cmd_power_state(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_DEVICE_POWER_STATE,
      command_data, 3, &response_data, &response_data_size,
      MIP_INTERFACE_LONG_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_3DM_REPLY_POWER_STATE) &&
        (field_header_ptr->size >= sizeof(mip_field_header) + sizeof(u8) * 2)) {
      memcpy(power_state, response_data + sizeof(mip_field_header) + 1,
             sizeof(u8));
    } else
      return_code = MIP_INTERFACE_ERROR;
  }
...
//
//! @fn
//! u16 mip_3dm_cmd_device_status(mip_interface *device_interface, u16
//! model_number,
//!                               u8 status_selector, u8 *response_buffer, u16
//!                               *response_size)
//
//! @section DESCRIPTION
...
//!
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [in] u16 model_number                - The model number of the
//! device.
//! @param [in] u8 status_selector              - The type of status desired.
//! @param [out] u8 *response_buffer            - A buffer to hold the response.
//! @param [in,out] u16 *response_size          - On entry, the size of the
//! buffer.  On exit, the size of the returned status message.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
//!                              the device communications failed.\n
...
//! @section NOTES
//!
//! Due to the variable response from the device, this function \b DOES \b NOT
//! byteswap the output.  This is the responsibility of the developer.
//!
...
//!
//!
//! Please see the device DCP for valid status selector values and the format of
...
u16 mip_3dm_cmd_device_status(mip_interface *device_interface, u16 model_number,
  ...
  *response_size = 0;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_DEVICE_STATUS,
      command_data, 3, &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->size - sizeof(mip_field_header)) <=
        user_buffer_size) {
...
//
//! @fn
//! u16 mip_3dm_cmd_low_pass_filter_settings(mip_interface *device_interface, u8
//! function_selector, mip_low_pass_filter_settings *filter_settings)
//
//! @section DESCRIPTION
...
//!
//! @param [in] mip_interface *device_interface	- The device interface.
//! @param [in] u8 function_selector            - The model number of the
//! device.
//! @param [in,out] mip_low_pass_filter_settings *filter_settings - structure
//! specifying the data type to be filtered and the filter parameters.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_low_pass_filter_settings(
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET,
      MIP_3DM_CMD_LOW_PASS_FILTER_SETTINGS, command_data,
      sizeof(u8) + sizeof(mip_low_pass_filter_settings), &response_data,
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_3DM_REPLY_ADVANCED_DATA_FILTER) &&
        (field_header_ptr->size >=
         sizeof(mip_field_header) + sizeof(mip_low_pass_filter_settings))) {
      memcpy(filter_settings, response_data + sizeof(mip_field_header),
...
//
//! @fn
//! u16 mip_3dm_cmd_accel_bias(mip_interface *device_interface, u8
//! function_selector, float *bias_vector)
//
//! @section DESCRIPTION
...
//! @param [in] u8 function_selector            - The model number of the
//! device.
//! @param [in,out] float *bias_vector		- pointer to array containing
//! input bias vector on write commands or to contain output bias vector on read
//! commands.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
...
u16 mip_3dm_cmd_accel_bias(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_ACCEL_BIAS,
      command_data, sizeof(u8) + 3 * sizeof(float), &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_3DM_REPLY_ACCEL_BIAS_VECTOR) &&
        (field_header_ptr->size >=
         sizeof(mip_field_header) + 3 * sizeof(float))) {
      memcpy(bias_vector, response_data + sizeof(mip_field_header),
             3 * sizeof(float));
...
//
//! @fn
//! u16 mip_3dm_cmd_gyro_bias(mip_interface *device_interface, u8
//! function_selector, float *bias_vector)
//
//! @section DESCRIPTION
//! Get or set Accelerometer x, y, z bias vector.
...
u16 mip_3dm_cmd_gyro_bias(mip_interface *device_interface, u8 function_selector,
                          float *bias_vector) {
  u8 *response_data, i;
  u16 response_data_size;
  u16 return_code;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_GYRO_BIAS,
      command_data, sizeof(u8) + 3 * sizeof(float), &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_3dm_cmd_coning_sculling_compensation(mip_interface
//! *device_interface, u8 function_selector, u8 *enable)
//
//! @section DESCRIPTION
...
u16 mip_3dm_cmd_coning_sculling_compensation(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET,
      MIP_3DM_CDM_CONING_AND_SCULLING_ENABLE, command_data, 2, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor ==
         MIP_3DM_REPLY_CONING_AND_SCULLING_ENABLE) &&
        (field_header_ptr->size >= sizeof(mip_field_header) + sizeof(u8))) {
...
//
//! @fn
//! u16 mip_3dm_cmd_capture_gyro_bias(mip_interface *device_interface, u8
//! function_selector, float *bias_vector)
//
//! @section DESCRIPTION
//! Get or set Accelerometer x, y, z bias vector.
...
u16 mip_3dm_cmd_capture_gyro_bias(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_CAPTURE_GYRO_BIAS,
      (u8 *)&command_data, sizeof(u16), &response_data, &response_data_size,
...
//
//! @fn
//! u16 mip_3dm_cmd_hard_iron(mip_interface *device_interface, u8
//! function_selector, float *vector)
//
//! @section DESCRIPTION
...
//! @param [in] u8 function_selector            - The model number of the
//! device.
//! @param [in,out] float *vector		        - Pointer to array containing
//! input hard iron vector on write commands or to contain output hard iron
//! vector on read commands.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_hard_iron(mip_interface *device_interface, u8 function_selector,
                          float *vector) {
  u8 *response_data, i;
  u16 response_data_size;
  u16 return_code;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_HARD_IRON_VECTOR,
      command_data, sizeof(u8) + 3 * sizeof(float), &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_3DM_REPLY_HARD_IRON_VECTOR) &&
        (field_header_ptr->size >=
         sizeof(mip_field_header) + 3 * sizeof(float))) {
      memcpy(vector, response_data + sizeof(mip_field_header),
             3 * sizeof(float));
...
//
//! @fn
//! u16 mip_3dm_cmd_soft_iron(mip_interface *device_interface, u8
//! function_selector, float *matrix)
//
//! @section DESCRIPTION
...
//! device.
//! @param [in,out] float *vector		        - Pointer to array containing
//! input soft iron 3x3 matrix on write commands or to contain output soft iron
//! 3x3 matrix on read commands.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_soft_iron(mip_interface *device_interface, u8 function_selector,
                          float *matrix) {
  u8 *response_data, i;
  u16 response_data_size;
  u16 return_code;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_SOFT_IRON_MATRIX,
      command_data, sizeof(u8) + 9 * sizeof(float), &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_3DM_REPLY_SOFT_IRON_MATRIX) &&
        (field_header_ptr->size >=
         sizeof(mip_field_header) + 9 * sizeof(float))) {
      memcpy(matrix, response_data + sizeof(mip_field_header),
             9 * sizeof(float));
...
//
//! @fn
//! u16 mip_3dm_sensor2vehicle_tranformation(mip_interface *device_interface, u8
//! function_selector, float euler_angles[3])
//
//! @section DESCRIPTION
...
//! device.
//! @param [in,out] float *vector		        - Pointer to array containing
//! input Euler angles on write commands or to contain output Euler angles on
//! read commands.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
...
u16 mip_3dm_sensor2vehicle_tranformation(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET,
      MIP_3DM_CDM_SENSOR2VEHICLE_TRANSFORMATION, command_data,
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor ==
         MIP_3DM_REPLY_SENSOR2VEHICLE_TRANSFORMATION) &&
        (field_header_ptr->size >=
         sizeof(mip_field_header) + 3 * sizeof(float))) {
...
//
//! @fn
//! u16 mip_3dm_cmd_complementary_filter_settings(mip_interface
//! *device_interface, u8 function_selector, mip_complementary_filter_settings
//! *settings)
//
//! @section DESCRIPTION
...
//! @param [in] u8 function_selector            - The model number of the
//! device.
//! @param [in,out] mip_complementary_filter_settings *settings - structure for
//! the complementary filter parameters.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
//!                              the device communications failed.\n
...
u16 mip_3dm_cmd_complementary_filter_settings(
    mip_interface *device_interface, u8 function_selector,
  ...
  if (function_selector == MIP_FUNCTION_SELECTOR_WRITE) {
    memcpy(&command_data[1], settings,
    ...
    // Byteswap if enabled
    if (MIP_SDK_CONFIG_BYTESWAP) {
      byteswap_inplace(&settings_pointer->north_compensation_time_constant,
                       sizeof(float));
      byteswap_inplace(&settings_pointer->up_compensation_time_constant,
                       sizeof(float));
    }
  }

  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_3DM_COMMAND_SET, MIP_3DM_CMD_COMPLEMENTARY_FILTER,
      command_data, sizeof(u8) + sizeof(mip_complementary_filter_settings),
      &response_data, &response_data_size,
      MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
  ...
  if ((return_code == MIP_INTERFACE_OK) && (response_data != NULL)) {
    ...
    if ((field_header_ptr->descriptor == MIP_3DM_REPLY_COMPLEMENTARY_FILTER) &&
      ...
      // Byteswap the baudrate if enabled
      if (MIP_SDK_CONFIG_BYTESWAP) {
        byteswap_inplace(&settings->north_compensation_time_constant,
                         sizeof(float));
        byteswap_inplace(&settings->up_compensation_time_constant,
                         sizeof(float));
      }
    } else
      return_code = MIP_INTERFACE_ERROR;
...
//
//! @fn
//! u16 mip_3dm_cmd_rtcm_23_message(mip_interface *device_interface, u8*
//! raw_data, u16 num_bytes)
//
//! @section DESCRIPTION
...
//!
//! @param [in] mip_interface *device_interface	- The device interface.
//! @param [in] u8* raw_data                    - Buffer containing "num_bytes"
//! of a RTCM 2.3 message.
...
//! @section NOTES
//!
//! This function will send multiple commands when the number of bytes in
//! "raw_data"\n is larger than MIP_MAX_PAYLOAD_DATA_SIZE.
//
/////////////////////////////////////////////////////////////////////////////

u16 mip_3dm_cmd_rtcm_23_message(mip_interface *device_interface, u8 *raw_data,
                                u16 num_bytes) {
  u16 curr_byte = 0, bytes_to_send;

  // Loop through the raw data bytes and send in MIP_MAX_PAYLOAD_DATA_SIZE size
  // chunks
  while (curr_byte < num_bytes) {
    ...
    // Send the data
    if (mip_interface_send_command(
            device_interface, MIP_3DM_COMMAND_SET,
            MIP_3DM_CMD_RAW_RTCM_2_3_MESSAGE, &raw_data[curr_byte],
            bytes_to_send, 1,
            MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS) !=
        MIP_INTERFACE_OK) {
      return MIP_INTERFACE_ERROR;
