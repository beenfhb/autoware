...
u16 mip_base_cmd_ping(mip_interface *device_interface) {
  return mip_interface_send_command(
      device_interface, MIP_BASE_COMMAND_DESC_SET, MIP_CMD_DESC_BASE_PING, NULL,
      0, 1, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
...
u16 mip_base_cmd_idle(mip_interface *device_interface) {
  return mip_interface_send_command(
      device_interface, MIP_BASE_COMMAND_DESC_SET,
      MIP_CMD_DESC_BASE_SET_TO_IDLE, NULL, 0, 1,
...
//!
//! @param [in] mip_interface *device_interface       - The device interface.
//! @param [out] base_device_info_field *device_info  - The returned device info
//! structure.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
...
u16 mip_base_cmd_get_device_info(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_BASE_COMMAND_DESC_SET,
      MIP_CMD_DESC_BASE_GET_DEVICE_INFO, NULL, 0, &response_data,
  ...
  if (return_code == MIP_INTERFACE_OK) {
    ...
    if ((field_header_ptr->descriptor == MIP_REPLY_DESC_BASE_DEVICE_INFO) &&
        (field_header_ptr->size >=
         sizeof(mip_field_header) + sizeof(base_device_info_field))) {
      memcpy(device_info, response_data + sizeof(mip_field_header),
             response_data_size - sizeof(mip_field_header));
...
//
//! @fn
//! u16 mip_base_cmd_get_device_supported_descriptors(mip_interface
//! *device_interface, u8 *response_buffer, u16 *response_size)
//
//! @section DESCRIPTION
...
//!
//! @param [in] mip_interface *device_interface - The device interface.
//! @param [out] u8 *response_buffer            - The returned array of
//! descriptors.
//! @param [in, out] u16 *response_size         - On entry, the size of the
//! buffer; On exit, the size of the data returned.
//
//! @retval MIP_INTERFACE_ERROR  When there is a problem with the command format
//! or the\n
//!                              the device communications failed.\n
...
u16 mip_base_cmd_get_device_supported_descriptors(
    mip_interface *device_interface, u8 *response_buffer, u16 *response_size) {
  u8 *response_data;
  u16 response_data_size;
  u16 return_code;
  mip_field_header *field_header_ptr;
  ...
  *response_size = 0;
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_BASE_COMMAND_DESC_SET,
      MIP_CMD_DESC_BASE_GET_DEVICE_DESCRIPTORS, NULL, 0, &response_data,
      &response_data_size, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
...
//
//! @fn
//! u16 mip_base_cmd_built_in_test(mip_interface *device_interface, u32
//! *bit_result)
//
//! @section DESCRIPTION
...
u16 mip_base_cmd_built_in_test(mip_interface *device_interface,
  ...
  return_code = mip_interface_send_command_with_response(
      device_interface, MIP_BASE_COMMAND_DESC_SET,
      MIP_CMD_DESC_BASE_BUILT_IN_TEST, NULL, 0, &response_data,
  ...
  if (return_code == MIP_INTERFACE_OK) {
    ...
    if ((field_header_ptr->descriptor == MIP_REPLY_DESC_BASE_BUILT_IN_TEST) &&
      ...
      memcpy(bit_result, response_data + sizeof(mip_field_header),
             response_data_size - sizeof(mip_field_header));
...
u16 mip_base_cmd_resume(mip_interface *device_interface) {
  return mip_interface_send_command(
      device_interface, MIP_BASE_COMMAND_DESC_SET, MIP_CMD_DESC_BASE_RESUME,
...
//! @section NOTES
//!
//! If communicating via USB, it is important to close the port immediately
//! after\n calling this function.  Failure to do so may result in the
//! designated port\n hanging.
//!
//
/////////////////////////////////////////////////////////////////////////////
...
u16 mip_base_cmd_reset_device(mip_interface *device_interface) {
  return mip_interface_send_command(
      device_interface, MIP_BASE_COMMAND_DESC_SET, MIP_CMD_DESC_BASE_SOFT_RESET,
      NULL, 0, 1, MIP_INTERFACE_DEFAULT_COMMAND_RESPONSE_TIMEOUT_MS);
}
