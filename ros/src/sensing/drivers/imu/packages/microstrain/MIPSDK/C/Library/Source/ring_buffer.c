...
//! @param [in]  u8 *data_buffer_ptr - data pointer (must be allocated and size
//! = \c max_entries * \c entry_size).
//! @param [in]  u32 max_entries     - maximum entries the ring buffer may
//! contain.
...
//
//! @fn
//! u16 ring_buffer_init_malloc(ring_buffer *buffer, u32 max_entries, u8
//! entry_size)
//
//! @section DESCRIPTION
...
//!
//! @param [out] ring_buffer *buffer - pointer to a ring buffer structure.
//! @param [in] u8 *entry            - pointer to memory buffer holding the
//! entry.
//! @param [in] u32 num_bytes        - number of bytes to write.
//
//! @retval RING_BUFFER_ERROR        Entry not written, buffer not
//! initialized.\n
//! @retval RING_BUFFER_FULL         Entry not written, the buffer is full.\n
//! @retval RING_BUFFER_MEMORY_ERROR Entry not written, num_bytes is larger than
//! the ring buffer entry size.\n
//! @retval RING_BUFFER_OK           Entry written.\n
//
//! @section NOTES
//!
//! \c num_bytes included for ring buffers with variable entry sizes (e.g.
//! strings). \c num_bytes must be less than the max entry size.
//!
//
/////////////////////////////////////////////////////////////////////////////
...
//
//! @fn
//! u16 ring_buffer_write_multi(ring_buffer *buffer, u8 *entry_buff, u32
//! num_entries)
//
//! @section DESCRIPTION
...
//!
//! @param [out] ring_buffer *buffer - pointer to a ring buffer structure.
//! @param [in] u8 *entry_buff       - pointer to memory buffer holding the
//! entries.
//! @param [in] u32 num_entries      - number of entries to write.
//
//! @retval RING_BUFFER_ERROR        Entries not written, buffer not
//! initialized.\n
//! @retval RING_BUFFER_FULL         Entries not written, the buffer is full.\n
//! @retval RING_BUFFER_MEMORY_ERROR Entries not written, num_bytes is larger
//! than the ring buffer entry size.\n
//! @retval RING_BUFFER_OK           Entries written.\n
//
//! @section NOTES
//!
//! All entries are assumed to be of size \c entry_size, which is set by the
//! ring_buffer_init_xxxxxx call.
//!
//
/////////////////////////////////////////////////////////////////////////////
  ...
  *num_written = 0;
  ...
  for (i = 0; i < num_entries; i++) {
    ...
    for (j = 0; j < buffer->entry_size; j++)
      buffer->entries[entry_pos * buffer->entry_size + j] =
          entry_buff[i * buffer->entry_size + j];
...
//!
//! @param [in] ring_buffer *buffer - pointer to a ring buffer structure.
//! @param [out] u8 *entry          - pointer to data buffer where the entry
//! will be stored.
...
//! @retval RING_BUFFER_MEMORY_ERROR Entry not read, max_bytes is smaller than
//! the ring buffer entry size.\n
...
//!
//! @param [in] ring_buffer *buffer - pointer to a ring buffer structure.
//! @param [in] u32 offset          - offset from the head of the ring buffer to
//! read.
//! @param [out] u8 *entry          - pointer to data buffer where the entry
//! will be stored.
//! @param [in] u32 max_bytes       - maximum size of the data buffer.
...
//
//! @fn
//! u16 ring_buffer_read_multi(ring_buffer *buffer, u8 *entry_buff, u32
//! entry_buff_size,
...
//!
//! @param [in]  ring_buffer *buffer - pointer to a ring buffer structure.
//! @param [out] u8 *entry           - pointer to data buffer where the entries
//! will be stored.
...
u16 ring_buffer_read_multi(ring_buffer *buffer, u8 *entry_buff,
  ...
  *num_read = 0;
  ...
  for (i = 0; i < num_requested; i++) {
    ...
    for (j = 0; j < buffer->entry_size; j++)
      entry_buff[i * buffer->entry_size + j] =
          buffer->entries[buffer->position * buffer->entry_size + j];
...
//
//! @fn
//! u16 ring_buffer_lookahead_read_multi(ring_buffer *buffer, u32 offset, u8
//! *entry_buff,
//!                                      u32 entry_buff_size, u32 num_requested,
//!                                      u32 *num_read)
//
...
//!
//! @param [in]  ring_buffer *buffer - pointer to a ring buffer structure.
//! @param [in]  u32 offset          - offset from the head of the ring buffer
//! to read.
//! @param [out] u8 *entry_buff      - pointer to data buffer where the entries
//! will be stored.
//! @param [in]  u32 entry_buff_size - maximum size of the data buffer in bytes.
//! @param [in]  u32 num_requested   - the number of entries requested.
//! @param [out] u32 num_read        - the number of entries actually read.
...
u16 ring_buffer_lookahead_read_multi(ring_buffer *buffer, u32 offset,
  ...
  *num_read = 0;
  ...
  for (i = 0; i < num_requested; i++) {
    ...
    for (j = 0; j < buffer->entry_size; j++)
      entry_buff[i * buffer->entry_size + j] =
          buffer->entries[local_position * buffer->entry_size + j];
...
//! @section DETAILS
//!
//! @param [out] ring_buffer *to   - pointer to a ring buffer structure to copy
//! to.
//! @param [in]  ring_buffer *from - pointer to a ring buffer structure to copy
//! from.
...
u32 ring_buffer_copy(ring_buffer *to, ring_buffer *from, u32 num_entries) {
  ...
  for (i = 0; i < num_entries; i++) {
    ...
    for (j = 0; j < to->entry_size; j++)
      to->entries[write_pos * to->entry_size + j] =
          from->entries[from->position * from->entry_size + j];
...
//
//! @fn
//! u16 ring_buffer_lookahead_copy(ring_buffer *to, u32 offset, ring_buffer
//! *from, u32 num_entries)
//
//! @section DESCRIPTION
...
//! @param [out] ring_buffer *to   - pointer to a ring buffer structure to copy
//! to.
//! @param [in]  u32 offset        - offset from the first element to start the
//! copy.
//! @param [in]  ring_buffer *from - pointer to a ring buffer structure to copy
//! from.
//! @param [in]  num_entries       - the number of entries to copy.
...
u32 ring_buffer_lookahead_copy(ring_buffer *to, u32 offset, ring_buffer *from,
                               u32 num_entries) {
  ...
  for (i = 0; i < num_entries; i++) {
    ...
    for (j = 0; j < to->entry_size; j++)
      to->entries[write_pos * to->entry_size + j] =
          from->entries[read_pos * from->entry_size + j];
...
//! @param [in] ring_buffer *buffer - pointer to a ring buffer structure.
//
//! @returns NULL (0) If the ring buffer pointer is invalid or there are no
//! elements.\n
