...
VectaCam::VectaCam(std::string in_camera_ip, unsigned int in_configuration_port,
                   unsigned int in_data_port, std::string in_parameter_file) {
...
void VectaCam::_parse_parameter_file(
    std::string in_parameter_file, std::vector<VectaCamCommand> &out_commands) {
  std::string line;
  ...
  while (getline(config_file, line)) {
    ...
    if (tokens.size() == 2) {
      ...
      current_command.low_nibble_address =
          (unsigned char)((full_address >> 0) & 0xFF);
      current_command.high_nibble_address =
          (unsigned char)((full_address >> 8) & 0xFF);
      current_command.command_data = std::stoi(tokens[1], NULL, 16);
      out_commands.push_back(current_command);
    } else
      std::cout << "Unrecognized command in configuration file: "
                << parameter_file_ << std::endl;
  }
  config_file.close();
}

void VectaCam::_send_commands_to_camera(
    unsigned int in_port, std::vector<VectaCamCommand> in_commands) {
  ...
  for (unsigned int i = 0; i < in_commands.size(); i++) {
    ...
    unsigned char command_buffer[12] = {0x05,
                                        0x00,
                                        0x36,
                                        0x00,
                                        0x00,
                                        ...
                                        0x00};
    ssize_t sent_data =
        sendto(socket_descriptor, command_buffer, sizeof(command_buffer), 0,
...
void VectaCam::_enable_camera(unsigned int in_port, bool in_enable) {
  ...
  if (sendto(socket_descriptor, enable_command, sizeof(enable_command), 0,
             reinterpret_cast<sockaddr *>(&socket_address),
             sizeof(socket_address)) < 0) {
    std::cout << "Could not send the enable command to the camera socket"
              << std::endl;
  }
}

void VectaCam::_initialize_camera(unsigned int in_configuration_port,
                                  unsigned int in_data_port,
                                  std::string in_parameter_file) {
...
long int VectaCam::GetFrameNumber() { return this->frame_number_; }
...
void VectaCam::StartCamera() {
  ...
  if (bind(socket_descriptor, reinterpret_cast<sockaddr *>(&socket_address),
           sizeof(socket_address)) < 0) {
  ...
  if (getsockname(socket_descriptor,
                  reinterpret_cast<sockaddr *>(&socket_address), &length) < 0) {
  ...
  std::cout << "Listening camera in UDP port number "
            << ntohs(socket_address.sin_port) << std::endl;

  _udp_receive(socket_descriptor);
}

void VectaCam::_form_image(int in_line_number, char *in_buffer,
                           uint32_t in_packet_offset,
  ...
  if (in_line_number < image_height_ && total < 1500 && p < image_width_ * 3 &&
    ...
    for (int i = 0; i < total; ++i) {
      image_buffer_[in_line_number * image_width_ * 3 + p + 2] =
          (((((int)in_buffer[k + 0]) << 4) & 0x3f0) +
           ((((int)in_buffer[k + 1]) >> 4) & 0xf)) >>
          2; // R
      image_buffer_[in_line_number * image_width_ * 3 + p + 1] =
          (((((int)in_buffer[k + 1]) << 6) & 0x3c0) +
           ((((int)in_buffer[k + 2]) >> 2) & 0x3f)) >>
          2; // G
      image_buffer_[in_line_number * image_width_ * 3 + p + 0] =
          (((((int)in_buffer[k + 2]) << 8) & 0x300) +
           ((((int)in_buffer[k + 3]) >> 0) & 0xff)) >>
          2; // B
...
void VectaCam::_udp_receive(int in_socket_descriptor) {
  ...
  while (this->running_) {
    n = recvfrom(in_socket_descriptor, buffer_in, VECTACAM_MAX_BUFFER, 0,
    ...
    if (n < 0) {
      ...
      if (header == VECTACAM_FRAME_START) {
        ...
        // frame end
        this->camera_image_ =
            cv::Mat(image_height_, image_width_, CV_8UC3, image_buffer_);
        frame_number_++;
      } else {
