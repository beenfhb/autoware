...
class VectaCam {
public:
  VectaCam(std::string in_camera_ip, unsigned int in_configuration_port,
...
private:
  ...
  void _initialize_camera(unsigned int in_configuration_port,
                          unsigned int in_data_port,
                          std::string in_parameter_file);
  void _parse_parameter_file(std::string in_parameter_file,
                             std::vector<VectaCamCommand> &out_commands);
  void _send_commands_to_camera(unsigned int in_port,
                                std::vector<VectaCamCommand> in_commands);
  void _enable_camera(unsigned int in_port, bool in_enable);
  void _form_image(int in_line_number, char *in_buffer,
                   uint32_t in_packet_offset, uint32_t in_packet_length);
