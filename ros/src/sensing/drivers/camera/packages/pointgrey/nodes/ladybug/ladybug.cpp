...
void parseCameraInfo(const cv::Mat &camMat, const cv::Mat &disCoeff,
                     const cv::Size &imgSize, sensor_msgs::CameraInfo &msg) {
...
void GetMatricesFromFile(ros::NodeHandle nh,
...
LadybugError start_camera() {
  ...
  error = ladybugSetAbsPropertyEx(m_context, LADYBUG_FRAME_RATE, false, true,
                                  m_isFrameRateAuto, m_frameRate);
  if (error != LADYBUG_OK) {
    return error;
  }
...
LadybugError stop_camera() {
  ...
  if (cameraError != LADYBUG_OK) {
    ROS_INFO("Error: Unable to stop camera (%s)",
             ladybugErrorToString(cameraError));
  }
  return cameraError;
}
...
int main(int argc, char **argv) {
  ...
  if (LADYBUG_OK != init_camera()) {
    ROS_INFO("Error: Failed to initialize camera (%s). Terminating...",
             ladybugErrorToString(grabberInitError));
    return -1;
  }
  ...
  if (startError != LADYBUG_OK) {
    ROS_INFO("Error: Failed to start camera (%s). Terminating...",
             ladybugErrorToString(startError));
    return -1;
  }
  /////////////////////
  // ROS
  ...
  int image_scale = 100;
  if (private_nh.getParam("scale", image_scale) && image_scale > 0 &&
      image_scale < 100) {
  ...
  camera_info_pub =
      n.advertise<sensor_msgs::CameraInfo>("/camera/camera_info", 1, true);
  ...
  while (running_ && ros::ok()) {
    ...
    if (acquisitionError != LADYBUG_OK) {
      ROS_INFO("Failed to acquire image. Error (%s). Trying to continue..",
    ...
    for (size_t i = 0; i < LADYBUG_NUM_CAMERAS; i++) {
      ...
      cv::Mat rawImage(size, CV_8UC1,
                       currentImage.pData + (i * size.width * size.height));
      cv::Mat image(size, CV_8UC3);
      cv::cvtColor(rawImage, image, cv::COLOR_BayerBG2RGB);
      cv::resize(image, image,
                 cv::Size(size.width * image_scale / 100,
                          size.height * image_scale / 100));
      //
