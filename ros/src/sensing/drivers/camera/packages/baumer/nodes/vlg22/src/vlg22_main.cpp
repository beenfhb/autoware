...
/*
 * Initialize cameras
 * @return Returns True if the camera systems and at least one camera was
...
bool baumer_startup(std::vector<BGAPI::System *> &ppSystem,
                    ...
                    std::vector<BGAPI::Image *> &imagePointers, int &camera_num,
                    int &system_count, BGAPI::Image *pImage) {
  int currSystem = 0;
  int i = 0;
  ...
  res = init_cameras(system_count, &ppSystem, &currSystem, camera_num,
                     cameraPointers);
  if (res != BGAPI_RESULT_OK) {
...
/*!
 * brightness in this program is defined as the ratio between the pixels with
 * higher values over the lowest ones. Ratio is calculated splitting the
 * histogram in half, and dividing the number of pixels in the seconf (higher)
 * part of the histogram (brightness higher than 128) over the total number of
 * pixels.
 * @param in_image
 * @return
 */
float get_image_brightness_percent(cv::Mat &in_image) {
  ...
  cv::calcHist(&in_image, 1, 0, cv::Mat(), histogram, 1, &histSize, &histRange,
               uniform, accumulate);
  ...
  cv::normalize(histogram, histogram, 0, histImage.rows, cv::NORM_MINMAX, -1,
                cv::Mat());
  for (int j = 1; j < histSize; j++) {
    total_samples += histogram.at<float>(j);
    cv::line(histImage,
             cv::Point(bin_w * (j - 1),
                       hist_h - cvRound(histogram.at<float>(j - 1))),
             cv::Point(bin_w * (j), hist_h - cvRound(histogram.at<float>(j))),
             cv::Scalar(255, 255, 255), 2, 8, 0);
  }

  std::cout << high_half_samples << "/" << total_samples << "==>" << brightness
            << std::endl;
  /// Display
...
void get_ros_params(ros::NodeHandle &node_handle, int &fps, double &scale,
                    double &brightness) {
...
void ros_init_publishers(ros::NodeHandle node_handle,
                         ros::Publisher publishers[], int camera_num) {
...
void adjust_exposure(BGAPI::Camera *camera_pointer, cv::Mat &bayer_image,
                     float &current_brightness, float threshold_brightness,
                     int &prev_exposure, int &exposure, int exposure_delta) {
...
int main(int argc, char **argv) {
  ...
  if (!baumer_startup(ppSystem, systemIter, res, cameraPointers, imagePointers,
                      camera_num, system_count, pImage)) {
    return -1;
  }
  ...
  while (ros::ok()) {
    ...
    for (unsigned int i = 0; i < cameraPointers.size(); i++) {
      ...
      if (res != BGAPI_RESULT_OK) {
        ...
        adjust_exposure(cameraPointers[i], bayer_image, current_brightness,
                        brightness, prev_exposure, exposure, exposure_delta);

        sensor_msgs::Image msg =
            ros_prepare_image(count, resized_color_image, i);
