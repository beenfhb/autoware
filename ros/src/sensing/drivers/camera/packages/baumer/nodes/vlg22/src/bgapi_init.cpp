...
int init_systems(int *system_count,
                 std::vector<BGAPI::System *> *externppSystem) {
  ...
  for (i = 0; i < *system_count; i++) {
    ...
    if (res != BGAPI_RESULT_OK) {
      ROS_INFO(
          "BGAPI::CreateSystem Errorcode: %d Systemnumber %d SysPointer 0x%p\n",
          res, i, (void *)pSystem);
...
void stop_cameras(std::vector<BGAPI::Camera *> &cameraObjects) {
  BGAPI_RESULT res = BGAPI_RESULT_FAIL;
  for (unsigned int i = 0; i > cameraObjects.size();
       i++) { // stop the camera when you are done with the capture
...
bool setup_cameras(std::vector<BGAPI::Camera *> &cameraObjects,
                   std::string camera_pixel_format_str) {
  BGAPI_RESULT res = BGAPI_RESULT_FAIL;
  for (unsigned int i = 0; i < cameraObjects.size(); i++) {
    ...
    res = cameraObjects[i]->getPixelFormat(formatlist.current, &state,
                                           &pixellist);
    if (res != BGAPI_RESULT_OK) {
    ...
    for (int p = 0; p < pixellist.length; p++) {
      res = cameraObjects[i]->getPixelFormatDescription(
          formatlist.current, pixellist.array[p], &pformat);
      if (res != BGAPI_RESULT_OK) {
      ...
      if (strcmp(camera_pixel_format_str.c_str(), (char *)pformat.sName) == 0) {
        // change pixel format if current setting does not match arg pixel
        // format.
    ...
    if (pixel_formart_id_to_select != -1) {
      res = cameraObjects[i]->setPixelFormat(
          pixellist.array[pixel_formart_id_to_select]);
      if (res != BGAPI_RESULT_OK) {
      ...
      res = cameraObjects[i]->getPixelFormatDescription(
          formatlist.current, pixellist.array[pixel_formart_id_to_select],
          &pformat);
      if (res != BGAPI_RESULT_OK) {
        ROS_INFO("BGAPI::Camera::getPixelFormatDesc error:%d\n", res);
        return false;
      }
      ROS_INFO("setPixelFormat => (ID %x:%s | %d)\n", pformat.iPixelFormat,
      ...
      if (pixel_formart_id_to_select != pixellist.current) {
        ROS_INFO("can\'t set PixelFormat to =>(%d, %s) (%d)\n",
                 pixellist.array[pixel_formart_id_to_select],
                 camera_pixel_format_str.c_str(), pixellist.current);
        return false;
      }
    }
    ...
    ROS_INFO("Current Gain: %g max:%g min:%g\n", gainrange.current,
             gainrange.maximum, gainrange.minimum);
...
int init_cameras(int system_count, std::vector<BGAPI::System *> *externppSystem,
                 int *pCurrSystem, int &camera_count,
                 std::vector<BGAPI::Camera *> &cameraObjects) {
  BGAPI_RESULT res = BGAPI_RESULT_FAIL;
  int cam = 0;
  camera_count = 0;
  ...
  for (systemIter = externppSystem->begin();
       systemIter != externppSystem->end(); systemIter++) {
    int count = 0;
    // this is an example how to count available cameras for all available
    // systems
    ...
    for (cam = 0; cam < count; cam++) {
      ...
      printf("%d select Camera %d of system %d - %s SN: %s\n", camera_count,
             cam, (int)(systemIter - externppSystem->begin()),
             cameradeviceinfo.modelName, cameradeviceinfo.serialNumber);
