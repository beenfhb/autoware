...
CalibrateCameraChessboardBase::CalibrateCameraChessboardBase(
  ...
  chessboardtype->addItems(QStringList() << "Box Grid"
                                         << "Circle Grid");
...
bool CalibrateCameraVelodyneChessboardBase::loadCalibResult(
  ...
  if (button == QMessageBox::Yes) {
    ...
    for (i = 0; i < n; i++) {
      ...
      int j, m = tmpmat.rows;
      calibvelodynespoints[i] = pcl::PointCloud<pcl::PointXYZI>::Ptr(
      ...
      for (j = 0; j < m; j++) {
        ...
        tmpvelodynetable->setItem(j, 0,
                                  new QTableWidgetItem(QString("%1").arg(
                                      calibvelodynespoints[i]->points[j].x)));
        tmpvelodynetable->setItem(j, 1,
                                  new QTableWidgetItem(QString("%1").arg(
                                      calibvelodynespoints[i]->points[j].y)));
        tmpvelodynetable->setItem(j, 2,
                                  new QTableWidgetItem(QString("%1").arg(
                                      calibvelodynespoints[i]->points[j].z)));
        tmpvelodynetable->setItem(
            j, 3,
            new QTableWidgetItem(QString("%1").arg(
                calibvelodynespoints[i]->points[j].intensity)));
      }
      ...
      PlaneExtractor *planeextraction =
          new PlaneExtractor(calibvelodynespoints[i], i, radius);
