...
void trans_depth_points_to_image_points(
  ...
  for (i = 0; i < (int)scan_points_dataset->scan_points.x.size(); i++) {
    ...
    /*
     * Coordinate transformation. Change from laser range finder coordinate to
     * camera coordinate
     */
    camera_x = (cameraExtrinsicMat.at<double>(0, 0) *
                    scan_points_dataset->scan_points.x.at(i) * 1000 +
                cameraExtrinsicMat.at<double>(0, 1) *
                    scan_points_dataset->scan_points.y.at(i) * 1000 +
                cameraExtrinsicMat.at<double>(0, 2) *
                    scan_points_dataset->scan_points.z.at(i) * 1000) +
               (cameraExtrinsicMat.at<double>(0, 3));
    camera_y = (cameraExtrinsicMat.at<double>(1, 0) *
                    scan_points_dataset->scan_points.x.at(i) * 1000 +
                cameraExtrinsicMat.at<double>(1, 1) *
                    scan_points_dataset->scan_points.y.at(i) * 1000 +
                cameraExtrinsicMat.at<double>(1, 2) *
                    scan_points_dataset->scan_points.z.at(i) * 1000) +
               (cameraExtrinsicMat.at<double>(1, 3));
    camera_z = (cameraExtrinsicMat.at<double>(2, 0) *
                    scan_points_dataset->scan_points.x.at(i) * 1000 +
                cameraExtrinsicMat.at<double>(2, 1) *
                    scan_points_dataset->scan_points.y.at(i) * 1000 +
                cameraExtrinsicMat.at<double>(2, 2) *
                    scan_points_dataset->scan_points.z.at(i) * 1000) +
               (cameraExtrinsicMat.at<double>(2, 3));
    if (camera_z > 0.0) {
      /*
       * Projection transformation. Change from camera coordinate to image
       * coordinate
       */
      image_points_dataset->image_points.x.push_back(
          (camera_x * cameraMat.at<double>(0, 0) / camera_z) +
          cameraMat.at<double>(0, 2));
      image_points_dataset->image_points.y.push_back(
          (camera_y * cameraMat.at<double>(1, 1) / camera_z) +
          cameraMat.at<double>(1, 2));
      /*
       * Calculate euclidean distance from the camera to objects
       */
      image_points_dataset->distance.push_back(
          sqrt(camera_x * camera_x + camera_y * camera_y +
               camera_z * camera_z) *
          100); // unit of length is centimeter

      /*
       * Copy to intensity
       */
      if (!(scan_points_dataset->intensity.empty())) {
        image_points_dataset->intensity.push_back(
            scan_points_dataset->intensity.at(i));
      }
    }
  }
}
...
static void projection_callback(const autoware_msgs::ProjectionMatrix &msg) {
  ...
  for (int row = 0; row < 4; row++) {
    for (int col = 0; col < 4; col++) {
      cameraExtrinsicMat.at<double>(row, col) =
          msg.projection_matrix[row * 4 + col];
...
static void intrinsic_callback(const sensor_msgs::CameraInfo &msg) {
  ...
  if (!isIntrinsic || imageSize.height != msg.height ||
    ...
    scan_image.distance =
        (float *)calloc(msg.height * msg.width, sizeof(float));
    scan_image.intensity =
        (float *)calloc(msg.height * msg.width, sizeof(float));
...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr &msg) {
  ...
  //    msg->angle_min * 180 / 3.141592, msg->angle_max * 180 / 3.141592,
  //    msg->angle_increment * 180 / 3.141592, msg->time_increment,
  ...
  for (i = 0; i < (int)msg->ranges.size(); i++) {
    scan_points_dataset.scan_points.x.at(i) =
        msg->ranges.at(i) *
        sin(msg->angle_min +
            msg->angle_increment * i);           // unit of length is meter
    scan_points_dataset.scan_points.y.at(i) = 0; // unit of length is meter
    scan_points_dataset.scan_points.z.at(i) =
        msg->ranges.at(i) *
        cos(msg->angle_min +
            msg->angle_increment * i); // unit of length is meter
   ...
   */
  trans_depth_points_to_image_points(&scan_points_dataset,
                                     &image_points_dataset);
  ...
  for (i = 0; i < (int)image_points_dataset.image_points.x.size(); i++) {
    /* Judge NaN */
    if (isnan(image_points_dataset.image_points.x.at(i)) == 1 ||
        isnan(image_points_dataset.image_points.y.at(i)) == 1) {
      std::cout << "Not a Number is i:" << i << std::endl;
      continue;
    }
    /* Judge out of X-axis image */
    if (0 > (int)image_points_dataset.image_points.x.at(i) ||
        (int)image_points_dataset.image_points.x.at(i) > imageSize.width - 1) {
      continue;
    }
    /* Judge out of Y-axis image */
    if (0 > (int)image_points_dataset.image_points.y.at(i) ||
        (int)image_points_dataset.image_points.y.at(i) > imageSize.height - 1) {
      continue;
    }

    scan_image.distance[(int)image_points_dataset.image_points.x.at(i) *
                            imageSize.height +
                        (int)image_points_dataset.image_points.y.at(i)] =
        image_points_dataset.distance.at(i);

    if (!msg->intensities.empty()) {
      scan_image.intensity[(int)image_points_dataset.image_points.x.at(i) *
                               imageSize.height +
                           (int)image_points_dataset.image_points.y.at(i)] =
          image_points_dataset.intensity.at(i);
    }

    if ((scan_image.max_y < (int)image_points_dataset.image_points.y.at(i)) ||
        (scan_image.max_y == NO_DATA)) {
      scan_image.max_y = (int)image_points_dataset.image_points.y.at(i);
    } else if ((scan_image.min_y >
                (int)image_points_dataset.image_points.y.at(i)) ||
               (scan_image.min_y == NO_DATA)) {
  ...
  scan_image_msg.distance.assign(scan_image.distance,
                                 scan_image.distance +
                                     imageSize.width * imageSize.height);
  scan_image_msg.intensity.assign(scan_image.intensity,
                                  scan_image.intensity +
                                      imageSize.width * imageSize.height);
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber projection_sub =
      n.subscribe("projection_matrix", 1, projection_callback);
  ros::Subscriber intrinsic_sub =
      n.subscribe("camera/camera_info", 1, intrinsic_callback);
  transformed_point_data =
      n.advertise<autoware_msgs::ScanImage>("scan_image", 1);
