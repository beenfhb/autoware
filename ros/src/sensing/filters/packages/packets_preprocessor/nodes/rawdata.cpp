...
inline float SQR(float val) { return val*val; }
  ...
  int RawData::getNumLasers() const
  {
    ...
    for (int i = 0; i < BLOCKS_PER_PACKET; i++) {
      ...
      for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
        ...
        if ((block.rotation >= config_.min_angle
          ...
          float time_offset = i * 55.296 / 1000.0 / 1000.0 + j * 2.304 / 1000.0 / 1000.0;

          //std::cout << i << " " << j << " " << std::fixed << time_offset << std::endl;

          data.addPoint(x_coord, y_coord, z_coord, corrections.laser_ring, block.rotation, distance, intensity, time_offset);
        }
      }
    }
  }
      ...
      for (int firing=0, k=0; firing < VLP16_FIRINGS_PER_BLOCK; firing++){
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          ...
          if ((azimuth_corrected >= config_.min_angle
            ...
            intensity = (intensity < min_intensity) ? min_intensity : intensity;
            intensity = (intensity > max_intensity) ? max_intensity : intensity;
            float time_offset = (block*2 + firing) * 55.296 / 1000.0 / 1000.0 + dsr * 2.304 / 1000.0 / 1000.0;

            // std::cout << block << " " << firing << " " <<  dsr << " " << std::fixed << time_offset << std::endl;

            data.addPoint(x_coord, y_coord, z_coord, corrections.laser_ring, azimuth_corrected, distance, intensity, time_offset);
          }
        }
      }
    }
