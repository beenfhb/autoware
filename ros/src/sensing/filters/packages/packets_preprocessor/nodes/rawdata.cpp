...
inline float SQR(float val) { return val*val; }
   ...
   *  @param pc shared pointer to point cloud (points are appended)
   */
  void RawData::unpack_invalid(const velodyne_msgs::VelodynePacket &pkt, DataContainerBase& data)
  {
    using velodyne_pointcloud::LaserCorrection;
    ROS_DEBUG_STREAM("Received packet, time: " << pkt.stamp);
      ...
      for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
        ...
        //      || raw->blocks[i].rotation >= config_.min_angle))){
        {
          ...
          if ( is_invalid_distance
               ...
               && (raw->blocks[i].rotation <= config_.max_angle
               || raw->blocks[i].rotation >= config_.min_angle))){
                   data.addPoint(x_coord, y_coord, z_coord, corrections.laser_ring, raw->blocks[i].rotation, 0, intensity);
               }
          else {
              data.addPoint(x_coord, y_coord, z_coord, corrections.laser_ring, raw->blocks[i].rotation, distance, intensity);
          }
   ...
   *  @param pc shared pointer to point cloud (points are appended)
   */
  void RawData::unpack_vlp16_invalid(const velodyne_msgs::VelodynePacket &pkt, DataContainerBase& data)
  {
    float azimuth;
    float azimuth_diff;
    int raw_azimuth_diff;
        ...
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          ...
          //      || azimuth_corrected >= config_.min_angle))){
            ...
            if ( is_invalid_distance
                 ...
                 && (azimuth_corrected <= config_.max_angle
                 || azimuth_corrected >= config_.min_angle))) {
                data.addPoint(x_coord, y_coord, z_coord, corrections.laser_ring, azimuth_corrected, 0, intensity);
            }
            else {
              data.addPoint(x_coord, y_coord, z_coord, corrections.laser_ring, azimuth_corrected, distance, intensity);
            }
