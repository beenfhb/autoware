...
private:
  ...
  void callback_config_(image_processor::topview_publisherConfig &config,
                        uint32_t level);
  // function generate top_view
  cv::Mat create_top_view_(cv::Mat src, double theta, double height,
  ...
  dynamic_reconfigure::Server<image_processor::topview_publisherConfig>
      dynamic_params_server_;
  dynamic_reconfigure::Server<image_processor::topview_publisherConfig>::
      CallbackType dynamic_params_callback_type_;
};
#endif