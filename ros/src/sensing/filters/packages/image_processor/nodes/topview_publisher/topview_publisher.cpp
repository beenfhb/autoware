...
topview_publisher::topview_publisher() : it_(nh_) {
  ...
  camera_info_sub_ =
      nh_.subscribe(camera_info_topic_name_, 1,
                    &topview_publisher::camera_info_callback_, this);
  image_sub_ = it_.subscribe(camera_topic_name_, 1,
                             &topview_publisher::image_callback_, this);
}

topview_publisher::~topview_publisher() {}

void topview_publisher::camera_info_callback_(
    const sensor_msgs::CameraInfoConstPtr &msg) {
  K_matrix_ = cv::Matx33d(msg->K[0], msg->K[1], msg->K[2], msg->K[3], msg->K[4],
                          msg->K[5], msg->K[6], msg->K[7], msg->K[8]);
  D_vec_ = cv::Vec4d(msg->D[0], msg->D[1], msg->D[2], msg->D[3]);
}

void topview_publisher::callback_config_(
...
cv::Mat topview_publisher::create_top_view_(cv::Mat src, double theta,
  ...
  cv::Mat top_image =
      cv::Mat::zeros(undistorted_image.rows, undistorted_image.cols, CV_8UC3);
  ...
  for (int y = 0; y < top_image.rows; y++) {
    for (int x = 0; x < top_image.cols; x++) {
      ...
      double oldX = 0.5 +
                    (Hvc / Hc) * (f / fp) * c *
                        (s / c -
                         (yOrg * Hvc * s - fp * Hc * c + fp * Dvc * s) /
                             (fp * Hc * s + Hvc * yOrg * c + fp * Dvc * c)) *
                        xOrg;
      double oldY = 0.5 +
                    f * ((yOrg * Hvc * s - fp * Hc * c + fp * Dvc * s) /
                         (fp * Hc * s + Hvc * yOrg * c + fp * Dvc * c));
      oldX = oldX + cxp;
      oldY = -oldY + cyp;
      if (oldX < 0 || top_image.cols - 1 < oldX || oldY < 0 ||
          top_image.rows - 1 < oldY) {
        continue;
      }
      if ((int)oldX + 1 >= top_image.cols || (int)oldY + 1 >= top_image.rows) {
        top_image.data[(y * top_image.cols + x) * top_image.channels()] =
            undistorted_image.data[((int)oldY * top_image.cols + (int)oldX) *
                                   top_image.channels()];
        top_image.data[(y * top_image.cols + x) * top_image.channels() + 1] =
            undistorted_image.data[((int)oldY * top_image.cols + (int)oldX) *
                                       top_image.channels() +
                                   1];
        top_image.data[(y * top_image.cols + x) * top_image.channels() + 2] =
            undistorted_image.data[((int)oldY * top_image.cols + (int)oldX) *
                                       top_image.channels() +
                                   2];
        continue;
      ...
      for (int i = 0; i < top_image.channels(); i++) {
        uchar f11 =
            undistorted_image.data[((int)oldY * top_image.cols + (int)oldX) *
                                       top_image.channels() +
                                   i];
        uchar f12 = undistorted_image
                        .data[(((int)oldY + 1) * top_image.cols + (int)oldX) *
                                  top_image.channels() +
                              i];
        uchar f21 = undistorted_image
                        .data[((int)oldY * top_image.cols + (int)oldX + 1) *
                                  top_image.channels() +
                              i];
        uchar f22 =
            undistorted_image
                .data[(((int)oldY + 1) * top_image.cols + (int)oldX + 1) *
                          top_image.channels() +
                      i];
        ...
        top_image.data[(y * top_image.cols + x) * top_image.channels() + i] =
            dy2 * (f11 * dx2 + f21 * dx1) + dy1 * (f12 * dx2 + f22 * dx1);
      }
    }
  }
  return top_image;
