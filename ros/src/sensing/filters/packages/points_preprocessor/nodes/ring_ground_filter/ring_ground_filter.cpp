...
GroundFilter::GroundFilter() : node_handle_("~") {
  ...
  points_node_sub_ = node_handle_.subscribe(
      point_topic_, 10000, &GroundFilter::VelodyneCallback, this);
  groundless_points_pub_ =
      node_handle_.advertise<sensor_msgs::PointCloud2>(no_ground_topic, 10000);
  ground_points_pub_ =
      node_handle_.advertise<sensor_msgs::PointCloud2>(ground_topic, 10000);
...
void GroundFilter::InitRadiusTable(int in_model) {
  ...
  case 64:
    ...
    for (int i = 0; i < 64; i++) {
      if (i <= 31) {
        ...
        radius_table_[i] =
            fabs(sensor_height_ * (1.0 / (tan(theta) + tan(b)) -
                                   1.0 / (tan(a + theta) + tan(b))));
      } else {
        a = 0.5 * M_PI / 180;
  ...
  case 32:
    ...
    for (int i = 0; i < 32; i++) {
      ...
      radius_table_[i] =
          fabs(sensor_height_ *
               (1.0 / (tan(theta) + tan(b)) - 1.0 / (tan(a + theta) + tan(b))));
    }
    break;
  case 16:
    a = 2.0 * M_PI / 180;
...
void GroundFilter::FilterGround(
  ...
  for (size_t i = 0; i < in_cloud_msg->points.size(); i++) {
    double u = atan2(in_cloud_msg->points[i].y, in_cloud_msg->points[i].x) *
               180 / M_PI;
    if (u < 0) {
      u = 360 + u;
    }
    int column =
        horizontal_res_ - (int)((double)horizontal_res_ * u / 360.0) - 1;
...
void GroundFilter::VelodyneCallback(
  ...
  // t2_ = ros::Time().now();
  // elap_time_ = t2_ -
  // t1_;//boost::chrono::duration_cast<boost::chrono::nanoseconds>(t2_-t1_);
