...
private:
  ...
  void
  VelodyneCallback(const sensor_msgs::PointCloud2::Ptr &in_sensor_cloud_ptr);
...
SpaceFilter::SpaceFilter() : node_handle_("~") {
  ...
  node_handle_.param<std::string>("subscribe_topic", subscribe_topic_,
                                  "/points_raw");
  ...
  cloud_sub_ = node_handle_.subscribe(subscribe_topic_, 10,
                                      &SpaceFilter::VelodyneCallback, this);
  cloud_pub_ =
      node_handle_.advertise<sensor_msgs::PointCloud2>("/points_clipped", 10);
}
...
void SpaceFilter::ClipCloud(
    const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr,
    pcl::PointCloud<pcl::PointXYZ>::Ptr out_cloud_ptr, float in_min_height,
    float in_max_height) {
  out_cloud_ptr->points.clear();
  for (unsigned int i = 0; i < in_cloud_ptr->points.size(); i++) {
...
void SpaceFilter::VelodyneCallback(
  ...
  if (vertical_removal_) {
    ClipCloud(inlanes_cloud_ptr, clipped_cloud_ptr, below_distance_,
              above_distance_);
  } else {
